 ...
 * 規模守衛與 F025 `AC-J16`／F042 `AC-27`（矩陣新增第 14 列 `OJT_PROGRESS_MANAGEMENT`）互斥——
 * 本檔為「新增功能鍵不得污染 MENU／連帶未被本輪 ring 清單觸碰」之連坐盲區（`git log` 確認
 * 本檔未被本輪任何 F042 commit 改過）。13→14（兩處），標題同步更正。
 * 🔴 F043 仲裁修正（test-generator 仲裁 2026-09-02，tdd-implementation 申訴）：同一連坐盲區
 * 再度重演——`AC-43`／`AC-B28` 新增第 15 列 `BUSINESS_CATEGORY_MANAGEMENT`，本檔於建 F043 ring
 * 時未被同步觸碰。就地再擴充 14→15（兩處）。本案之真正標的（「首頁不得進入矩陣」——
 * `not.toContain('首頁')`、鍵集合與 `FUNCTION_MATRIX` 逐字一致）之鑑別力一格未減。
 */
describe('F002 AC-D5 — 「首頁」不得進入 F025 功能矩陣', () => {
  it('TS-D1-010 FUNCTION_MATRIX 之功能鍵集合仍為 15 個，且不含「首頁」', () => {
