...
export const FUNC_DISPLAY: MatrixDisplayRow[] = [
  ...
  /**
   * 🔴 2026-08-28 F042 delta（`AC-27`／`AC-J16`）——新增第 14 列，位置逐列對齊 FUNCTION_MATRIX
   * （附錄管理之後、文件索引管理之前）。`受限CRUD` 於本列之語意＝僅可新增場次、不可刪除
   * （`AC-19` 由端點層把關，矩陣格值擋不住它），與「角色指派」列之受限語意互不相同。
   */
  {
    label: 'OJT 進度管理',
  ...
  /**
   * 🔴 2026-09-02 F043 delta（`AC-43`／F025 `AC-B28`）——新增第 15 列，位置逐列對齊
   * FUNCTION_MATRIX（最末列），與 `prototypes/18-permission-matrix.html` 之 `FUNC_ROWS`
   * 相對位置一致（置於既有全部列之後）。
   * 🔴 `AC-44`：本列主管欄 `唯讀` 與上方「循環管理（DAG）」列之 `無` **刻意不同**——
   * 同日兩項人類裁決（一移出、一放進）；兩列並列於同一頁可直接對讀，**不得對齊**。
   */
  {
    label: '業務/功能類別管理',
...
export const FIELD_DISPLAY: MatrixDisplayRow[] = [
  ...
  /**
   * 🔴 2026-08-28 F042 E11 delta（`AC-J7`／`AC-J8`）——「OJT 簽到表」改為**純衍生唯讀**：
   * 五格（含 ICSOP管理員）皆「唯讀」，其值由「OJT 進度管理」之場次彙總而得，無人可寫。
   * 📝 被反轉之 D9 破例列逐字保留供追溯：OLD> `['唯讀', '可寫', '可寫', '可寫', '唯讀']`。
   * 🔴 **本列是顯示鏡射，`domain/field-matrix.ts` 之 `FIELD_MATRIX` 為權威**——兩者不一致時，
   *    本頁會把矩陣說成另一回事，使唯一的權威矩陣頁自身說謊。
   */
  { label: 'OJT 實體簽到表', cells: ['唯讀', '唯讀', '唯讀', '唯讀', '唯讀'] },
