      ...
      >
        ...
        {drawerNodeId && !nodeDocsError && subtreeRows !== subtreeTotal && (
          ...
          >
            ...
            {/*
              🔵 2026-09-23 全站文案稽核：「（兩個數字不同為正常）」是為實作辯護的語氣（F044 同型）。
              📝 已作廢（⚠ 不得復原）：
                OLD> 此子樹共有 {subtreeRows} 筆掛載紀錄、對應 {subtreeTotal} 份相異程序書——同一份
                OLD> 程序書掛在多個節點時，於各節點下各列一次（兩個數字不同為正常）。
              🔴 **兩個數字都留在可見文字**（不得移入 ⓘ）；移入 ⓘ 的只有「為什麼會不同」。
            */}
            <span className="flex items-center gap-1 flex-wrap">
              此子樹共有 {subtreeRows} 筆掛載紀錄、對應 {subtreeTotal} 份相異程序書。
              <InfoNote
                infoKey="subtree-dup"
                paragraphs={['同一份程序書掛在多個節點時，會在各節點下各列一次，所以掛載筆數通常多於相異份數。']}
              />
            </span>
          </div>
        )}
