...
describe('UsageFormManagementPage — 使用表單管理（F018）', () => {
  beforeEach(() => {
    ...
    vi.mocked(endpoints.downloadUsageForm).mockResolvedValue({ url: 'blob:zzz', expiresInSeconds: 300 });
  ...
  it('TS-F018-026 主管無權 → 顯示封鎖畫面、不呼叫查詢端點', () => {
    ...
    expect(endpoints.getUsageFormPool).not.toHaveBeenCalled();
  });
  ...
  it('TS-F018-001 上傳合法 xlsx → 呼叫 uploadUsageForms([file])', async () => {
    mockAuth('ICSOPAdmin');
    render(<UsageFormManagementPage />);
    await waitFor(() => expect(screen.getByText('進件申請書.xlsx')).toBeInTheDocument());
    ...
    await waitFor(() => expect(endpoints.uploadUsageForms).toHaveBeenCalledWith([file]));
  });
  ...
  it('TS-F018-013 個別下載 → 呼叫 downloadUsageForm(formId) 並開啟 URL', async () => {
    mockAuth('ICSOPAdmin');
    render(<UsageFormManagementPage />);
    await waitFor(() => expect(screen.getByText('進件申請書.xlsx')).toBeInTheDocument());
    await userEvent.click(screen.getAllByRole('button', { name: '下載' })[0]);
    await waitFor(() => expect(endpoints.downloadUsageForm).toHaveBeenCalledWith('uf1'));
