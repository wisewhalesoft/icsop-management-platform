...
describe('endpoints — 端點契約對映', () => {
  ...
  it('getUsageFormPool → GET /admin/usage-forms/overview', async () => {
    vi.mocked(fetch).mockResolvedValue(jsonResponse([]));
    await getUsageFormPool();
    const [url, init] = vi.mocked(fetch).mock.calls[0];
  ...
  it('downloadUsageForm → GET /admin/usage-forms/:id/download', async () => {
    vi.mocked(fetch).mockResolvedValue(jsonResponse({ url: 'blob:z', expiresInSeconds: 300 }));
    const g = await downloadUsageForm('uf1');
