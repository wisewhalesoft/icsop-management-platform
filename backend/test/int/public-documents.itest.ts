...
describe('[int] F019 前台清單 × DOC_USING_DEPT 真實 join vs SOP', () => {
  ...
  it('TS-PS-INT-001 置頂：文件使用部門為使用者部門之上層（Z9A00 ⊃ Z9AB0）→ pinned', async () => {
    ...
    expect(d1.usingDeptIds).toEqual(['Z9A00']);
  ...
  it('TS-PS-INT-004 無使用部門列之文件 → usingDeptIds/Names 為空陣列、不置頂、不從清單消失', async () => {
    const items = await listMarkers(childCookie);
    const d4 = items.find((i) => i.documentNumber === NUM('004'))!;
    expect(d4).toBeDefined();
    expect(d4.usingDeptIds).toEqual([]);
    expect(d4.usingDeptNames).toEqual([]);
  ...
  it('TS-PS-INT-006 部門篩選選上層 Z9A00 → 涵蓋其下所有使用部門（001/002/003），不含 005', async () => {
    const items = await listMarkers(childCookie, '&deptCode=Z9A00');
    const nums = items.map((i) => i.documentNumber).sort();
    expect(nums).toEqual([NUM('001'), NUM('002'), NUM('003')]);
  });

  it('TS-PS-INT-007 部門篩選選同層兄弟 Z9AC0 → 僅命中該處室（003），不含 Z9AB0（002）', async () => {
    const items = await listMarkers(childCookie, '&deptCode=Z9AC0');
    expect(items.map((i) => i.documentNumber)).toEqual([NUM('003')]);
  });
  ...
  it('TS-PS-INT-008 一份文件多筆 DOC_USING_DEPT → 清單不重複回傳同一文件（1:N 不膨脹）', async () => {
    ...
    await repo.save(repo.create({ documentId: docIds['001'], orgCode: 'Z9AB0' }));

    const items = await listMarkers(childCookie);
    const d1 = items.filter((i) => i.documentNumber === NUM('001'));
    expect(d1).toHaveLength(1); // id 僅出現一次
    expect([...d1[0].usingDeptIds].sort()).toEqual(['Z9A00', 'Z9AB0']);
