 ...
 *
 * ⚠ **2026-09-07 補述——本 migration 留下的一種殘餘錯值**：室長欄不是只有「NULL 待補」一種
 *   狀態。舊 seed 以寫死的 `AS` 解析姓名時，凡是**在 AS 也唯一命中**的名字都會落下一個
 *   **AS 的員編**；公司改成 AD／AE／AJ 之後，那個非 NULL 的員編在該文件的公司裡查無此人，
 *   而「重跑 seed 只補 NULL」對它天然無效。dev 實測命中 1 筆：
 *   `ICSOP-SRC-304-1-10 潤興撥款文件作業程序書`（AD）室長 `20781`（AS 的周家宏，他在 AD 另有
 *   帳號 `70003`）；畫面症狀是清單顯示裸員編、編輯頁卻顯示姓名（後者以登入者公司搜尋）。
 *   修法已內建於 `seed-document-catalog.ts` 之 `planPrimaryChiefWrite()`（規則 ③），故
 *   各站「跑完 migration 再重跑一次 seed」這個既有步驟即可一併修掉，本檔不需再改。
 */
