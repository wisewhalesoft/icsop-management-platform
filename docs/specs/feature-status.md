...
| 🔵 進行中 | **0** | — |
| ⬜ 未開始 | **6** | F032 F033 F034 F035 **F039 F042** |
| | **42** | |

> **🔵 2026-08-27 新增 F042 OJT 進度管理（E11，US-103／104／105）——Phase A 規格，實作為 ⬜ 未開始**：規格層產出 [F042](features/F042-ojt-progress-management.md)（`AC-01`～`AC-29`）＋六檔之 `AC-J#` delta（`AC-J1`～`AC-J25`）＋[open-questions §E11](open-questions.md#e11-2026-08-27)（`OQ-E11-01`～`OQ-E11-16`）＋[error-handling #ojt-progress](error-handling.md#ojt-progress)。tally 由 ✅27 🟡9 🔵0 ⬜5（41）改為 **✅27 🟡9 🔵0 ⬜6（42）**。
> ✅ **2026-08-28 人類閘門：16 題 OQ 全數裁決，Phase A 完成、待閘門核可整包**。裁決摘要——01=C 待指派單位｜02=C 軟標記 `orphanedAt`｜03=B 裁撤不計分母｜04=A 僅 ICSOPAdmin 可刪｜05=A（主管·部門窗口＝`受限CRUD`，刪除於端點層把關、**不擴充 `PermissionAction` 值域**）｜06=B **四值含「部分完成」**｜07=B rollup 至**部層**＋窗口 30 天＋PII 防線不變｜08=A 建立頁上傳卡改提示卡｜09=A 日期必填·不可未來日·單檔｜10=A 沿用 pdf/jpg/jpeg/png ≤50MB｜11=A **舊端點直接移除回 404**｜12=A OJT 列改純衍生唯讀（20 欄、38→**40 案**）｜13=B **新立 `OJT_SESSION_UPLOAD`／`OJT_SESSION_DELETE` ＋ `AUDIT_LOG` additive `orgCode` 欄（獨立 migration）**｜14=A 前台唯讀顯示｜15=A TAB2 篩選兩項｜16=B **場次不可編輯**（更正＝刪除後重登）。
> 🔴 **06=B 之連帶：`prototypes/13` 必須改版**——ux-ojt 本輪依 (A) 案作業、`13` **版面一字未動**；B 案下需 **OJT 欄兩態改三態**（新增「部分完成」之 icon 與逐字文案）、**`data-has-ojt` 值域擴為三值**、**篩選三選項改四選項**。另 `hasOjt` 之型別由 `boolean` 改為**三值列舉**（回應形狀變更，交 sa-ojt）。
> 🔴 **本輪為 Phase A（僅 docs/）**：零產品程式碼、零 migration 檔。⚠ **migration 支數由 1 支修正為至少 3 支**：① `OJT_SESSION` 建表（`1724889600000`，含 `orgCode` nullable＋pending 旗標＋`orphanedAt`）｜② **`AUDIT_LOG` additive `orgCode` 欄（13=B 明文要求獨立 migration）**｜③ 既有 `OJT_SIGNIN` 之「待指派」遷移（01=C）。
> ✅ **2026-08-28 lead 覆核 `OQ-E11-17`～`OQ-E11-20` 完畢（3 核可、1 否決），規格側凍結**：**17 核可**（`targetType='OJT_SESSION'`、F024 類型值四→五）｜**18 否決裁量案**，改採 **TAB2 比對「列自身」之三選項**（`所有完成狀態`／`已完成`／`尚未完成`）——四值詞彙屬**文件層**（TAB1 區一逐筆表與 `prototypes/13` 共用常數），兩軸刻意分離｜**19 核可**（重掛回即清 `orphanedAt`、場次復活）｜**20 ①否決裁量案**，維持 prototype 之「總覽比率＋依文件逐筆**兩者皆有**」、**②自成一組不排除核可**。
> ✅ **三方回填全數完成**：**sa-ojt** ⇒ [data-model v1.10](data-model.md#ojt-session-entity)＋[error-handling #ojt-progress](error-handling.md#ojt-progress)＋F042 §架構設計（8 項）；**ux-ojt** ⇒ `prototypes/04`／`13`／`14`／`15`／`16`／`25` 改版＋F042 §prototype DOM 對照（11 項＋第三輪連帶）；**spec-writer** ⇒ 正文、`AC-J1`～**`AC-J26`**、open-questions、本檔、test 設計。落差消化紀錄見 [F042 §待同步清單](features/F042-ojt-progress-management.md#post-decision-sync)。
> ✅ **一項跨檔字面衝突已發現並修復**（spec-writer 比對時揪出、ux-ojt 已改）：日期錯誤碼兩處相差一個 `IN_`——[error-handling](error-handling.md#ojt-progress) 定稿 `OJT_TRAINING_DATE_FUTURE` vs `prototypes/25` 之 `_IN_FUTURE`。⚠ **差一個 `IN_` 沒人會一眼看出來，但測試會照著錯的那份寫、整合時才炸**。🔒 **ux-ojt 已加常設斷言**：畫面上每個 `CODE · 4xx` 字面自動對照 `error-handling.md`，未定義即紅＋反向哨兵確保打錯的變體兩邊皆不存在 ⇒ **自此不再依賴「剛好有人看到」**。
> 🔴 **由此確立之職權分工（本批教訓，值得推廣）**：**error-handling ＝ 錯誤碼字面之單一權威；prototype ＝ 使用者訊息逐字之單一權威**。✅ **連帶處置（sa-ojt，2026-08-28）**：`error-handling.md` 之「使用者訊息（示意，逐字待棒 4 定稿）」欄**整欄移除**（原為 `請填寫訓練日期。` 等佔位字——欄名說待定稿、內容卻是可照抄的字串，下游會照抄佔位字），改置職權分工說明並指向 [F042 §6 ⑬](features/F042-ojt-progress-management.md#prototype-25-dom-contract)。<br>　📌 **選「移除」而非「填入逐字」之理由值得記**：兩處各放一份完整逐字句，就變成**兩份文案要保持同步**——正是本批已因錯誤碼字面吃過一次虧的那個坑。**移除後只剩一份，沒有同步負擔。**
> 🔴 **歸位失敗之三碼不得合流**（ux-ojt 於 `prototypes/25` 原一律回 404，已修）：已被他人歸位 ⇒ **409**、整筆不存在 ⇒ **404**、單位非使用部門 ⇒ **400**。⚠ **409 是多人並行清理 legacy 時最可能發生者**，回成 404 會讓操作者以為資料被刪而去追查。
> 🔴 **三項於回填期間新發現之連帶（原盤點未涵蓋）**：① **[F025](features/F025-role-function-matrix.md) RA delta ③「`受限CRUD` 僅用於角色指派列」失效**（本功能列為第二處消費，兩處「受限」語意不同、明文禁止抽共用判定式）；② **`hasOjt` 改名為 `ojtStatus`**（sa-ojt 定案——`has` 前綴在三值下使 `if(x.hasOjt)` 對 `'partial'`／`'all'` 同為 truthy，與本 repo 已記錄之 `every([])` 恆真屬同類陷阱）；③ **[F020](features/F020-watermark.md#ojt-frontstage-note-delta) `AC-D2` 之斷言定位點失效**（`04` 移除附件區 OJT 項後，`此格式不支援浮水印` 在附件區之唯一載體消失，改由使用表單／附錄區承載）——⚠ **是定位點失效、不是文案作廢**，新增 `AC-J26` 登記。
> 🔴 **本批為本專案第二次「大規模反轉既有已驗收行為」（前例＝2026-08-20 D9 批），且推翻 D9 批之兩項使用者裁決**：`OQ-D9-19`／`OQ-D9-20` 於「文件表單維持唯讀」前提下唯獨為 OJT 開一個可寫例外；**F042 連這個唯一的例外也收回**（文件表單自此對全部 20 欄皆徹底唯讀，登記整批搬至新獨立管理頁）。⚠ **推翻之理由是「模型改變使該例外之載體不復存在」（單份覆蓋式附件 → 文件×使用單位×多場次），不是「主管／部門窗口不該登記 OJT」**——後者為使用者原始需求，由 [F042](features/F042-ojt-progress-management.md) `AC-05` 承接。
> 🔴 **對既有 ✅ 功能之影響（比照 D9 批之既有處置）**：[F016](features/F016-pdf-ojt-attachment.md)／[F017](features/F017-backend-document-list.md)／[F023](features/F023-audit-logging.md)／[F024](features/F024-access-history-query.md)／[F025](features/F025-role-function-matrix.md)／[F026](features/F026-role-field-matrix.md) 六者之既有 AC 有多條被反轉、作廢或改寫（逐條見 [F042 §既有行為反轉總表](features/F042-ojt-progress-management.md#reversal-table)，**原條文皆逐字保留供追溯**）。⇒ **六者之狀態於實作完成前一律維持 ✅**（既有測試仍為當前 main 之真實描述），但**實作時必有既有測試轉紅，此為預期、非回歸**——須就地改寫為新行為之背書、**不得刪除**（比照 `AC-F17`／`AC-N` 批之既有處置慣例）。
> 🔴 **交 test-generator 之硬性 handoff（三項，建環時必讀）**：① **`AC-D5` 之既有 fixture 於新模型下不再可建構**（「文件 A 有 `OJT_SIGNIN` 附件」——新模型無此形狀），須改以「全部使用單位皆已完成」重寫（[F017](features/F017-backend-document-list.md#ojt-derived-semantics-delta) `AC-J14`）；② 🔴 **`AC-N25` 第三輪之 `.ojt-write`／`.write-only` 集合互斥斷言將退化為恆真之假綠**（OJT 鈕移除 ⇒ `.ojt-write` 恆為空集合，空集合與任何集合之交集皆為空），須整段作廢而非留著（[F026](features/F026-role-field-matrix.md#ojt-field-retire-delta) `AC-J10`）；③ 🔴 **`AC-N76` ④ 之三條逐元素斷言只得縮為 2 條、不得縮為 0 條**——其中 `xls`／`icsop_pdf` 兩列與 OJT 無關，是 lead 於 2026-08-20 第四輪特別授權用來擋「有人把 `.write-only` 整個刪掉」之失誤（[F026](features/F026-role-field-matrix.md#ojt-field-retire-delta) `AC-J11` ⑤）。
> 🔴 **交 system-architect 之效能紅線**：`hasOjt` 由「查一張 `DOCUMENT_ATTACHMENT`」改為「每份文件之全部使用單位 × 各單位場次數之聚合」後，**最直覺的寫法就是逐列查一次＝N+1**，違反 [NFR-001](nfr.md#performance)、`AC-N40` 末段與 `AC-E10` 之同一條紅線。可測形狀已寫成 [F017](features/F017-backend-document-list.md#ojt-derived-semantics-delta) `AC-J15` ⑤。
> ⚠ **spec-writer 追加提報之 9 項**（原題未涵蓋、必須一併裁決者，含 `OQ-E11-05` 之值域缺口、`OQ-E11-12` 與 `OQ-D9-36` 之逐格斷言互相污染、`OQ-E11-13` 之 `orgCode` 落點）：見 [open-questions §E11 四](open-questions.md#e11-2026-08-27)。
...
> ⚠ **AC-33 與 AC-40 是兩件不同的字串，實作與建環時不得混為一談**：AC-33＝查無結果之**空狀態**文案 `查無符合結果`（逐字、**不分支**）；AC-40＝清單頂部之**範圍說明句**（`#scopeNotice`，**依 viewer 分支**）。業務使用者查無結果時兩者同時出現。<br>🔴 **2026-08-27 更新**：`AC-40` 已作廢（說明列整條移除，[F019](features/F019-public-list-browsing.md#ux-20260827-public-delta) `AC-Y1`）⇒ 兩者不再同時出現，**`AC-33` 之空狀態文案為該畫面唯一說明**，且**不得**因說明列移除而被一併拿掉（`AC-Y2` 零漣漪鎖）。
...
| F026 | 角色×欄位權限矩陣 | P0 | 1 | ✅ 已完成-已驗證 | 欄位寫入拒絕**建立＋編輯兩路徑**（含多值欄 all-or-nothing）；**使用部門子樹前綴判定**（共用 `isWithinSubtree`，與 F019 共用，AC8/9 覆蓋）；**附件/使用表單下載權限**（AC6 角色×動作覆蓋：主管/部門窗口可下載、取代被拒 `PERMISSION_DENIED`）。<br>🔴 **2026-08-27 就地更正本列之過時敘述**：原文記「**AC5-6 浮水印釐清（人類定案 OQ-FM-01）**：後台下載＝原始檔（管理存取 SAS，不燒錄），燒錄/稽核僅前台檢視器路徑（F020）；.xlsx 無 PDF 浮水印可燒」——該敘述**已於 2026-08-20 由 `OQ-D9-08`（選項 B）全面推翻**（`OQ-FM-01` 與 `OQ-D18-01` 正式失效），本列卻未同步更新，屬**規格生態系內之殘留**。**現行行為＝後台四條下載端點一律燒錄浮水印（僅 PDF）並寫入調閱稽核、無例外角色**（含 ICSOPAdmin，`OQ-D9-09`）；權威＝[F020 §backend-burn-delta](features/F020-watermark.md#backend-burn-delta) `AC-N14`～`AC-N21`、[F026 Edge Cases](features/F026-role-field-matrix.md) 已全面改寫。**原敘述逐字保留於本列供追溯。**<br>🔵 **2026-08-27 E11 影響**：`AC-N22`～`AC-N27`／`AC-N75`／`AC-N76` 將被反轉／作廢／改寫（OJT 破例收回，見 [§OJT 欄位退場 delta](features/F026-role-field-matrix.md#ojt-field-retire-delta) `AC-J7`～`AC-J11`）；**本列狀態於實作前維持 ✅，實作時既有測試轉紅為預期、非回歸。** |
...
| ID | 功能 | P | Ph | 狀態 | 關鍵缺口 / 為何未達 Done |
|----|------|---|----|------|--------------------------|
| F042 | OJT 進度管理（文件×使用單位×場次） | P1 | 1 | ⬜ 未開始 | **✅ 規格側凍結（2026-08-28）**——16 題 OQ 全數裁決 ＋ 4 題衍生後續（`OQ-E11-17`～`20`）覆核結案（3 核可／1 否決），[open-questions §E11](open-questions.md#e11-2026-08-27) 已無待決項；三方（spec-writer／sa-ojt／ux-ojt）回填完成、[§待同步清單](features/F042-ojt-progress-management.md#post-decision-sync) 之 19 項落差全數消化。**待閘門核可整包後進入 Phase B。**<br>🔴 **06=B 之連帶（已落地）**：`prototypes/13` 已改版——OJT 欄**三態**（`file-check-2`／**`file-minus-2` 新鍵**／`file-x-2`，文字 `已全部完成`／`部分完成`／`尚未開始`）、篩選**四選項**、🔴 **`data-has-ojt` 值域改 `"all"｜"partial"｜"none"`、不保留 `true`／`false`**（舊值域斷言將配對 0 個元素而大聲失敗＝刻意，防語意悄悄變窄之假綠）；`hasOjt` **改名 `ojtStatus`**、型別改三值聯集。<br>🔴 **TAB2 篩選為三值（列層）、與清單頁四值（文件層）刻意不同**——兩軸不得對齊。<br>🔴 **migration 至少 3 支**（建表含 `orgCode` nullable＋`orphanedAt`、🔒 **不另設 pending 旗標**／`AUDIT_LOG` additive `orgCode`／legacy **1:1 所有權轉移**遷移；遷移後 `DOCUMENT_ATTACHMENT` 之 `OJT_SIGNIN` **列舉值完全移除**）。<br>**無任何實作**：場次實體未建表、功能鍵 `OJT 進度管理` 未入 [F025](features/F025-role-function-matrix.md) 矩陣、側選單未新增、TAB1／TAB2 頁面與端點群皆未建、`AUDIT_LOG` 之使用單位維度未擴充、既有 `OJT_SIGNIN` 資料未遷移。**prototype 部分完成**（`25-ojt-progress.html` **已由 ux-ojt 建立**、`14`／`15`／`16` 已改版；⚠ **`13` 尚未改版**——其依裁決前之 (A) 案作業，06=B 定案後必須補三態與四選項；前台詳情頁亦待改版，14=A）。<br>🔴 **部署順序之硬性約束（非建議）**：`AC-22`（移除文件表單之 OJT 上傳入口）**不得早於**本功能上線，否則出現「無任何入口可登記 OJT」之功能真空——[F042](features/F042-ojt-progress-management.md) `AC-22` 與 `AC-05` **必須同批上線**。<br>🔴 **本功能反轉六檔之既有已驗收 AC**，逐條見 [F042 §既有行為反轉總表](features/F042-ojt-progress-management.md#reversal-table)；六檔狀態於實作前維持 ✅，實作時既有測試轉紅為預期、非回歸。<br>**⚠ 未決之前置資訊**：既有 `DOCUMENT_ATTACHMENT.type='OJT_SIGNIN'` 之**現況筆數尚未清點**（Phase A 不動資料庫），為 `OQ-E11-01`（遷移方式）之裁決前置。<br>🔴 **2026-08-28 Phase A+B 後之實機修正三項（狀態維持現況、僅記載規格側定稿）**：① **真實資料規模缺陷**——TAB1 區一「依文件逐筆」表**無筆數上限**，dev 環境近 600 份 ICSOP 文件時變成 600 列巨長表（prototype 原 mock 僅 6 份，**問題被假資料整個藏住**）⇒ 定稿為「**預設僅未全部完成 ＋ 上限 15 ＋ 三值顯示範圍 ＋ 截斷告知（三要素）**」，🔴 **三值不得簡化為二值**（否則 `已全部完成` 晶片在任何範圍下都畫不出來，`OQ-E11-20` ① 所倚賴之「唯一載體」失守），🔒 **統計口徑一律不受上限影響**；連帶 `GET /admin/ojt-progress/summary` 新增查詢參數 `docScope` 且 `docCoverage` **由陣列改為物件**（受限切片 ＋ 完整母體計數，**破壞性變更、既有斷言必然轉紅為預期**）。② **版面貼齊**——唯讀 bar 移入 topbar 內側（`border-t`、full-bleed）、分頁列緊接 `</header>` 且 full-bleed，以 `AppShell` 之 `TopbarBanner`／`BelowTopbar` 兩 portal 插槽承載（🔒 `OrgSyncPage`／`PermissionMatrixPage` 之分頁列**不得**一併搬入）。③ **區三「最近完成」之同型節流**——同樣無筆數上限（僅靠 30 天窗口收斂）⇒ `RECENT_MAX_ROWS` **8**、排序＝最近一次訓練日期由新至舊、`[data-recent-truncation]`（🔴 未截斷時不進 DOM）、🔒 **刻意不加捲軸、無顯示範圍控制項**；🔒 四條既有規則（30 天窗口／PII 硬防線／孤兒排除／**不排除**裁撤單位）一字未改。🔴 **①③ 為同一形狀之缺陷但定值刻意不同**（上限 15 vs 8、有無捲軸、有無範圍控制項、截斷句有無名詞變體），**不得互相對齊**。<br>裁決見 [`OQ-E11-21`](open-questions.md#e11-post-impl)，條文落點＝[F042](features/F042-ojt-progress-management.md) `AC-14`／`AC-16`／`AC-28` ⑯⑰⑱／§架構設計 一-2／§6 ⑯·⑱ 群列。<br>✅ **同批另收斂兩項 prototype 偏離**（ux-fix）：④ `prototypes/25` 之 `docCoverage()` 原取用已排除裁撤與孤兒之 `coverageRows()`，**與正文／後端／既有測試三方不一致**（係 prototype 單方偏離）⇒ 已改為 `allRows().filter(r => !r.orphaned)`；🔴 **連帶新增必要載體 `[data-doc-coverage-basis-note]`**——新口徑下逐筆表各文件分母合計（含裁撤，57）與 KPI 進度列數（排除裁撤，56）**必然對不上**，需一行可見說明避免被讀成 bug（比照同頁既有 `#inactiveNote`）。⑤ 摘要行總份數補專屬掛鉤 `[data-doc-coverage-total]`（與既有 `-shown` 成對）⇒ 區一新掛鉤由 7 組增為 **9 組**。<br>✅ **本輪無待補項**：F042 §7-B 之「不得建環」清單同批再解除 1 項（區三上限），現僅存「側選單新項之位置」。<br>🔴 **2026-08-28 第二輪實機修正（[`OQ-E11-22`](open-questions.md#e11-post-impl)）——「未指定使用部門」拆為 TAB1 區一逐筆表之第四種呈現態**：backend 對 dev **真庫實打**揭露 **591 份 ICSOP 文件中僅 4 份設定了「使用部門」**，其餘 **587 份 `totalUnits = 0`** ⇒ 依 `AC-04` 判為 `none`、覆蓋率退化為 `0 / 0`⇒`0%`，**與「真的一列都沒完成」共用同一個排序鍵** ⇒ 依上一輪之升冪＋上限 15，**無義務文件整批占滿前 15 名**，唯一一份真有進度落差的文件排到第 591 名、預設範圍下永遠看不到。定稿＝**獨立一態 ＋ 排序沉底（排序鍵三段：有無訓練義務 → 覆蓋率 → 編號）＋ 不計入 `incompleteTotal`**，連帶：顯示範圍 **三值 → 四值**（增 `僅未指定使用部門`）、比值與百分比欄顯 **`—`**（非 `0 / 0`／`0%`）、摘要行**重構為兩行**（上行切「有沒有訓練義務」、下行才是有義務者之三態細分）、截斷名詞表增為四變體且 `unassigned` 範圍之**排序描述與去處另行分岔**（該範圍導向「ICSOP 文件管理」而非「OJT 資料清單」）；端點側 `docScope` 增為四值、`docCoverage.byState` 增第四鍵 `unassigned`。🔴 **核心負向鎖定**：`data-doc-ojt-state` 值域**維持三值**、第四態另以 `[data-doc-no-using-dept]`（無值屬性）表達（⚠ 同一列同時是 `none` 且無義務**正是事實**、不得互相對齊）；🔒 **`ojtStatusView()`／`OJT_DOC_STATE` 不得新增第四鍵**（與 `prototypes/13` 共用，加了會漣漪到清單頁圖示欄與四值篩選）。🔴 **既有不變式 ③ 就地更正**：`incompleteTotal === byState.partial + byState.none − byState.unassigned`（原式在上一輪為真僅因當時語料無 `totalUnits = 0` 之文件）。🔒 **統計口徑、`AC-04` 三態、TAB2、區三一律未動**；條文落點＝[F042](features/F042-ojt-progress-management.md) `AC-14` ⑧～⑮／`AC-28` ⑲／§架構設計 一-2／§6 ⑲ 群列，測試方向見 [F042-test §三-3](../test-specs/features/F042-test.md)。 |
...
| **③** | **瀏覽器實測兩種 persona（＋孤兒）** | 以真瀏覽器分別以 **業務**／**其他** 子分類帳號登入前台，逐項核對：<br>(a) 清單筆數與 `total` 差異（業務只見使用部門相符者）；<br>(b) 🔴 **2026-08-27 改為反向核對**：頁面上**沒有**任何頂部範圍說明列、亦不出現兩條被推翻之逐字說明句（`AC-Y1`）〔📝 OLD> 「頂部 `#scopeNotice` 說明句逐字正確且**兩種 persona 不同**（AC-40）」〕；<br>(c) 查無結果時**空狀態仍為 `查無符合結果`**（`AC-33`；📝 OLD> 「且**頂部說明句同時仍在**（AC-33 vs AC-40 不得互相取代）」）；<br>(d) 直連他部門文件之詳情 URL → **404、且畫面不洩漏文件編號/書名**（AC-21）；<br>(e) 檢視器／下載／列印他部門文件 → 拒絕、**無 PDF 位元組**（AC-25／AC-26）；<br>(f) **孤兒帳號**（`orgCode` 空）→ 清單為空、**無任何範圍說明句**、無「帳號異常」字樣（AC-12／`AC-Y1`／`AC-Y2`；📝 OLD> 「說明句沿用業務句」）；<br>(g) 帳號管理 modal：角色選「一般使用者」才出現子分類選擇器（AC-32） | 本輪**簡易版 ring 無 Playwright、亦未做瀏覽器煙霧測試**。2026-07-25 之 Chrome MCP 煙霧測試曾揪出 3 個「只有真瀏覽器會踩」的部署／代理層 bug（nginx/vite 代理白名單、viewer PDF iframe 之 Accept 撞 SPA bypass、裸 `/admin` 絕對轉址掉 port），**單元全綠完全測不到** |
