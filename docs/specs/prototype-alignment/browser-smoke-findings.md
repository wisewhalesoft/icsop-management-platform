...
- 🟡 GAP-07-2：歡迎標題用 loginId「AS22455」而非顯示名（proto 用姓名；此帳號 /auth/me name=游博丞）。
  **已修復（2026-08-27）**：改用 `/auth/me` 之 `name`；姓名缺漏（手動帳號未填）才退回帳號。
- ℹ️ GAP-07-3（LOW）：副標缺當日日期「· 2026-07-16（週四）」。**已補（2026-08-27）**，同上一次修改。
- 🔺 GAP-07-4：**儀表板缺整個「最近活動」區塊**（proto 07 之 ACTIVITY 五列）。
  **已補（2026-08-27）**：`GET /admin/dashboard/activity`，五類事件取自真實來源，
  **伺服端**依 F025 逐類過濾（proto 對五種角色顯示同一份示範清單，但活動列承載 PII
  ⇒ 主管／部門窗口僅見其功能範圍內之活動，此為刻意偏離 prototype 之處）。
  🔴 **真庫實跑（2026-08-27）揪出兩件單元測試看不見的事**（整合測試 `dashboard-activity.itest.ts` 已鎖）：
  ① AUDIT_LOG 之文件下載列 **85 筆中 22 筆** `documentNumber`／`targetName` 兩個快照欄皆為 null
     （`documentId` 則恆非 null）——部分寫入路徑未帶 targetNumber/targetName。直用快照會使活動列
     顯示成「— — 被下載（某人）」；已於顯示層以 `documentId` 回查 ICSOP_DOCUMENT 補位（不回寫稽核列）。
     ⚠ 稽核列快照本身仍缺——F023 之既有缺口，登記待查（活動區塊只是第一個看見它的地方）。
  ② LIFECYCLE_CHANGE_LOG **83 筆中 33 筆**之 `lifecycleId` 指向已刪除的循環（多為整合測試殘留），
     該類活動退回僅顯示摘要、無「循環『X』」前綴——已知且刻意（不隱藏該次結構變更事實）。
