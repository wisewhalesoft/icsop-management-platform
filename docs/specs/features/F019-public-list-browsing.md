...
## 2026-09-24 組織篩選連動 delta — 制定公司鎖定下級三欄（`AC-OC#` 批，前台落點） {#org-cascade-delta}

> 權威與人類裁決同 [F017 §組織篩選連動 delta](F017-backend-document-list.md#org-cascade-delta)；`AC-OC1`～`AC-OC5` 之行為**逐字適用於前台**，本節只列前台特有之落點。
> **前台缺陷實況**：`GET /public/documents/filter-options` 之 `draftingDepts`／`draftingSections` 以**裸代碼**跨公司 distinct（`buildFilterOptions`），label 以 `collapseByCode` 收斂為**其中一家**之名稱 ⇒ 兩家公司同碼不同名之部門只剩一個選項、且名稱可能是別家的。清單查詢本身以 `companyCode ∧ draftingDeptId` 比對，故 `AC-OC1` 鎖定後**查詢結果即正確**，缺的只是「依公司收斂之選項」。

- **AC-OC7**（🔴 選項之上級歸屬，additive）：`GET /public/documents/filter-options` 回應**新增**鍵 `draftingOrgUnits: { companyCode, divisionId, divisionName, deptId, deptName, sectionId, sectionName }[]`——自**同一份** `visibleCandidates()` 衍生之 distinct 組合（`AC-D5` 之可見性紀律延續：不可見文件之單位不得出現）；名稱以 **(companyCode, 代碼)** 配對解析（🔴 不得經 `collapseByCode`）；未命中名稱者 fallback 為代碼。🔒 既有六鍵之值、label、排序**一字不變**（其純函式回歸鎖 `public-list-filter-options.spec.ts` 不改）。前台以 `draftingOrgUnits` 依 `AC-OC2` 收斂 `制定本部`／`制定部門`／`制定室別` 之選項；`制定公司` 選項仍取自 `draftingCompanies`。
- **AC-OC8**（🔴 網址參數）：前台篩選狀態住在網址（`co`／`mkdiv`／`mkdept`／`section`）。Given 網址帶有 `mkdiv`／`mkdept`／`section` 任一但**無 `co`**（且不符合 `AC-OC5` 自動帶入）, Then 該等下級參數**不施加**並自網址移除（`replace`）——與 `AC-OC1`「無公司即不可選下級」一致，避免出現畫面鎖著、清單卻被縮小之狀態。`AC-OC3` 之清空同樣反映到網址。<br>
  🔴 **（2026-09-24 實機補訂）值不在選項內者同樣不施加並移除**：Given 選項已載入, When 網址之 `co` 不在 `draftingCompanies`、或 `mkdiv`／`mkdept`／`section` 不在依 `AC-OC2` 收斂後之對應選項內（他公司之代碼、viewer 不可見之單位、已不存在之單位）, Then 該參數**及其全部下級**自網址移除（`replace`）、不施加。📌 **發現經過**：實機以 `?mkdept=JA000` 進入、自動帶入 `co=AS` 後，JA000 不在該 viewer 可見之部門中 ⇒ 下拉顯示「全部」、清單卻為 **0 筆**——畫面上找不到任何原因，即本 repo 反覆修過之「看不見的篩選條件」形狀。🔒 選項載入**失敗**時不做本判定（否則一次網路錯誤會把使用者分享來的網址條件全數清掉）。
