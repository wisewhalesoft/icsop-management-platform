...
4. 每筆顯示：程序書編號、程序書書名、內容摘要、文件狀態、制定公司、制定部門、制定室別、版次、公告日期。<br>📝 **2026-08-16 使用者裁決推翻，理由：使用者指定之顯示欄位清單**——原條文為「文件編號、文件名稱、制定部門、**使用部門**、文件狀態、公告日期、內容摘要」；**移除 使用部門**，**新增 制定公司／制定室別／版次**（循環別原僅存在於 prototype 卡片、不在本條文，一併移除見 AC-D8）。
...
- Given 清單載入完成, When 呈現, Then 每筆顯示 程序書編號／程序書書名／內容摘要／文件狀態／制定公司／制定部門／制定室別／版次／公告日期。<br>📝 **2026-08-16 使用者裁決推翻，理由：使用者指定之顯示欄位清單（缺失 delta 第 3 項）**——原條文為「每筆至少顯示編號/名稱/制定部門/**使用部門**/狀態/公告日期（可含內容摘要）」。「至少」之措辭一併收斂為**恰為**上列九項（逐項斷言見 AC-D8）。
...
- **AC-D8**（清單卡顯示欄位）：Given 清單卡渲染完成, When 逐項檢視, Then 其欄位標籤集合**恰為** `程序書編號`／`程序書書名`／`內容摘要`／`文件狀態`／`制定公司`／`制定部門`／`制定室別`／`版次`／`公告日期` 九項；`<dl>` 區塊之標籤順序為 `制定公司：`／`制定部門：`／`制定室別：`／`版次：`／`公告日期：`／`內容摘要：`（編號、書名、狀態徽章維持於卡片標頭，位置不變）；且卡片 DOM **不得出現** `使用部門：` 與 `循環別：` 兩個標籤（`queryByText('使用部門：') === null`、`queryByText('循環別：') === null`）。`版次` 以等寬字（`mono`）呈現，格式 `{YY}'{NN}`。
...
- **AC-D10**（🔒 文案回歸鎖定；OQ-D18-06）：Given 清單頁渲染, When 檢視頂部說明句與置頂／其餘兩區塊標題, Then `SCOPE_NOTICE_BUSINESS`／`SCOPE_NOTICE_OTHER`（AC-U7 之兩條常數）、置頂區標題 `您部門相關文件`、其餘區標題 `其他文件`、空狀態文案 `查無符合結果` 五者**逐字與本 delta 導入前完全相同**——**縱使說明句內文提及「使用部門」而該欄位已不再顯示於卡片，仍不得修改**（說明句解釋的是「可見範圍」，非「畫面上有哪個欄位」）。
