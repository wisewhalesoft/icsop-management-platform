...
> **🔴 2026-08-27 第三輪 UX delta（使用者裁決；三項）**：① **再透明一點** ⇒ 不透明度 `0.388` → **`0.30`**（對比度 ≈1.603 → **≈1.429**），`AC-N1` 門檻同步由 `1.60` 下修為 **`1.40`**。⚠ **這是該門檻的第三次下修**（`3.0` → `1.70` → `1.60` → `1.40`）；門檻的用途是保證浮水印**仍看得見**（防遮蔽／防抹除之最低可辨識度），每降一次就削弱一次該保證。**下一次再降之前，請先在真實列印品上確認人眼仍可辨識**——測試只能證明算術，證明不了看不看得見。<br>② **燒錄疊字** ⇒ 平鋪改於**旋轉後之座標系**鋪排（＝前端 `watermarkOverlayGeometry()` 同一套做法），`WATERMARK_TILE_STEP_X`／`_Y` 兩常數整組作廢、改為 `WATERMARK_TILE_PAD_X`／`_Y`。🔴 **根因不是密度沒調好，是格點選錯了**：舊迴圈以**頁面座標軸**平鋪而文字以 `rotate(45°)` 畫，位移 `(m·stepX, n·stepY)` 之兩枚 tile 其平行文字線垂距 ＝ `|m·stepX − n·stepY| × cos45°`；`stepX 325` 與 `stepY 357` 接近 ⇒ 對角鄰居 `(1,1)` 垂距僅 **22.6pt** < 字級 `24pt` ⇒ **必然疊字**。⚠ 該缺陷**與 stepX/stepY 的大小無關**——只要兩者接近，放多疏都會在對角線方向撞在一起。<br>③ **固定機密聲明只在正中央出現一次** ⇒ tile 只重複「①身分列 ③時間戳」兩行，②固定機密聲明於頁面／畫板正中央渲染一次（`AC-N68` 已就地擴充）。
> **🔴 2026-08-27 第二輪 UX delta（使用者裁決）——比照參考文件 `reference/企金撥款作業調整.pdf` 之量測值**：該文件之浮水印為**全頁點陣疊圖**（`/I6`，595×841 DeviceRGB ＋ DeviceGray SMask），以像素量測得：墨跡色 **`#7C7C7C`（中性灰）**、SMask 峰值 **`99/255` ＝ `0.388`**、全形字進距 **24px ＝ 24pt**（其疊圖 1px ＝ 1pt）、橫向 tile pitch **325pt**、列距 153pt、0° 水平單行。<br>就地改寫之落點：`AC-N2` 色值／不透明度欄、`AC-T2`／`AC-T4` 之字級（`14`→**`24`**，行距 `28`→**`48`**）與平鋪（`stepX` `260`→**`325`**、`stepY` `208`→**`357`**）；前端 DOM 字級 `16px`→**`32px`**（＝24pt @96dpi）、tile 內距隨字級同倍放大。<br>🔴 **`AC-N1` 之門檻 `1.60` 維持不動**（使用者明示）——新定稿值實算 ≈ **1.603**，餘裕僅 `0.003`。⚠ **再往淡的方向動任何一步都會破線**，屆時必須連同 `AC-N1` 之數字一起裁決，不得逕自把門檻改小以讓測試轉綠。<br>🔴 **`WATERMARK_TILE_STEP_Y` 之組成規則本輪由「間隙固定 `138`」改為「間隙隨字級等比放大」**（`138 × 24/14 ≈ 236.6` → `237`）：字級放大 1.71 倍而間隙不動，整頁墨跡會比放大前**更重**，與本輪「比照參考之稀疏感」之目的相反。不變式改為 `stepY − (2 × LINE_STEP + FONT_SIZE) === 237`。<br>⚠ **只抄參數、不抄做法**：參考文件之點陣疊圖放大會糊、文字不可搜尋，且圖層可被整層抽離（本次量測即是這樣抽出來的）；本系統維持**向量燒錄進內容層**，**不得**改為疊圖。<br>📌 **與參考之密度落差（如實記錄，非缺陷）**：參考為**單行**浮水印、每頁約 12 行；本系統為 [NFR-007](../nfr.md#watermark) 之**三行**格式，於本組參數下每頁約 36 行。三行式是規格硬需求，密度無法逐字照抄。
...
- **AC-D2**（策略 A：非 PDF 不燒錄且 UI 明示）：Given 某**附件、附錄或使用表單**之格式**非 PDF**（`xlsx`／`xls`／`jpg`／`png`）, When 於**前台**文件詳情頁下載, Then 回應為**原始檔位元組、未經任何浮水印處理**（不轉檔、不失真）；且 When 渲染該檔案所在之清單列, Then 該列顯示逐字文案 **`此格式不支援浮水印`**（`queryByText('此格式不支援浮水印')` 於該列內可命中）；PDF 格式之列**不得**出現該文案。**三類檔案（附件／附錄／使用表單）適用同一規則、同一文案，不得分歧。**
  - 🔴 **[2026-08-28 E11] `AC-D2` 之「附件區」載體已消失，斷言範圍縮減**（`OQ-E11-14`→A ＋ [F042](F042-ojt-progress-management.md) `AC-24`）：`prototypes/04` 前台詳情頁依裁決移除附件區之 OJT 項後，**附件區不再有任何非 PDF 之列**（ICSOP PDF 恆為 pdf）⇒ 該逐字文案在**附件區**之唯一載體（OJT 之 jpg 分支）不復存在。🔒 **策略 A 與逐字文案 `此格式不支援浮水印` 本身完全不變、續為有效**，改由**使用表單區／附錄區**之 `.xlsx` 列承載。見 [§前台 OJT 項移除之連帶 delta](#ojt-frontstage-note-delta) `AC-J26`。**原條文逐字保留於上。**
...
- **AC-N1**（🔴 對比度門檻，可計算之量化驗收；🔴 **2026-08-27 第三輪就地改寫——門檻由 `1.60` 再降為 `1.40`**（使用者裁決「再透明一點」）；🔴 同日第一輪曾由 `1.70` 降為 `1.60`；🔴 2026-08-20 曾由 `3.0` 降為 `1.70`（`OQ-D9-31`）：Given 浮水印之呈現色值與不透明度, When 以 sRGB alpha 合成於**純白背景**（`#FFFFFF`）求其有效色，並依 **WCAG 2.1 相對亮度公式**計算該有效色與 `#FFFFFF` 之對比度, Then 其值 **≥ 1.40**（含邊界）。<br>⚠ **這是本門檻的第三次下修**。門檻的用途是保證浮水印**仍看得見**（防遮蔽／防抹除之最低可辨識度），每降一次就削弱一次該保證。**下一次再降之前，請先在真實列印品上確認人眼仍可辨識**——測試只能證明算術，證明不了看不看得見。<br>📝 **被推翻之原門檻逐字保留供追溯**：「Then 其值 **≥ 3.0**（含邊界）。」｜「Then 其值 **≥ 1.70**（含邊界）」（對應 `#334155` @ `0.30`、實算 ≈ 1.716）｜「Then 其值 **≥ 1.60**（含邊界）」（對應 `#475569` @ `0.30` ≈ 1.613、`#7C7C7C` @ `0.388` ≈ 1.603）。**推翻理由（使用者裁決）＝ 3:1 之可讀性代價過高**——lead 已獨立驗算並確認原算式正確（3:1 ⇒ 有效色不得淺於 `rgb(149,149,149)` ⇒ `#334155` 需 `opacity ≈ 0.57`），但使用者判定該不透明度會過度遮蓋文件內容。<br>📌 **驗證載體**：純函式單元測試——被測輸入為 `AC-N3` 所要求之具名常數（色值＋不透明度），測試自備合成與對比度計算之工具函式（**不得**改為讀取瀏覽器計算樣式，jsdom 不做 alpha 合成）。<br>📌 **合成與亮度公式（供測試逐字實作，避免各自臆造）**：`effective = 255 − alpha × (255 − channel)`，逐通道套用（**不四捨五入為整數亦可，兩種算法於本定稿值下皆落在 1.429 附近，同樣 ≥ 1.40**）；相對亮度 `L = 0.2126·R' + 0.7152·G' + 0.0722·B'`，其中 `C' = c/255 ≤ 0.03928 ? (c/255)/12.92 : ((c/255 + 0.055)/1.055)^2.4`；對比度 `= 1.05 / (L + 0.05)`。<br>⚠ **門檻為 `≥ 1.40`、不是 `≈ 1.43`**——請以**不等式**斷言，**不得**寫成對某個小數點後兩位之相等比較（浮點與四捨五入差異會使等值斷言脆裂）。
- **AC-N2**（🔴 定稿值與各載體一致；🔴 **2026-08-20 兩處就地改寫——不透明度 `0.57`→`0.30`（`OQ-D9-31`）、載體由 5 處減為 4 處（`OQ-D9-32`）**）：Given 本 delta 實作完成, When 檢視**下表各載體**之色值／不透明度常數, Then **逐字為下列定稿值**——
  ...
  | 2 | 變更歷程新舊並列疊加 | `frontend/src/pages/ChangeHistoryPage.tsx` | **`#7C7C7C`** | **`0.30`** |
  | 3 | 循環樹狀圖預覽疊加 | `frontend/src/pages/LifecycleTreePreviewPage.tsx` | **`#7C7C7C`** | **`0.30`** |
  | 4 | PDF 燒錄（內容層；**檢視器所見位元組亦由此產生**） | `backend/src/public/pdf-burner.ts` | **`rgb(0.4863, 0.4863, 0.4863)`**〔＝`#7C7C7C` 之 0–1 正規化〕 | **`0.30`** |
  | 5 | Prototype 權威 | `prototypes/22-*`、`prototypes/23-*` ＋ **`prototypes/00-design-system.html` 之浮水印示範**（🔴 2026-08-20 第三輪**追認納入**：`00` 為設計系統之文件，留著舊值會與全部四個實際載體矛盾；**它非行為載體、無回歸風險**，lead 已核可）（⚠ `prototypes/05-public-viewer-watermark.html` 之 `.wm-layer` **整段移除**，見 `AC-N7`；惟其 canvas 燒錄之 `WM_COLOR` 同為本欄定稿值） | **`#7C7C7C`** | **`0.30`** |
  ...
  <br>🔴 **2026-08-27 第二輪就地改寫（使用者裁決：比照參考文件之量測值）**：色值由 Tailwind slate 色階改為**中性灰** `#7C7C7C`（R=G=B），不透明度 `0.30` → `0.388`。<br>🔴 **「中性灰」是本輪之裁決重點，不只是三個字面值**：前兩版皆為偏藍之 slate；測試除逐字斷言外，**應另立一條 `R === G === B`**，否則日後有人改成另一個偏色而三值仍「看起來像灰」時不會紅。
  <br>🔴 **2026-08-27 第三輪就地改寫（「再透明一點」）**：不透明度 `0.388` → **`0.30`**；色值不動。
  <br>📌 **現行定稿值之驗算**：`#7C7C7C` @ `0.30` ⇒ 有效色 `rgb(215.7, 215.7, 215.7)`、對比度 **≈ 1.429:1**，滿足下修後之 `AC-N1` 門檻 ≥ 1.40。**ui-ux-designer 逐字照抄，不得自行發明**（比照 [F018](F018-usage-form-management.md#edit-number-action) 之既有慣例）。<br>📝 **被推翻之前一版定稿值逐字保留供追溯**：`#7C7C7C` @ `0.388`（有效色 `#CCCCCC`、對比度 ≈ **1.603:1**）｜`#475569` @ `0.30`（有效色 `rgb(199.8, 204.0, 210.0)`、對比度 ≈ **1.613:1**）｜`#334155` @ `0.30`（有效色 `rgb(193.8, 198.0, 204.0)`、對比度 ≈ **1.716:1**）｜`#334155` @ `0.57`（有效色 ≈ `rgb(138.7, 146.7, 158.1)`、對比度 ≈ **3.115:1**，為滿足原 ≥ 3.0 門檻之最小不透明度）。<br>⚠ **與 lead 裁決備忘之數字差異（如實記錄）**：lead 之裁決文字記為「≈ 1.73:1」；spec-writer 以本條所載公式重新驗算為 **≈ 1.716:1**（整數化合成色後為 ≈ 1.715）。**兩者皆滿足 ≥ 1.70 之門檻，裁決不受影響**；本檔採實算值 1.716，以免測試把 1.73 當成期望值而恆紅。若人類決定調整門檻，**只需改本表兩個字面值與 `AC-N1` 之門檻數字**，其餘 AC 一字不動。
...
- **AC-N66**（🔴 **另兩頁之疊加層必須保留——正向斷言**；`OQ-D9-32` 之範圍界線）：Given 後台**變更歷程頁**（`ChangeHistoryPage`，新舊並列 diff）與**循環樹狀圖預覽頁**（`LifecycleTreePreviewPage`）渲染完成, When 檢視 DOM, Then 兩頁之浮水印疊加層**必須存在**——
  ...
  - 兩頁之色值／不透明度為 `AC-N2` 表列之**現行定稿值**（**`#7C7C7C`／`0.30`**；📝 OLD> `#7C7C7C`／`0.388`｜`#475569`／`0.30`｜`#334155`／`0.30`）。
...
- **AC-N68**（三層式呈現契約之新載體；🔴 **2026-08-27 第三輪就地擴充：機密聲明只出現一次**）：Given 任一浮水印快照字串, When 於**檢視器路徑**呈現（＝ `AC-N6` 之已燒錄位元組）, Then 其三層式結構（①身分資料列 ②固定機密聲明**另起一行** ③時間戳）由 `backend/src/public/pdf-burner.ts` 之 **`toDisplayLines(snapshot)`** 承載——該純函式對同一輸入之回傳**恰為 3 行**，且**逐行與前端 `watermarkLines(snapshot)` 之對應行字串相同**。
  <br>🔴 **版面分派（第三輪新增；🔒 `toDisplayLines()`／`watermarkLines()` 本身一字不動）**：三行之**呈現位置**由疊在其上之純呈現層函式分派——後端 `splitWatermarkPresentation(snapshot)`、前端 `watermarkPresentation(snapshot)`，兩者皆回傳 `{ tiled, centre }`：
  - `tiled` ＝ **隨 tile 重複**之兩行（①身分列、③時間戳）；
  - `centre` ＝ **只渲染一次**、置於頁面（PDF）或畫板（DOM）**正中央**之②固定機密聲明。
  <br>🔴 **可逆性必須成立且必須被斷言**：`[tiled[0], centre, tiled[1]].join('-')` 恆等於原快照。**這是「呈現怎麼排」與「稽核記了什麼」不會漂移的唯一機器保證**——少了它，日後有人為了版面把某一欄從 `tiled` 拿掉，稽核快照仍然是對的，於是沒有任何測試會紅。
  <br>🔴 **必須另有負向鎖**：機密聲明**不得**出現在任何一枚 tile 內。只驗「中央有一份」是不夠的——tile 照舊渲染三行時，中央那份也會存在而測試照樣綠。
  <br>🔒 **線性稽核快照字串、欄位順序、收合規則（[NFR-007](../nfr.md#watermark)）一字不變**；`AC-N68` 原文之「恰為 3 行」約束的是 `toDisplayLines()` 之**回傳**，不是「每枚 tile 畫幾行」。
  <br>📝 已作廢（⚠ 不得復原）：OLD> 每枚 tile 皆渲染三行（含固定機密聲明）。
...
- **AC-T2**（各載體逐一綁定；數值逐字）：Given 本 delta 實作完成, When 檢視下表各載體, Then 其行距**逐字為下列定稿值**——
  ...
  | 3 | PDF 燒錄（內容層；**檢視器所見位元組亦由此產生**，`AC-N6`） | `backend/src/public/pdf-burner.ts` | 每行 y 位移 | `WATERMARK_FONT_SIZE × WATERMARK_LINE_HEIGHT` ＝ **`24 × 2.0`** ＝ **`48`**（🔴 2026-08-27 第二輪就地改寫：字級 `14`→`24`；📝 OLD> `14 × 2.0` ＝ `28`、OLD> `12 × 2.0` ＝ `24`、OLD> `size + 8` ＝ `20`、OLD> `size + 3` ＝ `15`） | ✅ 是（`AC-T4`） |
  ...
  | 5 | Prototype canvas 權威（＝後端燒錄之視覺代理） | `prototypes/05-public-viewer-watermark.html` | `WM_LINE_STEP`／`stepX`／`stepY` | **`64`**（＝`WM_FONT_SIZE 32 × WM_LINE_HEIGHT 2.0`）／**`920`**／**`2 × 64 + 280`**（📝 OLD> `64`／`700`／`308`、`32`／`560`／`154`、`28`／`560`／`144`） | ❌ 否（設計權威，見 `AC-T5`） |
  <br>📌 **`05` 之平鋪本來就是對的**：它先 `translate` 到頁面中心、`rotate(-45°)`，才在**旋轉後的座標系**鋪 `gx`／`gy`。後端舊實作反而是在頁面座標軸上鋪、文字才旋轉——第三輪之疊字缺陷即源於此，已改為與 `05` 同一套做法。
...
- **AC-T3**（🔴 **INV-WM-LH ＝ 跨載體一致性不變式**；本節最關鍵之一條）：Given 於**同一個測試檔**內分別渲染 `ChangeHistoryPage` 與 `LifecycleTreePreviewPage`, When 蒐集兩頁全部 `[data-testid="watermark-text"]` 之 `line-height` 並取其**相異值集合**, Then 該集合之 `size` **恰為 `1`**，且其唯一元素 **等於 `WATERMARK_LINE_HEIGHT`**。
  ...
  <br>⚠ **本條 ①②（集合）之涵蓋範圍僅限 DOM 疊加載體**（型別相同、單位相同、可直接比大小）。燒錄載體（canvas／PDF）之行距落在**不同單位系統**（px vs PDF point；2026-08-27 第二輪起字級 canvas `32` vs PDF `24`，📝 OLD> `16` vs `14`、`14` vs `12`），**刻意不納入本集合**；其定稿值由 `AC-T4` 單獨鎖定。
  ...
  <br>🔴 **③ 也不得併入 ①② 的集合**：即使四個載體現在都是 `2.0`，把 PDF point 的行距與 DOM 的無單位 `line-height` 放進同一個 `Set` 比較，比的是兩種單位系統下**恰好相等的數字**，而不是同一個量——`WM_FONT_SIZE` 一旦不同（canvas 32 vs PDF 24），該集合就會無故轉紅或無故轉綠。**③ 比的是「行高倍數」這個無單位量，①② 比的是 DOM 載體的實際 `line-height`，兩者是不同的斷言。**

- **AC-T4**（後端燒錄之每行位移與平鋪間距；🔴 **2026-08-21 第三輪就地改寫**，`OQ-T3-01` 選項 (c) ＋ `OQ-T3-02`）：Given `PdfLibBurner.burnPdf` 對一份三行式快照燒錄, When 檢視其每頁每個 tile 之三行文字, Then 第 `i` 行（`i` 由 `0` 起）之 y 座標為 `y − i × WATERMARK_LINE_STEP`，其中 `WATERMARK_LINE_STEP` **逐字為 `48`**（＝`WATERMARK_FONT_SIZE(24) × WATERMARK_LINE_HEIGHT(2)`；🔴 2026-08-27 第二輪就地改寫，📝 OLD> `28`、`24`）；且 `drawText` 之 `size` 逐字為 `WATERMARK_FONT_SIZE`。
  <br>🔴 **2026-08-27 第三輪：`WATERMARK_TILE_STEP_X`／`_Y` 兩常數整組作廢**，平鋪改於**旋轉後之座標系**鋪排（＝前端 `watermarkOverlayGeometry()` 同一套做法：先建邊長 `(W + H) × cos45°` 之正方形、以頁面中心為中心，於該框內以 `tileW × tileH` 規則鋪排，再把 `(u, v)` 映回頁面座標）。新常數：
  - `WATERMARK_TILE_PAD_X === 45`（沿文字方向之左右留白；相鄰 tile 之文字間隔 ＝ `2 × 45` ＝ `90pt`）
  - `WATERMARK_TILE_PAD_Y === 105`（垂直於文字方向之上下留白；取值使**每行之平均垂距** ＝ `tileH / 行數` ＝ `306 / 2` ＝ **`153pt`**，逐字對齊參考文件量測到的列距）
  <br>📌 兩者與前端 `WM_TILE_PAD` 之比例相同（`pad / 字級` ＝ `1.875` / `4.375`），只是單位不同（pt vs px）——「更貼近前端顯示方式」在此不只是講法，是可核對的比例關係。
  <br>🔴 **必須斷言的是「不會撞在一起」這個性質本身，不是某個 step 數值**：`旋轉框內任兩條文字線之垂距 ≥ WATERMARK_FONT_SIZE`（同 tile 內＝`WATERMARK_LINE_STEP`；跨 tile ＝`tileH − (行數−1) × WATERMARK_LINE_STEP`）。舊實作壞掉的方式正是「每個數值單看都合理，組合起來卻在對角線方向撞上」——只鎖數值的測試抓不到它。
  <br>📝 已作廢（⚠ 不得復原）：OLD> `WATERMARK_TILE_STEP_X` `260`／`325`；OLD> `WATERMARK_TILE_STEP_Y` `180`／`198`／`208`／`357`；OLD> 間隙不變式 `=== 138`／`=== 237`。
  <br>⚠ **字級 `24` 僅在維持 45° 對角時可行**：機密聲明為 26 個全形字 ⇒ 24pt 需 `624pt` 寬，已超過 A4 寬（`595.28pt`）；A4 對角線 `1030pt` 才容得下。若日後把燒錄改回水平 `0°`，字級上限只有 **`22pt`**（且左右零留白），**必須連字級一起改**。
  <br>📌 **可斷言形狀（比照 `AC-N3` 之既有處置與 `pdf-burner.spec.ts` 之既有慣例）**：以 `import` 直接斷言具名匯出常數——`WATERMARK_FONT_SIZE === 24`、`WATERMARK_LINE_HEIGHT === 2`、`WATERMARK_LINE_STEP === 48`，**且** `WATERMARK_LINE_STEP === WATERMARK_FONT_SIZE * WATERMARK_LINE_HEIGHT`（**推導關係本身也要斷**，見 `AC-T1`），**不需**真的解析 PDF 位元組。
  <br>📌 **負向回歸鎖（🔴 兩個作廢值都要鎖）**：`WATERMARK_LINE_STEP !== 15`（＝`size + 3`，本 delta 前之原始值）**且** `WATERMARK_LINE_STEP !== 20`（＝`size + 8`，**2026-08-21 第一輪之定稿值，曾進入本條 AC**——不鎖住它，實作者照第一輪 AC 寫出 `20` 也會綠）**且** `WATERMARK_LINE_STEP !== 24`（＝字級 `12` 之已作廢定稿值）**且** `WATERMARK_LINE_STEP !== 28`（＝字級 `14` 之已作廢定稿值，2026-08-27 當日曾短暫進入本條 AC）。
...
### 前台 OJT 項移除之連帶 delta（🔴 2026-08-28 E11；權威＝[F042](F042-ojt-progress-management.md)） {#ojt-frontstage-note-delta}

> **本節之性質**：[F042](F042-ojt-progress-management.md) `AC-24`（`OQ-E11-14`→**A**）使前台文件詳情頁之 OJT 改為**唯讀衍生之已完成單位清單**、且**不提供任何場次檔下載**
> ⇒ `prototypes/04` 之**附件區已無 OJT 項**。本節僅登記其對 `AC-D2` 之**斷言定位點**之影響；🔒 **策略 A（`OQ-D18-02`）與其逐字文案本身一字未動、續為有效**。
> **本 delta 之 AC 編號採 `AC-J#`**（配發表見 [F042 §庚](F042-ojt-progress-management.md#reversal-table)；本檔僅 `AC-J26` 一條）。
> 📌 **本節為 2026-08-28 第三輪由 ux-ojt 於改版 `04` 時發現之連帶，原盤點未涵蓋**——逐條登記見 [F042 §反轉總表 辛節](F042-ojt-progress-management.md#reversal-table)。

- **AC-J26**（`AC-D2` 之斷言範圍縮減——**非文案作廢**）：Given 前台文件詳情頁（`prototypes/04` 為權威）, When 檢視各區之非 PDF 列, Then ——① **附件區**已無 OJT 項，其餘附件（ICSOP PDF）恆為 `pdf` ⇒ **該區不出現** `此格式不支援浮水印`；② **使用表單區與附錄區**之 `.xlsx`／`.xls` 列**仍逐字顯示** `此格式不支援浮水印`，且其下載仍回**原始檔位元組、未經浮水印處理**（`AC-D2` 之行為契約逐字不變）。<br>🔒 **三類檔案「同一規則、同一文案、不得分歧」之既有要求續為有效**——⚠ 現在只剩**兩類**（使用表單／附錄）在前台有非 PDF 之可能，但該要求之語意不變：任一類日後若再出現非 PDF，仍須用同一句。<br>🔴 **本條最易被誤讀之處**：`AC-D2` 之既有測試若以「附件區之 OJT 列」定位該文案，**必然轉紅**——⚠ **那是定位點失效，不是文案作廢**。**須就地改寫為以使用表單／附錄列定位、不得刪除該斷言**（比照 `AC-F17` 之既有處置慣例）。<br>📌 **前台不提供 OJT 場次檔下載**（[F042](F042-ojt-progress-management.md) `AC-24`）：簽到表為出席紀錄，與該檔 `AC-16` 之 PII 防線同源 ⇒ 本檔之燒錄／不燒錄規則**自始不適用於場次檔之前台路徑**（該路徑不存在）。<br>🔒 **後台場次檔下載仍適用既有政策**：`format = pdf` 者沿用 `OQ-D9-08` 之後台燒錄＋寫稽核（見 [error-handling #ojt-progress](../error-handling.md#ojt-progress) 之明文延伸）。
