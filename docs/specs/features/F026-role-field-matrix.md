...
>
> ⚠ **推翻範圍嚴格限於「OJT 簽到表」一列**——其餘 19 欄（含 ICSOP PDF、使用表單、附錄）之格值**逐格不變**（`OQ-D9-20` 選項 A；`AC-N24`／`AC-N25` 回歸鎖定，為本 delta **最重要之防護**：防「開一個洞、鬆一片牆」）。「所屬節點」雖列 ICSOPAdmin 可寫，但維護入口為節點抽屜（F009），非文件編輯表單。🔴 **2026-08-27 收斂**：「制定公司」之承載欄位由 `draftingCompanyId`（該公司 ROOT 之 orgCode，已 DROP）改為 `companyCode`（公司代碼，NOT NULL）。🔴 **2026-09-04 人類裁決推翻其「於建立時決定即固定」**：程序書目錄清單匯入把 126 筆非和潤企業之文件記成和潤企業（見 migration `1725580800000`），而編輯端唯讀代表這種錯誤**在畫面上永遠改不掉**——鎖死擋掉「不小心改錯」的同時也把「改正」一起擋掉了。📝 被推翻之原條文逐字保留供追溯：「且**於建立時決定即固定**——編輯端一律靜默剔除（比照 `nodeId`）。理由：改公司會使既有 `draftingDeptId`／`draftingSectionId`／`usingDeptIds`（皆為各公司獨立編碼之 5 碼 orgCode）整批指向他公司之單位，並直接影響 F041 之資料列可見性判定。」該連動風險**未被否認**，改由「變更公司即連動清空那三欄」承接（`DocumentsService.update()` 1e）——而非鎖死欄位。本列之**角色格值逐格不變**（僅 ICSOPAdmin 可寫）。共 **20 欄位**（原 19 欄 ＋ **2026-08-06 新增「附錄（多）」**；詳見 [data-model](../data-model.md#document-entity)；2026-07-17 移除「當責部門」、新增 制定公司/制定部門/制定室別/內容摘要，發布日期→公告日期、人為版本號→版次）。
...
> 🔴 **[2026-08-27 E11] 上方「現行定案（2026-08-20 起）」之 OJT 例外已被 `OQ-E11-12`／[F042](F042-ojt-progress-management.md) `AC-22` 改寫**，見 [§OJT 欄位退場 delta](#ojt-field-retire-delta) `AC-J7`。**文件表單自此對全部 20 欄（含 OJT）皆為徹底唯讀，2026-08-20 之唯一例外收回**；矩陣表「OJT 簽到表」列之主管／部門窗口兩格隨之反轉回「唯讀」（具體形狀依 `OQ-E11-12` 之 A／B 案裁決）。**上方原定案句與其 📝 追溯段落逐字保留、不刪不改。**
...
| 文件狀態 | 唯讀 | 可寫 | 唯讀 | 唯讀 | 唯讀 |
| 制定公司 | 唯讀 | 可寫（建立與編輯皆可；🔴 2026-09-04 起解除「僅建立時」之限制）| 唯讀 | 唯讀 | 唯讀 |
| 制定部門 | 唯讀 | 可寫 | 唯讀 | 唯讀 | 唯讀 |
| 制定室別 | 唯讀 | 可寫 | 唯讀 | 唯讀 | 唯讀 |
...
> 🔴 **[2026-08-27 E11] 上表「OJT 簽到表」列之兩個 `可寫` 格已被 `OQ-E11-12`／[F042](F042-ojt-progress-management.md) 反轉回 `唯讀`**（或整列移除，依該題之 A／B 案），見 [§OJT 欄位退場 delta](#ojt-field-retire-delta) `AC-J7`。⚠ **A 案下本列之五格將與上方 📝 所保留之原格值逐字相同**——亦即回到 D9 導入前之狀態，惟其**語意已不同**：彼時為「尚未開放」，此後為「已改為系統衍生之唯讀值」（[F042](F042-ojt-progress-management.md) `AC-04`）。
> 🔴 **2026-08-28 更正（test-generator 仲裁申訴 10 連帶發現；原句誤記，保留於上供追溯）**：上句「A 案下本列五格將與上方 📝 所保留之原格值逐字相同」為**誤記**——`AC-J7`②已定案「五格全數為唯讀」，但上方 📝 所保留之原格值（D9 之前）為 `唯讀｜可寫｜唯讀｜唯讀｜唯讀`，**ICSOPAdmin 一格為「可寫」**（`git show 03cc8f0^:backend/src/rbac/field-matrix.ts` 舉證：D9 之前 `OJT_SIGNIN` 歸在 `ICSOP_WRITABLE` 共用列，`ICSOPAdmin: 'WRITABLE'`），與「五格全數唯讀」不符。**應為**：本列五格與 D9 導入前相比**恰 1 格不同**——`ICSOPAdmin` 由「可寫」改為「唯讀」，其餘四格（`SysAdmin`／`Supervisor`／`DeptContact`／`User`）與 D9 導入前相同。⚠ **若依原句誤記逕自把 `ICSOPAdmin` 改回可寫，正是 `AC-J8`「不得只驗曾經破例之兩角色、遺漏 ICSOPAdmin」明文要防的那個洞。**
...
- **AC-N22**（🔴 矩陣逐格斷言——恰兩格改值）：Given `FIELD_MATRIX` 之全部 20 個欄位鍵 × 5 種角色（共 100 格）, When 逐格取值, Then **恰有 2 格與本 delta 導入前不同**——欄位鍵 `OJT 簽到表` × `Supervisor` 與 `OJT 簽到表` × `DeptContact`，兩者皆由「唯讀」改為「可寫」；**其餘 98 格逐格相同**；欄位鍵集合與角色集合亦未增減（仍為 20 × 5）。
  - 🔴 **[2026-08-27 E11] `AC-N22` 之「恰 2 格改值」已被 `OQ-E11-12`／[F042](F042-ojt-progress-management.md) `AC-22` 反轉**，見 [§OJT 欄位退場 delta](#ojt-field-retire-delta) `AC-J7`。**原條文逐字保留於上。**
- **AC-N23**（主管／部門窗口對 OJT 欄之寫入解析為允許）：Given 角色為 `Supervisor` 或 `DeptContact`, When 對欄位鍵 `OJT 簽到表` 呼叫欄位權限解析函式, Then 回傳「可寫」；When 該角色經 OJT 上傳路徑送出寫入, Then **不得**回 `FIELD_WRITE_FORBIDDEN`（行為面契約見 [F016](F016-pdf-ojt-attachment.md#ojt-role-open-delta) `AC-N28`）。
  - 🔴 **[2026-08-27 E11] `AC-N23` 之期望值已被反轉為「唯讀」**，見 [§OJT 欄位退場 delta](#ojt-field-retire-delta) `AC-J8`。**原條文逐字保留於上。**
- **AC-N24**（🔒 **19 欄回歸鎖定——本 delta 最重要之防護**）：Given 角色為 `Supervisor` 或 `DeptContact`, When 逐一嘗試寫入 `OJT 簽到表` **以外之 19 個欄位鍵**中的任一個（含 `文件狀態`／`制定公司`／`制定部門`／`制定室別`／`文件編號`／`當責室長-主要`／`當責室長-次要`／`文件使用部門`／`版次`／`所屬循環`／`所屬節點`／`文件連結點`／`公告日期`／`文件名稱`／`內容摘要`／`系統 UUID`）, Then **一律回 403 `FIELD_WRITE_FORBIDDEN`**、該更新不寫入 DB。<br>📌 **可測形狀**：以 `Supervisor`／`DeptContact` × 19 個欄位鍵之**全組合逐案斷言**（38 案），**不得**只抽驗其中數欄——本條要偵測的正是「整頁鬆綁」，抽驗會漏掉未抽到的欄位。<br>🔴 **all-or-nothing 語意不變**：既有「一次請求夾帶多欄、其中任一為唯讀即整批拒絕」之判定**逐字不變**；`Supervisor` 送出「OJT ＋ 任一其他欄」之混合 payload 時**仍整批拒絕**（OJT 之寫入路徑為獨立之附件上傳端點，不經文件欄位 payload）。
  - 🔴 **[2026-08-27 E11] `AC-N24` 之措辭已被 `OQ-E11-12` 改寫**（「`OJT 簽到表` 以外之 **19** 個欄位」→「**全部 20** 個欄位」；可測形狀之全組合案數 **38 → 40**），見 [§OJT 欄位退場 delta](#ojt-field-retire-delta) `AC-J9`。🔒 **本條之防護本身不但續存且範圍擴大**（自 19 欄擴為 20 欄），**不得**因 OJT 破例收回而誤判為「本條已無必要」。**原條文逐字保留於上。**
- **AC-N25**（🔒 另兩類附件與附錄仍拒——同一防護之第二面）：Given 角色為 `Supervisor` 或 `DeptContact`, When 嘗試上傳／取代 **ICSOP PDF**、上傳／關聯／移除**使用表單**、或上傳／覆蓋／關聯／解除**附錄**, Then 一律拒絕——ICSOP PDF 與附錄之欄位層回 403 `FIELD_WRITE_FORBIDDEN`；使用表單管理端點之路由層回 403 `PERMISSION_DENIED`（[F025](F025-role-function-matrix.md) 矩陣「文件使用表單管理」對兩者為「無」）、附錄管理端點同理（「附錄管理」為「無」，[F039](F039-appendix-management.md) AC-33）。<br>⚠ **本條與 `AC-N24` 不可合併**：`AC-N24` 鎖的是**文件欄位 payload** 路徑，本條鎖的是**檔案上傳／關聯端點**路徑；兩條路徑之守門機制不同（欄位層 vs 路由層），只驗其一會漏掉另一。<br>🔴 **2026-08-20 第三輪擴充——前端呈現層之隔離（`15`／`16` 兩頁，lead 追認 designer 之超範圍改動）**：本條之回歸鎖定範圍自此**含編輯頁（`DocumentEditPage`／`prototypes/15-document-edit.html`）之控制項可見性規則**。Given 以 `Supervisor` 或 `DeptContact` 渲染後台**編輯頁**, When 檢視其寫入控制項之 class 指派, Then——<br>　① **OJT 取代鈕**之 class **含 `ojt-write`、不含 `write-only`**，且帶 `data-ojt-upload`；<br>　② **ICSOP PDF 取代鈕**與 **`.xls` 上傳鈕**（及其餘全部寫入控制項）之 class **含 `write-only`、不含 `ojt-write`**；<br>　③ 兩組 class **互斥**——`querySelectorAll('.ojt-write')` 之結果集合與 `querySelectorAll('.write-only')` 之結果集合**交集為空**，且對該兩角色**可寫控制項恰為 1 個**（＝ OJT 取代鈕；`container.querySelectorAll('[data-ojt-upload]').length === 1`）。<br>🔴 **本子條所防之失誤形狀（lead 明確點名）**：把 OJT 取代鈕**併入既有 `.write-only`** ⇒ `.write-only` 之角色條件一旦為兩角色放寬，**ICSOP PDF 取代鈕與 `.xls` 上傳鈕會一起對主管放行**——一個 class 之誤用即造成整片牆鬆動，且後端 `AC-N33` 雖仍會擋（403），畫面上卻已出現不該存在的入口。<br>⚠ **不得**以 `offsetParent === null` 或 `toBeVisible()` 作為 vitest 斷言——**jsdom 不做版面計算，`offsetParent` 對所有元素恆為 `null`**，該斷言會**恆真而毫無鑑別力**（假綠）。該形狀僅適用於 prototype 於**真實瀏覽器**中之自檢；**約束環側一律以 class 指派與 `data-*` 掛鉤斷言**（如上 ①②③）。<br>📌 **`15` 之改動屬 lead 追認之超範圍項**：`AC-N20` 明文含「編輯頁」，且不改則同一 delta 下 `16` 說「OJT 可寫」、`15` 說「全欄位唯讀不可取代」，兩頁自相矛盾。DOM／文案契約見 `AC-N76`。
  - 🔴 **[2026-08-27 E11] `AC-N25` 之前半（另兩類附件與附錄仍拒）維持有效；其 2026-08-20 第三輪擴充之 `.ojt-write`／`.write-only` class 互斥契約已整段作廢**（OJT 取代鈕不再存在 ⇒ `.ojt-write` 結果集合恆空、交集斷言退化為**恆真之假綠**），見 [§OJT 欄位退場 delta](#ojt-field-retire-delta) `AC-J10`。**原條文逐字保留於上。**
- **AC-N26**（🔒 系統管理員對 OJT 仍唯讀——`OQ-D9-24` 之明文排除）：Given 角色為 `SysAdmin`, When 對欄位鍵 `OJT 簽到表` 呼叫欄位權限解析函式, Then 回傳「唯讀」；When 經 OJT 上傳路徑送出, Then 回 **403 `FIELD_WRITE_FORBIDDEN`**。<br>📌 **本條之存在理由**：使用者原文僅提及「主管/部門窗口」；`OQ-D9-24` 明文排除系統管理員，正是為了防止實作時把「非 ICSOPAdmin 之後台角色」一併放行。
  - 🔴 **[2026-08-27 E11] `AC-N26` 之期望值不變、存在理由消失**（其理由「防止把非 ICSOPAdmin 之後台角色一併放行」隨破例收回而不再適用）：併入 [§OJT 欄位退場 delta](#ojt-field-retire-delta) `AC-J8` 之「五角色皆唯讀」單一斷言。
- **AC-N27**（🔒 一般使用者對 OJT 仍唯讀）：Given 角色為 `User`（`userSubtype` 為 `business` 或 `other` 皆然）, When 對欄位鍵 `OJT 簽到表` 呼叫欄位權限解析函式, Then 回傳「唯讀」；When 呼叫 OJT 上傳端點, Then 回 **403 `PERMISSION_DENIED`**（路由層；該端點之閘門為 `ICSOP 文件管理` read，`User` 為 `NONE`，見 [F025](F025-role-function-matrix.md) `AC-N36`）。
  ...
  - 🔴 **[2026-08-27 E11] `AC-N27` 之期望值不變、存在理由消失**（同 `AC-N26`）：併入 [§OJT 欄位退場 delta](#ojt-field-retire-delta) `AC-J8`。
...
- **AC-N75**（🔴 唯讀頁附件區之 DOM 契約與徽章文案；權威＝`prototypes/16-document-readonly.html`）：Given 後台文件唯讀頁渲染完成, When 檢視附件區, Then 下列**逐字成立**——
  ...
  - 🔴 **[2026-08-27 E11] `AC-N75` 之「`[data-writable-attachment]` 恰 1 個且其列為 `ojt`」已被 [F042](F042-ojt-progress-management.md) `AC-22` 反轉為「恰 0 個」**（四種 `data-attachment-kind` 之列**皆帶** `data-readonly-attachment`），見 [§OJT 欄位退場 delta](#ojt-field-retire-delta) `AC-J11`。🔒 **`data-attachment-kind` 之四值本身維持不變**（`icsop_pdf`／`ojt`／`usageform`／`appendix`）——OJT 仍是一種附件類別，只是不再可寫。**原條文逐字保留於上。**
- **AC-N76**（🔴 編輯頁之 `.ojt-write` 隔離契約與徽章文案；權威＝`prototypes/15-document-edit.html`；lead 追認之超範圍改動）：Given 後台文件**編輯頁**渲染完成, When 檢視 OJT 區塊, Then——
  ...
  - 🔴 **[2026-08-27 E11] `AC-N76` 整條作廢**（編輯頁之 OJT 取代鈕整個移除 ⇒ ①②③④ 四個子條之標的皆不存在），見 [§OJT 欄位退場 delta](#ojt-field-retire-delta) `AC-J11`。<br>🔴 **④ 之逐元素斷言中，`data-attachment-write="xls"` 與 `"icsop_pdf"` 兩條必須保留並改寫為 2 條（原 3 條）**——它們鎖的是**另兩顆鈕仍為 `write-only`**，與 OJT 無關；**不得**因本條整條作廢而順手一併刪除。⚠ 那正是 `AC-N76` ④ 之授權理由所點名之「**有人直接把 `.xls` 上傳鈕的 `.write-only` 整個刪掉**」失誤形狀——集合式斷言在該情況下**仍然全綠**。**原條文逐字保留於上。**

### OJT 欄位退場 delta（🔴 2026-08-27 E11；權威＝[F042](F042-ojt-progress-management.md)） {#ojt-field-retire-delta}

> **本節之性質**：2026-08-20 之 `OQ-D9-19` 推翻本檔頂部定案、**僅為「OJT 簽到表」一欄**對主管／部門窗口開放寫入，是本系統首次也是唯一一次之欄位層破例。
> [F042](F042-ojt-progress-management.md) **把這個唯一的例外收回**——OJT 之登記路徑整批搬到獨立管理頁面（[F042](F042-ojt-progress-management.md) `AC-05`／`AC-22`），文件表單自此對**全部 20 欄皆為徹底唯讀**。
> 🔴 **收回之理由**：**模型已改變**（單份覆蓋式 → 多使用單位 × 多場次），文件表單之欄位形狀承載不下新模型；**不是推翻「主管／部門窗口需要能登記 OJT」之使用者需求本身**。
> **本 delta 之 AC 編號採 `AC-J#`**（配發表見 [F042 §庚](F042-ojt-progress-management.md#reversal-table)；🔴 **禁止續編 `AC-N77` 以後**）。
> ✅ **2026-08-28 人類閘門：`OQ-E11-12`→**A**（OJT 列改「純衍生唯讀」、**五角色皆唯讀**、**欄位鍵集合維持 20**、`AC-N24` 措辭改「全部 20 欄」、案數 **38 → 40**）。本節之 A／B 分支已全數收斂。
> ✅ **outcome 分類連帶確定為 `FORBIDDEN`（唯讀，寫入回 403 `FIELD_WRITE_FORBIDDEN`）**，非 `IGNORE`——理由：裁決逐字為「五角色皆**唯讀**」且明訂 **40 案**，而 40 案之斷言形狀即「一律回 403」；採 `IGNORE`（靜默忽略）則該 40 案無法成立。
> 📌 **與 `OQ-D9-36` 之污染風險現為 moot**：該題仍為「只登錄、本輪不修」，`系統 UUID` 列一格未動 ⇒ 本節之逐格斷言基準乾淨。⚠ **警告保留**：日後啟動 `OQ-D9-36` 時須與本批分批。
> 📌 **逐條反轉之單一真相來源＝[F042 §既有行為反轉總表](F042-ojt-progress-management.md#reversal-table) 乙節**；本節為其落點，不得與之分歧。

- **AC-J7**（🔴 矩陣格值反轉——`AC-N22` 之反轉；`OQ-E11-12`→**A** 定值）：Given `FIELD_MATRIX` 之全部欄位鍵 × 5 種角色, When 逐格取值, Then ——① 欄位鍵集合仍為 **20**、角色仍為 5（共 **100 格**）；② 「OJT 簽到表」列之**五格全數為「唯讀」**（outcome 分類 **`FORBIDDEN`**）；③ ⇒ 與 **D9 delta 導入前**之格值**逐格相同**，即 `AC-N22` 之「恰 2 格改值」**歸零為「恰 0 格」**；④ 其餘 19 列 × 5 欄**逐格不變**。<br>🔴 **格值雖回到 D9 前，語意已不同**：彼時為「尚未開放」，此後為「該欄之值**由系統衍生、無人可寫**」（[F042](F042-ojt-progress-management.md) `AC-04`）。<br>🔴 **outcome 分類為 `FORBIDDEN` 而非 `IGNORE`**（裁決連帶確定）——`IGNORE` 為靜默忽略（比照「系統 UUID」列），`FORBIDDEN` 為回 403；**兩者行為不同**，而 `AC-J9` 之 40 案斷言（「一律回 403 `FIELD_WRITE_FORBIDDEN`」）只在 `FORBIDDEN` 下成立。⚠ **斷言時須一併驗 outcome 分類，不得只驗顯示字面。**<br>📌 **與 `OQ-D9-36` 之交互現為 moot**：該題（「系統 UUID」列顯示值三方不一致）仍為「**只登錄、本輪不修**」，`系統 UUID` 列一格未改 ⇒ 本條之逐格比對基準乾淨。⚠ **警告保留**：日後啟動 `OQ-D9-36` 時**須與本批分批**，否則兩者之逐格斷言互相污染（該題選項 (a) 之代價欄已明載此點）。<br>🔴 **2026-08-28 更正（test-generator 仲裁申訴 10 連帶發現，原句③誤記，保留於上供追溯）**：③「與 D9 delta 導入前之格值逐格相同」為**誤記**——`git show 03cc8f0^:backend/src/rbac/field-matrix.ts` 舉證 D9 之前 `OJT_SIGNIN` 歸在 `ICSOP_WRITABLE` 共用列（`ICSOPAdmin: 'WRITABLE'`），與②「五格全數唯讀」不符（`ICSOPAdmin` 一格於 D9 之前為「可寫」）。**應為**：與 D9 導入前基準**恰 1 格不同**——`OJT簽到表×ICSOPAdmin`（`可寫`→`唯讀`），此即本 delta 真正收斂之處；其餘 99 格（含 OJT 列之 `SysAdmin`／`Supervisor`／`DeptContact`／`User` 四格）與 D9 導入前相同。⚠ **下一位讀者若照③原句把 `ICSOPAdmin` 改回可寫，正是 `AC-J8`「不得只驗曾經破例之兩角色、遺漏 ICSOPAdmin」明文所防之洞**——可測形狀（自我一致性）：`FIELD_MATRIX['OJT簽到表'].ICSOPAdmin` 之逐格斷言須為 `'FORBIDDEN'`，且與「D9 導入前基準」比對時須落在**唯一**之差異清單中（非「與基準逐格相同」）。
- **AC-J8**（🔴 五角色對 OJT 欄皆唯讀——`AC-N23`／`AC-N26`／`AC-N27` 之收斂）：Given 欄位鍵 `OJT 簽到表`（`OQ-E11-12`→A 下**仍存在於集合中**）, When 以 `SysAdmin`／`ICSOPAdmin`／`Supervisor`／`DeptContact`／`User` **五種角色逐一**呼叫欄位權限解析函式, Then **五者皆回傳「唯讀」**（outcome `FORBIDDEN`）；When 任一角色經任何路徑送出對該欄之寫入, Then 回 **403 `FIELD_WRITE_FORBIDDEN`**。<br>🔴 **本條之可測形狀為 5 案全組合，不得只驗主管與部門窗口兩角色**——`AC-N23` 反轉後最可能的失誤是「只把兩格改回唯讀，卻讓 `ICSOPAdmin` 之『可寫』留著」，那會使文件表單上重新長出一個僅對 ICSOPAdmin 可見之上傳入口，直接違反 [F042](F042-ojt-progress-management.md) `AC-22`（該條明文含 `ICSOPAdmin`）。<br>📝 `AC-N26`／`AC-N27` 之期望值於本條中**逐字延續**（僅其「明文排除以防實作誤擴大」之存在理由已不適用）。
- **AC-J9**（🔒 **20 欄回歸鎖定**——`AC-N24` 之措辭改寫，**本 delta 最重要之防護**）：Given 角色為 `Supervisor` 或 `DeptContact`, When 逐一嘗試寫入 **全部 20 個欄位鍵**中的任一個（**含 `OJT 簽到表`**）, Then **一律回 403 `FIELD_WRITE_FORBIDDEN`**、該更新不寫入 DB。<br>📌 **可測形狀**：2 角色 × 20 欄位鍵之**全組合逐案斷言**＝**40 案**（原 `AC-N24` 為 38 案，因 OJT 欄回歸而 +2），**不得**只抽驗其中數欄。<br>🔴 **本條要偵測的失誤與 `AC-N24` 同型但方向相反**：`AC-N24` 防的是「開一個洞、鬆一片牆」；本條防的是「**收回那個洞時，順手把整面牆一起重砌錯**」——例如把 `FIELD_MATRIX` 之整列刪掉卻忘了在服務層補上拒絕分支，造成該欄之寫入請求**靜默通過**（既不寫入也不報錯，或反而被視為未知欄位而忽略）。<br>🔒 **all-or-nothing 語意逐字不變**：一次請求夾帶多欄、其中任一為唯讀即整批拒絕。<br>✅ **案數 40 為裁決明訂之值**（`OQ-E11-12`→A：「措辭改『全部 20 欄』38→40 案」）——**不是 spec-writer 推算**，實作與建環時以此為準。
- **AC-J10**（🔴 `.ojt-write` class 隔離契約整段作廢——`AC-N25` 第三輪擴充之處置）：Given [F042](F042-ojt-progress-management.md) 實作完成, When 以 `Supervisor` 或 `DeptContact` 渲染後台編輯頁, Then ① **`querySelectorAll('.ojt-write').length === 0`**；② **`querySelectorAll('[data-ojt-upload]').length === 0`**；③ 該兩角色之**可寫控制項恰為 0 個**。<br>🔴 **`.ojt-write` 這條 CSS class 本身應自 prototype 與實作一併移除**（不是留著讓它恆空）——留著一條永遠匹配不到元素的 class，下一位讀者無從判斷它是「刻意保留的擴充點」還是「刪漏的殘留」。<br>⚠ **`AC-N25` 之前半（ICSOP PDF／使用表單／附錄之寫入嘗試仍一律拒絕）維持有效、逐字不變**——本條只作廢其第三輪之 class 互斥擴充。<br>📌 **為何必須明文作廢而非留著**：原 ③ 之斷言為「`.ojt-write` 與 `.write-only` 兩集合交集為空」；OJT 鈕移除後 `.ojt-write` 恆為空集合，**空集合與任何集合之交集皆為空** ⇒ 該斷言**恆真而毫無鑑別力**（假綠），與本 repo 已明文禁止之 `offsetParent === null` 同型。
- **AC-J11**（🔴 兩頁 DOM 契約之反轉與作廢——`AC-N75`／`AC-N76` 之處置；權威＝`prototypes/16`／`15`，改版後由棒 4 回寫）：Given 後台文件**唯讀頁**（`16`）渲染完成, When 檢視附件區, Then ① **`querySelectorAll('[data-writable-attachment]').length === 0`**；② 四種 `data-attachment-kind`（`icsop_pdf`／`ojt`／`usageform`／`appendix`）之列**皆帶 `data-readonly-attachment`** 並顯示鎖頭徽章（可見文字逐字 `唯讀`）；③ **不存在**任何 `[data-ojt-upload]` 元素。<br>Given 後台文件**編輯頁**（`15`）渲染完成, Then ④ **不存在** `[data-ojt-exception]` 徽章、**不存在** `[data-attachment-write="ojt"]`；⑤ 🔒 **`[data-attachment-write="xls"]` 與 `[data-attachment-write="icsop_pdf"]` 兩個掛鉤各恰出現一次、其 `classList` 各自含 `write-only` 且不含 `ojt-write`**（原 `AC-N76` ④ 之三條逐元素斷言**縮為 2 條，不得縮為 0 條**）。<br>🔴 **⑤ 為本條最容易被漏掉之處**：`AC-N76` 整條作廢時，最順手的做法是把它底下那張三列的表一起刪掉——**但其中兩列與 OJT 完全無關**，刪掉等於同時撤掉 `AC-N76` ④ 之授權理由所要防的那個失誤（有人把 `.xls` 上傳鈕的 `.write-only` 整個刪掉，集合式斷言仍全綠）。<br>⚠ **`data-attachment-kind` 之四值、`data-readonly-attachment`、`data-field-readonly-note` 三個掛鉤名逐字不變**（`AC-N75` ①③⑤ 續為有效）；改變的只有「哪些列是可寫的」這一件事。<br>📌 **`FIELD_RO_NOTE` 等文案之逐字新值見 [F016](F016-pdf-ojt-attachment.md#ojt-progress-supersede-delta) `AC-J4`**（兩頁共用同一組常數，不得各自重打——`AC-N76` ③ 之既有規則逐字延續）；**逐字值已由 ux-ojt 定稿**，權威＝[F042 §prototype 25 §6](F042-ojt-progress-management.md#prototype-25-dom-contract)，其中 `FIELD_RO_NOTE` 之欄位數**確定為 20**（`OQ-E11-12`→A）。<br>✅ **ux-ojt 已提供本條之逐字可照抄斷言**，見 [F042 §prototype 25 §8](F042-ojt-progress-management.md#prototype-25-dom-contract)——含 `16` 之三條回歸鎖定（四種 `data-attachment-kind` 值不變、每列皆帶 `data-readonly-attachment`）與 `15` 之**三條最易誤刪項**。
