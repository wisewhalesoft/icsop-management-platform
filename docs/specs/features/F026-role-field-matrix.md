...
## Edge Cases
- 主管/部門窗口可下載 ICSOP PDF/使用表單/**附錄**（[F039](F039-appendix-management.md)，同一原則）：**後台下載提供原始檔案**（管理存取，經短效期 SAS URL 核發，伺服器不經手位元組故不燒錄浮水印），但上傳/取代該附件被拒。**浮水印燒錄與調閱稽核僅發生於前台路徑**——涵蓋前台檢視器（[F020](F020-watermark.md) VIEW／DOWNLOAD／PRINT）、**前台文件詳情頁之附件下載（ICSOP PDF／OJT）**、**前台文件詳情頁之附錄下載**、**前台文件詳情頁之使用表單下載**共四者；後台原始下載與前台燒錄下載係「管理存取 vs 消費存取」之刻意區分。（OQ-FM-01 人類裁決，2026-07-24：後台維持 RAW、不接線 PdfBurner。）
