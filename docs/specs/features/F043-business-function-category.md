# F043: 業務/功能類別管理
Priority: P1 | Status: 🟢 **APPROVED（2026-09-02 人類閘門通過，三輪共 12 項裁決）**——`OQ-B-01`～`OQ-B-10` **10 題全數結案**、12 項 `[ASSUMPTION]` **全數確認**、system-architect 8 項已裁定（[第 14 章](../architecture-spec.md#ch14-f043) 決策 E1～E9）；**已核准進入 Phase B（實作）** | Last Updated: 2026-09-02 <br>🔵 **2026-09-22 UX16 delta（項 5／7／8／12／13）：見 [§UX16 delta](#ux16-delta)（`AC-UX27～AC-UX39、AC-UX56`；🟢 裁決全數 APPROVED，見 [open-questions §UX16](../open-questions.md#ux16-2026-09-22)）。**
...
- **AC-55**（🔴 **候選之循環別篩選——使用者主動縮小範圍，非系統過濾**｜🟢 2026-09-04 使用者裁決，實機測試揭露）：
  Given 真庫有 **591 份文件、分屬 14 個循環**，而候選清單依 `documentNumber` 遞增排序、且**文件編號第 2 段即循環代碼**（`ICSOP-**CIPS**-101-1-00`）⇒ 依編號排序等同**依循環分群**，第一頁 20 筆恆為同一循環；且抽屜**無翻頁機制** ⇒ 其餘 569 筆僅能經關鍵字搜尋抵達。
  When 使用者於候選區之「循環別」下拉**主動選取**某循環，
  Then ① 候選僅回該循環之文件；② `candidateTotal` 與 `candidateLifecycleCount` 為**套用該選取後**之全集統計（後者變為 `1`）；③ 🔴 **`candidateLifecycles`（下拉選項來源）仍列出全部循環**——其基準為「keyword／`excludeDocumentIds` 已套用、**使用者所選循環未套用**」之全集分組，否則選定後下拉只剩該循環、使用者**出不來**；④ 選回「全部循環」須完全還原。
  - 🔒 **與 `AC-20` 之分界（本條最重要的不變式）**：`AC-20` 禁的是「**系統靜默地**只給同循環文件」；**使用者主動縮小範圍是另一回事**。兩者必須在程式碼層面長得不一樣——參數逐字為 **`userSelectedLifecycleId`**，**刻意不叫 `lifecycleId`**；`listCandidateDocs` 之**查詢輸入型別永遠不得出現 `lifecycleId` 這個鍵**（既有兩條 `@ts-expect-error` 結構性防線因此**一格未鬆**，新增的是另一個名字的鍵）。
  - 🔒 該參數**無預設值**（不傳＝不過濾，行為與本 delta 前完全一致）、**不得由節點或類別推導**、只能由使用者互動設定。
  - 🔴 **可測形狀之鑑別力要求**：語料必須讓「**當前頁的循環集合**」與「**`candidateLifecycles` 的內容**」**不同**（例如當前頁只含 1 個循環、而選項有 5 個），否則第 ③ 款恆真——而第 ③ 款正是本功能存在的理由（頁內看不到的循環也要選得到）。
  - 📝 UI 決定：下拉選項**不顯示筆數**（沿用本 repo 唯一先例 `ChangeHistoryPage.tsx:906` 之循環別下拉）；`count` 欄位仍回傳，日後要顯示不需改契約。
...
## UX16 delta — 2026-09-22 業務/功能類別五項（項 5 子樹資料／7／8／12／13；`AC-UX#` 批） {#ux16-delta}

> **來源**＝[stories/2026-09-22-ux-delta-16.md](../../stories/2026-09-22-ux-delta-16.md) 第 5／7／8／12／13 項；裁決紀錄＝[open-questions §UX16](../open-questions.md#ux16-2026-09-22)（`OQ-UX16-06`～`08`／`12`～`16`／`23`～`26`，🟢 全數 APPROVED）。
> **本 delta 之 AC 編號採 `AC-UX#`**（編號規則與跨檔範圍見 [F002 §UX16 delta](F002-role-based-routing.md#ux16-delta) 檔頭）。🔴 **明文不沿用本檔既有之 `AC-B#`**（該批已跨三檔配發完畢）。
> 🔴 **人類已裁決之方向（2026-09-22，不得再翻案）**：**B**＝新增真正的排序欄位（`BUSINESS_CATEGORY.sortOrder`，需 migration）｜**D**＝樹狀圖節點內以小字逐列列出各制定公司計數｜**E**＝「DAG 畫布」欄依寫權門控（主管**與系統管理員**皆不顯示）。
> 🔴 **本輪之約束環為簡化版（只有 `backend jest` ＋ `frontend vitest`）** ⇒ **AC 是唯一防線**。
> 🔴 **本 delta 需一支 migration**（`BUSINESS_CATEGORY.sortOrder`）——**單元測試全綠證明不了欄位存在**；驗收條件見 `AC-UX30` 末段。

### 一、前台子樹導向鈕之資料面與 `AC-56` 之前台半句作廢（項 5）

- **AC-UX27**（🔴 **`AC-56` 之前台負向半句作廢；後台正向半句一字不動**）：Given [F019](F019-public-list-browsing.md#ux16-delta) `AC-UX13` 實作完成, Then ——
  - ① 🔴 **本檔 `AC-56` 之「前台負向半句」作廢**（`OQ-UX16-06`＝選項 A，人類 2026-09-22 確認推翻）：其載體為 [F019](F019-public-list-browsing.md) `AC-B20` 末段、`PublicCategoryTreePage.tsx:510-511` 之註解、`prototypes/30-public-category-tree.html:189-190`、`PublicCategoryTreePage.test.tsx:185`～`:191` 之負向鎖定測試——逐處處置見 [F019](F019-public-list-browsing.md#ux16-delta) `AC-UX14` ①～⑦，🔴 **本節不再抄一份**（避免「同一件事兩個定義點」）。
  - ② 🔒 **`AC-56` 之後台正向半句（①～⑤ 全部）一字不動**：後台鈕之逐字文案 `在文件管理中檢視這 {N} 份程序書`、`N` 取自相異文件數、deep link 參數 `businessCategoryId` ＋ `bcNodeSubtreeId`、chip 文案與 ⑤ 之落地形狀——**一律逐字不變**，其既有測試（`BusinessCategoryTreePreviewPage.subtreeDrawer.test.tsx`）**維持綠燈且期望值未經修改**。
  - ③ 🔴 **兩套 deep link 參數自此為三套之其二，三者互不共用鍵名**：循環側 `lifecycleId` + `nodeSubtreeId`（後台）｜類別側 `businessCategoryId` + `bcNodeSubtreeId`（後台）｜**前台子樹**（[F019](F019-public-list-browsing.md#ux16-delta) `AC-UX15` ⑤，鍵名待 system-architect 定案）。🔒 **明文禁止**前台沿用後台之任一組鍵名——兩邊之路由與可見性口徑不同，共用會讓「後台網址貼到前台」產生一個看似成功卻略過可見性檢查的請求形狀。
  - ④ 📌 **資料面**：前台子樹文件查詢沿用既有 `getPublicBusinessCategorySubtreeDocuments`（`/public/business-categories/:id/nodes/:nodeId/subtree-documents`）之**同一份可見性口徑**；🔴 `AC-B22` 之「查詢層過濾、不得先取全量再前端過濾」一字不改。

### 二、樹狀圖容器（項 7 之後台側）

- **AC-UX28**（🔒 **後台樹狀圖預覽頁不受項 7 影響**）：Given [F019](F019-public-list-browsing.md#ux16-delta) `AC-UX17` 實作完成, When 檢視**後台**類別樹狀圖預覽頁（`BusinessCategoryTreePreviewPage`）與循環樹狀圖預覽頁（`LifecycleTreePreviewPage`）之畫布容器, Then 其版面 class 與捲動行為**與本 delta 導入前逐字相同**。<br>
  🔴 **理由**：使用者原文之範圍逐字為「前台-ICSOP 文件瀏覽-業務/功能類別樹狀圖」；後台兩個預覽頁各自另有版面契約（含 `AC-36` 之 PDF 下載與列印之版面前提），順手改它們會在一個**沒有任何斷言覆蓋視覺結果**的輪次裡動到列印輸出。<br>
  📌 **本條為負向回歸鎖**，其鑑別力來自「前台那一頁**確實改了**」——兩者須在同一輪成對斷言（只寫後台不變時，「兩邊都沒改」也會綠）。

### 三、類別排序欄位 `sortOrder`（項 8）

- **AC-UX29**（🔴 **新增排序欄位與後台維護能力；調序介面＝數值輸入框 ＋ 上下移動鈕，兩者皆提供**；`OQ-UX16-34`，人類 2026-09-22 第三輪）：Given 後台「業務/功能類別管理」清單頁, When ICSOP 管理員檢視, Then 每一筆類別具備一個**可維護之排序值** `sortOrder`（整數；🔒 **鎖定之識別子為 `sortOrder`**，🔴 明文禁止 `order`／`seq`／`displayOrder` 等變體），且該列**同時**提供下列**兩種**調序載體——
  - 🔒 **① 數值輸入框**：`aria-label` **逐字**為 **`排序值（{類別顯示名}）`**（`{類別顯示名}` ＝ `businessCategoryDisplayName` 之輸出）；DOM 掛鉤 **`data-sort-order-input`**。
  - 🔒 **② 上移／下移鈕各一**：`aria-label` **逐字**為 **`上移（{類別顯示名}）`**／**`下移（{類別顯示名}）`**；🔒 **其 `title` 與 `aria-label` 同值**（2026-09-22 第四輪補訂；沿用本 repo 既有作法——chip ✕ 之 `AC-UX16`、導向鈕之 `AC-UX13` 皆為「可見文字／`aria-label`／`title` 同值」）；DOM 掛鉤 **`data-sort-up`**／**`data-sort-down`**（皆為 `<button type="button">`）。<br>
  　📌 **可測形狀**：`getByLabelText('上移（{顯示名}）').getAttribute('title')` **等於**該 `aria-label`（下移同）——🔴 **兩者須在同一句斷言中比對**，分開寫時「其中一個忘了跟著改」不會紅。
  - 🔒 **②-b 移動成功後之 toast**（2026-09-22 第四輪補訂，權威＝`prototypes/26-business-category-list.html`；🔴 **編號為 ②-b 而非 ③——本條之 ③ 已由「篩選生效時停用」佔用，見下方**）：其**可見文字逐字**為 ——<br>
  　**`已將「{類別顯示名}」上移一位（排序值 {n}）；前台類別下拉與文件清單之類別篩選皆依同一順序呈現。`**（下移者「上移」二字改為 **`下移`**，其餘一字不差）<br>
  　🔒 **句型沿用第一輪 `setSortOrder` 之既有 toast**（同頁、同一件事的兩種入口，不得各說各話）。<br>
  　🔴 **以 `AC-UX55` 之同一組紀律受檢**：① **使用者語言**——本句回答的是「**我剛做了什麼、它會影響哪裡**」⇒ 🟢 可見；🔴 **明文禁止**出現 `sortOrder`／`data-sort-*`／任何 AC 或不變式編號／「次要排序鍵」「重編號」這類只對實作者有意義的措辭。② **數字必須是可見文字**——`{n}`（該列變更後之新 `sortOrder`）**不得**只存在於 `title`、`aria-label` 或屬性中。<br>
  　🔴 **`{n}` 必須是「寫入後之實際值」，不是「使用者以為的位置」**：ⓓⓔ 之**重編號分支**下 `{n}` 為重編號後之新值（如 `15`）——🔴 語料須涵蓋該分支，否則「toast 印的是舊值」這種錯法在非重編號路徑上看不出來。<br>
  　🔒 **本句與 `AC-UX55` 之匯出 toast 明文不共用常數**（兩者是不同的事）。

  🔒 **`AC-UX31` ③（值相同時以 `name` 昇冪）維持原條文、不放寬**——🔴 **理由必須留檔**：ui-ux-designer 曾正確指出「上下鈕若實作為互換值，**結構上生不出重複值** ⇒ ③ 在畫面上永遠不可達」，但本裁決保留**數值輸入框** ⇒ 使用者仍可把兩列打成同值 ⇒ **③ 仍然可達、仍然必要**。⚠ **若日後有人拿掉輸入框只留上下鈕，③ 才會變成不可達**，屆時要重新裁決的是「要不要拿掉輸入框」，不是「要不要刪掉 ③」。

  🔴 **③ 上下鈕在任何篩選或搜尋生效時必須停用（`disabled`）或不進 DOM**：Given 清單已套用名稱關鍵字或狀態篩選（Main Flow 甲-3）, When 檢視任一列, Then 其 `data-sort-up`／`data-sort-down` **不可用**；When 清除全部篩選, Then 恢復可用。<br>
  　📌 **理由（ui-ux-designer 提報之真實歧義，不得省略）**：在**被篩過**的檢視裡，「上移一位」指的是「**畫面上的前一列**」還是「**完整清單裡的前一列**」——兩種答案都說得通，不裁定就會變成實作者各自解讀，而兩種解讀在同一份語料下會產生不同的資料狀態。<br>
  　📌 **可測形狀**：施加任一篩選後斷言兩顆鈕 `disabled`（或 `queryByLabelText(...) === null`），清除後斷言恢復——🔴 **成對**，只驗其一時「乾脆永遠停用」也會綠。

  🔴 **④ 上下鈕之寫入規則（🔒 三步，恆有效；明文禁止「單純互換兩列的 `sortOrder`」）**：設 `T` 為目標列；在**當前完整清單之排序**（`sortOrder` 昇冪、同值時 `name` 昇冪，`AC-UX31` ③）中，`T` 的前一列為 `P`、`P` 的前一列為 `PP`。**上移**＝
  - **(a)** `P` 不存在（`T` 已在首位）⇒ 該鈕 `disabled`，**不寫入**；
  - **(b)** `PP` 不存在 ⇒ `T.sortOrder = P.sortOrder − 10`；
  - **(c)** 否則 ⇒ `T.sortOrder = ⌊(PP.sortOrder + P.sortOrder) / 2⌋`；🔴 **若該結果等於 `PP.sortOrder` 或 `P.sortOrder`**（間距已用盡，**含 `PP` 與 `P` 平手之情形**）⇒ **先對全表依當前排序重編號為 `10, 20, 30, …`，再重跑本規則一次**。
  - **下移**為其鏡像（`P` → 後一列 `N`、`PP` → `N` 的後一列 `NN`；(b) 為 `T.sortOrder = N.sortOrder + 10`）。
  - 🔴 **為何不能用「單純互換值」（本條存在的全部理由）**：**兩列 `sortOrder` 相等時互換是 no-op** ⇒ 使用者連按無反應、畫面毫無說明。而平手是**可達狀態**（輸入框允許，見上方 🔒）⇒ 這不是理論情形。
  - 📌 **可測形狀（純函式 `moveUp(rows, id)`／`moveDown(rows, id) → rows'`，**七條固定向量**，缺任一條即漏掉一支）**：<br>
    🔴 **標籤逐字為 `A`／`B`／`C`（📝 2026-09-22 第四輪就地改寫，原 `甲`／`乙`／`丙` 之整張表 `OLD>` 逐字保留於本表下方）。**
    | # | 方向 | 輸入（`sortOrder(name)`） | 目標 | 期望 | 它守的是 |
    |---|---|---|---|---|---|
    | ⓐ 一般 | 上移 | `10(A), 20(B), 30(C)` | C | C ＝ `15` ⇒ 次序 `A, C, B` | 基本路徑 (c) |
    | ⓑ **平手** | 上移 | `10(A), 20(B), 20(C)` | C | C ＝ `15` ⇒ 次序 `A, C, B` | 🔴 **互換實作在此為 no-op** |
    | ⓒ 首位 | 上移 | `10(A), 20(B)` | A | 鈕 `disabled`、**無任何寫入** | (a) |
    | ⓓ 間距用盡 | 上移 | `10(A), 11(B), 12(C)` | C | 觸發全表重編號 ⇒ `10, 20, 30` ⇒ C ＝ `15` ⇒ 次序 `A, C, B` | (c) 之 🔴 分支 |
    | ⓔ 三方平手 | 上移 | `20(A), 20(B), 20(C)` | C | 同 ⓓ（重編號後 C ＝ `15`） | (c) 之 🔴 分支之退化形 |
    | **ⓕ 末位之前一步** | **下移** | `10(A), 20(B), 30(C)` | A | A ＝ `⌊(30+20)/2⌋` ＝ `25` ⇒ 次序 `B, A, C` | 🔴 **鏡像之 (c)** |
    | **ⓖ 移到末位** | **下移** | `10(A), 20(B)` | A | A ＝ `N.sortOrder + 10` ＝ `30` ⇒ 次序 `B, A` | 🔴 **鏡像之 (b)——`+10` 而非 `−10`** |
    - 🔴 **ⓕⓖ 兩條非有不可（理由不得省略）**：下移之鏡像中**唯一不對稱的就是 (b)**（`N + 10` 而非 `P − 10`），🔴 **那正是複製貼上最容易寫錯的一行**；只給上移五條時，環**只會建上移那一支**，下移整條路徑零覆蓋。
    - 🔴 **斷言「結果次序」而非只斷言「該列的新值」**：只驗 `C === 15` 時，一個把 `B` 也改壞的實作照樣綠。<br>
    - 🔴 **並須斷言「其餘各列之 `sortOrder` 未被改動」**（ⓓⓔ 之重編號分支除外，該分支改為斷言**相對次序保持不變**）。
    - 🔒 **通則：固定向量的標籤，必須在「被實際使用的那個比較器」下遞增，且最好在所有常見比較器下都遞增。**<br>
    　🔴 **明文禁止**以 `甲/乙/丙`、`一二三`、`上中下` 這類「**閱讀習慣上的序數**」當排序標籤——它們的序數性**只存在於人腦裡，不存在於任何實際的字串比較器裡**。<br>
    　📌 **本輪之實測（lead 2026-09-22 查證）**：`甲`＝U+7532、`乙`＝U+4E59、`丙`＝U+4E19 ⇒ **碼位序 `丙 < 乙 < 甲`｜`localeCompare` `乙 < 丙 < 甲`｜`zh-Hant` `乙 < 丙 < 甲`——三種比較器給出三種順序，沒有一種是遞增**。<br>
    　🔴 **它同時踩中本 repo 兩個既有坑**：① 既有血訓「`localeCompare` 定序隨環境漂移」（[F017](F017-backend-document-list.md) `AC-B9` ② 因此明文禁用它）；② **次序與人腦預期相反** ⇒ 寫條文的人與讀條文的人都不會察覺期望值是錯的。<br>
    　📌 **`A`／`B`／`C` 在三種比較器下皆遞增** ⇒ 向量之期望值**與比較器的選擇無關**；ui-ux-designer 以 `A/B/C` 實跑五條上移向量，輸出與上表期望欄**逐字相同** ⇒ **證明規則本身正確、當時錯的只有標籤**。
    - 📝 **已作廢（⚠ 不得復原、不得用於斷言）**：`OLD>` 原五列之標籤與期望為——ⓐ `10(甲), 20(乙), 30(丙)` 移 丙 ⇒ 丙＝`15`、次序 `甲, 丙, 乙`｜ⓑ `10(甲), 20(乙), 20(丙)` 移 丙 ⇒ 丙＝`15`、次序 `甲, 丙, 乙`｜ⓒ `10(甲), 20(乙)` 移 甲 ⇒ `disabled`｜ⓓ `10(甲), 11(乙), 12(丙)` 移 丙 ⇒ 重編號後 丙＝`15`｜ⓔ `20(甲), 20(乙), 20(丙)` 移 丙 ⇒ 重編號後 丙＝`15`。<br>
    　🔴 **ⓐⓒⓓ 三條之期望值碰巧不受影響**（`sortOrder` 互異 ⇒ 用不到 `name` 次要鍵）；🔴 **錯的恰是 ⓑ 與 ⓔ——本表唯二靠平手觸發次要鍵的兩條**，實跑結果為 ⓑ `丙:0, 甲:10, 乙:20`、ⓔ **無任何寫入**。<br>
    　🔴 **ⓔ 之失敗形狀特別惡劣、值得單獨記住**：平手排序把 `丙` 推到第一列 ⇒ 走 (a) 不寫入 ⇒ **一個完全正確的實作看起來像壞掉的**（按鈕停用、按不動）。而該條原本要守的正是「互換實作在平手時 no-op」——**它自己變成了 no-op 的期望值**。

  🔒 **⑤ 兩套寫入路徑必須產生相同的資料狀態**：Given 同一份初始語料, When ① 以上下鈕達成某個次序、② 由使用者**自行把數值改成同樣次序**, Then 兩者之**排序後 id 序列完全相同**。<br>
  　🔴 **`AC-UX31` 之值域與邊界規則（① 停用仍佔序位、② 新增為 `max+10` 且 `max()` 不分 `status`、③ 同值以 `name` 昇冪）對兩套路徑一體適用**——🔴 **明文禁止**上下鈕走一套「只給鈕用」的旁路規則。

  🔒 **寫權沿用既有閘門**：`canPerform(role, BUSINESS_CATEGORY_MANAGEMENT, 'write')` ⇒ 僅 `ICSOPAdmin` 可調序；`SysAdmin` 與 `Supervisor` 為唯讀（`AC-44`／`AC-45` 之既有規則一字不改，**不新增功能矩陣列**）⇒ 🔒 **唯讀角色下三個調序載體一律不進 DOM**（比照 `AC-UX37` 之處置，非 `disabled`、非 CSS 隱藏）。<br>
  🔒 **DOM 契約**：清單列帶 **`data-sort-order="{值}"`**（值為整數之字串，`0` 亦不得省略）——🔴 它是「這一列的序位是多少」在 DOM 層唯一的機器可讀載體，🔴 **輸入框與上下鈕皆不承載值**（輸入框之 `value` 是編輯中的草稿，不是已持久化的事實）。<br>
  📝 **已作廢（⚠ 不得復原、不得用於斷言）**：`OLD>` 「…且提供**可操作之調序載體**（🔵 具體互動形式由 ui-ux-designer 定稿：可為數值輸入、上下移動鈕或拖曳）。」（2026-09-22 第一輪；同日第三輪由使用者裁定「兩者皆提供」，並明文**不含拖曳**——`OQ-UX16-34`）。

- **AC-UX30**（🔴 **既有類別之初始值＝`name ASC` 逐一編號、間距 10**；`OQ-UX16-14`＝選項 A）：Given `sortOrder` 之 migration 執行, When 對既有類別賦予初始值, Then 其值**等同** `ROW_NUMBER() OVER (ORDER BY name) * 10`（即依 `name` 升冪為 `10, 20, 30, …`；實際 SQL 語法與型別由 system-architect 定案）。<br>
  🔴 **⇒ day-0 之前台下拉次序與上線前逐字相同**（前台現行即依 `name ASC`，`typeorm-public-business-category.store.ts:54`）；🔒 **改變的只有後台清單之次序**（由 `updatedAt DESC` 改為與前台一致）——後台之 `updatedAt DESC` 本身非任何 AC 鎖定之刻意值。<br>
  🔴 **本 migration 之 `ORDER BY name ASC` 刻意維持原樣、不套用 `AC-UX31` ③ 之碼位序修正**（2026-09-22 第六輪補訂；`sa-ux16` 已提出同一警告）：<br>
  　📌 **理由**：本條要的是「**day-0 次序與上線前逐字相同**」，而**上線前的次序就是那支未修正的 SQL 所產生的次序**（走 DB 預設 collation `Chinese_Taiwan_Stroke_BIN`，非碼位序）⇒ **一次性初始化要複製的是「現況」，不是「碼位序」**。<br>
  　🔴 **兩處 `ORDER BY name` 的職責在 migration 執行完那一刻分岔**：<br>
  　　- **本條（migration，只跑一次）**＝**凍結上線前的既有次序** ⇒ 🔒 **維持 SQL 定序、不動**；<br>
  　　- **`AC-UX31` ③（執行期，每次查詢）**＝**平手時之確定性次要鍵** ⇒ 🔒 **由應用層之 `sortByOrderThenName()` 施加碼位序**。<br>
  　🔴 **明文禁止把本 migration 之 `ORDER BY name` 一併改成碼位序**——那會讓 day-0 的前台下拉次序**與上線前不同**，直接違反本條，而「有沒有違反」在上線後**無從回頭比對**（舊次序已不存在）。<br>
  　⚠ **這正是 `sa-ux16` 警告的失誤形狀**：下一個人看到「同一個 repo 裡兩處 `ORDER BY name`、一處改了一處沒改」，會傾向把它當成漏改而順手統一。🔒 **它們不是同一件事，只是碰巧同字。**
  🔴 **間距為 10 而非 1（理由不得省略）**：`AC-UX31` ② 要求新增類別排最後、且允許日後在兩者之間插入而**不需全表重編號**；間距 1 會讓第一次插入就必須重寫整張表。<br>
  📌 **可測形狀**：以三筆 `name` 刻意**不同於** `updatedAt` 次序之語料驅動該初始化之純函式投影，斷言輸出恰為 `[10, 20, 30]` 且其對應之 `name` 為字典序。🔴 語料若讓兩種次序相同，「依 name」與「依 updatedAt」輸出相同，本條恆真。<br>
  🔴 **驗收條件（缺一不可，`migration 寫了 ≠ 已兌現`）**：① 該 migration 對 dev 真庫**實跑 COMMIT**；② 以 `SELECT id, name, sortOrder FROM BUSINESS_CATEGORY ORDER BY sortOrder` **覆核**其值確為 `10, 20, 30, …` 且對應字典序；③ 重建 image 後實際開過一次後台類別清單與前台下拉。**本 repo 已三度重演「migration 寫了但沒對真庫跑」之形狀**（見檔頭實作前置警語）。

- **AC-UX31**（🔴 `sortOrder` 之配置規則）：Given 後台類別管理, Then ——
  - ① **停用之類別仍佔用其序位**（`OQ-UX16-15`）：`sortOrder` 是**純欄位**，🔴 **不隨 `status` 變動而重整**；停用一筆類別**不改變任何其他類別之 `sortOrder`**。<br>　📌 **理由**：重新啟用時次序不會跳掉。<br>　📌 **可測形狀**：以 `[10, 20, 30]` 三筆之語料，停用中間那筆後斷言三筆之 `sortOrder` **仍為 `[10, 20, 30]`**（🔴 不得只斷言「前台下拉少了一筆」——那對「把 30 重編為 20」完全無感）。
  - ② **新增類別之預設 `sortOrder` ＝ `max(sortOrder) + 10`**（`OQ-UX16-16`）⇒ 排在最後；Given 表中無任何類別, Then 首筆為 **`10`**。<br>　🔴 **`max()` 之計算範圍涵蓋全部列、不分 `status`**（與 ① 一致：停用者仍佔序位，若排除它就會產生重複序位）。<br>　📌 **可測形狀**：語料中之最大 `sortOrder` 須落在**一筆已停用之類別**上 ⇒ 斷言新增者為該值 +10。🔴 語料若讓最大值落在啟用類別上，「分不分 status」輸出相同，本條恆真。
  - ③ ⚠ **`sortOrder` 值**不保證唯一、不保證連續**——它是排序鍵而非識別碼。🔒 **值相同時之次要排序鍵為 `name` 昇冪，其「昇冪」＝ 🔒 UTF-16 碼位序（code unit order）**（🟢 `OQ-UX16-35` 已裁決，2026-09-22 第五輪【lead 收斂】）——即 JavaScript `<`／`>` 運算子與 `Array.prototype.sort()` **不帶 comparator 時之預設行為**；🔴 **明文禁止 `localeCompare()`**（含任何帶 `locale`／`Intl.Collator` 之定序）。<br>
  　🔴 **「昇冪」不是一個規格，它是三個（本款之核心，逐字不得刪節）**：<br>
    | 途徑 | 得到的次序 | 本條之處置 |
    |---|---|---|
    | **SQL `ORDER BY name`** | 🔴 **筆畫序為主** —— 本專案**資料庫預設** collation 為 **`Chinese_Taiwan_Stroke_BIN`**（實測 `DATABASEPROPERTYEX(DB_NAME(),'Collation')`，逐字載於 `backend/src/database/migrations/1724025600000-usage-form-number-collation.ts:11`）；🔒 **`BUSINESS_CATEGORY.name` 無任何 collation 覆寫** ⇒ 走資料庫預設 | 🔴 **不等於本條所要求的次序** |
    | **JS `sort()`／`<`** | **UTF-16 碼位序** | 🟢 **本條採用** |
    | **JS `localeCompare()`** | 地區／ICU 相依 | 🔴 **明文排除** |
  　🔴 **`_BIN` 不是 `_BIN2`——這是一個「名字會騙人」的地方，必須逐字留檔**：`Chinese_Taiwan_Stroke_BIN` 之 `_BIN` **只有第一個字元**採 locale 排序權重（此處 locale ＝ `Stroke`，其權重表即**筆畫序**），其餘字元才比位元組；**唯有 `_BIN2` 才保證 100% 依 code point**。<br>
  　　🔴 ⇒ **`Chinese_Taiwan_Stroke_BIN` 與 `Chinese_Taiwan_Stroke_CI_AS` 一樣都不是 UTF-16 碼位序**；SQL 的 `ORDER BY name`（**不論套哪一種本專案既有 collation**）與本條所要求的碼位序**依然是兩回事**。<br>
  　　⚠ **不寫這一句的後果**：下一個人看到 collation 名字裡有 `BIN` 就會推論「那不就是二進位、不就是碼位序嗎」而**把整條缺口判成誤報**，然後把應用層的定序拿掉。<br>
  　📝 **已作廢（⚠ 出處與 collation 名稱更正，2026-09-22 第六輪；🔒 結論與裁決一字未變）**：`OLD>` 「本專案 DB collation 為 **`Chinese_Taiwan_Stroke_CI_AS`**（載明於 `…1723939200000-usage-form-number.ts:17` 與 `…1724025600000-usage-form-number-collation.ts:43`）」。<br>
  　　🔴 **原引之 `_CI_AS` 有兩個問題**：① `…collation.ts:43` 那一處是對 **`USAGE_FORM_POOL.formNumber` 這一個欄位**的覆寫，**不是資料庫預設**；② `…1723939200000:17` 之 `_CI_` 假設**已被同目錄下一支 migration 自己推翻**（該檔 `:15` 明載「§10.7 決策表只列了 `_CI_` 與 `_CS_` 兩格、**漏了 `_BIN`**」）。<br>
  　　📌 **本次更正之意義**：**缺口成立、裁決不變**——換的只是「它為什麼不是碼位序」的具體理由，而**新理由比舊理由更隱蔽**（`_BIN` 這個名字本身就在誤導）。<br>
  　🔴 **⇒ 照抄 SQL 的 `ORDER BY name` 次序是錯的**：[architecture-spec §16](../architecture-spec.md) 原把三處排序統一寫成 `ORDER BY sortOrder ASC, name ASC`，🔴 **該 SQL 在本專案 collation 下產生的不是碼位序、與本條不一致**。<br>
  　🟢 **單一來源機制已由 system-architect 裁定＝路線 (b)**（`sa-ux16`，2026-09-22）：**SQL 只留 `ORDER BY sortOrder`**；`name` 次鍵改由新增之**共用純函式 `sortByOrderThenName()`**（`backend/src/business-categories/business-category-sort.ts`）於**應用層**施加，🔒 **三處消費者共用同一支**。<br>
  　　📌 **否決路線 (a)（SQL 加 `COLLATE` 覆寫）之理由**：`COLLATE` 覆寫會**同時**改變 `INV-B1` 唯一鍵 `(name, subcategory)` 所倚賴的**相等判定**，而本題**只需要排序、不需要改比較語意**。<br>
  　🔴 **不變之明文要求（與路線無關）**：**三處消費者（後台清單、前台下拉、[F017](F017-backend-document-list.md) 第 14 項篩選之下拉，`AC-UX32`）之可觀測次序必須一致且為碼位序**。<br>
  　🔴 **裁決理由（逐字落檔）**：<br>
  　　**①** **與既有兩處先例一致且不矛盾**：[F017](F017-backend-document-list.md) `AC-B9` ②、[F019](F019-public-list-browsing.md#ux16-delta) `AC-UX19` 皆已明訂碼位序並禁用 `localeCompare`。<br>
  　　**②** **與環境無關**。本 repo 已有既有血訓「`localeCompare` 定序隨環境漂移」；而本款有**三處消費者**，任一處挑了不同比較器，就會出現「**同一批類別在同一套系統裡兩種順序**」。<br>
  　　**③** 🔒 **它只是平手時的次要鍵**——主序由管理員自己設定的 `sortOrder` 決定。碼位序對中文讀者「看起來沒有意義」這件事，**在這個位置的成本趨近於零**，而**確定性與三處一致**的價值很高。<br>
  　　**④** 🟢 **改採碼位序不會使任何既有向量失效**：`AC-UX29` ④ 之七條向量已改用 `A/B/C`，其期望值**與比較器的選擇無關**。<br>
  　🔒 **通則（本輪收斂，適用於全 repo）**：**凡規格說「依某欄位排序」，必須指名比較器。**🔴 **「昇冪」不是一個規格，它是三個**（SQL collation／碼位序／`localeCompare`），而三者對中文字串**給出三種不同的答案**。<br>
  　📝 **本缺口之發現與更正過程（留檔）**：spec-writer 於第四輪覆核向量標籤時發現本款**未指定比較器**（lead 原以為條文已指定 `localeCompare`，經覆核為誤記，lead 已自行更正）；🔴 **而 lead 續查 DB collation 後揭露它比原提報**再深一層**——不是「兩種比較器二選一」，是**三種**，且第三種（筆畫序）正是架構師已寫進 §16 的那一條 SQL 會實際產生的。<br>
  　📌 **可測形狀**：以**必然分岔**之中文語料驅動定序純函式（🔴 語料須含一組「碼位序與筆畫序給出不同結果」之同值列），斷言輸出為碼位序；🔴 **不得**以 SQL 查詢結果作為本條之期望值來源。

- **AC-UX32**（🔴 **前後台皆依 `sortOrder` 排序**）：Given `AC-UX29`～`AC-UX31` 實作完成, When 檢視 ——
  - ① **後台類別池清單**（`typeorm-business-category.store.ts:52`，現行 `updatedAt DESC`）→ 改為 **`sortOrder ASC, name ASC`**；
  - ② **前台類別切換下拉**（`typeorm-public-business-category.store.ts:54`，現行 `name ASC`）→ 改為 **`sortOrder ASC, name ASC`**；
  - ③ 🔒 **[F017](F017-backend-document-list.md) 第 14 項「業務/功能類別」篩選之下拉**（`AC-B7` ②③）亦依同一排序——🔴 **三處必須是同一個排序口徑**，否則「後台調了序、某一個下拉沒跟著動」會在畫面上表現為「同一份清單有兩種順序」。
  - 🔴 **可測形狀（本條之全部鑑別力所在）**：語料之 `sortOrder` 必須**同時不同於** `name` 字典序**與** `updatedAt` 次序（如 `name` 為 `甲乙丙`、`sortOrder` 為 `30, 10, 20`）⇒ 斷言三處之輸出順序**皆為** `乙丙甲`。<br>　🔴 語料若讓 `sortOrder` 次序碰巧等於字典序，① ② 兩處「有沒有改」輸出相同，本條恆真——**這正是本 repo「建環語料無鑑別力」之第五形狀**。
  - 🔒 **`AC-B18` 之納入條件一字不改**（前台下拉仍僅納入 `status='active'` 且對該 viewer 至少一份可見文件之類別）：本條改的是**次序**，不是**成員**。

### 四、樹狀圖節點之制定公司統計（項 12）

- **AC-UX33**（🔴 **節點內逐列列出各制定公司計數**；人類裁決 D）：Given 後台類別樹狀圖預覽頁（`BusinessCategoryTreePreviewPage`）已渲染, When 檢視任一節點, Then 其既有徽章 `掛載 {N} 份程序書`／`尚未掛載程序書`（`AC-32`，含 `data-mounted-doc-count`）**一字不改**，並**於其下方**以小字**逐列**列出該節點各制定公司之計數。<br>
  🔒 **DOM 契約**：<br>
  　① 計數列之容器 `role="list"`、`data-testid="node-company-counts"`；<br>
  　② 每一列 `role="listitem"`、**`data-company-code="{companyCode}"`**、**`data-company-doc-count="{n}"`**（值為整數之字串）；<br>
  　③ 每一列之**可見文字逐字**為 **`{公司簡稱} {n}`**（公司簡稱與數字之間恰一個半形空格，例：`和潤 3`）——🔴 數字**必須是可見文字**，不得只存在於屬性中（[F044](F044-admin-dashboard-analytics.md) `AC-G71` 之同一紀律）。<br>
  🔴 **總和不變式 INV-UX1（本項唯一能抓到「查錯欄位」的斷言）**：**Σ 各 `data-company-doc-count` ＝ 該節點之 `data-mounted-doc-count`**，對**每一個**節點皆成立。<br>
  　🔴 **為何非有不可（理由逐字，不得省略）**：既有 `docCountsByNode()` 之形狀是 `try { … } catch { return new Map(); }`（`typeorm-business-category-dag.store.ts`）——照抄這個形狀時，**欄名寫錯就會永遠拿到空明細**，而 `data-mounted-doc-count` 走的是另一支查詢、**仍然正確**，於是任何「計數列存在嗎」「徽章對嗎」之斷言**全部照樣綠**。只有總和不變式會翻紅。<br>
  　📌 **語料鑑別力要求**：語料須含**跨 ≥2 家公司**之掛載（如某節點 `和潤 3`／`和勁 2`，`docCount = 5`）——單一公司時「有沒有拆分」輸出相同，本條恆真。<br>
  🔒 **`N = 0` 之節點**：`AC-32` 之 `尚未掛載程序書` 與 `data-mounted-doc-count="0"` 不變，計數列容器**整個不進 DOM**（0 個公司 ⇒ 沒有列可列；🔴 不得渲染一個空的 `role="list"`，那會讓 `AC-UX34` 之「0 不顯示」失去對照）。

- **AC-UX34**（🔴 計數列之**順序**與**零計數處置**；`OQ-UX16-23`／`OQ-UX16-24`）：Given `AC-UX33` 之計數列, Then ——
  - ① **順序恆依 `companyCode` 昇冪**（`localeCompare` 之字典序），即現行四家逐字為 **`AD` → `AE` → `AJ` → `AS`**；🔴 **明文禁止**依計數大小排序、亦**禁止**依 `COMPANY_FULL_NAMES` 之登錄順序排序。<br>
    　📌 **理由 ①（穩定）**：依計數大小排序會讓同一個節點的列序**隨資料變動而跳動**（今天和潤在上、明天和勁在上），使用者無法建立穩定的閱讀位置。`companyCode` 昇冪與登錄順序同為**資料無關之確定序**，兩者在這一點上等價。<br>
    　📌 **理由 ②（決定性理由——全站單一順序）**：`companyCode` 昇冪**才是使用者在本產品裡到處看到的那一個順序**——`GET /companies`（帳號管理之公司下拉、文件建立／編輯之公司下拉、前台制定公司篩選）全部走 `listSelectableCompanies()` 之 `AD, AE, AJ, AS`。鎖登錄順序等於讓**同一組四家公司在同一套系統裡有兩種排法**，而其中一種（`AS` 排第一）在畫面上**沒有任何地方解釋得出來**——那正是本 repo 反覆記取的「同一個概念兩個定義點」形狀。<br>
    　🔒 **本條鎖的是「呈現順序」這個可觀測結果，不指定實作從哪裡取序**：實作**可以**取用 `listSelectableCompanies()` 之輸出（其順序正好即本條所要者），🔴 惟**仍不得**把它當成「可選公司集合」之第二個定義點——`company-name.ts` 之 **`INV-C1`（`SELECTABLE_COMPANIES ≡ Object.keys(COMPANY_FULL_NAMES)`）一字不動**。<br>
    　🔒 **`__unspecified__` 那一列固定排在最後、不參與昇冪比較**（`AC-UX35`）——它不是一個 `companyCode`。<br>
    　　🔴 **理由（📝 2026-09-22 第三輪就地更正；ui-ux-designer 提報之事實錯誤，規則本身不變）**：`_` 在 **`localeCompare` 之定序**下會排在字母**之前** ⇒ 讓它參與比較會把它推到 `AD` 前面。<br>
    　　⚠ **而它的 UTF-16 碼位（`0x5F` ＝ 95）其實大於大寫字母 `A`–`Z`（65–90）** ⇒ 改用碼位序時它反而落在最後。**兩種比較法給出相反的位置，這正是不能讓它參與比較的更強理由**——排除它之後，`__unspecified__` 殿後這件事就不再取決於採用哪一種字串比較法。<br>
    　　📝 `OLD>` 原括號逐字為「（`_` 之碼位小於字母）」——**該陳述為假**，已更正如上。<br>
    　📌 **可測形狀（🔴 2026-09-22 第三輪就地改寫；原「單一節點須含三家公司」不可達）**：本條之三種鑑別力**分置三個節點、以「跨節點合計」涵蓋**，合計須涵蓋**四家公司 ＋ 一筆 `companyCode` 未指定**——<br>
    　　ⓐ **昇冪鑑別點**：某節點之計數列須**同時含 `AS` 與至少一個字母序在前之公司**（如 `AD`）⇒ 斷言 `data-company-code` 之有序陣列中 **`AS` 在最後**。🔴 **`AS` 恰是登錄順序之首、昇冪之末 ⇒ 它是兩種排法唯一的鑑別點**；語料若不含 `AS`，本條對「改錯成登錄順序」完全無感。<br>
    　　ⓑ **sentinel 殿後**：另一節點含一筆 `companyCode` 未指定之掛載 ⇒ 斷言 `__unspecified__` 為**最後一列**（`AC-UX35`）。<br>
    　　ⓒ **0 不顯示**：第三個節點之掛載全部來自單一公司 ⇒ 斷言 ② 之負向半句。<br>
    　🔴 **為何不能把三者塞進同一個節點（理由不得省略，否則下一個人會「修正」成單節點）**：prototype `29`（後台樹狀圖預覽）與 `30`（前台樹狀圖）**共用同一份示範語料**，其節點掛載數皆自該語料推導；為了湊出「單一節點三家公司」而硬塞第三份文件，會讓該節點在兩支 prototype 之間**失去共同的推導來源**（`29` 之「掛載 N」與 `30` 之可見數不再出自同一批列）。⇒ **語料的形狀是被既有 prototype 一致性約束的，不是可以隨測試需要捏造的**。<br>
    　🔴 **改為跨節點之後仍須逐節點斷言，不得合併成一條「全頁 `data-company-code` 之集合」**——合併後「三個鑑別點各自落在哪一個節點」就不再被驗證，而 ⓐ 之順序斷言必須在**單一節點內**才有意義。<br>
    　📝 **已作廢（⚠ 不得復原、不得用於斷言）**：`OLD>` 「① **順序恆依 `company-name.ts` 之 `COMPANY_FULL_NAMES` 固定登錄順序**，即逐字 **`AS` → `AD` → `AE` → `AJ`**；🔴 **明文禁止**依計數大小排序、亦**禁止**依公司代碼字典序排序。…⚠ **本序與 `listSelectableCompanies()` 之輸出不同**…🔴 **本條鎖定的是登錄順序（`AS` 在首位），不是該函式之輸出**——實作時**不得**直接呼叫 `listSelectableCompanies()` 取序。」（2026-09-22 第一輪之 `OQ-UX16-23` 裁決；同日第二輪由 lead 自行推翻，理由見上方 📌 理由 ②。**該禁令一併解除。**）
  - ② **計數為 0 之公司不顯示**（`OQ-UX16-24`）：Given 某節點之掛載文件全部來自 `AS`, When 渲染計數列, Then 該容器內**恰一列**，`queryByText(/^和勁 0$/) === null`（其餘三家同）。<br>　📌 **理由**：「和運 0」無資訊量，且會讓每個節點無謂地增高四列。<br>　🔴 **可測形狀（成對）**：同一輪中另斷言**該一列確實存在且其值正確**——只寫負向半句時，「整個計數列功能沒做」照樣綠。

- **AC-UX35**（🔴 **`companyCode` 為 NULL 或空字串之歷史列必須有明訂去處**）：Given 語料中存在**掛載於某節點、但其 `companyCode` 為 `NULL` 或空字串**之文件, When 渲染該節點之計數列, Then 該文件**計入一列 `data-company-code="__unspecified__"`**，其可見文字**逐字**為 **`未指定 {n}`**，位置**恆為最後一列**（🔴 **不參與 `AC-UX34` ① 之 `companyCode` 昇冪比較**——它不是一個 `companyCode`；📝 `OLD>` 本括號原為「在 `AJ` 之後」，該字面隨 `OQ-UX16-23` 之第二輪改裁而失效，**順序之權威一律在 `AC-UX34` ①，此處不再重述具體公司代碼**）。<br>
  🔴 **明文禁止靜默丟棄（理由不得省略）**：丟棄會使 `INV-UX1` 之總和**短少**，而 `data-mounted-doc-count` 之徽章**仍然正確** ⇒ 畫面上出現「5 份程序書」但明細只加到 4，且沒有任何斷言會紅（除非本條存在）。<br>
  🔒 **`__unspecified__` 之字面逐字沿用 [F044](F044-admin-dashboard-analytics.md) 之既有 `SEG_UNSPECIFIED_KEY`／`SEG_UNSPECIFIED_LABEL`**（該常數已為「解析不出來的桶」而存在，不另立第二套詞彙）。<br>
  🔒 **與 [F019](F019-public-list-browsing.md#ux16-delta) `AC-UX24` 刻意不同**：前台之本部篩選**不加** sentinel（那是篩選維度，沒有選項就是篩不到）；本條**必須加** sentinel（這是**分組維度**，每一份掛載都必須落進某一列，否則合計對不上）。**兩者不得互相對齊。**<br>
  📌 **可測形狀**：語料須含**一份 `companyCode` 為空之掛載** ⇒ 斷言 `INV-UX1` **仍然成立**。🔴 語料若全部公司別齊全，「有沒有處理空值」輸出相同，本條恆真。

- **AC-UX36**（🔒 **不套用於前台樹狀圖**；`OQ-UX16-25`）：Given [F019](F019-public-list-browsing.md) 前台 `業務/功能類別樹狀圖` 模式已渲染, When 檢視任一節點, Then **不存在** `data-testid="node-company-counts"` 之容器、**不存在**任何 `data-company-doc-count` 屬性；🔒 其節點徽章**仍為** `data-visible-doc-count` 與 `掛載 {N} 份程序書`／`尚未掛載程序書`（`AC-B16` ③／`AC-B21`）、**一字不改**。<br>
  🔴 **理由（不得省略）**：使用者原文逐字僅提及「後台」；且前台之 `docCount` 語意為「**該 viewer 可見數**」、後台為「**全部掛載數**」——[F019](F019-public-list-browsing.md) `AC-B16` ③ 已**明文禁止**兩者共用屬性名，若前台也要公司別統計，該統計同樣須逐列套可見性，複雜度與洩漏風險都顯著不同。<br>
  📌 **可測形狀（成對）**：同一輪中另斷言**後台那一頁確實有**該容器——只寫前台負向半句時，「後台也沒做」照樣綠。

### 五、「DAG 畫布」欄依寫權門控（項 13）

- **AC-UX37**（🔴 **「DAG 畫布」鈕依 `canPerform(role, BUSINESS_CATEGORY_MANAGEMENT, 'write')` 門控**；人類裁決 E）：Given 進入後台「業務/功能類別管理」清單頁, When 該角色對 `業務/功能類別管理` 之權限**不含寫權**（現行矩陣下＝`Supervisor` 主管 **與** `SysAdmin` 系統管理員，兩者皆為 `READ`）, Then 「操作」欄**不含** `DAG 畫布` 按鈕（`queryByText('DAG 畫布') === null`；🔴 **非** `disabled`、**非** CSS 隱藏——CSS 隱藏會讓負向斷言恆真＝假綠）；<br>
  Given 該角色具寫權（現行矩陣下＝`ICSOPAdmin`，`CRUD`）, Then 該鈕**照常顯示**、其導向 `/admin/business-categories/{id}/canvas` 之行為一字不改。<br>
  🔴 **判準必須是 `canPerform(...,'write')` 本身，明文禁止寫成角色清單**（如 `roleCode === 'ICSOPAdmin'`）——`FUNCTION_MATRIX` 是可調整的，寫死角色會讓矩陣與畫面在下一次調整時分歧。<br>
  🔒 **與同儲存格既有三個動作共用同一道閘門**：`編輯`／`停用`／`刪除` 現行即以 `canWrite` 門控（`BusinessCategoryListPage.tsx:248`）⇒ 本條是把 `DAG 畫布` **併入既有那一道閘門**，🔴 **不得**為它另寫第二份判定。<br>
  📌 **可測形狀（本條之全部鑑別力所在）**：以三種可進入本頁之角色（`SysAdmin`／`ICSOPAdmin`／`Supervisor`）逐一渲染同一列，斷言 `DAG 畫布` 存在與否之布林陣列為 **`[false, true, false]`**。<br>
  　🔴 **三種都要驗**：只驗 `Supervisor` 不顯示時，一個「寫死排除主管」的實作會讓 `SysAdmin` 照樣看得到——而那正是人類裁決 E 明確要一併關掉的。<br>
  　🔴 **注入點是 `canPerform` 本身，不是 `FUNCTION_MATRIX`**：mock `canPerform` 即具真實鑑別力（[F044](F044-admin-dashboard-analytics.md) 2026-09-21 之既有結論）；🔴 **禁止**以 spy 斷言「`canPerform` 被呼叫過」代替行為斷言——那抓不到「呼叫了但忽略回傳值」。<br>
  ⚠ **`DeptContact` 與 `User` 對本功能為 `NONE` ⇒ 不進本頁**，不在本條之驗證向量內。

- **AC-UX38**（🔒 **「檢視樹狀圖預覽」鈕維持現狀、不受本次變更影響**；`OQ-UX16-26`＝選項 A）：Given `AC-UX37` 實作完成, When 以**無寫權**之角色（`SysAdmin`／`Supervisor`）檢視同一儲存格, Then 「檢視樹狀圖預覽」圖示鈕（`BusinessCategoryListPage.tsx:218-225`）**仍然存在且可用**——其 `aria-label` 逐字仍為 `檢視「{類別顯示名}」樹狀圖預覽`、`title` 仍為 `檢視樹狀圖預覽（開新分頁）`、其 `window.open` 之固定視窗名稱與目標路由**一格未動**。<br>
  🔴 **本條必須明文寫成 AC**：兩顆鈕住在同一個儲存格裡，下一個人最可能的動作是「順手把整個儲存格包進 `canWrite &&`」。<br>
  📌 **理由（人類裁決）**：嚴格依使用者原文之字面範圍（只提「DAG 畫布」），且「檢視樹狀圖預覽」**本身即為唯讀功能**——對唯讀角色隱藏它會**減損其既有可用能力**（他們現在看得到，改完就看不到了）。<br>
  📌 **可測形狀（成對，與 `AC-UX37` 同一輪）**：同一列、同一角色（`Supervisor`）下斷言 ⓐ `DAG 畫布` **不存在** ∧ ⓑ `檢視樹狀圖預覽` **存在**。⚠ 只寫 ⓐ 時，「整格砍掉」照樣綠。

- **AC-UX56**（🔴 **唯讀角色下「操作」欄之空儲存格呈現**；2026-09-22 第三輪補訂，權威＝`prototypes/26-business-category-list.html`）：Given 角色對 `業務/功能類別管理` **不含寫權**（`SysAdmin`／`Supervisor`）, When 檢視清單之「操作」欄, Then 該儲存格**恆有內容**——其可見文字為**逐字 `—`**（U+2014 em dash），🔴 **不得**是空白格、`null`、`0` 或整個 `<td>` 消失。<br>
  📌 **理由**：沿用本 repo 對「欄位無值」之既有通則（[F017](F017-backend-document-list.md) `AC-B2`、[F019](F019-public-list-browsing.md) `AC-D14` ②、本頁「掛載文件數」為 0 時之既有 `—`）；🔴 **空白格與「這一格壞掉了」在畫面上完全一樣**，破折號是「這裡本來就沒有你能做的事」的唯一可讀訊號。<br>
  🔴 **本條同時是「表頭與列必須同進退」之保險**：`AC-UX37` 移除的是**格內的按鈕**，🔒 **「操作」欄之 `<th>` 與每列之 `<td>` 一律保留**——只拿掉 `<td>` 會讓整張表**錯位一格**，而「按鈕不存在」這條斷言對錯位**完全無感**（[F017](F017-backend-document-list.md) `AC-D17` 已記錄過同型代價）。<br>
  📌 **可測形狀（三段，缺一即漏）**：以 `Supervisor` 渲染 ⇒ ⓐ 該列「操作」`<td>` **存在**；ⓑ 其 `textContent.trim()` **逐字為 `—`**；ⓒ 表頭欄數 **== 每列儲存格數**。🔴 並以 `ICSOPAdmin` 成對斷言該格**不是** `—`（含 `編輯`／`停用`／`刪除`／`DAG 畫布` 四個動作）——只驗唯讀那一側時，「兩種角色都印破折號」照樣綠。<br>
  🔴 **ⓑ 之實作義務：凡以 `textContent` 逐字斷言的格子，其內容物一律不得以 CSS 隱藏**（2026-09-22 第四輪補訂）——🔴 **CSS 隱藏的節點仍留在 `textContent` 裡** ⇒ 一個「用 `.write-only` 之類的 class 把三顆鈕藏起來」的實作，其 `textContent` 實際為 `編輯\n停用\n刪除\n\n—`，**本條看起來可滿足、實際上不可滿足**。<br>
  　🔒 **正確落地＝條件渲染（不進 DOM），畫面可以逐像素相同、改的只是 DOM**。<br>
  　📌 **本款與本 repo 既有紀律同源**：[F042](F042-ojt-progress-management.md) `AC-28` ⑳-(a)「**折疊必須驗『不進 DOM』而非『有沒有 class』**」；`AC-UX37` 之「🔴 **非** `disabled`、**非** CSS 隱藏」亦是同一條。<br>
  　📝 **本款由 ui-ux-designer 於第四輪實作 prototype 時逼出（FYI 留檔）**：`編輯`／`停用`／`刪除` 三顆鈕原以 `.write-only` CSS 隱藏，第一輪實作**其實過不了 ⓑ**（實測 `編輯\n停用\n刪除\n\n—`），已改為條件渲染；並已查證 `.write-only` **仍有其他使用者**（topbar 新增鈕）⇒ 針對該 class 的互斥型斷言**不會退化成假綠**。<br>
  　🔒 **這正是「一條 AC 可不可滿足，要由實際 DOM 決定、不是由條文讀起來合不合理決定」的實例**——⚠ 寫下任何 `textContent` 逐字斷言時，必須同時指定「不該出現的東西**不進 DOM**」，否則條文與實作會各自以為對方錯了。
  ⚠ **本條不涵蓋「檢視樹狀圖預覽」那一格**：該鈕不在「操作」欄內（它與類別名同格，`BusinessCategoryListPage.tsx:218-225`），且依 `AC-UX38` **對所有角色恆存在** ⇒ 那一格永遠不會變空。

- **AC-UX39**（🔒 **F043 之零漣漪回歸鎖定**）：Given 本 delta 全數實作完成, When 逐項檢視, Then 下列**一律與本 delta 導入前逐字相同**——
  - ① **INV-B1～INV-B6 六條不變式**、全部錯誤碼與其 HTTP 狀態、`businessCategoryDisplayName`／`normalizeSubcategory` 之行為；
  - ② **`AC-44`／`AC-45` 之角色權限**（SysAdmin 唯讀／ICSOPAdmin CRUD／Supervisor 唯讀）與 [F025](F025-role-function-matrix.md) 矩陣之任一格——🔴 **本 delta 不動任何權限值**，`AC-UX37` 改的是「畫面有沒有行使既有的那道閘門」，不是閘門本身；
  - ③ **`AC-32` 之節點徽章逐字與 `data-mounted-doc-count` 屬性名**、`AC-35` 之跨組不去重、`AC-36` 之樹狀圖 PDF 下載與列印、`AC-53` 之「前台不提供 PDF」；
  - ④ **`AC-56` 之後台正向半句全部五款**（`AC-UX27` ②）；
  - ⑤ **[F017](F017-backend-document-list.md) 第 16 欄之呈現規則**（`AC-B1`～`AC-B5`）與 **CSV 15 欄**（`AC-B9`）——🔴 `sortOrder` **不進 CSV**、**不改變** `AC-B9` ② 之碼位序規則；
  - ⑥ **`BUSINESS_CATEGORY_DOC` 之 `(nodeId, documentId)` 唯一鍵**與 `INV-B4`（兩套掛載互不干涉）；
  - ⑦ **結構變更歷程與快照**（`AC-38`～`AC-42`／`AC-54`）——🟢 **`sortOrder` 之調整不落入變更歷程，且不為它單獨新增稽核**（`OQ-UX16-32`，🟢 **已裁決**【lead 收斂，2026-09-22 第二輪】）：<br>
    　① 🔒 **不進 `AC-38`～`AC-42`**：該組條文之既有語意是「**DAG 結構**（節點與邊）之變更」，`sortOrder` 是**類別池清單的呈現次序**、不改變任何結構；記進去會讓歷程被大量純排序事件淹沒。<br>
    　② 🔴 **也不為 `sortOrder` 單獨新增 `AUDIT_LOG` 事件**。理由（已查證 `backend/src/business-categories/business-category.service.ts`）：`createBusinessCategory`（`:104`）與 `updateBusinessCategory`（`:119`）**今日皆無任何稽核寫入**，全檔僅 `BUSINESS_CATEGORY_DELETE`（`:196`）有 ⇒ **類別的名稱／說明／狀態編輯本來就沒有稽核**；為 `sortOrder` 單開一條會造成「**改順序有紀錄、改名沒紀錄**」的不對稱，**比缺口本身更難解釋**。<br>
    　③ 🔒 **「類別編輯無稽核」為既有缺口、非本輪引入；本輪刻意不補，另立追蹤項。**🔴 **此句必須留在檔案裡**——否則下一個讀到「調順序查不到是誰調的」的人會以為是本 delta 漏做，而順手補一條只涵蓋 `sortOrder` 的稽核，正好做出 ② 所要避免的那個不對稱。
