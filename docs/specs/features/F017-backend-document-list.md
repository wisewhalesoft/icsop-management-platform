# F017: 後台文件清單與搜尋
Priority: P0-MVP | Status: 🟡 實作（unit 綠；14 欄清單＋9 篩選＋排序分頁；**「檔案」與「連結點程序書」兩欄之後端富化與前端渲染已補**（doc-seams，批次注入不 N+1）；int 已寫未跑，見 implementation-logs/doc-seams-impl.md）｜**循環子分類顯示 delta：🟢 APPROVED（2026-08-07）**｜**連結點程序書欄摺疊 delta：🟢 APPROVED（2026-08-18 使用者裁決，AC-E1～AC-E9）**｜**連結點無 PDF 事前標示 delta：🟢 APPROVED（2026-08-27 使用者裁決，AC-E10～AC-E14；已實作）**｜**清單匯出（CSV）delta：🟢 APPROVED（2026-08-31 使用者裁決，`AC-X1`～`AC-X17`；規格層完成、實作未開始）**｜**業務/功能類別欄 delta：🟢 APPROVED（2026-09-02 人類閘門通過）（`AC-B1`～`AC-B11`；畫面 15→16 欄、篩選 13→14 項、CSV 14→15 欄；權威＝[F043](F043-business-function-category.md)）** | Last Updated: 2026-09-02 <br>🔵 **2026-09-22 UX16 delta（項 11）：見 [§UX16 delta](#ux16-delta)（`AC-UX40～AC-UX44`；🟢 裁決全數 APPROVED，見 [open-questions §UX16](../open-questions.md#ux16-2026-09-22)）。**
...
### 2026-09-08 「循環別」欄與篩選之角色可見性 delta（🔴 使用者裁決） {#cycle-role-visibility-delta}

> **裁決逐字**：「後台-ICSOP 文件管理：角色『主管』及『部門窗口』，隱藏『循環別』清單欄位與篩選欄位」。
> **本節 AC 編號採 `AC-D17`**（沿用 `AC-D#` 批次）。驗證層次：**前端 DOM 層**（不動任何後端判定、不新增錯誤碼、不動 CSV 匯出）。

- **AC-D17**（「循環別」欄與篩選對主管／部門窗口不進 DOM）：Given 登入角色對 `循環管理` 之權限為 `無`（現行矩陣下＝`主管`／`部門窗口`）, When 進入後台文件清單頁, Then ① 表頭**不含** `循環別`；② 每列**不含**該欄之儲存格（`[data-cycle-cell]` 查無）；③ 篩選控制項**不含** `aria-label="循環別"` 者，**桌面篩選列與行動底部 sheet 兩處皆然**；Given 角色對 `循環管理` 為 `唯讀`／`CRUD`（`系統管理員`／`ICSOP管理員`）, Then 三者**皆維持原狀**。<br>
  🔒 **判定與「樹狀圖」欄共用同一個述詞**（`LIFECYCLE_MANAGEMENT read`，見本檔 2026-09-02 之同一條紀律），🔴 **不得**寫成角色清單、🔴 **不得**為本欄另寫第二份判定——三個載體（樹狀圖欄／循環別欄／循環別篩選）是同一個「循環維度」的三種呈現，各寫一份判定時，日後漏改的那一處會讓被隱藏的角色從另一個入口看見同一份資訊。<br>
  🔴 **表頭 `<th>` 與列 `<td>` 必須同進退**：只隱藏其中一邊會使整張表**錯位一格**，而「欄位不見了」這條斷言對錯位**完全無感** ⇒ 建環須以「表頭欄數 == 每列儲存格數」鎖住，並以**精確差值**（可見角色之欄數 − 不可見角色之欄數 **== 2**，樹狀圖＋循環別）斷言——只驗「變少了」時，「乾脆把整個欄群組砍掉」也會綠。<br>
  🔒 **其餘欄之集合與順序、統計卡／排序／分頁、其餘 13 項篩選一律不變**。<br>
  ⚠ **CSV 匯出之 15 欄一字未動**（含 `循環別`）：本輪裁決之範圍逐字為「清單欄位與篩選欄位」，匯出未在其列；**不得**順手改成「依角色輸出不同欄數」——那是另一個裁決，且會使「匯出恆等於畫面所見」與「匯出欄位固定」兩條既有鎖定同時失效。若日後需要，應為匯出另立一條 AC。<br>
  ⚠ **與前台之同日裁決刻意不同構**：[F019](F019-public-list-browsing.md) `AC-D16` 是**不分角色**無條件移除；本條是**依權限**隱藏。明文禁止把兩者「整理」成同一個判定。

### 業務/功能類別欄 delta（🟢 **APPROVED（2026-09-02 人類閘門通過）**，2026-09-02；權威＝[F043](F043-business-function-category.md)） {#business-category-column-delta}

> 🔵 **本節為 DRAFT，未經人類閘門核准前不得實作。** 規則權威＝[F043](F043-business-function-category.md)；本節僅加「本頁確實行使該契約」之 AC。
> **使用者原文**：「另外需要在 ICSOP 文件管理清單新增一欄「業務/功能類別」，需要做成篩選欄位，與比照連結點文件的 UI 顯示方式呈現該文件有掛的業務/功能類別。」
> **人類裁決（2026-09-02，決 4）**：本欄**納入 CSV 匯出**——現行 **14 欄增為 15 欄**，多值以分隔符併為一格。
> **本 delta 之 AC 編號採 `AC-B#`**（B＝2026-09-02 business category 批；跨 [F017](F017-backend-document-list.md)／[F019](F019-public-list-browsing.md)／[F025](F025-role-function-matrix.md) 三檔為 `AC-B1`～`AC-B29`，與既有 `AC-D#`／`AC-E#`／`AC-J#`／`AC-N#`／`AC-S#`／`AC-T#`／`AC-X#` 批次區隔、不重號）。
> 🔒 **本 delta 不動**：既有 15 欄之集合與相對順序、13 項篩選之組成／順序／比對語意、節點子樹 chip、3 張統計卡、排序、分頁、空狀態、第 12 欄之摺疊與無檔案態、CSV 既有 14 個表頭之字面，一律逐字不變（`AC-B11`）。**本 delta 只在最末**各加一欄／一項。

- **AC-B1**（🔴 畫面 15 → **16 欄**，新欄置於最末）：Given 進入後台文件清單頁, When 載入, Then 清單分頁顯示 **16 欄**，第 16 欄（最末欄）之表頭**逐字**為 `業務/功能類別`（**半形斜線 `/`，前後無空白**）；Then 既有第 0～14 欄之集合、相對順序與顯示規則**逐項不變**（`AC-B11`）。<br>📝 **原數值逐字保留供追溯**：本頁欄數演進為 14（2026-07）→ **15**（2026-08-20 `OQ-D9-25` 新增最左 OJT 欄）→ **16**（本 delta）。<br>📌 **置於最末之理由**（`[ASSUMPTION]` A1，見 [F043 §審查清單](F043-business-function-category.md#human-gate-review)）：使用者原文未指定位置；置末對既有欄序**零影響**，且與第 14 欄「循環別」相鄰，兩條分類軸最易對讀。
- **AC-B2**（空值態）：Given 某文件未掛載於任何業務/功能類別節點, When 呈現第 16 欄, Then 顯示 **`—`**（U+2014，比照第 12 欄 0 筆之既有呈現）；**不得**顯示 `0`、空白格或 `null`。
- **AC-B3**（🔴 **pill 摺疊呈現 ＋ 依類別去重**；比照第 12 欄 `AC-E1`～`AC-E3`）：Given 某文件掛載之類別集合（**依 `businessCategoryId` 去重後**）之大小為 `N`, When 呈現第 16 欄, Then ——
  - ① `N = 0` → `—`（`AC-B2`）；
  - ② `N = 1` → **單顆 pill**，其文字為 `businessCategoryDisplayName` 之輸出（有子分類 → `授信（消金）`；無 → `授信`）；🔴 **不得**出現 `+0` 或任何摺疊徽章；
  - ③ `N ≥ 2` → **第一顆 pill ＋ 可點的 `+{N−1}` 徽章**，點擊就地展開列出全部 `N` 顆；再次點擊收合；
  - ④ 該儲存格於收合態**恆為一行高**（比照 `AC-E1` 之既有版面契約）。
  - 🔴 **去重規則（本條之核心）**：同一份文件若掛在**同一類別之多個節點**（[F043](F043-business-function-category.md) `AC-21`），該類別**只呈現一顆 pill**；`N` 為**相異類別數**，**非**掛載列數。<br>📌 **可測形狀（防「數了列數」）**：語料須含一份掛在**同一類別 2 個節點 ＋ 另一類別 1 個節點**之文件（共 3 筆掛載列、2 個相異類別）⇒ 斷言 `N = 2`、pill 文字集合恰為那兩個類別。**只放「一類別一節點」之語料時，列數與類別數恆相等，該斷言等於沒寫。**
- **AC-B4**（pill 本身不可點擊；`[ASSUMPTION]` A11）：Given 第 16 欄之任一顆類別 pill, When 檢視其 DOM, Then 其**不是** `<a>`、**不是** `<button>`、**無** `onClick`、**無** `href`——為純顯示元素；**唯一可互動者為 `+{N−1}` 徽章**（展開／收合）。<br>📌 **理由**：第 12 欄之 pill 可點是因為「連結點程序書」背後有**可下載之 PDF**；類別背後**沒有檔案**，而通往類別樹狀圖之導覽入口不在本頁（見 [F043](F043-business-function-category.md) `AC-32` 之類別池清單入口）。**若人類裁定 pill 應可點擊並導向類別樹狀圖預覽**，本條須改寫並新增一條導向 AC。
- **AC-B5**（展開狀態之列身分；比照 `AC-E5`）：Given 已展開某列之第 16 欄, When 改變篩選或換頁, Then 展開狀態以**列身分（`documentId`）為鍵**，**不得**落到其他列上；該列若已不在結果集中則自然不呈現，**非錯誤**。
- **AC-B6**（🔴 篩選 13 → **14 項**，新項置於最末）：Given 進入後台文件清單頁, When 檢視篩選區, Then 其篩選控制項**恰為 14 個**，順序（桌面：由左至右、逐列換行；行動 sheet：由上而下）逐字為 `制定公司`／`制定部門`／`制定室別`／`當責室長`／`狀態`／`程序書編號`／`程序書書名內`／`公告日期`／`連結點程序書`／`附錄`／`使用表單`／`OJT`／`循環別`／**`業務/功能類別`**；各控制項之無障礙名稱即為上列逐字字串。<br>🔒 **既有 13 項之組成、順序與比對語意（`AC-D1`／`AC-D2`）逐字不變**——本項為第 14 項、置於最末。<br>📝 **原數值逐字保留供追溯**：篩選項數演進為 9（初版）→ **13**（2026-08-16 缺失 delta 第 9 項）→ **14**（本 delta）。
- **AC-B7**（第 14 項之控制項型態、選項來源與比對語意）：Given 展開 `業務/功能類別` 篩選, Then ——
  - ① 控制項型態為**可搜尋下拉（combobox）**（比照 `循環別`），輸入關鍵字時選項即時縮小；
  - ② **選項之顯示字串** ＝ `businessCategoryDisplayName` 之輸出；**選項值** ＝ **`businessCategoryId`**（🔴 **非** `name` 字串——同名不同子分類之兩個類別必須可分別被選取，比照 `AC-S2`）；
  - ③ **預設選項集僅含 `status = 'active'` 之類別**（停用之類別不入下拉）；
  - ④ 比對語意＝「該文件**至少存在一筆**掛載，其節點所屬類別 ＝ 所選 `businessCategoryId`」（**存在量詞**，非等值）；未提供者不施加限制；與其餘 13 項並用為 **AND**。
  - ⚠ **③ 與第 16 欄之呈現刻意不同、不得對齊**：**停用類別之既有掛載仍會出現在第 16 欄**（[F043](F043-business-function-category.md) `OQ-B-04` 現採甲案）——掛載關係並未消失，隱藏會使管理員無法理解「為何這個類別刪不掉」（`BUSINESS_CATEGORY_HAS_DOCUMENTS` 指向一個看不見的關係）。**顯示歷史事實 vs 不引導新篩選，是兩件不同的事。**
- **AC-B8**（篩選命中者排第一顆；比照 `AC-E6`）：Given 已套用 `業務/功能類別` 篩選且某列之命中類別非其第 16 欄之第一顆, When 呈現該列, Then **改以命中者為收合態唯一可見之第一顆**，其餘順序不變（穩定排序，兩段內部各自維持原相對順序）。
- **AC-B9**（🔴 **CSV 14 → 15 欄**；人類裁決決 4）：Given 匯出成功, When 檢視檔案位元組, Then ——
  - ① 第 1 列表頭之欄位由左至右**逐字**為 `OJT,制定公司,制定部門,制定室別,當責室長,狀態,檔案,程序書編號,程序書書名,版次,內容摘要,連結點程序書,公告日期,循環別,業務/功能類別`（**15 欄**）；🔒 **既有 14 個表頭字面與其順序一字不改**，新欄置於最末；
  - ② 第 15 欄之值＝該文件之**相異類別**（去重規則同 `AC-B3`）之 `businessCategoryDisplayName`，以**全形頓號 `、`** 相接（前後**無空白**）；<br>　🔴 **其順序恆為「依 `businessCategoryDisplayName` 之 UTF-16 碼位序（code unit order）遞增」**——即 JavaScript `<`／`>` 運算子與 `Array.prototype.sort()` **不帶 comparator 時之預設行為**；🔴 **明文禁止 `localeCompare()`**（含任何帶 `locale`／`Intl.Collator` 之定序）。<br>　**禁用 `localeCompare` 之理由（不得省略）**：中文之 locale 定序由**執行環境的 ICU 版本**決定——ui-ux 於 2026-09-02 實測，Chromium 之 `localeCompare` 與碼位序對同一組類別名**排出不同順序**。本欄之立條前提（`AC-B10`）是「**穩定、可斷言、與請求無關**」，而 locale 定序**恰好違背它**：期望值會隨執行環境漂移，產生「在某台機器綠、在另一台紅」之經典不穩定測試（CI 容器與開發機的 ICU 幾乎必然不同）。碼位序**無外部相依**，是本 repo 目前唯一能寫死成固定期望值的排序。<br>　📌 **已接受之代價（明列，非缺陷）**：碼位序對中文而言**不是**筆劃序、注音序或任何人類直覺的順序（例：`授信` 與 `帳務處理` 之先後由 Unicode 編號決定）。**這是刻意的取捨**——CSV 為存查用途，可預測性優先於閱讀順序；若日後要求人類可讀之排序，須改為由**後端持有一份顯式的排序鍵**（而非改用 `localeCompare`）。
  - ③ `N = 0` → **空儲存格**（🔴 **非** `—`、**非** `0`——`—` 是畫面之空值佔位符而非資料，[error-handling.md#export](../error-handling.md#export) 值層通則 ＋ `AC-X2` ② 之既有規則）；
  - ④ 🔴 **順序恆依 ② 之碼位序，明文不套用 `AC-B8` 之「命中者置前」**——本欄之 CSV 順序**與請求、與畫面篩選狀態完全無關**（理由與代價見 `AC-B10`：為一個排序細節再開一個匯出 body 鍵，代價高於收益）。<br>　⚠ **此為本欄與第 12 欄之刻意不同**：第 12 欄以 `linkTargetId` 承載命中值故可置前（`AC-X6`），本欄沒有那個鍵。**已於此明列，非疏漏。**<br>　📝 **原條文逐字保留供追溯**：`OLD>` 「④ **順序＝畫面第 16 欄展開後所見之順序**（含 `AC-B8` 之命中者置前）——與 `AC-X6` 對第 12 欄之既有基準（**展開態**）一致；」<br>　🔴 **改寫理由**：原 ④ 與 `AC-B10` 之裁定**直接矛盾**（前者說跟畫面走、後者說恆依字典序），兩條並存會讓下游各取一句、建出兩套互相衝突的期望值。本輪就地收斂為單一規則。
  - ⑤ 其餘一切（UTF-8 BOM／CRLF／RFC 4180 逸出／CSV 注入前綴／列序＝畫面當前排序／0 筆僅表頭／檔名）**沿用 [error-handling.md#export](../error-handling.md#export) 與 `AC-X1`／`AC-X2` 之既有規則，一字不改**。
  - 🔴 **分隔符為全形頓號 `、`、明文禁止半形逗號 `,`**（`[ASSUMPTION]` A3）：<br>　**(a) 禁用逗號之理由**——逗號會觸發 RFC 4180 之引號包覆，使欄內逗號與欄間逗號**在肉眼上無從分辨**（理由逐字同 `AC-X5` ①）。<br>　**(b) 為何是頓號而非分號**——本欄之值是**中文顯示名**（同「當責室長」欄，`AC-X5` 用 `、`）；半形分號 `;` 在本頁專用於**編號**類（`AC-X6` 之 `LINKED_DOC_NUMBER_SEPARATOR`），與中文名並置會不一致。**三欄之分隔符（`、`／`;`／`、`）之異同為刻意，不得統一。**<br>　**(c)** ⚠ 類別顯示名本身可能含**全形括號**（`授信（消金）`），與兩種分隔符皆不衝突。
  - ⚠ **注入前綴之影響**：本欄值若以 `=`／`+`／`-`／`@`／Tab／CR 開頭（類別名為 ICSOPAdmin 可自由輸入之字串 ⇒ **真實可達之注入面**），須先加前導 `'` 再套 RFC 4180 包覆（`AC-X2` ③）；**期望值一律讀作「畫面所見字串經該規則轉換後之結果」**。
- **AC-B10**（🔒 **匯出端點之 body 形狀一格不動**）：Given 本 delta 實作完成, When 檢視 `POST /admin/documents/export` 之 request body, Then 其**仍恰為兩鍵** `{ documentIds: string[]; linkTargetId?: string }`——**不新增** `businessCategoryId` 或任何第三個鍵。<br>🔴 **理由（不得省略）**：第 15 欄之值由**後端依 `documentIds` 自行 join `BUSINESS_CATEGORY_DOC` 取得**，前端無須也不得把畫面算好的字串塞進請求（`AC-X3` 之既有紀律「十四欄全部由後端解析」，本欄為第十五，同此）。多送一個鍵即違反 [§Interface Contract](#interface-contract) 之「恰兩鍵」語意契約——**多一個篩選鍵，就等於在後端開了一扇「也許該重跑一下篩選」的門**，而那正是 `OQ-X-01` 之裁決要關掉的東西。<br>📌 `AC-B8` 之命中者置前需要「命中之類別」這項資訊——🔴 **本 delta 明文裁定：CSV 第 15 欄之順序不套用 `AC-B8` 之命中者置前**，改為**恆依 `businessCategoryDisplayName` 之 **UTF-16 碼位序**（🔴 逐字定義與 `localeCompare` 禁令見 `AC-B9` ②；📝 `OLD>` 原措辭為「字典序」，因未指定定序來源而**可被實作成 `localeCompare`**，2026-09-02 就地精確化）**（穩定、可斷言、與請求無關）。⚠ **此為本欄與第 12 欄之刻意不同**（第 12 欄以 `linkTargetId` 承載命中值），理由＝為了一個排序細節再開一個 body 鍵，代價高於收益。**④ 之「順序＝畫面展開後所見」因此僅適用於畫面，不適用於 CSV**——兩者刻意不同，已於此明列，**非疏漏**。
- **AC-B11**（🔒 **回歸鎖定：本 delta 只增不改**）：Given 本 delta 實作完成, When 逐項檢視, Then 下列**一律與本 delta 導入前逐字相同**——① 既有 15 欄（第 0～14 欄）之集合、順序與各欄顯示規則；② 既有 13 項篩選之組成、順序與 `AC-D2` 之比對語意；③ 節點子樹 chip（`AC-T40`～`AC-T48`）；④ 3 張統計卡；⑤ 排序、分頁與空狀態；⑥ 第 12 欄之摺疊（`AC-E1`～`AC-E14`）；⑦ CSV 既有 14 個表頭之字面與其值層規則（`AC-X1`～`AC-X8`）；⑧ `GET /admin/documents` 之既有參數與回應形狀（🔴 **本欄之資料若需 additive 新增回應鍵，其形狀待 system-architect 裁定**，見 [F043 §待 system-architect](F043-business-function-category.md#for-architect) 第 5 項）；⑨ `backend/src/storage/csv-export.ts` **一行未改**。<br>🔴 **本條為本 delta 之「鬆一片牆」偵測器**：新增一欄時最可能的失誤，是順手重排欄序或把某個既有欄一起「整理」掉。
...
## UX16 delta — 2026-09-22 後台文件清單新增「制定本部」篩選（項 11；`AC-UX#` 批） {#ux16-delta}

> **來源**＝[stories/2026-09-22-ux-delta-16.md](../../stories/2026-09-22-ux-delta-16.md) 第 11 項（使用者原文：「後台-ICSOP 文件管理：增加『制定本部』篩選欄位(依公司組織推導)，於制定公司後、制定部門前」）；裁決紀錄＝[open-questions §UX16](../open-questions.md#ux16-2026-09-22)（`OQ-UX16-19`／`22`，🟢 APPROVED）。
> **本 delta 之 AC 編號採 `AC-UX#`**（編號規則與跨檔範圍見 [F002 §UX16 delta](F002-role-based-routing.md#ux16-delta) 檔頭）。🔴 **明文不沿用 `AC-D#`／`AC-B#`／`AC-T#`／`AC-X#`**。
> 🔴 **本項與既有唯一擴充先例之手法不同**：`AC-B6`／`AC-B7`（2026-09-02）之手法為「**附加於最末、既有順序一字不動**」；本項使用者原文明確指定**插入中段**（制定公司後、制定部門前），將牽動既有順序陣列中第 2 項以後之全部相對位置。**此為預期之轉紅、須就地改寫，非回歸**（`OQ-UX16-22`，人類確認）。
> 🔴 **本輪之約束環為簡化版（只有 `backend jest` ＋ `frontend vitest`）** ⇒ **AC 是唯一防線**。
> 🔒 **本 delta 不動**：畫面 16 欄之集合與相對順序、CSV 15 欄、節點子樹 chip、3 張統計卡、排序、分頁、空狀態、第 12 欄摺疊、第 16 欄之呈現規則（`AC-UX44`）。

- **AC-UX40**（🔴 篩選器組成與順序：**插入中段**）：Given 登入角色對 `循環管理` 之權限**非 `無`**（＝`SysAdmin`／`ICSOPAdmin`）進入後台文件清單頁, When 檢視篩選區, Then 其篩選控制項**恰為 15 個**，順序（桌面：由左至右、逐列換行；行動 sheet：由上而下）**逐字**為 `制定公司`／**`制定本部`**／`制定部門`／`制定室別`／`當責室長`／`狀態`／`程序書編號`／`程序書書名內`／`公告日期`／`連結點程序書`／`附錄`／`使用表單`／`OJT`／`循環別`／`業務/功能類別`；各控制項之無障礙名稱即為上列逐字字串。<br>
  🔴 **篩選數為角色相依**：Given 角色對 `循環管理` 為 `無`（＝`Supervisor`／`DeptContact`，`AC-D17`）, Then `循環別` 不進 DOM ⇒ 控制項**恰為 14 個**，其餘順序不變。<br>
  　🔴 **兩種角色都要驗**：只驗 `ICSOPAdmin` 之 15 時，一個「把 `制定本部` 也一併對主管隱藏」的實作不會紅；只驗 `Supervisor` 之 14 時，`循環別` 被誤刪也不會紅。<br>
  🔒 **控制項型態＝可搜尋下拉（combobox）**，比照其左右兩側之 `制定公司` 與 `制定部門`；🔒 其 DOM id 沿用既有慣例 `cbD_{key}_input`／`cbD_{key}_clear`、`aria-label` ＝ `制定本部`、清除鈕 `aria-label` ＝ **`清除制定本部`**、placeholder 逐字 **`全部`**（`AC-D10` 之既有表格規則擴及本項，🔴 **不新增第二套命名慣例**）。

- **AC-UX41**（🔴 比對語意與選項來源）：Given 已選定 `制定本部` 之某值, When 後端執行查詢, Then ——
  - ① **等值比對**，比對鍵為**本部之組織識別**（🔵 建議 `draftingDivisionId`，形狀由 system-architect 定案，**非顯示名稱字串**）；
  - ② 語意為「該文件之**制定組織沿 `parentCode` 上溯所抵達之本部** ＝ 所選值」⇒ **該本部下轄之全部部與處室之文件皆納入**；
  - ③ 未提供者不施加限制；與其餘各項並用為 **AND**（`AC-D2` 之既有規則擴及本項）；
  - ④ **選項之 `label` 為人類可讀之本部名稱**；🔒 **`value` 之形狀逐字為 `` `${公司代碼}__{本部代碼}` ``**（例：`AS__A0000`；**兩個半形底線**為分隔符）——🔴 **2026-09-22 第三輪補訂**（權威＝`prototypes/13-document-list.html`／`03-public-list.html:463`），原措辭「`value` 為 id 或 code」**未指定形狀、不足以約束下游**。<br>　🔴 **必須是複合鍵**：`ORG_UNIT` 之唯一鍵為 `(companyCode, orgCode)`，5 碼組織代碼**各公司獨立編碼** ⇒ 單用本部代碼會把不同公司的同碼本部併成同一個選項。<br>　🔒 **與前台同一形狀、同一分隔符**（[F019](F019-public-list-browsing.md#ux16-delta) `AC-UX22`；`__` 沿用 `orgGroupKeyOf()` 與 [F044](F044-admin-dashboard-analytics.md) `data-org-key` 之全站既有慣例，**不另立第二種**）——🔴 兩頁共用同一份本部推導實作（`AC-UX43` ④），**值的形狀也必須共用**，否則同一個本部在兩頁會有兩個不同的識別字串。<br>　🔒 **本頁之 deep link 相容性**：本頁已有兩套子樹 deep link 參數（`lifecycleId`+`nodeSubtreeId`、`businessCategoryId`+`bcNodeSubtreeId`，[F043](F043-business-function-category.md) `AC-56` ③）；🔴 本項之參數鍵名由 system-architect 定案，**不得**與該四者任一同名。
  - 📌 **可測形狀（防「只比對了部層」）**：語料須含**同一本部下之兩個不同部**各一份文件 ⇒ 選定該本部時**兩份皆回傳**。🔴 語料若該本部下只有一個部，「上溯到本部」與「直接比對部」輸出相同，本條恆真。
  - 📌 **與前台之同一語意**：[F019](F019-public-list-browsing.md#ux16-delta) `AC-UX22` 為同一推導之另一處落點；🔴 **兩處必須共用同一份本部推導實作**（落點由 system-architect 定案），**不得**各寫一套——本 repo 已於 `org-path.ts:19-22` 記錄過「同一演算法兩份實作分歧」之真實回歸。
  - 🔒 **`AC-D2` 表中既有 13 列之比對語意一字不改**；🔒 **`制定公司`／`制定部門`／`制定室別` 三項之比對鍵與語意亦一字不改**——本項是**新增第四個組織維度**，不是把既有三項改成階層式連動。<br>　📝 **2026-09-24 已推翻**：自 [組織篩選連動 delta](#org-cascade-delta) 起四個組織維度**改為階層式連動**（`AC-OC1`～`AC-OC5`）；本款上文保留為 `OLD>` 追溯。

- **AC-UX42**（🔴 **既有絕對值鎖之就地改寫清單；一律改為回歸鎖，不得只把數字加一**）：Given `AC-UX40` 實作完成, Then 下列**每一處**皆為預期轉紅、須就地改寫（非回歸）——

  | # | 位置 | 現行內容 | 改寫方向 |
  |---|---|---|---|
  | ① | 本檔 `AC-D1`（[§篩選 9 → 13 項 delta](#filter-13-delta)） | 「其篩選控制項**恰為 13 個**」＋逐字 13 項順序 | 就地改為 `AC-UX40` 之**十五項順序**（🔵 本條已由 `AC-B6` 擴為 14，本次為第二次擴充）；🔒 保留其 `OLD>` 演進紀錄 |
  | ② | 本檔 `AC-B6` | 「**恰為 14 個**」＋逐字 14 項順序 ＋ 「🔒 既有 13 項之組成、順序…逐字不變」＋「**本 delta 只在最末**各加一欄／一項」 | 就地改為 **15 項**；🔴 **其「新項只附加於最末」之明文措辭須以 `OLD>` 標註為「該手法僅適用於 2026-09-02 那一批」**——本項是**插入中段**，`OQ-UX16-22` 人類確認 |
  | ③ | 本檔 `AC-B11` ② | 「既有 **13 項**篩選之組成、順序與 `AC-D2` 之比對語意」逐字不變 | 就地改為 **14 項**（＝本 delta 導入前之實況）；🔒 其餘七款一字不動 |
  | ④ | 本檔 `AC-D17` 末段 | 「其餘 **13 項**篩選一律不變」 | 就地改為 **14 項**；🔒 其「表頭與列必須同進退、以精確差值 `== 2` 斷言」之既有形狀**一字不動** |
  | ⑤ | `frontend/src/pages/DocumentListPage.tsx:193-216`（`FILTERS`） | 14 元陣列，`{ key: 'company' }` 之後直接是 `{ key: 'dept' }` | 於 `company` 與 `dept` **之間**插入 `制定本部`；`EMPTY_FILTERS` 與 `FilterKey` 同步 |
  | ⑥ | `frontend/src/pages/DocumentListPage.test.tsx:148` 與 `:188` | 逐字順序陣列（含 `制定公司`／`制定部門` **相鄰**之斷言） | 整段就地改寫為新順序；🔴 **相鄰斷言之語意反轉**（兩者自此**不相鄰**），須改為「`制定公司` 之後恰為 `制定本部`、其後恰為 `制定部門`」 |
  | ⑦ | 🔴 `frontend/src/pages/DocumentListPage.export.test.tsx:428-432` | `expect(controls).toHaveLength(14)` ＋逐字 14 標籤陣列 | 同 ⑥。🔴 **本處住在 export delta 檔裡、與「匯出」主題無關，是本批最容易漏掉的一處**——漏改即整檔紅，且紅的原因與該檔各案主題無關 |
  | ⑧ | `frontend/src/pages/DocumentListPage.filterDelta.test.tsx:187` 及其上方註解（「14 項＝11 個 combobox ＋ 2 個固定值 select ＋ 1 個 role=group 之日期區間」） | 同 ⑦ | 改為 **15 項＝12 個 combobox ＋ 2 個 select ＋ 1 個 group**；🔴 **註解裡的拆解算式也要改**，否則下一個人會照著錯的算式重建語料 |

  - 🔴 **通則（本 repo 血訓，2026-09-02 一天重演三次）**：**新斷言要 N+1、既有他處斷言鎖死 N ⇒ 同一份環不可能全綠。** 每一處都必須改為**回歸鎖**——「既有各項**逐一仍在（含其相對順序）** ＋ 新項**存在且位於 `制定公司` 與 `制定部門` 之間** ＋ 總數恰 15（或 14）」三句並存；🔴 **不得只把數字從 14 改成 15**。
  - 🔴 **grep 時須以「值的字面」為鍵，不得以函式或常數名為鍵**：⑥⑦⑧ 三處之載體各不相同（就地字面／就地字面／`FILTER_LABELS` 類常數），以名稱搜尋必漏。**本批已知落點恰為上表八處**——🔴 **本表是「已知清單」、不是窮舉**，凡表外出現之同型絕對值鎖一律照本條通則處置，不得以「不在表裡」為由放著不管。

- **AC-UX43**（🔴 **本部推導與 `org-path.ts` 之「捨本部層」原則之關係**）：Given 本 delta 與 [F019](F019-public-list-browsing.md#ux16-delta)／[F042](F042-ojt-progress-management.md#ux16-delta) 之同批項目實作完成, Then ——
  - ① 🔴 **`backend/src/org-directory/org-path.ts:9` 之陳述「**捨本部層**——與本專案三級組織模型（公司 / 部 / 處室）及 prototype 一致」須就地更新為**：「**顯示路徑三級（公司 / 部 / 處室）；篩選與分組維度可含本部**」（`OQ-UX16-19`，人類 2026-09-22 確認本部回歸為全站篩選／顯示維度）。
  - ② 🔒 **既有三級顯示欄一格未動**——帳號清單之部門欄、[F020](F020-watermark.md) 浮水印之部門欄、文件之制定部門與制定室別欄：🔴 **本裁決只放寬原則的適用範圍，不改變任何既有欄位的輸出**。<br>　🔴 **可測形狀（本條唯一的鑑別力）**：`orgPathLabel()`（前後端兩份）之既有固定向量測試**維持綠燈且期望值未經修改**——⚠ 只改註解不改行為時，本款自然成立；它防的是**反過來**的那個失誤（有人讀了 ① 就順手把本部加進顯示路徑）。
  - ③ 🔒 **前後端兩份 `org-path.ts` 之同步義務不變**（`org-path.ts:19-22` 之既有警示）：任一側調整取值規則、分隔符或 fallback，另一側必須同步。
  - ④ 📌 **本部推導之實作落點由 system-architect 定案**（是否重用 `backend/src/dashboard/division-resolver.ts` 之 `divisionOf()`、或將其上移為共用地基模組）；🔴 惟**全系統只能有一份本部推導實作**——現行 `divisionOf()` 之唯一消費者為 [F044](F044-admin-dashboard-analytics.md) 儀表板，本批新增**四個**消費者（本檔／[F019](F019-public-list-browsing.md#ux16-delta)／[F042](F042-ojt-progress-management.md#ux16-delta) 之單位名稱與篩選），**第二份實作出現的機率因此上升，而不是下降**。

- **AC-UX44**（🔒 **F017 之零漣漪回歸鎖定**）：Given 本 delta 實作完成, When 逐項檢視, Then 下列**一律與本 delta 導入前逐字相同**——
  - ① **畫面 16 欄之集合、由左至右之相對順序與各欄顯示規則**（`AC-B1`／`AC-B11` ①；🔴 **本 delta 只動篩選、不動欄位**——使用者原文逐字為「篩選欄位」）；
  - ② **CSV 匯出之 15 欄表頭字面、順序與全部值層規則**（`AC-B9`／`AC-X1`～`AC-X8`）——🔴 `制定本部` **不進 CSV**；⚠ 若日後要進，須另立一條 AC（比照 `AC-D17` 之既有紀律）；
  - ③ **匯出端點之 body 恰兩鍵** `{ documentIds, linkTargetId? }`（`AC-B10`）——🔴 **不新增第三個鍵**；
  - ④ **節點子樹 chip 之兩套 deep link**（循環側 `AC-T40`～`AC-T48`、類別側 [F043](F043-business-function-category.md) `AC-56` ③④）、其文案與清除之方向性不對稱（`AC-D8`／`AC-T46`）——🔴 惟 `AC-D8` 之「**13 項**篩選、關鍵字與 chip 三者同時清空」須就地改為 **15 項（或 14 項，依角色）**；
  - ⑤ **3 張統計卡、排序、分頁與空狀態**；
  - ⑥ **`AC-D7` 之當責室長主要∪次要語意**與其與 [F019](F019-public-list-browsing.md) 之共用實作義務；
  - ⑦ **`GET /admin/documents` 之既有參數一律相容**——本 delta 為 **additive**，🔴 未帶新參數之既有請求**行為完全不變**。

## 2026-09-24 組織篩選連動 delta — 制定公司鎖定下級三欄（`AC-OC#` 批） {#org-cascade-delta}

> **來源**＝使用者回報（2026-09-24）：「篩選的欄位在 制定公司、制定本部、制定部門、制定室別 有跨公司名稱重複的混淆問題」；人類裁決（同日）：**① 以「制定公司」鎖定下級欄位，使用者由公司層級往下逐步篩選；更改上級欄位時清空下級欄位**；② **只鎖公司、本部不強制**（推導不出本部之單位須仍可被篩到）；③ **僅一家公司可選時自動帶入**；④ **前台（[F019](F019-public-list-browsing.md#org-cascade-delta)）一起改**。
> **缺陷實況（本 delta 之可測動機）**：後台 `制定部門`／`制定室別` 之選項值與比對鍵皆為**名稱字串**（`DocumentListPage.tsx` 之 `d.draftingDeptName !== filters.dept`）⇒ 兩家公司各有「資訊部」時下拉只剩一個「資訊部」，選了它會**連帶篩出另一家公司的文件**，畫面上無從察覺；`制定本部` 值雖為複合鍵，但 label 為裸本部名稱 ⇒ 兩家各有「管理本部」時下拉出現兩個**一模一樣**的選項。
> 📌 **推翻**：本檔 `AC-UX41` 末款「🔒 `制定公司`／`制定部門`／`制定室別` 三項之比對鍵與語意亦一字不改——本項是新增第四個組織維度，**不是把既有三項改成階層式連動**」——自本日起**改為階層式連動**（`OLD>` 見該款，本 delta 不就地刪除其文字）。
> **本 delta 之 AC 編號採 `AC-OC#`**（前後台共用同一批編號；前台之落點差異見 F019 同名 delta）。

- **AC-OC1**（🔴 鎖定）：Given `制定公司` 未選, Then `制定本部`／`制定部門`／`制定室別` 三個 combobox 為 **disabled**、placeholder 逐字 **`請先選擇制定公司`**；🔒 三者**仍渲染於原位**——`AC-UX40` 之控制項數（15／14）、順序、無障礙名稱一字不變（disabled ≠ 不進 DOM）。Given `制定公司` 已選, Then 三者 enabled、placeholder 回到 `全部`。
- **AC-OC2**（🔴 選項收斂）：Given `制定公司`＝C, Then ——
  - `制定本部` 選項＝C 之文件所衍生之本部；
  - `制定部門` 選項＝C 之文件所衍生之部門，**且若 `制定本部`＝V 已選則僅限 V 所轄**；
  - `制定室別` 選項＝C 之文件所衍生之室別，**且依已選之 `制定本部`／`制定部門` 同理收斂**；
  - 🔴 **本部不強制**：`制定本部` 未選時，`制定部門`／`制定室別` 列出 C 之**全部**部門／室別（含推導不出本部者）——否則無本部之單位永遠選不到。
  - 📌 可測形狀：語料須含兩家公司**同名**部門各一份文件；選定 C₁ 後部門選項**只有一個**該名稱、選取後清單**不含** C₂ 之文件。🔴 只用單一公司語料時本條恆真。
- **AC-OC3**（🔴 改上級清下級）：When `制定公司` 被改值或清除, Then `制定本部`／`制定部門`／`制定室別` **一併清空**；When `制定本部` 被改值或清除, Then `制定部門`／`制定室別` 清空；When `制定部門` 被改值或清除, Then `制定室別` 清空。🔒 **不論原下級值是否仍與新上級相容，一律清空**（人類裁決原文「更改上級欄位時清空下級欄位」）。已套用條件 chip 之 ✕ 走**同一條路徑**（移除 `制定公司` chip ⇒ 三個下級 chip 一併消失）。🔒 下級之改變**不影響**上級。
- **AC-OC4**（🔴 選項識別改以代碼、同名時加註上級）：
  - ① `制定部門`／`制定室別` 之選項值與比對鍵由**名稱字串**改為**單位代碼**（`draftingDeptId`／`draftingSectionId`），label 仍為名稱；因 `AC-OC1` 已保證公司先定，代碼在公司內唯一。
  - ② 收斂後之選項清單中若有 **label 相同**之兩個選項（同公司、不同上級之同名單位）, Then 各自 label 加註上級名稱為 `{名稱}（{上級名稱}）`（部門之上級＝本部；室別之上級＝部門）；上級名稱不可得時加註代碼 `{名稱}（{代碼}）`。label 不重複者**不加註**。
  - ③ 已套用條件 chip 之值文字取選項 label（`{標籤}：{label}`，沿用 2026-09-24 收合 delta 之規則）。
- **AC-OC5**（🔴 單一公司自動帶入）：Given `制定公司` 之選項**恰一個**且使用者**尚未**對 `制定公司` 做過任何操作, When 頁面完成載入, Then `制定公司` 自動帶入該值（下級三欄因而 enabled），並如同使用者選取般計入「已套用 N 項」與 chip。🔒 使用者清除它（combobox 清除鈕、chip ✕、「清除全部篩選」）後**本次瀏覽不再自動帶入**——否則無法清除。選項 ≥ 2 或 0 時不帶入。
- **AC-OC6**（🔒 零漣漪）：`狀態`／`程序書編號`／…／`業務/功能類別` 等其餘 11 項之組成、比對語意一字不變；各項並用仍為 AND；CSV 15 欄、匯出 body、節點子樹 chip、統計卡、排序、分頁不變。
