...
### D9 delta：新增之稽核事件於查詢／匯出之呈現（🔴 2026-08-20；缺失／變更 delta 第 5／8 項之連動） {#d9-audit-view-delta}

> 前提裁決：`OQ-D9-10`（後台燒錄下載寫稽核）／`OQ-D9-23`（OJT 上傳寫稽核）——落列規則之權威＝[F023](F023-audit-logging.md#d9-audit-delta) `AC-N50`／`AC-N51` 與 [data-model AUDIT_LOG](../data-model.md#auditlog-entity)。
> **本 delta 之 AC 編號採 `AC-N#`**。⚠ **本節不改動 F024 之查詢契約、篩選組成、分頁或 CSV 欄位**——僅補齊「新資料落進既有查詢後怎麼顯示」。

- **AC-N53**（OJT 上傳事件之類型歸屬與標籤；🔴 **2026-08-20 第二輪就地修訂**——`OQ-D9-29` 裁決要求本頁能將其**排除／篩出**）：Given `AUDIT_LOG` 中存在一筆 `actionType='ATTACHMENT_UPLOAD'`／`targetType='DOCUMENT_ATTACHMENT'` 之紀錄, When 渲染該列, Then 「類型」欄逐字為 **`上傳`**（`auditKindLabel('DOCUMENT_ATTACHMENT') === '上傳'`）、「操作類型」欄之中文標籤逐字為 **`附件上傳`**（`actionTypeLabel('ATTACHMENT_UPLOAD') === '附件上傳'`）；「對象」欄以該文件之編號／名稱呈現（`documentId` 條件必填，`AC-N50`）；「浮水印快照」欄**留空**（該動作之 `watermarkSnapshot` 為 `null`，本 feature 既有 AC「無浮水印之動作類型該欄留空」逐字適用）。<br>📌 **兩份對照表各新增同一組鍵值**（`backend/src/audit/access-history-labels.ts` 之 `ACTION_TYPE_LABEL`／新增之 kind 對映，與前端 `AccessHistoryPage.tsx` 之 `ACT_LABEL`／`rowKind()`），比照 `access-history-labels.ts` 檔頭所載之「**兩份逐字相同**」既有不變式（前後端無共用 package 之既定處置）。<br>📝 **被修訂之原條文逐字保留供追溯**：「Given `AUDIT_LOG` 中存在一筆 `actionType='ATTACHMENT_UPLOAD'`／**`targetType='DOCUMENT'`** 之紀錄, When 於本頁以類型「**文件**」篩選查詢, Then **該筆納入結果**（`kindToTargetTypes('文件') = ['DOCUMENT','USAGE_FORM','APPENDIX']` 之既有對映**不變**、**不新增第四種類型篩選值**）；When 渲染該列, Then 「類型」欄逐字為 `文件`…」<br>🔴 **修訂理由（`OQ-D9-29`，lead 裁決）**：原案「只解決了一半」——它讓上傳事件落入「文件」類，使**「文件調閱歷程」被非調閱事件污染且無從排除**。現行設計以專屬 `targetType` ＋ 第四種類型篩選值同時滿足「排除」與「篩出」，逐條見 `AC-N69`。
  - 🔴 **[2026-08-28 E11] `AC-N53` 語意改寫**（`OQ-E11-13`→**B**：`actionType` 改為 `OJT_SESSION_UPLOAD`／`OJT_SESSION_DELETE` ⇒ 標籤字面改變），見 [§OJT 進度稽核呈現 delta](#ojt-progress-audit-view-delta) `AC-J22`。🔒 **既有 `ATTACHMENT_UPLOAD` 之標籤對照不得移除**——`AUDIT_LOG` 為 append-only，歷史列永久存在且本頁仍須渲染。**原條文與其 📝 追溯段落逐字保留於上。**
- **AC-N54**（後台燒錄下載列之呈現）：Given `AUDIT_LOG` 中存在後台下載所產生之列（`actionType='DOWNLOAD'`，`targetType ∈ {DOCUMENT, USAGE_FORM, APPENDIX}`）, When 查詢與渲染, Then 其呈現**與前台下載所產生之列完全同形**——「類型」欄為 `文件`、「操作類型」欄為既有標籤 `下載`、「浮水印快照」欄落值（PDF）或留空（非 PDF）；**本頁不新增任何用以區分前台／後台之欄位或篩選**。<br>📌 **`documentId` 為 `null` 之兩種情形**（自表單池管理頁／附錄池管理頁下載）：本頁既有 Edge Case「非文件類型之紀錄無 `documentId` ⇒ 表格與匯出須容許該欄為空並改以『對象』欄呈現」**逐字適用**，不需新規則。<br>⚠ **「前後台於本頁不可區分」已提報為 [open-questions](../open-questions.md) `OQ-D9-30` `[CLARIFY]`**（`OQ-D9-10` 僅裁定「寫稽核，比照前台」，未裁定是否需來源區分欄）；**本輪假設＝不新增區分欄位、零 schema 變更**。
- **AC-N55**（🔒 F024 查詢與匯出之回歸鎖定）：Given 本 delta 實作完成, When 執行本 feature 之全部既有 AC 與 `AC-F1`～`AC-F19`, Then **全數維持綠燈**——查詢之篩選組成、比對語意、30 天預設、RBAC（僅 SysAdmin／ICSOPAdmin）、分頁、明細展開、以及 CSV 匯出之**表頭逐字**（`操作人員,員工編號,公司,部門,處/室,角色,類型,對象（文件／循環）,操作類型,操作時間`）、UTF-8 BOM、RFC 4180 逸出、注入前綴、上限 10,000 與 `EXPORT_ROW_LIMIT_EXCEEDED`、匯出稽核（`ACCESS_HISTORY_EXPORT`）**一律不變**。<br>🔒 **特別鎖定「公司」欄仍為全稱**：[F020](F020-watermark.md#d9-watermark-delta) `AC-N13` 之三處全稱回歸鎖定涵蓋本頁——本頁之「公司」欄與其 CSV 值**不得**因浮水印改用簡稱而連帶改為 `和潤企業`。

- **AC-N69**（🔴 **上傳事件必須可排除、可篩出**——`OQ-D9-29` 裁決之核心兌現）：Given `AUDIT_LOG` 中同時存在 ① 一筆調閱紀錄（`targetType='DOCUMENT'`／`actionType='VIEW'`）與 ② 一筆 OJT 上傳紀錄（`targetType='DOCUMENT_ATTACHMENT'`／`actionType='ATTACHMENT_UPLOAD'`）, When 以下列三種「類型」篩選查詢, Then——
  - ① **類型＝`文件`** ⇒ **僅回傳 ①、不含 ②**（**排除**；`kindToTargetTypes('文件') = ['DOCUMENT','USAGE_FORM','APPENDIX']` **逐字不變**，天然不含 `DOCUMENT_ATTACHMENT`）。
  - ② **類型＝`上傳`**（🔴 **本 delta 新增之第四種類型篩選值**）⇒ **僅回傳 ②、不含 ①**（**篩出**；`kindToTargetTypes('上傳') = ['DOCUMENT_ATTACHMENT']`）。
  - ③ **類型＝`全部`（預設）** ⇒ **兩筆皆回傳**（既有「全部」語意不變）。
  - **且（🔴 2026-08-20 第三輪就地改寫；`OQ-D9-35` 裁決＝採納 ui-ux-designer 之唯一自洽讀法）**：**類型值恰為四種**——`文件`／`循環`／`變更`／`上傳`；**控制項連同既有之預設項「全部類型」共 5 個 `option`**。`上傳` 置於既有三個類型值**之後**，既有三者與預設項之**字面與相對順序逐字不變**。<br>📝 **被推翻之原條文逐字保留供追溯**：「**且**：篩選控制項之選項**恰為四個**，其可見文字由左至右／由上而下逐字為 `全部`／`文件`／`循環`／`變更`／`上傳`——⚠ **`上傳` 置於既有三者之後**，既有三個選項之**字面與相對順序逐字不變**。」<br>🔴 **推翻理由（`OQ-D9-35`）**：原條文**自相矛盾**——寫「恰為四個」卻列出**五個**字面值。designer 如實提報且**未自行選邊**（正確判斷）：採「恰為四個」就必須刪掉 `全部`，而那會直接牴觸**同一條之 ③**（類型＝`全部` ⇒ 兩筆皆回傳）與 `AC-N55` 之查詢回歸鎖定 ⇒ **該讀法不可實作**。<br>📌 **「第四種類型篩選值」一詞自此明確指「類型值之第四個」，不是 `option` 總數**（本檔與 [F023](F023-audit-logging.md)／[data-model](../data-model.md#auditlog-entity) 之相同措辭皆依此讀）。
  - 🔴 **[2026-08-28 E11] 本條 ① 之「類型值恰為四種／控制項共 5 個 `option`」已被推翻**（`OQ-E11-17` 覆核核可）：現為 **類型值恰五種**（`文件`／`循環`／`變更`／`上傳`／**`OJT 場次`**）、**控制項連同預設項 `全部類型` 共 6 個 `option`**。📝 **原條文逐字保留於上供追溯。** 🔒 **既有四者之字面與相對順序逐字不動**，`OJT 場次` 置於其後。見 [§OJT 進度稽核呈現 delta](#ojt-progress-audit-view-delta) `AC-J23`。
  - 🔴 **本條為 `OQ-D9-29` 裁決「F024 必須能將其排除／篩出」之唯一驗證載體**。⚠ **「排除」與「篩出」是兩件事、必須各自斷言**：只驗「選『上傳』看得到」不足以證明「選『文件』看不到」——而後者才是分類學污染之真正防線。
  - 📌 **既有三類之對映一格未動** ⇒ [F039](F039-appendix-management.md) `AC-30`（`targetType='APPENDIX'` 於類型「文件」可查得）等既有 AC **維持綠燈**。
  - 🔴 **[2026-08-27 E11] `AC-N69` 之類型值集合依 `OQ-E11-13`／`OQ-E11-04` 裁決可能擴充**（新 `actionType` 是否落入既有「上傳」類；**刪除動作**是否需第五種類型值），見 [§OJT 進度稽核呈現 delta](#ojt-progress-audit-view-delta) `AC-J23`。🔒 **其核心不變式「排除與篩出是兩件事、必須各自斷言」逐字續為有效**，且**適用範圍擴及新事件**。**原條文逐字保留於上。**
- **AC-N70**（上傳事件於匯出與明細之呈現）：Given `AC-N69` 之查詢結果含一筆上傳紀錄, When 匯出 CSV, Then 該列照常輸出且——「類型」欄之值逐字為 `上傳`、「操作類型」欄之值逐字為 `附件上傳`（**列舉欄一律輸出中文標籤**，沿用 [error-handling.md#export](../error-handling.md#export) 之既有值層通則與 `AC-F5` ②③）、「對象（文件／循環）」欄為該文件之編號／名稱；**CSV 表頭逐字不變**（`AC-F5`／`AC-N55`）。<br>When 展開該筆明細, Then 「浮水印快照」欄**留空**（既有 AC「無浮水印之動作類型該欄留空」）。<br>🔒 **匯出範圍規則不變**：範圍恆為「符合當前查詢條件之全部結果」——若使用者以類型 `文件` 匯出，上傳列**不在匯出檔中**（`AC-N69` ① 之直接後果）。<br>⚠ **不新增匯出欄位、不新增錯誤碼、不改變 10,000 筆上限與 `EXPORT_ROW_LIMIT_EXCEEDED`。**

  - 🔴 **[2026-08-28 E11] `AC-N70` 語意改寫**（`OQ-E11-13`→**B**：`類型` 欄與 `操作類型` 欄之中文標籤隨新 `actionType` 而變；逐字值待 ux-ojt 定於 `prototypes/17`），見 [§OJT 進度稽核呈現 delta](#ojt-progress-audit-view-delta) `AC-J24`。🔒 **CSV 表頭逐字不變、不新增匯出欄位、不新增錯誤碼、10,000 筆上限與 `EXPORT_ROW_LIMIT_EXCEEDED` 不變**（`AC-F5`／`AC-N55`）。**原條文逐字保留於上。**

### OJT 進度稽核呈現 delta（🔴 2026-08-27 E11；權威＝[F042](F042-ojt-progress-management.md)） {#ojt-progress-audit-view-delta}

> **本節之性質**：[F023 §OJT 進度稽核 delta](F023-audit-logging.md#ojt-progress-audit-delta) 決定「新事件怎麼落列」，本節只補齊「**落進來之後在本頁怎麼顯示、怎麼篩、怎麼匯出**」。
> ⚠ **本節不改動 F024 之查詢契約、篩選組成、分頁或 CSV 欄位**——與 `AC-N53`～`AC-N55` 之既有範圍紀律一致。
> **本 delta 之 AC 編號採 `AC-J#`**（配發表見 [F042 §庚](F042-ojt-progress-management.md#reversal-table)；🔴 **禁止續編 `AC-N77` 以後**）。
> ✅ **2026-08-28 人類閘門**：`OQ-E11-13`→**B**（新立 `OJT_SESSION_UPLOAD`／`OJT_SESSION_DELETE`）；`OQ-E11-04`→**A**（僅 ICSOPAdmin 可刪、寫稽核）。
> ✅ **`OQ-E11-17` 已於 2026-08-28 覆核核可**：新 `actionType` 之 **`targetType` ＝ `OJT_SESSION`**（第 9 個，[data-model v1.10](../data-model.md#auditlog-entity)）；**本頁「類型」值由四種增為五種**、控制項共 6 個 `option`。本節已無 `[ASSUMPTION]`。
> 🔒 **兩個新 `actionType` 之中文標籤與第五個類型值之逐字文案＝`prototypes/17-access-history.html` 權威**（ux-ojt 定稿中）——本節**不臆造字面**，僅鎖定可機器驗證之性質（值集合大小、兩標籤必須互異）。
> 📌 **逐條反轉之單一真相來源＝[F042 §既有行為反轉總表](F042-ojt-progress-management.md#reversal-table) 戊節**；本節為其落點，不得與之分歧。

- **AC-J22**（🔴 新事件之標籤——`AC-N53` 之語意改寫；`OQ-E11-13`→**B** 定值）：Given `AUDIT_LOG` 中存在一筆 `actionType='OJT_SESSION_UPLOAD'` 或 `'OJT_SESSION_DELETE'` 之紀錄, When 渲染該列, Then ——① 「操作類型」欄之中文標籤為**兩個互異之逐字值**（`actionTypeLabel('OJT_SESSION_UPLOAD')` ≠ `actionTypeLabel('OJT_SESSION_DELETE')`）；② 「類型」欄之值依 `AC-J23` 之類型歸屬；③ 「對象」欄以該文件之編號／名稱呈現；④ 「浮水印快照」欄**留空**——`AC-N80` 之逐字說明句 `（此動作類型無浮水印，該欄留空）` **續為適用**。<br>✅ **逐字已定稿（ux-ojt，`prototypes/17` 為權威）**——<br>
  | 項目 | 逐字值 |
  |---|---|
  | 第五個「類型」值 | **`OJT 場次`**（sky；🔒 **置於既有四者之後，`文件`／`循環`／`變更`／`上傳` 之字面與相對順序逐字不動**） |
  | `actionTypeLabel('OJT_SESSION_UPLOAD')` | **`場次登記`**（sky） |
  | `actionTypeLabel('OJT_SESSION_DELETE')` | **`場次刪除`**（rose） |

  🔴 **兩個操作類型標籤互異之要求已由定稿兌現**（`場次登記` ≠ `場次刪除`）——⚠ 該性質仍須有**獨立斷言**，不得只驗字面：字面日後若被改動，互異性是那條真正要保住的東西。<br>🔴 **使用單位之承載方式＝既有「對象名稱／說明」欄之文字，不新增欄位**（`AC-J25` 之欄位集合鎖定因此不破）：格式如 **`OJT 場次登記（營運管理部 / 審查室 · 訓練日期 2026-06-18）`**。<br>　✅ **連帶效果（已定為斷言）**：使用單位與訓練日期因此落在既有可搜尋之欄位內 ⇒ **可被本頁既有之文件搜尋框搜到**。<br>　📌 **本設計使「稽核查得到是哪個單位」與「本頁不新增欄位／不新增篩選」兩個要求同時成立**——⚠ 這正是 `AC-J19` 所警告之「`orgCode` 若無處可放即形同虛設」之解法：它有落在 `AUDIT_LOG` 欄位上供機器查詢，也有落在人看得到的說明文字裡。<br>🔴 **① 之「互異」為硬性**：把登記與刪除標成同一個詞，等於在畫面上抹掉兩者之差別——與 `AC-J20` 之 `actionType` 獨立性同源。<br>🔒 **既有 `ATTACHMENT_UPLOAD` 之標籤對照不得移除**：`AUDIT_LOG` 為 **append-only**，2026-08-20～E11 上線期間所產生之歷史列**永久存在**且本頁仍須渲染 ⇒ `actionTypeLabel('ATTACHMENT_UPLOAD')` 與 `kindToTargetTypes` 對 `DOCUMENT_ATTACHMENT` 之既有對映**必須保留**。⚠ **這是最容易「順手清乾淨」而使歷史列渲染成空白之處**。<br>📌 **兩份對照表須各新增同一組鍵值**（`backend/src/audit/access-history-labels.ts` 與前端 `AccessHistoryPage.tsx` 之 `ACT_LABEL`／`rowKind()`），比照該檔檔頭所載之「**兩份逐字相同**」既有不變式——前後端無共用 package 之既定處置，逐字不變。<br>🔒 **本頁不新增「使用單位」欄位、不新增其篩選**（比照 `AC-N54` 對 `OQ-D9-30` 之既有處置）——`orgCode` 落在 `AUDIT_LOG` 之欄位上供追溯，但**本頁之欄位集合與篩選組成逐字不變**（`AC-J25`）。
- **AC-J23**（🔴 新事件必須可**排除**、可**篩出**——`AC-N69` 之延伸）：Given `AUDIT_LOG` 中同時存在 ① 一筆調閱紀錄（`targetType='DOCUMENT'`／`actionType='VIEW'`）與 ② 一筆 OJT 場次登記事件, When 以「類型」篩選查詢, Then ——① **類型＝`文件`** ⇒ **僅回傳 ①、不含 ②**（**排除**）；② **新事件所屬之類型值** ⇒ **僅回傳 ②、不含 ①**（**篩出**）；③ **類型＝`全部`（預設）** ⇒ 兩筆皆回傳。<br>🔴 **「排除」與「篩出」是兩件事、必須各自斷言**——只驗「選對類型看得到」不足以證明「選『文件』看不到」，而後者才是分類學污染之真正防線（`AC-N69` 之既有明文，逐字適用於新事件）。<br>✅ **類型值之集合已定案（`OQ-E11-17` 覆核核可，2026-08-28）**：**新增第 9 個 `targetType` `OJT_SESSION`**（[data-model v1.10](../data-model.md#auditlog-entity)），**本頁「類型」值由四種增為五種**（控制項連同預設項 `全部類型` 共 **6 個 `option`**），`AC-N69` 末子句「類型值恰為四種／共 5 個 `option`」隨之**就地改寫為五種／6 個**。<br>　**定案理由**：① 場次**不是** `DOCUMENT_ATTACHMENT`——`buildAuditRow()` 之 `targetType → targetId` 對映會落到場次 id，沿用舊值等於指鹿為馬；② 沿用既有「上傳」類會使 **`OJT_SESSION_DELETE` 於畫面上顯示為「上傳」**，那是畫面層的謊言，而本 repo 已因同型分類學問題（`OQ-D9-29`）付出過一次代價。<br>　✅ **交叉驗證**：sa-ojt 於獨立收斂 data-model 時得出**同一答案**，兩方獨立推導一致。<br>✅ **第五個類型值逐字為 `OJT 場次`**（ux-ojt 定稿於 `prototypes/17`）；🔒 **既有四者之字面與相對順序逐字不動**。<br>🔴 **`AC-N69` ① 之「類型值恰為四種／共 5 個 `option`」已就地推翻為「恰五種／共 6 個」**（原文逐字保留於該條）。<br>🔒 **兩個新 `actionType` 同屬該第五類型值** ⇒ 選該類型時**兩者皆回傳**；⚠ 若需區分登記與刪除，其手段是「操作類型」欄之**互異標籤**（`AC-J22` ①），**非**再拆一個類型值。<br>🔴 **`OJT_SESSION_DELETE` 之可篩出性亦須成立**（`OQ-E11-04`→A 已開放刪除）：兩個新 `actionType` **同屬**新類型值 ⇒ 選該類型時**兩者皆回傳**；⚠ 若日後需區分登記與刪除，其手段是**「操作類型」欄之互異標籤**（`AC-J22` ①），**非**再拆一個類型值。<br>🔒 **`kindToTargetTypes('文件') = ['DOCUMENT','USAGE_FORM','APPENDIX']` 之既有對映逐字不變** ⇒ [F039](F039-appendix-management.md) `AC-30` 等既有 AC 維持綠燈。<br>🔒 **既有「上傳」類（`DOCUMENT_ATTACHMENT`）之對映亦不得移除**——歷史列仍須可篩出（`AC-J22` 之 append-only 鎖）。
- **AC-J24**（新事件於匯出與明細之呈現——`AC-N70` 之語意改寫；`OQ-E11-13`→**B** 定值）：Given `AC-J23` 之查詢結果含一筆 `OJT_SESSION_UPLOAD` 或 `OJT_SESSION_DELETE` 事件, When 匯出 CSV, Then 該列照常輸出，「類型」與「操作類型」兩欄輸出 `AC-J22` 之**中文標籤**（列舉欄一律輸出中文標籤，沿用 [error-handling.md#export](../error-handling.md#export) 之既有值層通則與 `AC-F5` ②③）、「對象（文件／循環）」欄為該文件之編號／名稱。<br>🔒 **CSV 表頭逐字不變**（`AC-F5`／`AC-N55`）、**不新增匯出欄位、不新增錯誤碼、不改變 10,000 筆上限與 `EXPORT_ROW_LIMIT_EXCEEDED`**。<br>🔒 **匯出範圍規則不變**：範圍恆為「符合當前查詢條件之全部結果」——以類型 `文件` 匯出時，新事件**不在匯出檔中**（`AC-J23` ① 之直接後果）。<br>When 展開該筆明細, Then 「浮水印快照」欄之文字**逐字為** `（此動作類型無浮水印，該欄留空）`（`AC-N80` 之既有逐字契約，適用範圍自此含新事件）。
- **AC-J25**（🔒 F024 查詢與匯出之回歸鎖定）：Given 本 delta 實作完成, When 執行本 feature 之全部既有 AC 與 `AC-F1`～`AC-F19`／`AC-N53`～`AC-N55`／`AC-N69`／`AC-N70`／`AC-N80`／`AC-N81`, Then **除本 delta 明列之改寫外全數維持綠燈**——查詢之篩選組成、比對語意、30 天預設、RBAC（僅 SysAdmin／ICSOPAdmin）、分頁、明細展開、CSV 之 UTF-8 BOM／RFC 4180 逸出／注入前綴、匯出稽核（`ACCESS_HISTORY_EXPORT`）**一律不變**。<br>🔒 **特別鎖定 `AC-N81` 之「同頁刻意分歧」**：表格與 CSV 之「公司」欄仍為**全稱**（`和潤企業股份有限公司`）、浮水印快照仍為**簡稱**（`和潤企業`），**兩者不得被統一**——⚠ 本 delta 新增之事件其 `watermarkSnapshot` 為 `null`，**不得**因「反正這類事件沒有快照」而順手放寬 `AC-N81` 之斷言範圍。<br>🔒 **本頁之欄位集合與 13 項查詢篩選之組成逐字不變**：`AUDIT_LOG` 雖新增 `orgCode` 欄（[F023](F023-audit-logging.md#ojt-progress-audit-delta) `AC-J19` ②），**本頁不新增「使用單位」欄、不新增其篩選**——使用單位改**承載於既有「對象名稱／說明」欄之文字**（`AC-J22`），故欄位集合不破而資訊仍可見、可搜。<br>✅ **浮水印快照之排除已收成具名常數 `NON_WATERMARK_ACTIONS`**（ux-ojt 定稿於 `prototypes/17`），其成員**含兩個新 `actionType`**（`OJT_SESSION_UPLOAD`／`OJT_SESSION_DELETE`）。<br>　🔴 **本常數之目的是「不為無浮水印之動作編造快照」**：`AC-N80` 之逐字說明句 `（此動作類型無浮水印，該欄留空）` 必須**由一份可列舉的集合驅動**，而非散落各處之 `if`——⚠ 漏列一個新動作，畫面就會對它顯示一個**不存在的浮水印欄位**（空字串會被讀成「資料遺失」，正是 `AC-N80` 當初立條要防的事）。<br>　📌 **可測形狀**：斷言 `NON_WATERMARK_ACTIONS` **恰含**目前全部無快照之 `actionType`（含 `ACCESS_HISTORY_EXPORT`、`ATTACHMENT_UPLOAD` 與兩個新值）；新增任何無浮水印動作而未登錄者，本斷言即轉紅。<br>✅ **`prototypes/17` 之示範列刻意保留兩項事實**：① **舊 `ATTACHMENT_UPLOAD` 示範列保留**——`AUDIT_LOG` 為 append-only，稽核歷史不會消失，本頁**永遠**須渲染得出它（`AC-J22` 之 🔒 段）；② **`ICSOPAdmin` 出現於新事件之示範列**——這是「**角色不對稱不延續**」（[F023](F023-audit-logging.md#ojt-progress-audit-delta) `AC-J21` ③）在畫面上之載體。⚠ **兩者皆為刻意，不得當成 mock 資料清掉。**
...
- **AC-N80**（浮水印快照欄之 DOM 契約與留空文案；權威＝`prototypes/17-access-history.html`）：Given 展開任一筆稽核明細, When 檢視「浮水印快照」欄, Then 該欄帶 **`data-wm-snapshot`**；Given 該筆之 `watermarkSnapshot` 有值（`VIEW`／`DOWNLOAD`／`PRINT` 等浮水印動作）, Then 其文字**逐字等於該快照字串**；Given 該筆之 `watermarkSnapshot` 為 `null`（如 `ATTACHMENT_UPLOAD`／`ACCESS_HISTORY_EXPORT`）, Then 其文字**逐字為** **`（此動作類型無浮水印，該欄留空）`**（全形括號）。
  <br>🔴 **本條之必要性**：本 feature 既有 AC 只寫「無浮水印之動作類型**該欄留空**」——「留空」有三種可能實作（空字串／`—`／說明句），**三者對使用者之意義完全不同**：空字串會被讀成「資料遺失」。明訂說明句可使「本來就沒有」與「應該有卻沒寫進去」在畫面上可區分。
  <br>📌 **明列為設計裁量、刻意不入 AC 者**：`ATTACHMENT_UPLOAD` 之操作類型徽章色（`violet`）——比照 `AC-N38`（兩態圖示顏色）與 [F018](F018-usage-form-management.md) `AC-D21` ④（錯誤邊框色票）之既有慣例，色票不入 AC。
- **AC-N81**（🔴 **同頁刻意分歧：浮水印用簡稱、表格與 CSV 用全稱——不得統一**）：Given 本頁同時呈現 ① 表格之「公司」欄與其 CSV 匯出值、② 展開明細之浮水印快照（`data-wm-snapshot`）, When 同一筆紀錄之操作者所屬公司為 `AS`, Then——
  - ① **表格「公司」欄與 CSV 之對應欄位逐字為 `和潤企業股份有限公司`**（**全稱**；`AC-N13` ③ 之回歸鎖定）；
  - ② **同一列展開後之浮水印快照字串中，公司名稱片段逐字為 `和潤企業`**（**簡稱**；`AC-N12`）；
  - ③ 兩者**同時成立於同一頁面、同一筆紀錄**，且**不得**被「統一」為其中任一種。
  - 🔴 **本條之存在理由（不得省略）**：這是全系統中**唯一**能同時看到 `AC-N12`（浮水印用簡稱）與 `AC-N13` ③（稽核公司欄用全稱）兩條規則之畫面。**兩者不同源是刻意的**——浮水印之公司名稱取自 `COMPANY_SHORT_NAMES`（`OQ-D9-06` 選項 A：推翻範圍嚴格限於浮水印），稽核公司欄取自 `COMPANY_FULL_NAMES`（三處全稱消費點之一）。
  - ⚠ **若不立本條，任何人（含日後之 AI agent）在同一頁看到同一家公司出現兩種寫法，都會判定為 bug 並「修正」為一致**——而那個「修正」會直接推翻 `OQ-D9-06` 之使用者裁決，且**兩個方向都錯**：改成全都用簡稱會波及 F003 帳號管理與 `GET /companies`；改成全都用全稱則使 #3 需求（浮水印縮短）失效。
  - 📌 **`watermarkSnapshot` 為稽核當下之字串快照**，非顯示時再組裝 ⇒ ② 之值來自寫入時之浮水印，本條同時鎖住「快照不得於顯示層被改寫」。
...
- **2026-08-20 使用者裁決（D9 delta）**：`OQ-D9-10`（後台燒錄下載寫稽核）／`OQ-D9-23`（OJT 上傳寫稽核）之**顯示側連動**，見 [§D9 delta](#d9-audit-view-delta)（`AC-N53`～`AC-N55`）。**新增 OQ（交回 lead）**：`OQ-D9-29`（調閱歷程表承載寫入事件之分類學衝突）、`OQ-D9-30`（前後台稽核列不可區分）。🔒 **本頁「公司」欄仍為全稱**（[F020](F020-watermark.md#d9-watermark-delta) `AC-N13`）。
