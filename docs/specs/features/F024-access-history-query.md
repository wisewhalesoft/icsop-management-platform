...
- **AC-N53**（OJT 上傳事件之類型歸屬與標籤）：Given `AUDIT_LOG` 中存在一筆 `actionType='ATTACHMENT_UPLOAD'`／`targetType='DOCUMENT'` 之紀錄, When 於本頁以類型「**文件**」篩選查詢, Then **該筆納入結果**（`kindToTargetTypes('文件') = ['DOCUMENT','USAGE_FORM','APPENDIX']` 之既有對映**不變**、**不新增第四種類型篩選值**）；When 渲染該列, Then 「類型」欄逐字為 `文件`、「操作類型」欄之中文標籤逐字為 **`附件上傳`**（`ACTION_TYPE_LABEL` 與前端 `ACT_LABEL` 兩份對照表**各新增同一組鍵值**，比照 `access-history-labels.ts` 檔頭所載之「兩份逐字相同」不變式）；「浮水印快照」欄**留空**（該動作之 `watermarkSnapshot` 為 `null`，本 feature 既有 AC「無浮水印之動作類型該欄留空」逐字適用）。<br>⚠ **本項使「文件調閱歷程」開始承載一筆非調閱之寫入事件**——此分類學衝突已提報為 [open-questions](../open-questions.md) `OQ-D9-29` 交回 lead；**在該題定案前，本條為現行規格**（`OQ-D9-23` 逐字裁定寫入 `AUDIT_LOG`）。
