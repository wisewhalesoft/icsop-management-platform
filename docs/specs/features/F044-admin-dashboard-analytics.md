...
#### `AC-G89` — 卡④ 之排除註記純函式 `ojtOnTimeNote`：**不共用** `exclusionNote()`
...
#### (i) 🔴 兩個「人類／agent 覆核抓到、機器抓不到」之實例（本輪紀錄）

> 🔴 **本節不是語料要求，是對下一輪的警語。** 本輪發生兩次「規格是對的、測試也綠的，但那條 AC 實際上已經沒有人在驗」，兩次都是 **ui-ux-designer 主動提報**才發現。
