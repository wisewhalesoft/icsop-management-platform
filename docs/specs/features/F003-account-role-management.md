...
- **AC-P13（部門主檔＝沿用既有 `GET /org-units`，不新增端點）**：Given 已登入之使用者, When 呼叫 `GET /org-units?companyCode={code}`（既有端點，預設僅回 `isActive=true`）, Then 回 `OrgUnitRecord[]`；權限沿用既有 `PUBLIC_BROWSING` read（5 角色皆可，[org-directory.controller](../../../backend/src/org-directory/org-directory.controller.ts) 既有裁定），**不新增 F025 功能鍵、不改動既有權限**。部門下拉之候選＝該回應之全部列（**含最上層 `ROOT`**），依 `orgCode` 昇冪排序；**不得**限縮 tier——上游帳號之 `orgCode` 實測分布於 `ROOT`／`DIVISION`／`DEPARTMENT`／`SECTION`／`SUBSECTION` 各層，限縮將使手動帳號無法與上游帳號同層對齊。（2026-09-24 修訂：原排除 `ROOT`，但 dev 真庫實查上游帳號確有掛在 `ROOT` 者——AS 2／AD 3／AJ 3，總經理層級——排除即無法建立對應之手動帳號，例如模擬總經理；使用者裁定開放。）
