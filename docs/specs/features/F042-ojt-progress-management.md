# F042: OJT 進度管理
Priority: P1 | Status: 🟢 **規格定稿（2026-08-28 人類閘門對 `OQ-E11-01`～`OQ-E11-16` 全數裁決）；Phase A 完成，待閘門核可整包** | Last Updated: 2026-08-28
...
|---|---|---|---|---|---|---|
| `/admin/ojt-progress/summary` | GET | TAB1 儀表板三區（`AC-14`／`AC-15`／`AC-16`） | 無 | `{ coverage: { numerator, denominator, rate, excludedInactive, excludedOrphaned }, docCoverage: [{ documentId, documentNumber, documentName, state, totalUnits, completedUnits }], deptRollup: [{ deptOrgCode, deptName, totalUnits, completedUnits, rate }], recentSessions: [{ documentId, documentNumber, documentName, orgCode, orgName, trainingDate }] }`——三區公式定值見 `AC-14`～`AC-16`／[data-model.md §建議查詢形狀](../data-model.md#ojt-session-query-shape)（`denominator=0` 時 `rate` 省略，前端呈現「尚無可統計之進度列」；`recentSessions` 明文不含上傳者姓名/員編，`AC-16` PII 防線） | `canPerform(role, OJT_PROGRESS_MANAGEMENT, 'read')` | `PERMISSION_DENIED`（403） |
...
| 項目 | 為何不得建環 | 處置 |
|---|---|---|
| 區三之**筆數上限** | `OQ-E11-07` **只裁了窗口天數（30 天），未裁上限**。prototype 目前**不設上限、全部列出**——此為 ux-ojt 之裁量值 | 🔒 **不入 AC**；若日後資料量使該區過長，屬另案 |
