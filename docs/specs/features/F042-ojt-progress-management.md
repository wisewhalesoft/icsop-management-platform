...
> 🔴 **2026-09-01 · 第三輪實機需求（人類核可）：TAB2「OJT 資料清單」新增第二種分組模式「以文件分組」**——現況 TAB2 恆以**使用單位**為群組（`AC-11`），要回答「這一份文件，各使用單位辦得如何」必須把同一份文件散落在各單位群組下的列逐一湊起來。本輪新增**分組模式切換（恰二態）**：`以使用單位分組`（🔒 **預設**，＝現況一格不改）／`以文件分組`。🔒 **切換不改變 rows 之資料來源**（同一份 `GET /admin/ojt-progress/rows` 之完整未分頁結果，只改前端分組方式），既有兩項篩選（單位搜尋、完成狀態）在兩種模式下皆持續生效。**條文落點恰三處**：[§九](#acceptance-criteria)（新增 `AC-30`～`AC-36`，🔒 **既有 `AC-01`～`AC-29` 一字未改**）／`AC-28` **⑳**（8 組新掛鉤與全部新逐字）／[§3-B](#prototype-25-dom-contract)·[§6](#prototype-25-dom-contract) ⑳ 群列。<br>🔴 **本輪之核心口徑鎖**：文件群組標題之「已完成 X / 共 Y 單位」**一律取自該文件在 TAB2 當下呈現之列**，**不得**改讀 TAB1 之 `docCoverage[].completedUnits`／`totalUnits`——⚠ **只用「裁撤單位」造不出差異**（`AC-04`／`docCoverage` 同樣**不套** `isActive` 過濾，兩者會剛好相等，那條斷言等於沒寫）；**真正的分岔在孤兒列**：`docCoverage` 之列由 `DOC_USING_DEPT` 驅動 ⇒ 孤兒**天然不成列**，TAB2 則**另行呈現**孤兒列（既有 `[data-row-orphaned]`）⇒ 同一份文件之兩個數字**必然可以不同**。<br>🔴 **規模收斂為硬需求、不得省略**：dev 現況僅 28 列，但正式站 **591 份** ICSOP 文件若都指定了使用部門，群組數會逼近文件數——**與 `OQ-E11-21` 砍 TAB1 區一那一次是同一形狀之缺陷**（真實資料才暴露、假資料整個藏住）。定稿＝**群組預設折疊（🔴 展開才渲染組內列，非 CSS 隱藏）＋ 文件搜尋**兩項並用。<br>🔒 **本輪未消耗任何 `AC-J#`**（全部條文落於 F042 本檔自身之 §九與 `AC-28` ⑳；`AC-J27` 仍為下一支可用之編號，配發條件仍為「跨檔 delta AC」）、**未新增任何後端行為**（`GET /admin/ojt-progress/rows` 之端點與回應形狀一格未動，本輪純為前端分組）。
> 🔴 **2026-09-02 · 第四輪（第三輪之就地修正）：文件分組模式下「列的身分維度」漏寫，`AC-31` ④ 就地改寫**——使用者實機切到「以文件分組」、展開群組後回報：**第二層每一列顯示的都是與群組標題相同的程序書名，看不出那一列是哪一個使用單位**。<br>🔴 **根因在規格、不在實作**：`AC-31` ④ 原文寫「組內列**沿用既有進度列之全部行為與掛鉤，一格不改**」，而既有進度列之主標籤恆為 `[data-progress-doc-number]`＋`[data-progress-doc-name]`——那在**使用單位分組**下是對的（群組＝單位、列＝文件）；到了**文件分組**下群組標題已經是那份文件 ⇒ 每一列重複同一個書名，真正該當主標籤的 `orgName`（其值為「**公司簡稱 / 部 / 處室**」全名，例 `和潤企業 / 財務會計部 / 財管室`）**整個沒出現**。**實作者照字面做完全正確**——⚠ **規格條文把「行為」與「身分呈現」綁進同一句話，環就只會鎖住其中一個。**<br>🔴 **⭐ 本輪之核心教訓：環少了一整個維度（不是語料無鑑別力）**——上一輪 22＋22 案鎖住了「列**有沒有**進 DOM」（`AC-33`①）與「列的**行為**對不對」（展開場次／新增／下載／刪除／角色守門／孤兒列規則），但**沒有任何一條斷言在問「這一列說明的是哪一個實體」** ⇒ **一個把群組內每一列都印成同一個字串的實作，可以 100% 通過既有的環。**<br>　⚠ **與本檔已記錄之另一個假綠形狀刻意區辨、不得混為一談**：`AC-32` 之陷阱是**語料無鑑別力**（只放「裁撤單位」時 TAB2 與 `docCoverage` 恆相等，斷言等於沒寫）；本輪是**斷言少了一整個維度**——**語料本身完全沒問題**（三列的 `orgName` 天生互異），只是**沒有人去讀它**。📌 **可推廣之檢查**：凡「把一批項目分組呈現」之畫面，環至少要有一條斷言**逐列比對列自身的身分**，且該身分之語料須**組內互異且至少 3 列**；只驗數量與行為的環，對「每一列都印同一個東西」**完全無感**。<br>**條文落點恰四處**：[`AC-31`](#acceptance-criteria) ④（就地改寫為 ④-a／④-b／④-c，📝 **原文逐字保留供追溯**）／[§3-B](#prototype-25-dom-contract)（新增 **2 列**身分掛鉤 ＋ 組內列那一列之說明就地收窄 ＋ 負向鎖定增列第 ④ 條）／`AC-28` **⑳**（新掛鉤數 **8 → 10**、增列第 **(e)** 條斷言紀律）／[`AC-36`](#acceptance-criteria)（補一句**對偶鎖**）。<br>🔴 **新掛鉤逐字**：**`[data-progress-org-name]`**（＝該列 `orgName` 全名）／**`[data-progress-org-code]`**（＝該列 `orgCode` 5 碼），🔒 **僅 `document` 模式進 DOM**、**每列恰 1 個**、**必須出現在列上之 `[data-progress-doc-number]`／`[data-progress-doc-name]` 之前**（DOM 順序＝閱讀順序，主標籤位之判定**不得靠 CSS**）。<br>🔒 **零漣漪仍為硬約束**：`org` 模式下 `[data-progress-doc-number]`／`[data-progress-doc-name]` 之位置、值與行為**一格未動**，既有測試須全數維持綠燈且**期望值未經修改**；兩個新掛鉤為 **additive**。🔒 **本輪未消耗任何 `AC-J#`**、**未新增任何後端行為**、**未新增本檔內部之 AC 編號**（`AC-37` 仍為下一支可用之編號）——📌 **刻意不新編一條 AC**：缺陷在 `AC-31` ④ 那一句話本身，另編新號會讓錯的那句話原封不動留在檔裡，下一個人仍會照它做（本 repo 之既有原則：**錯的條文就地收窄，不以增補條文迴避**）。<br>🔴 **2026-09-02 二次裁決（人類／lead）：`document` 模式下列上之 `[data-progress-doc-number]`／`[data-progress-doc-name]` 完全不進 DOM**——初版之折衷（保留兩掛鉤、僅以 DOM 順序鎖住主標籤位）**被否決**：畫面上仍看得到一份重複的書名，**就還是使用者抱怨的那個樣子**（原話：「第二層**應該會是使用單位的名稱**」）。<br>　⚠ **本次刻意放行修改一條既有測試斷言**（`OjtProgressPage.test.tsx` 之 `AC-31`④ 案中「`document` 模式下 doc-number／doc-name 存在」兩行）——📌 **理由必須留給後人**：那兩行生自 `AC-31` ④ **已確認寫錯**之原條文，斷言的正是使用者實測抱怨的行為，屬「**測試把缺陷釘成預期行為**」而**非**「既有已驗收行為」；「既有案不得刪改」之紀律保護的是**先前已驗證過**的行為，**不保護由錯誤條文生出的斷言**——那種斷言留著，等於把錯誤條文換一個地方繼續執行。🔒 **放行範圍僅限那兩行**，其餘既有案期望值一字未改；🔒 列之識別**屬性**（`data-progress-row` 鍵／`data-progress-doc`／`data-progress-org`）一格不動。
...
> 🔴 **2026-09-02 · 第五輪（人類需求三項，本輪同時觸及後端資料模型）**：使用者原文三句——「①**儀錶板頁籤對主管/部門窗口隱藏，僅顯示 OJT 資料清單頁籤**；②**OJT 資料清單粒度細緻到文件版本，但進度追蹤、OJT狀態顯示都以最新版本來追蹤，另外需注意版本太多時顯示問題(例如只顯示新版，其他先收合)**；③**OJT 資料清單在程序書名後面標示應完成訓練的日期，日期為公告日期+1個月**」。<br>
> 🔴 **② 之前置查證（實作前必讀，否則會做出一個沒有資料支撐的功能）**：本系統**沒有文件版本實體**——dev 實測 **591 份文件、程序書編號零重複**（一個編號恰一列），版次只是文件上的一個純量欄 `edition`（僅 **7/591** 有值），公告日期亦僅 **6/591** 有值。⇒ 人類裁決之模型＝**以版次欄為版本識別 ＋ 場次快照**：`ICSOP_DOCUMENT.ojtTrainingEdition`（**訓練基準版次**）＋ `OJT_SESSION.edition`（登記當下之基準版次快照），**完成 ⟺ 兩者相符**（`null` 對 `null` 亦相符）。<br>
> 🔴 **② 之第二項人類裁決（本輪最容易被實作成「規則」的地方）：改版時須**詢問** ICSOP 管理員是否要求重新訓練**——改版**不必然**要求各使用單位重跑一次教育訓練。⇒ 基準版次只在「版次真的變了 ∧ 管理員答『要求重新訓練』」時推進；答「否」或未改版次時**一格不動**。⚠ **把它做成「改版即重訓」是本輪最大的規格誤讀**：那會讓一次錯字修正把全公司的訓練完成狀態全部清成未完成。<br>
> 🔴 **② 之第三項人類裁決：既有場次「視為當下版次」**（migration 回填 `OJT_SESSION.edition := 其文件之 edition`）——不回填時，凡有版次之文件其既有場次都會與基準不符，**上線當天一批已完成的列無聲翻紅**。<br>
> 🔴 **① 為前端可見性、不是授權邊界**：`GET /admin/ojt-progress/summary` 之閘門仍是既有之 `OJT_PROGRESS_MANAGEMENT read`（主管／部門窗口本就看得到 TAB2 的**全部**列，儀表板不多揭露任何一列資料）。隱藏的是一個**對他們沒有用處的分頁**。🔒 判定住在 `canViewDashboard()` 一支純函式，**刻意不與 `canAddSession()` 合流**——本輪起兩者答案不同（主管可登記場次、卻看不到儀表板），合流會讓下次任一邊調整時另一邊被靜默地一起改掉。<br>
> 🔴 **③ 之推導點在前端唯一一處** `ojt-progress-view.ts#trainingDueDate`，後端只送 `announcedDate` 原料——同一個日期在兩層各算一次，遲早會在月底那幾天給出兩個不同的答案。**月底溢位必須夾回當月最後一日**（`1/31 + 1 月` → `2/28`，非 JS `Date` 預設之 `3/3`）。<br>
> **條文落點**：[§九](#acceptance-criteria) 新增 `AC-37`～`AC-40`（🔒 **既有 `AC-01`～`AC-36` 一字未改**）。🔒 **本輪未消耗任何 `AC-J#`**（`AC-J27` 仍為下一支可用之編號）；連帶之三檔 delta（[F025](F025-role-function-matrix.md)／[F036](F036-lifecycle-tree-preview.md)／[F017](F017-backend-document-list.md)）為**同日之另一組人類裁決**（循環管理與樹狀圖欄），與本輪無因果關係，各自落在其本檔。
...
<br>
<br>🔴 **第四種呈現態「未指定使用部門」（2026-08-28 第二輪修正；[`OQ-E11-22`](../open-questions.md#e11-post-impl)）**——🔒 **就地增列，本條上方之節流七項與四道負向鎖定原文一字未動**。📌 **成因＝真庫實打才暴露之退化值問題**：dev 真庫 **591 份 ICSOP 文件中僅 4 份設定了「使用部門」**，其餘 **587 份 `totalUnits = 0`** ⇒ 依 `AC-04` 判為 `none`、覆蓋率退化為 `0 / 0`⇒`0%`，**與「有使用部門卻一列都沒完成」共用同一個排序鍵**，數量又壓倒性 ⇒ 依上一輪之升冪排序，**587 份無義務文件全部並列第一、占滿前 15 名**，唯一一份真有進度落差的文件排到第 591 名、**預設範圍下永遠看不到**。⚠ **上一輪之節流沒有錯，是它與一個退化值相遇後失效**——排序規則仍照著跑，只是它排的東西不再是「最需要關注的」。🔒 **使用者裁決＝拆成獨立一態 ＋ 排序沉底 ＋ 不計入未完成合計**。🔒 **逐字值與掛鉤之權威＝ [§prototype 25 §2](#prototype-25-dom-contract)／[§6](#prototype-25-dom-contract) ⑲ 群列，本條不重打字面。**
<br>　**⑧ 第四態之定義（單一判準）**：**`totalUnits === 0`**（該文件未指定任何使用部門 ⇒ **沒有訓練義務**）。🔴 **本態不是「進度差」，是「不適用」**——區一問的是「**哪些文件需要關注**」，一份沒有使用部門的文件不需要關注（要處理的是去「ICSOP 文件管理」補使用部門，不是去登記場次）。⚠ **與 `AC-04` 之關係務必讀清楚**：`AC-04` 口徑下它**仍然是 `none`**（`AC-04` 明文覆寫「`every()` 對空陣列回 `true`」之語言預設，該覆寫**續為有效、本輪未動**）；本態是**區一之呈現態**，**不是 `AC-04` 的第四個狀態值**。
<br>　**⑨ 顯示範圍由三值增為恰四值**：`incomplete`（🔒 **仍為預設**）／`completed`／**`unassigned`**（新增）／`all`，🔒 **`option` 之順序即此順序**，逐字＝[§6](#prototype-25-dom-contract) ⑲。🔴 **`incomplete` 之集合定義同步收窄為 `totalUnits > 0 && state !== 'all'`**——「僅未全部完成」**不含**無義務者（它們沒有「未完成」可言）。🔒 **`completed`（`state === 'all'`）之定義一字未動**：`totalUnits === 0` 依 `AC-04` 恆為 `none`，**天然不入此集**，**不需要、也不得**為此另加一道排除。
<br>　**⑩ 排序沉底（本輪之核心修正）**：`totalUnits === 0` 之文件**一律排在所有有義務之文件之後**，🔴 **不得占用前 `maxRows` 名額**。排序鍵自此為三段：**① 有無訓練義務（有義務者在前）→ ② 覆蓋率由低至高 → ③ `documentNumber` 昇冪**。⚠ **只拆出第四態而不改排序等於沒修**：態分開了、排序鍵仍相同 ⇒ 前 15 名照樣被無義務文件占滿。**斷言形狀**：以「無義務文件之份數 > `maxRows`、且另有至少一份有義務且未完成之文件」之 fixture 驅動 `docScope='all'` ⇒ 該有義務文件**必須出現在 `items` 內**（🔴 **這條才是本輪修正之直接背書**，只驗「有 15 筆」與「態分得出來」都擋不住）。
<br>　**⑪ `incompleteTotal` 排除無義務者**：「尚未全部完成合計」＝**有使用部門卻尚未全部完成**之份數（＝`partial` ＋ **有義務之** `none`）。🔴 **本項使既有不變式 ③ 失效**——`byState.none` 為 `AC-04` 口徑（**含**無義務者），已就地更正為 **`incompleteTotal === byState.partial + byState.none − byState.unassigned`**，見 [§架構設計 一-2](#architecture)。📌 **理由**：把 587 份沒有訓練義務的文件計入「尚未全部完成」，等於在畫面上宣告 587 件待辦——那是一個**數量級錯誤的行動號召**。
<br>　**⑫ 比值欄之退化值不得照畫**：`totalUnits === 0` 時 `[data-doc-coverage-ratio]` 呈現 **`—`**（**非 `0 / 0`**）、`[data-doc-coverage-pct]` 呈現 **`—`**（**非 `0%`**，且**不畫進度條**）。📌 **理由**：`0 / 0` 與 `0%` 都在宣稱一個**不存在的量測結果**——那正是本條上段「分母為零不得退化為 `0%`／`NaN`／`100%`」（總覽比率）在**逐列層級**的同一條規則。
<br>　**⑬ 摘要行重構為兩行（🔒 四個數字必須構成一個可加總之分割）**：**上行**＝總份數 `[data-doc-coverage-total]` ＋ **`[data-doc-coverage-tracked]`（已指定使用部門份數）** ＋ **`[data-doc-coverage-unassigned]`（未指定使用部門份數）**；**下行**＝標籤 `已指定使用部門者之細分：` ＋ **既有**三態 `[data-doc-coverage-stat="all\|partial\|none"]` ＋ **既有** `[data-doc-coverage-incomplete]`。🔴 **兩條加總關係必須同時成立、且應各建一案**：**`tracked + unassigned === total`**（現行 fixture：34 ＋ 18 ＝ 52）且 **`stat.all + stat.partial + stat.none === tracked`**（13 ＋ 9 ＋ 12 ＝ 34）。⚠ **下行之 `尚未開始` 顯示的是「有義務卻一列都沒完成」（12），刻意不等於 `AC-04` 口徑之 `byState.none`（30，含 18 份無義務者）**——少了上行與那句標籤，讀者會把 12 當成 `byState.none` 而判為 bug。🔒 **三態份數恆為完整母體之分佈、不隨顯示範圍或上限改變**（既有規則未動）。
<br>　**⑭ 第四態之空狀態與截斷句各增一個變體**：`[data-doc-coverage-empty="unassigned"]` 為**第三句**範圍空狀態（🔒 `all` 仍恆為空字串、`no-docs` 仍與之互不相同）；截斷名詞表由三個變體增為**恰四個、一 `scope` 一個**。🔴 **`unassigned` 範圍下之截斷句必須另行分岔兩處**：**(a) 排序描述**——該範圍下所有列之覆蓋率皆為 `—`，**不得宣稱「依覆蓋率排序」**（那是假的），逐字改為「依程序書編號昇冪排序」；**(b) 「完整的去哪看」**——🔴 **不得把人導去「OJT 資料清單」**：這些文件沒有使用部門就**沒有進度列**，過去只會看到空的，正確去處是「**ICSOP 文件管理**」（去把使用部門補上）。⚠ **這是「不得靜默 top-N」三要素在本範圍下的實質內容改變、不是文案潤飾**——照抄另外三個範圍的句子會產生**兩句假話**。
<br>　**⑮ 導向 TAB2 之入口（`[data-doc-coverage-more]`）在本輪之處置＝不變**：🔒 **該鈕仍恆存在、逐字與行為一格未動**（`AC-14` ⑦）。📌 **與 ⑭(b) 之區別**：截斷句在描述「**本表這 15 份之外還有什麼、去哪看**」，故必須隨範圍分岔；該鈕是一個**恆定的導覽入口**，其可見文字已自述去處與落地篩選（`至「OJT 資料清單」檢視尚未完成之進度列`），不隨範圍改寫。🔵 **殘餘議題已登記**：`unassigned` 範圍下該鈕仍指向 TAB2，而該範圍之文件在 TAB2 沒有列 ⇒ 是否隨範圍調整其呈現，列為 [`OQ-E11-22`](../open-questions.md#e11-post-impl) 之 🔵 後續，**本輪不處理、亦不得由下游自行裁量**。
<br>
<br>🔴 **本次修正之三道負向鎖定（逐條可直接建環）**：
<br>　**① `data-doc-ojt-state` 之值域維持 `all｜partial｜none` 三值，🔴 明文禁止增為四值**：第四態另以 **`[data-doc-no-using-dept]`（無值屬性；🔒 僅在 `totalUnits === 0` 時進 DOM，**非** CSS 隱藏）** 表達。⚠ **同一列可同時是 `data-doc-ojt-state="none"` 且帶 `[data-doc-no-using-dept]`——這正是事實**（`AC-04` 說它是 `none`，區一說它沒有訓練義務），**兩者不衝突，亦不得互相對齊、不得寫成互斥斷言**。**斷言形狀**：以無義務文件之列斷言 `[data-doc-ojt-state]` 之值為 **`none`**（**不是** `unassigned`）**且**該列帶 `[data-doc-no-using-dept]`；並另建一條 `querySelectorAll('[data-doc-ojt-state="unassigned"]').length === 0` 之負向案。
<br>　**② 與 `prototypes/13` 共用之三態常數不得新增第四鍵**：`frontend/src/domain/ojt-status-view.ts` 之 `ojtStatusView()`／`OJT_DOC_STATE`（`prototypes/25` 之對應常數）為 **文件清單頁（`prototypes/13`／`DocumentListPage`）與本頁共用**之 `AC-04` 三態載體 ⇒ 🔴 **加第四鍵會直接漣漪到清單頁之圖示欄與四值篩選**（[F017](F017-backend-document-list.md#ojt-derived-semantics-delta) `AC-J13`／`AC-J14`）。**兩張表度量的東西不同**：清單頁問「**這份文件的訓練做完沒**」（`totalUnits = 0` 確實就是「尚未開始」），區一問「**哪些文件需要關注**」（`totalUnits = 0` 是**沒有義務**）。⇒ 第四態之視覺為**區一專屬、另立一份常數**，🔒 **不外流**。**斷言形狀**：`DocumentListPage` 之 OJT 篩選仍**恰四個 `option`**（`全部` ＋ 三態逐字）、其圖示欄仍**恰三種**狀態值（🔒 `AC-J14` 之既有斷言**期望值不得改**）。
<br>　**③ 第四態之視覺刻意跳出三態家族，不得「順手對齊」**：icon 為 **`circle-slash`**（🔴 **刻意不用 `file-*-2` 家族**——三態是同一把量尺上的三個刻度，本態**在尺之外**）；色票為 **`text-slate-500`**，🔴 **刻意不比照 `none` 之 `text-slate-300`**（白底對比約 1.7:1、遠低於 WCAG AA 之 4.5:1，**已列為待裁之既有議題**）。⚠ **新載體不必繼承既有載體的可讀性問題**——`text-slate-300` 是與 `13` 共用之鎖定常數、本輪不得改動，但那不構成把新態也畫成 1.7:1 的理由。
<br>
<br>🔒 **本輪明確未改動者（負向鎖定，覆核時逐項確認）**：① **上一輪節流之七項與四道負向鎖定**（顯示範圍控制項、上限 15、截斷告知三要素、摘要行防假綠、範圍空狀態、捲軸容器、導向入口）**原文一字未動**；② **統計口徑一格未動**——KPI 五張卡、`#inactiveNote`、區二、區三**一律以完整母體計算**，第四態**不改變任何既有分子／分母**（本條上段之覆蓋率、`AC-17` 之裁撤排除、`AC-25` 之孤兒排除**全部照舊**）；③ **`AC-04` 之文件層三態一格未動**（見 ⑧ 之 ⚠）；④ **TAB2 一格未動**（篩選仍恰兩項、完成狀態仍恰三個 `option`）；⑤ **區三（`AC-16`）一格未動**。
...
<br>✅ **2026-08-28 第二輪修正後補之一組（[`OQ-E11-22`](../open-questions.md#e11-post-impl)；區一逐筆表之第四種呈現態）**：⑲ **「未指定使用部門」態**＝**3 組新掛鉤**——`[data-doc-no-using-dept]`（🔴 **無值屬性**，掛於 `[data-doc-coverage-row]` 之列元素上；🔒 **僅在 `totalUnits === 0` 時進 DOM**，**非** CSS 隱藏）／`[data-doc-coverage-tracked="{n}"]`／`[data-doc-coverage-unassigned="{n}"]`（後二者為摘要行**上行**之兩個新片段）。🔒 **上一輪之 9 組（`-scope`／`-summary`／`-total`／`-stat`／`-incomplete`／`-truncation`／`-more`／`-empty`／`-basis-note`）與列上之 `[data-doc-coverage-row]`／`-ratio`／`-pct`／`[data-doc-ojt-state]`／`[data-doc-ojt-state-chip]` 一格未動**——本輪同樣為 **additive**，**無任何既有掛鉤改名或改值域**；其全部新逐字見 [§6](#prototype-25-dom-contract) 之 ⑲ 群列。
<br>　🔴 **⑲ 之三條斷言紀律（違反其一即為假綠或建出規格未授權之約束）**：**(a) `data-doc-ojt-state` 之值域仍為三值**——無義務列之該屬性值為 **`none`**（**不是** `unassigned`），第四態**只**由 `[data-doc-no-using-dept]` 表達；⚠ **同一列同時具備兩者是正確狀態，不得寫成互斥斷言**，且須另建一條 `querySelectorAll('[data-doc-ojt-state="unassigned"]').length === 0` 之負向案。**(b) 摘要行由五片段增為七片段、且分屬兩個 `<div>` 行**——🔒 **上一輪之兩條紀律（逐掛鉤斷言、不得對整行 `textContent` 下逐字）續為有效，且範圍擴及新增之兩個片段**；另須各建一案斷言兩條加總關係（`tracked + unassigned === total`、`stat.all + stat.partial + stat.none === tracked`）。**(c) 第四態之晶片逐字與視覺不得取自 `OJT_DOC_STATE`**——`[data-doc-ojt-state-chip]` 於本態承載之逐字為 `未指定使用部門`、icon `circle-slash`、色票 `text-slate-500`，🔒 **取自區一專屬之另一份常數**（`prototypes/25` 之 `DOC_UNASSIGNED_VISUAL`）；🔴 **`ojtStatusView()`／`OJT_DOC_STATE` 不得新增第四鍵**（會漣漪到 `prototypes/13` 之圖示欄與四值篩選，見 `AC-14` 本輪負向鎖定 ②）。
<br>　🔒 **⑲ 與 ⑯ 之關係（不得誤讀為取代）**：⑲ **不是** ⑯ 的替代或修訂，而是**同一張表上多出一種呈現態**——⑯ 之九組掛鉤、上限 15、截斷三要素、防假綠紀律**全部續為有效**；⑲ 只增列「這份文件沒有訓練義務」這一維，並因此連帶擴充**顯示範圍（三值 → 四值）**、**截斷名詞表（三變體 → 四變體）**、**範圍空狀態（兩句 → 三句）**與**摘要行（一行 → 兩行）**四處。
<br>✅ **2026-09-01 第三輪新增之一組（TAB2 分組模式；`AC-30`～`AC-36` 之落點）**：⑳ **「以文件分組」模式**＝**10 組新掛鉤**（📝 **原記 8 組；2026-09-02 隨 `AC-31` ④-a 增列「列身分」之兩組**）——`[data-ojt-group-mode]`（分組模式 `select`，值域**恰二值** `org`〔🔒 **預設**〕／`document`）／`[data-ojt-doc-search]`（文件搜尋，🔒 **僅 `document` 模式進 DOM**）／`[data-doc-group="{documentId}"]`／`[data-doc-group-number]`／`[data-doc-group-name]`／`[data-doc-group-ratio]`／`[data-doc-group-pct]`／`[data-doc-group-toggle="{documentId}"]`（帶 `aria-expanded`，🔒 **初始為 `"false"`**）／`[data-doc-group-basis-note]`／**`[data-progress-org-name]`**／**`[data-progress-org-code]`**（🔴 **組內列之主標籤**，🔒 **僅 `document` 模式進 DOM**、每列恰 1 個；2026-09-02 新增）。🔒 **TAB2 既有掛鉤之值與行為一格未動**——`[data-progress-group]`／`-name`／`-code`／`[data-org-inactive]`／`[data-progress-row]` 及其全部子掛鉤／`[data-ojt-filter-bar]`／`[data-ojt-filter="org|status"]`／`[data-ojt-row-count]`，本輪為 **additive**，無任何既有掛鉤改名或改值域；其全部新逐字見 [§3-B](#prototype-25-dom-contract) 與 [§6](#prototype-25-dom-contract) 之 ⑳ 群列。
<br>　🔴 **⑳ 之四條斷言紀律（違反其一即為假綠，或建出規格未授權之約束）**：**(a) 折疊必須驗「不進 DOM」而非「有沒有 class」**——未展開之群組其 `[data-progress-row]` **完全不進 DOM**；只驗 CSS 隱藏或 `toBeVisible()` 之否定，會讓「591 組全渲染」這個**本輪唯一要消除的缺陷**整個逃掉（同 `AC-14` ③ 之既有紀律，形狀相同）。**(b) 口徑那條必須用會分岔的語料**——語料須刻意讓「TAB2 當下之列」與 `docCoverage` 給出**不同**的 X/Y（最小可用形狀＝該文件在 TAB2 有 3 列、其中 1 列 `orphaned`），兩數相等時該斷言零鑑別力。**(c) 分組模式控制項與文件搜尋一律不得掛 `data-ojt-filter`**——`AC-13` 之「篩選**恰兩項**」為既有鎖，`document.querySelectorAll('[data-ojt-filter]').length === 2` 在**兩種模式下**皆須成立。**(d) 百分比不得另打一份推導**——必須重用既有 `coveragePercent()`（全頁單一百分比推導點）；直接寫 `Math.round` 或讀一個 API 未送之 `rate` 欄，**是本頁已發生過的真實缺陷**（見 §2 之 `[data-rollup-rate]` 與其註）。**(e) 列的身分必須逐列比對（2026-09-02 增補，本輪缺陷之直接對策）**——⚠ **上一輪之 22＋22 案沒有任何一條在問「這一列說明的是哪一個實體」**：它們鎖住了列**有沒有**進 DOM（`AC-33`①）與列的**行為**對不對（展開／新增／下載／刪除／角色守門／孤兒列規則），⇒ 一個把群組內每一列都印成同一個字串的實作可以 **100% 通過**。正確形狀＝語料使**組內至少 3 列之 `orgName` 互不相同**，斷言以**有序陣列 `toEqual` 逐列比對**；🔴 **與 (b) 之「語料無鑑別力」刻意區辨、不得混為一談**——(b) 是**語料**造不出分岔，本條是**斷言少了一整個維度**（語料本來就沒問題，三列的 `orgName` 天生互異，只是沒有人去讀它）。
...
### 九、TAB2 之分組模式（2026-09-01 第三輪新增；`AC-30`～`AC-36`）

> 🔒 **本節全部為 additive**：`AC-01`～`AC-29` 之既有規則**一字未改**——特別是 `AC-11`（以使用單位分組）**續為預設模式之規格**、`AC-13`（篩選**恰兩項**）**一格未動**、`AC-12`／`AC-03`／`AC-05`／`AC-06`／`AC-19`／`AC-25` 之進度列行為在兩種分組模式下**完全相同**。
> 🔴 **本節之單一真相**：分組只改「列被裝進哪個盒子、依什麼順序排」，**不改列本身、不改列的來源、不改任何列上的判定**。凡本節之條文讀起來會改動到某一列的完成狀態、可否新增場次、可否下載或可否刪除者，即為誤讀。

- **AC-30**（🔴 分組模式切換，恰二態，預設「以使用單位」）：Given 我在 TAB2, When 檢視篩選列, Then 存在一個分組模式控制項 `[data-ojt-group-mode]`，其值域**恰二值**——`org`（🔒 **預設**，可見文字逐字 `以使用單位分組`）／`document`（可見文字逐字 `以文件分組`）；When 切換為 `document`, Then 同一批列改以文件為群組呈現。<br>🔴 **切換不改變 rows 之資料來源**：**不得**因切換而發出任何新的 `GET /admin/ojt-progress/rows`（斷言形狀：切換前後該端點之呼叫次數**相同**且**大於 0**）——📌 **理由**：該端點已回傳完整、未分頁之進度列（[§架構設計 一](#architecture)），分組純為前端行為；為分組再打一次 API，等於把一個純呈現決策做成一次網路往返。<br>🔴 **本控制項不得掛 `data-ojt-filter`**：`AC-13` 之「篩選**恰兩項**」為既有鎖 ⇒ `[data-ojt-filter]` 之數量在**兩種模式下**恆為 **2**。📌 **它不是篩選**——它不移除任何列，只改變列的組織方式；把它算成第三個篩選會直接推翻 `OQ-E11-15`→A 之既有裁決。<br>🔒 **既有兩項篩選（單位搜尋、完成狀態）在兩種模式下皆持續生效**，其掛鉤、選項集合與逐字文案一格未動。
- **AC-31**（以文件分組之群組形狀）：Given 分組模式為 `document`, When 檢視清單, Then ——① 群組＝**一份 ICSOP 文件**，鍵為 **`documentId`**（`[data-doc-group="{documentId}"]`）；② 群組標題呈現**文件編號**（`[data-doc-group-number]`）與**書名**（`[data-doc-group-name]`）；③ 群組標題呈現該文件之部門完成度 `[data-doc-group-ratio]`（逐字形狀 `已完成 {X} / 共 {Y} 單位`）與百分比 `[data-doc-group-pct]`；④ 組內列之**行為與互動掛鉤**沿用既有進度列、**一格不改**（`[data-progress-row]` 之列鍵／`[data-progress-doc]`／`[data-progress-org]`／`[data-completion-badge]`／`[data-session-count]`／展開場次明細／新增場次／下載／刪除，含其角色守門與孤兒列規則）。<br>　🔴 **④-a（2026-09-02 就地修訂——⚠ 本條原文是「錯的」，不只是不完整）：列之「主標籤」必須隨分組維度切換**。既有進度列之主標籤恆為 `[data-progress-doc-number]`＋`[data-progress-doc-name]`，那在 `org` 模式下是對的（群組＝單位、列＝文件）；到了 `document` 模式，**群組標題已經是那份文件** ⇒ 沿用同一個主標籤會使**同一群組內每一列都印出同一個字串**，而真正該當主標籤的 `orgName` **整個沒出現**（2026-09-01 使用者實機實測之實際缺陷）。⇒ 本模式之列主標籤改為 **additive** 之兩個新掛鉤：**`[data-progress-org-name]`**（逐字＝該列之 `orgName` **全名**「公司簡稱 / 部 / 處室」，例 `和潤企業 / 財務會計部 / 財管室`）與 **`[data-progress-org-code]`**（逐字＝該列之 `orgCode` **5 碼**）；🔒 **僅 `document` 模式進 DOM**（`org` 模式下**完全不進 DOM**、非 CSS 隱藏 ⇒ `AC-36` 之零漣漪一格未動）；🔒 **與 `org` 模式群組標題之 `[data-progress-group-name]`／`[data-progress-group-code]` 採同一套呈現語彙**（同一個維度在兩處呈現，不得各自造一種寫法）。<br>　🔴 **④-b（2026-09-02 人類裁決後之定值）：`document` 模式下列上之 `[data-progress-doc-number]`／`[data-progress-doc-name]` **完全不進 DOM**（**非** CSS 隱藏、**非** `hidden` 屬性）** ⇒ `[data-progress-org-name]`／`[data-progress-org-code]` 為本模式下列之**唯一**主標籤。📌 **理由**：群組標題已經是那份文件，列再重複一次是**純噪音**——使用者原話：「第二層**應該會是使用單位的名稱**」；⚠ **「保留但降為次要脈絡位」之折衷已被明確否決**（畫面上仍看得到一份重複的書名，就還是使用者抱怨的那個樣子）。<br>　🔒 **列之識別「屬性」一格不動**：`data-progress-row`（鍵仍為 `{documentId}__{orgCode}`）／`data-progress-doc`／`data-progress-org` **一律保留**——它們是**識別屬性、不是可見文字**，移掉會打斷既有之展開／新增／下載／刪除行為與其定位。<br>　🔒 **`org` 模式一格不變**：兩掛鉤照舊進 DOM 且值不變（見 `AC-36` 對偶鎖）。<br>　⚠ **本項推翻了一條 2026-09-01 之既有測試斷言**（`frontend/src/pages/OjtProgressPage.test.tsx` 之 `AC-31`④ 案中「`document` 模式下 `[data-progress-doc-number]`／`[data-progress-doc-name]` 存在」兩行）：📌 **放行理由必須留給後人**——那兩行生自本條**已確認寫錯**之原文，斷言的正是使用者實測抱怨的那個行為，屬「**測試把缺陷釘成預期行為**」而**非**「既有已驗收行為」；「既有案不得刪改」之紀律保護的是**先前已驗證過**的行為，不保護由錯誤條文生出的斷言。🔒 **放行範圍僅限那兩行**（2026-09-02 lead 裁決），其餘既有案期望值一字未改。<br>　🔴 **④-c 書名恰 1 處、不得隨列數增生**：同一 `[data-doc-group]` 內，文字恰等於該文件書名之**葉節點**元素**恰 1 個**——即群組標題 `[data-doc-group-name]` 本身。📌 **本條為本輪缺陷之直接負向鎖**：把群組內每一列都印成書名的實作，其計數會隨列數增生為 `1 + 列數` ⇒ 立即翻紅。⚠ **只數葉節點**：包住群組標題的外層容器其 `textContent` 亦等於書名，計入會造成假紅。📝 **2026-09-01 之初版曾寫成「除群組標題與每列 `[data-progress-doc-name]` 之外恰 0 個」**（為了讓當時保留於列上的次要脈絡不翻紅）；④-b 改為「不進 DOM」後，該遷就已無必要，**強形式就此收回**。<br>　📝 **原文逐字保留於此供追溯**：「④ 組內列**沿用既有進度列之全部行為與掛鉤，一格不改**（`[data-progress-row]`／展開場次明細／新增場次／下載／刪除，含其角色守門與孤兒列規則）。」——🔴 **「一格不改」這四個字即為 2026-09-01 缺陷之根因**：它把「行為」與「身分呈現」綁進同一句話，實作者照字面做**完全正確**，缺陷仍然發生。**規格條文一旦把兩個維度綁成一句，環就只會鎖住其中一個。**<br>🔴 **鍵是 `documentId`、不是文件編號或書名**：兩份不同文件可以同名（書名非唯一），以書名分組會把它們併成一組而**憑空少掉一份文件**。<br>🔒 **兩種模式互斥渲染**：`org` 模式下 `[data-doc-group]` 恰 **0** 個；`document` 模式下 `[data-progress-group]` 恰 **0** 個——⚠ **不得**兩種群組容器同時進 DOM（那會使既有測試之 `[data-progress-group="ABA00"]` 在文件模式下仍然找得到，任何「模式真的切了嗎」之斷言就此失去鑑別力）。
- **AC-32**（🔴 **口徑鎖**——本輪最易寫錯處）：Given 文件群組之完成度 `已完成 {X} / 共 {Y} 單位`, Then ——**X** ＝該文件在 TAB2 **當下呈現之列**中 `completed === true` 者之列數；**Y** ＝該文件在 TAB2 **當下呈現之列**之總列數（🔴 **含 `inactive`（已裁撤）與 `orphaned`（已移出使用部門）之列**，且為既有兩項篩選與文件搜尋**套用後**之結果）。<br>🔴 **不得改用 TAB1 區一 `docCoverage[].completedUnits`／`totalUnits`**——兩者**刻意不同口徑**：`docCoverage` 之列由 `DOC_USING_DEPT` 驅動 ⇒ **孤兒天然不成列**（[§架構設計 一-2](#architecture) 之明文），而 TAB2 **另行呈現**孤兒列；`AC-04`／`docCoverage` 亦**不套** `isActive` 過濾，而 TAB2 之列**全數呈現**。<br>　⚠ **建環之語料陷阱（明文寫出以防環寫成假綠）**：**只放一個「裁撤單位」造不出差異**——`docCoverage` 同樣不排除裁撤，兩邊會剛好相等，該斷言即等於沒寫。**最小可用之分岔語料＝該文件在 TAB2 有 3 列、其中 1 列 `orphaned`**（TAB2 得 `2 / 3`，`docCoverage` 得 `1 / 2`）。<br>🔴 **百分比必須重用既有 `coveragePercent()`**（全頁**只有一個**百分比推導點）：直接寫 `Math.round(...)` 另打一份，或讀一個 API 未送之 `rate` 欄而印出 `undefined%`，**皆為本頁已發生過之真實缺陷形狀**（見 [§2](#prototype-25-dom-contract) `[data-rollup-rate]` 之註）。<br>🔴 **分母為 0 時不得出現 `NaN%`／`0%`／`100%`**，比照既有 `NO_STATISTICS_TEXT` 之處理（同 `AC-14` 之既有規則）。📌 **本分支於元件層不可達**——群組是「因為有列」才存在的，`Y ≥ 1` 恆成立 ⇒ **其載體為純函式層**（`coveragePercent` 之單一推導點），建環時須在該層直接餵 `(0, 0)`，不得因為畫不出來就不建約束。<br>✅ **既有簽章之查證結果（test-generator 首跑紅燈所揭露，2026-09-01）**：`coveragePercent(done, total)` 之既有簽章為 **`=> number | null`**（回傳 `67`／`100`／`0`；分母為 0 時回 **`null`**，即「無可統計」之信號，由呼叫端換成 `NO_STATISTICS_TEXT`），**不是**已格式化之字串。⇒ 文件群組之百分比**顯示字串**以 **`docGroupPercentText(done, total)`** 為其單一推導點，🔒 **其內部必須委派 `coveragePercent`、不得另打一份 `Math.round`**；`null` 時回 `NO_STATISTICS_TEXT`。📌 **本項更正的是介面形狀、不是行為**——四捨五入、`0%` 非錯誤、分母 0 不得退化為 `NaN%`／`0%`／`100%` 三者一字未鬆。<br>🔴 **畫面上必須有一句口徑說明** `[data-doc-group-basis-note]`（🔒 **全頁恰 1 個**、僅 `document` 模式進 DOM，逐字見 [§6](#prototype-25-dom-contract) ⑳）。📌 **處置比照同頁既有之 `[data-doc-coverage-basis-note]`**（同一種問題之既有解法）：同頁並置兩個口徑不同的數字，沒有這一行，使用者一對帳就會讀成 bug。
- **AC-33**（🔴 **規模收斂之兩項硬需求，缺一不可**）：Given 分組模式為 `document`, Then ——<br>　**① 群組預設折疊**：`[data-doc-group-toggle="{documentId}"]` 之 `aria-expanded` 初始為 **`"false"`**，🔴 **展開才渲染組內列**——未展開時該群組之 `[data-progress-row]` **完全不進 DOM**（**非** CSS 隱藏、**非** `hidden` 屬性）；When 點擊該控制項, Then `aria-expanded` 轉為 `"true"` 且組內列進 DOM；再點一次則收合並**再次自 DOM 移除**。<br>　**② 文件搜尋** `[data-ojt-doc-search]`：比對 **`documentNumber` 或 `documentName`** 之**不分大小寫子字串**；關鍵字 trim 後為空字串時**視為不過濾**（不得因空白字元而濾掉全部列）。🔒 **僅於 `document` 模式進 DOM**（`org` 模式下**完全不進 DOM**），🔴 **切回 `org` 模式時清空關鍵字**——📌 **理由與 `AC-14` ⑦ 之既有處置同源**：一個看不見的條件仍在過濾列，就是畫面說謊；不清空會使使用者切回單位模式後看到一個比實際小的集合，而畫面上找不到任何解釋。<br>　🔴 **本兩項為硬需求，不得省略、不得只做一項**：正式站 591 份 ICSOP 文件若都指定了使用部門，群組數逼近文件數 ⇒ 沒有折疊，一次會渲染近 600 個群組與其全部列；沒有搜尋，使用者只能靠捲軸找一份文件。**TAB1 區一已因同一形狀之缺陷被砍成上限 15**（`OQ-E11-21`）。<br>　⚠ **本輪刻意不採「上限 N ＋ 截斷告知」**（＝區一之解法）：文件分組是**逐份文件查閱**用的，截掉一部分文件會使「我要找的那一份」以機率消失；折疊＋搜尋在不隱藏任何一份文件的前提下達成同樣的渲染量收斂。🔒 **兩區之解法刻意不同，不得互相對齊**（同 `AC-28` ⑱ 之既有紀律）。
- **AC-34**（排序須具決定性）：Given 分組模式為 `document`, Then 文件群組依 **`documentNumber` 昇冪**；組內列依 **`orgName` 昇冪**（皆以 `localeCompare` 比較）。🔴 **不得依賴資料到達順序**（斷言形狀：以**亂序**語料餵入，渲染順序須為排序後之順序）。<br>📌 **`orgName` 之值為「公司簡稱 / 部 / 處室」全名**（2026-09-01 前置修正 `2f8acf1`）⇒ 依其昇冪即天然先依公司再依部、處室分群，**不需要也不得另建一套跨公司之排序鍵**。
- **AC-35**（空狀態與文案常數之沿用）：Given 分組模式為 `document`, Then 空狀態**沿用既有逐字常數、不另造詞**——① 套用篩選（含文件搜尋）後無任何列 ⇒ `EMPTY_ROWS_TEXT`（逐字 `查無符合條件的進度列`）；② 全域無任何進度列 ⇒ `EMPTY_ALL_TEXT`（逐字 `目前沒有任何 OJT 進度列`）＋ `EMPTY_ALL_HINT`（其補充提示）。<br>🔴 **本輪新增之逐字文案一律以具名常數寫在 `frontend/src/pages/ojt-progress-view.ts` 之文案常數區**（與既有慣例一致）；**測試 import 該常數作斷言、不得於測試內硬寫中文字面**——同一組文案在兩處各打一份即為分歧之起點（本規格集之既有原則）。
- **AC-36**（🔒 既有掛鉤零漣漪；新掛鉤為 additive）：Given 本輪實作完成, When 分組模式為 `org`（預設）, Then TAB2 之 DOM **與本輪之前完全相同**——`[data-progress-group="{orgCode}"]` 之值與其 `[data-progress-group-name]`／`[data-progress-group-code]`／`[data-org-inactive]`、`[data-progress-row="{documentId}__{orgCode}"]` 及其全部子掛鉤、`[data-ojt-filter-bar]`／`[data-ojt-filter="org|status"]`／`[data-ojt-row-count]` **一格未動**（既有測試須全數維持綠燈且期望值未經修改）。<br>🔒 **`[data-ojt-row-count]` 之文字不因分組模式改變**——同一批列、同一個口徑，只是裝進不同的盒子；⚠ 若它跟著分組模式變，代表分組動到了列本身（`AC-30` 之核心已被違反）。<br>🔒 **往返鎖**：`org` → `document` → `org` 之後，既有掛鉤之值與數量須與從未切換過時相同。<br>🔒 **對偶鎖（2026-09-02 隨 `AC-31` ④-a 增補）**：`org` 模式下 `[data-progress-org-name]`／`[data-progress-org-code]` **完全不進 DOM**（`length === 0`），且列之主標籤**仍為文件編號／書名**（`[data-progress-doc-number]`／`[data-progress-doc-name]` 之值即該列文件之編號與書名）。⚠ **修過頭把兩邊對調，會變成同一個缺陷的另一個方向**——使用單位群組下每一列都印出同一個單位名，而列究竟是哪一份文件又看不出來；🔴 **對偶回歸案必須斷言「值」而非只斷言「掛鉤存在」**：既有之存在性斷言（`not.toBeNull()`）對「元素還在、印的內容換了」完全無感。

### 十、版次追蹤、應完成訓練日期與儀表板可見性（2026-09-02 第五輪新增；`AC-37`～`AC-40`）

- **AC-37**（🔴 完成判定改以**訓練基準版次**為準）：Given 一列進度列（`documentId × orgCode`）, Then 該列 `completed === true` **⟺ 存在至少一筆場次，其 `edition` 與該文件當下之 `ojtTrainingEdition` 相符**（🔴 **`null` 對 `null` 亦視為相符**——591 份文件中 584 份未設版次，若判為不符，上線當天所有已完成的列會整批翻紅）。<br>🔴 **回應須同時攜帶兩個計數**：`sessionCount`（**全部版次**之場次總數）與 `currentEditionSessionCount`（符合基準者）——⚠ **不得合流成一個數字**：合流後「這個單位辦過訓練，但那是改版前的事」在畫面上就**沒有任何載體**，使用者只會看到「0 場次」而以為紀錄不見了。畫面上該情形以獨立註記 `[data-row-retrain]` 呈現，🔒 **完成徽章本身一格不動**（照實說「尚未完成」；處置比照既有之孤兒列註記 `AC-25`——做成第三種徽章態會讓人以為舊場次的紀錄失效了）。<br>🔴 **只問「等不等於基準」，不做任何新舊比較**：版次字串 `{YY}'{NN}` 沒有可靠的全序（跨年度、手動編號、`null`），任何 `>` 比較都會在某些真實值上給出錯誤答案。<br>🔴 **新場次快照的是 `ojtTrainingEdition`（基準），不是 `edition`（文件當下版次）**——⚠ 建環之語料須讓兩者**不同**（＝「改版但不要求重訓」之狀態）：快照寫錯時，這筆剛登記的場次會與基準不符、登記完仍顯示「尚未完成」；兩者相同的語料對這個錯誤**完全無感**。<br>🔒 **文件層三態（`AC-04`）與 TAB1 覆蓋率分子同步改用本判定**（`typeorm-ojt-completion.reader.ts` 之 `Q2`），⚠ 其 SQL 之 `NULL` 比對須顯式寫出 `IS NULL` 那半句——`s.edition = d.ojtTrainingEdition` 在兩者皆 `NULL` 時於 SQL 為 **UNKNOWN 而非 TRUE**，少了它後台清單之 OJT 欄會**整批**退回「尚未開始」。

- **AC-38**（🔴 改版時**詢問**是否要求重新訓練，非自動）：Given ICSOP 管理員於文件編輯頁變更**版次**並按儲存, When 送出前, Then 出現問句 `[data-retrain-modal]`，**恰兩個出口**——「要求重新訓練」／「不需重新訓練」；答前者 ⇒ PATCH 帶 `ojtRetrainRequired: true`，後端將 `ojtTrainingEdition` 推進為新版次（各使用單位之完成狀態回到「尚未完成」，**既有場次紀錄完整保留**）；答後者 ⇒ **不帶該鍵**，基準版次一格不動、完成狀態不變。<br>🔒 **兩個出口皆會送出**（沒有「取消儲存」那一支）：這是一個關於本次改版的**分類問題**，不是一道確認關卡；做成三選一等於在每次改版前多插一個可以按錯的岔路。使用者若不想儲存，路徑仍是既有的「取消」鈕。<br>🔴 **只在版次真的變了時才出現、也才生效**（負向鎖）：未變更版次卻送 `ojtRetrainRequired: true` 一律無效——📌 「要求重訓」是**改版的一個屬性**，不是一顆隨時可按的「全部單位重來」按鈕（那會讓人一次點掉全公司的訓練完成狀態）。⚠ **少了本條，一個「每次儲存都跳問句」或「旗標隨時生效」的實作可以通過其餘每一條**。<br>🔒 `ojtRetrainRequired` 為**控制旗標、不是文件欄位**（不在 `FIELD_KEY_BY_PROP` 白名單內，處置比照 F012 之 `reason`）⇒ 客戶端直送 `ojtTrainingEdition` 一律被丟棄。<br>🔒 **基準版次不進變更歷程**——它是 `edition` 變更的系統性後果，而 `edition` 那一列已經記了；多記一列只會讓使用者在歷程裡看到兩個長得很像的版次欄位。

- **AC-39**（場次明細依版次分組；「版本太多時只顯示新版、其他先收合」）：Given 展開一列之場次明細, Then 場次依其 `edition` **分組**呈現——① **當下訓練基準版次那一組恆為第一組、恆展開，且恆存在**（🔴 **即使一場都沒有**：改版並要求重訓後，畫面必須看得到「目前版次：0 場」；若「沒有場次就不長群組」，使用者只會看到一串舊版次的場次，讀不出「現在這一版還沒人受訓」）；② **其餘版次群組預設收合**，🔴 **收合時其 `[data-session-row]` 完全不進 DOM**（**非** CSS 隱藏、**非** `hidden` 屬性——同 `AC-33`① 之既有紀律）；③ 舊版次群組依**該組最新訓練日期遞減**排序，🔒 **不得依版次字串排序**（理由同 `AC-37` 之無全序）。<br>🔒 **場次列之行為一格不改**：下載／刪除／角色守門／`aria` 文案共用**同一個渲染點**（`SessionRow`）——📌 本 feature 於 2026-09-02 已因複製出第二份列而踩過一次，這是那個教訓的直接對策。<br>🔒 `null` 版次是一個**真正的群組鍵**（非「無群組」）：DOM 屬性值不得為 `null`，故以哨兵鍵表示；正常情形下（584/591 份文件）它就是每一列唯一的群組。

- **AC-40**（儀表板分頁之可見角色 ＋ 應完成訓練日期）：<br>　**① 儀表板分頁**：Given 角色為 **主管／部門窗口**, Then TAB1 之分頁鈕與其 `role="tabpanel"` **完全不進 DOM**（非 `disabled`、非 CSS 隱藏——一顆按得到卻沒有內容的分頁鈕比沒有那顆鈕更難理解），初始分頁即 `sessions`，且**完全不發** `GET /admin/ojt-progress/summary`（591 份文件之全池聚合，純浪費的往返）。Given 角色為 ICSOPAdmin／SysAdmin, Then 兩者皆在、且 summary 照發（**對偶鎖**，防「乾脆都不發」之過度修正）。<br>　**② 應完成訓練日期**：Given TAB2 之任一列, Then 於**程序書名之後**呈現 `[data-training-due]`，值＝**公告日期 + 1 個月**。🔴 **必須驗 DOM 順序**（只驗兩者都存在的斷言，對「日期印在編號前面」完全無感）。🔴 **月底溢位夾回當月最後一日**（`2026-01-31` → `2026-02-28`、`2028-01-31` → `2028-02-29`）——天真作法 `setMonth(+1)` 會自動跨月（→ `3/3`），比使用者預期晚三天，且一年只有少數幾天看得出來。🔴 **以 UTC 拆解**（比照既有 `todayIsoDate()`）。<br>　🔒 **無公告日期時仍進 DOM、值為 `—`**：整個消失會讓使用者以為這份文件沒有訓練期限，而事實是「還沒有人設公告日期」（6/591 有值）。<br>　🔒 **`document` 分組模式下該日期掛在群組標題之書名之後、全頁恰 1 個**——列上不印書名（`AC-31` ④-b），書名旁的日期自然也該待在群組標題那一份。
...
- 🔴 **`summary` 之 `docScope` 值域與 `docCoverage.byState` 就地更正（2026-08-28 第二輪，[`OQ-E11-22`](../open-questions.md#e11-post-impl)）**：📝 **上表 `/admin/ojt-progress/summary` 列之「關鍵參數」與「回應形狀」兩欄原文逐字保留**；🔴 **自本輪起** `docScope` 之值域為**恰四值** `incomplete`（**預設**）｜`completed`｜**`unassigned`**｜`all`，且 `docCoverage.byState` 增第四鍵成為 `{ all, partial, none, unassigned }`。逐項語意、母體與不變式見下方 §一-2 之 🔴 更正段。
...
| `all` | 全部文件 | 不過濾 |
| **`unassigned`**（🔴 **2026-08-28 第二輪新增**） | 僅未指定使用部門（**無訓練義務**者） | `totalUnits === 0`（🔒 依 `AC-04` 恆為 `none` 之**子集**，非第四個狀態值） |
...
- 🔴 **值域就地更正為恰四值（2026-08-28 第二輪；📝 上方標題行「值域**恰三值**」與 `incomplete` 列之原文逐字保留）**：`docScope` 自本輪起為 **`incomplete`｜`completed`｜`unassigned`｜`all`**，且 🔴 **`incomplete` 之集合定義同步收窄為 `totalUnits > 0 && state !== 'all'`**（原記為 `state !== 'all'`）——「僅未全部完成」**不含**無義務者。🔒 **`completed`／`all` 兩列一字未改**；🔒 **缺值或未知值仍一律正規化為 `incomplete` 並回聲於 `docCoverage.scope`**（該規則未動，僅其合法值域多一個）。⚠ **`unassigned` 與 `all` 不同型**：`all` 是「不過濾」，`unassigned` 是一道**正向**過濾。
...
| **`byState.unassigned`**（🔴 **2026-08-28 第二輪新增之第四鍵**） | `number` | **未指定使用部門**（`totalUnits === 0`）之文件份數 | 🔴 **完整母體** |
...
- 🔴 **切片產生順序就地更正為「四步、三段排序鍵」（2026-08-28 第二輪；📝 上一則之原文逐字保留）**：① 依 `docScope` 過濾 → ② 依**三段排序鍵**排序：**(1) 有無訓練義務（`totalUnits === 0` 者一律在後）→ (2) 覆蓋率（`completedUnits / totalUnits`）由低至高 → (3) `documentNumber` 昇冪** → ③ 取前 `maxRows` 筆。<br>　🔴 **原文之 🔒「`totalUnits === 0` 視為 0%」續為真、但已不決定名次**：該退化值仍是 `0%`，只是第 (1) 段排序鍵先把它推到最後 ⇒ **不得再據原文推論「無義務者排最前」**。⚠ **這正是本輪要修的東西**：`0 / 0`⇒`0%` 與「真的一列都沒完成」共用同一個排序鍵，而無義務者在真庫佔 587/591 ⇒ 前 `maxRows` 名額被整批占滿。<br>　**斷言形狀**：以「無義務文件份數 > `maxRows`、另有至少一份有義務且覆蓋率 < 100% 之文件」之 fixture 驅動 `docScope='all'` ⇒ 該有義務文件**必須在 `items` 內**（🔴 只驗「恰 `maxRows` 筆」與「排序非遞減」**都擋不住**——沉底缺失下兩者皆綠）。
...
- 🔴 **不變式 ③ 就地更正（2026-08-28 第二輪；📝 上一則之四條原文逐字保留）**——**③ 自本輪起不成立**，正確關係式為：
<br>　**③′ `incompleteTotal === byState.partial + byState.none − byState.unassigned`**（現行 prototype fixture 驗算：`21 === 9 + 30 − 18` ✅）。
<br>　**成因**：`byState.none` 為 **`AC-04` 口徑**（`totalUnits === 0` 依 `AC-04` 恆為 `none` ⇒ **含**無義務者），而 `incompleteTotal` 依使用者裁決**排除**它們（`AC-14` ⑪）⇒ 兩者自此**不同母體**，原式恆少扣一個 `byState.unassigned`。⚠ **原式在上一輪為真是因為當時語料無任何 `totalUnits === 0` 之文件**（`byState.unassigned` 恆為 0），**不是因為它是一條普遍成立的關係**——真庫 587/591 一到，差額就是 587。
<br>　🔒 **④ 一字未改、續為真**：`byState.all + byState.partial + byState.none === totalDocuments`（`13 + 9 + 30 = 52` ✅）——🔴 **`byState.unassigned` 不得加入這個和**：它是 `byState.none` 之**子集**，**不是第四個互斥類**；把四鍵相加會得到 70（=52+18），是本輪最容易犯的算術錯誤。
<br>　**另增兩條不變式（可直接建環）**：**⑤ `byState.unassigned ≤ byState.none`**（子集關係之機讀形式；等號成立即「所有未開始者都沒有使用部門」）；**⑥ `totalDocuments − byState.unassigned` ＝ 前端摘要行之 `[data-doc-coverage-tracked]`**（🔴 **端點不另給 `tracked` 欄，由前端做這個減法**）。
<br>　**現行 prototype fixture 之全套對帳值**：`totalDocuments`=**52**、`byState`={`all`:13, `partial`:9, `none`:**30**, `unassigned`:**18**}、`incompleteTotal`=**21**、`tracked`=**34**、摘要行下行三態=`13／9／12`。
...
- 📌 **欄名對映之延伸（2026-08-28 第二輪；📝 上一則原文逐字保留）**：新鍵 `byState.unassigned` ↔ 掛鉤 **`[data-doc-coverage-unassigned]`**（同一個字，無翻譯層）；摘要行之 **`[data-doc-coverage-tracked]` 刻意沒有對應欄位**——它是 `totalDocuments − byState.unassigned` 之**前端減法**，🔒 **端點不新增 `trackedDocuments` 之類的第三個計數欄**（多存一份等於多一個會漂移的真相；`total`／`unassigned` 兩個母體數已足以推出它）。<br>　🔴 **`unassigned` 亦為同字、但此處是同義**：`docScope === 'unassigned'` 與 `byState.unassigned` **指向同一個集合**（`totalUnits === 0` 之文件），**與 `all` 之同字不同義（顯示範圍 vs 文件層狀態）不同型**，不需要對照表隔離。<br>　🔴 **`byState` 之消費不得改用 `Object.keys()`**：摘要行下行之三個 `[data-doc-coverage-stat]` 片段**必須由寫死之 `['all','partial','none']` 陣列驅動**（prototype 25 即如此），🔒 **`unassigned` 不得渲染成第四個 `-stat` 片段**——它有自己的掛鉤與自己的行。⚠ 改用 `Object.keys(byState)` 會在畫面上多長出一個 `-stat="unassigned"`，並使「三態份數之和 === tracked」之斷言失去意義。<br>　🔒 **`docCoverage` 之其餘欄名、母體規則與「計數恆取自完整母體」一格未動**——`byState.unassigned` 同為**完整母體**之計數，🔴 **不隨 `docScope` 或 `maxRows` 改變**（四種範圍各請求一次，其值必須完全相同）。
...
> 📝 **2026-08-28 第二輪修正（[`OQ-E11-22`](../open-questions.md#e11-post-impl)，第四種呈現態）同樣未消耗任何 `AC-J#`**：其全部條文落於 **F042 本檔自身之 `AC-14`（增列 ⑧～⑮ 與三道負向鎖定）與 `AC-28` ⑲（增列）**，未於任何他檔產生 delta ⇒ **`AC-J27` 仍為下一支可用之編號**。
...
| **✅ 2026-08-28 第二輪修正新增之 3 組（`AC-14` 第四態條文之落點；`AC-28` ⑲）** | 🔒 **上一輪之 9 組與列上既有掛鉤一格未動**——本輪同為 **additive**。🔴 **`[data-doc-ojt-state]` 之值域維持 `all\|partial\|none` 三值、未增為四值** | — |
| `[data-doc-no-using-dept]` | 🔴 **無值屬性（bare attribute）**，掛於 `[data-doc-coverage-row]` 之 `<tr>` 上；🔒 **僅在 `totalUnits === 0` 時進 DOM**（**非** CSS 隱藏）。⚠ **同一列可同時是 `data-doc-ojt-state="none"` 且帶本屬性——這正是事實**（`AC-04` 說它是 `none`，區一說它沒有訓練義務），**兩者不衝突、不得互相對齊**。<br>🔒 本態列之 `[data-doc-coverage-ratio]`／`[data-doc-coverage-pct]` 皆呈現 **`—`**（**非 `0 / 0`**、**非 `0%`**），且**不畫進度條** | `AC-14` ⑧⑩⑫／`AC-28` ⑲ |
| ↳ `[data-doc-coverage-tracked="{n}"]` | 摘要行**上行**：已指定使用部門之份數（＝ `totalDocuments − byState.unassigned`，🔴 **由前端做這個減法**，端點不另給 `tracked` 欄）；🔒 **全頁恰 1 個**、屬性值即數字 | `AC-14` ⑬ |
| ↳ `[data-doc-coverage-unassigned="{n}"]` | 摘要行**上行**：未指定使用部門之份數（＝端點之 `docCoverage.byState.unassigned`）；🔒 **全頁恰 1 個**、屬性值即數字；🔒 **恆為完整母體之計數，不隨顯示範圍或上限改變**（同 `-stat` 之既有規則） | `AC-14` ⑬／`AC-14`（上一輪）負向鎖定 ① |
| ↳ 🔴 **`[data-doc-coverage-scope]` 之值域就地更正** | 📝 **上方該列原文逐字保留**（「值域**恰三值**」）；🔴 **自 2026-08-28 第二輪起為恰四值**：`incomplete`（🔒 **仍為預設**）／`completed`／**`unassigned`**／`all`，🔒 **`option` 順序即此順序**。🔒 **「切換＝重新請求、不得客端切換」一字未改** | `AC-14` ⑨ |
| ↳ 🔴 **`[data-doc-coverage-empty="{scope}"]` 之值域就地更正** | 📝 **上方該列原文逐字保留**；🔴 範圍空狀態由**兩句增為三句**（`incomplete`／`completed`／**`unassigned`**），🔒 `all` 仍恆為空字串、`no-docs` 仍與三者**互不相同、不得混用** | `AC-14` ⑭ |
| ↳ 🔴 **`[data-doc-coverage-summary]` 之內容結構就地更正** | 📝 **上方該列原文逐字保留**（其兩個屬性 `-scope-value`／`-shown` 未動）；🔴 **內容由一行增為兩個 `<div>` 行**——**上行**＝`-total`＋`-tracked`＋`-unassigned`；**下行**＝標籤 `已指定使用部門者之細分：` ＋ 三個 `-stat` ＋ `-incomplete`。⚠ **兩個行容器與該標籤目前皆無專屬掛鉤**（皆為裸元素）⇒ 🔵 **已回報 ux-fix；規格側不代為設計掛鉤**（沿用 `-total` 之既有處置路徑）。**建環時之可觀測形狀**＝`[data-doc-coverage-summary]` 恰 **2** 個元素子節點，且各自所含之掛鉤集合如上 | `AC-14` ⑬／`AC-28` ⑲(b) |
...
### 3-B. TAB2 文件分組模式（2026-09-01；`AC-30`～`AC-36`／`AC-28` ⑳）

> 🔴 **本節之掛鉤全部為新增（additive）**：[§3](#prototype-25-dom-contract) 之既有掛鉤在 `org`（預設）模式下**一格未動**；本節之掛鉤僅於 `document` 模式進 DOM。
> ⚠ **`prototypes/25-ojt-progress.html` 尚未含本模式**（本輪需求晚於 prototype 定稿）⇒ 🔒 **本節與 [§6](#prototype-25-dom-contract) ⑳ 群列為本模式逐字值與選擇器之權威**，非「照抄 prototype」；prototype 若日後補作，須以本節為準、不得另立一套。

| 掛鉤 | 說明 | 承載之 AC |
|---|---|---|
| `[data-ojt-group-mode]` | 分組模式 `select`，值域**恰二值** `org`（🔒 **預設**）／`document`；🔒 **`option` 順序即此順序**；`aria-label` 逐字見 [§6](#prototype-25-dom-contract) ⑳。🔴 **不得掛 `data-ojt-filter`**（`AC-13` 之「恰兩項」為既有鎖）；🔴 **切換不重新請求 rows** | `AC-30` |
| `[data-ojt-doc-search]` | 文件搜尋 `input[type="search"]`；比對 `documentNumber` 或 `documentName` 之**不分大小寫子字串**；🔒 **僅 `document` 模式進 DOM**（`org` 模式**完全不進 DOM**，非 CSS 隱藏），切回 `org` 時**清空關鍵字**；🔴 **同樣不得掛 `data-ojt-filter`** | `AC-33` ② |
| `[data-doc-group="{documentId}"]` | 🔒 **群組＝一份 ICSOP 文件**（非文件編號、非書名）；依 `documentNumber` 昇冪 | `AC-31` ①／`AC-34` |
| ↳ `[data-doc-group-number]`／`[data-doc-group-name]` | 文件編號／書名 | `AC-31` ② |
| ↳ `[data-doc-group-ratio]` | 逐字形狀 `已完成 {X} / 共 {Y} 單位`（🔴 **半形斜線**，與 `[data-doc-coverage-ratio]` 之 `{n} / {n}` 同家族；⚠ **與 `[data-ojt-derived-summary]` 之全形 `／` 刻意不同**——那是文件表單側之既有文案，**不得互相對齊**）。🔴 **X／Y 取自 TAB2 當下呈現之列**，不得改讀 `docCoverage` | `AC-31` ③／`AC-32` |
| ↳ `[data-doc-group-pct]` | `{n}%`；🔴 **必須由 `docGroupPercentText(X, Y)` 產出，而該函式必須委派既有 `coveragePercent(X, Y)`**（全頁單一百分比推導點；✅ 既有簽章＝`=> number | null`，見 `AC-32` 之查證註）；`Y === 0`（`coveragePercent` 回 `null`）⇒ 比照既有 `NO_STATISTICS_TEXT`，**不得** `NaN%`／`null%`／`0%`／`100%`（📌 該分支於元件層不可達，其載體為純函式層） | `AC-31` ③／`AC-32` |
| ↳ `[data-doc-group-toggle="{documentId}"]` | 折疊／展開控制項，帶 `aria-expanded`；🔒 **初始為 `"false"`（預設折疊）**；🔴 **展開才渲染組內列——未展開時該群組之 `[data-progress-row]` 完全不進 DOM（非 CSS 隱藏）**。🔒 其 `aria-label` **須含該文件之程序書編號**（📌 理由同 `AC-28` ⑩：多個群組之折疊鈕若無從分辨，螢幕閱讀器與 `getByRole` 皆點不到正確的那一顆）；🔵 **其完整句型為設計裁量、不入約束** | `AC-33` ① |
| ↳ 組內之 `[data-progress-row="{documentId}__{orgCode}"]` | 🔒 **既有掛鉤與其全部子掛鉤之「行為與互動」一格不改**（展開／新增／下載／刪除、角色守門、孤兒列規則）；組內依 `orgName` 昇冪。<br>🔴 **但「主標籤」隨分組維度切換**（2026-09-02 就地修訂）：本模式下列之主標籤為 `[data-progress-org-name]`＋`[data-progress-org-code]`，且為**唯一**主標籤；🔴 **`[data-progress-doc-number]`／`[data-progress-doc-name]` 於本模式完全不進 DOM**（非 CSS 隱藏）。🔒 列之識別**屬性**（`data-progress-row` 鍵／`data-progress-doc`／`data-progress-org`）**一格不動**。<br>📝 **原文逐字保留供追溯**：「🔒 **既有掛鉤與其全部子掛鉤、全部行為一格不改**」——⚠ 那句話正是 2026-09-01 實機缺陷之根因 | `AC-31` ④／`AC-34` |
| ↳ 組內列之 `[data-progress-org-name]` | 🔴 **2026-09-02 新增（`AC-31` ④-a）**：組內列之**主標籤**，逐字＝該列之 `orgName` **全名**（`公司簡稱 / 部 / 處室`，例 `和潤企業 / 財務會計部 / 財管室`；後端 `OjtOrgDirectory.nameOf` 之單一組裝點，`2f8acf1`）。🔒 **僅 `document` 模式進 DOM**（`org` 模式下 `querySelectorAll('[data-progress-org-name]').length === 0`，**非** CSS 隱藏）；🔒 **每列恰 1 個**；🔒 呈現語彙同 `org` 模式群組標題之 `[data-progress-group-name]` | `AC-31` ④-a／`AC-34` |
| ↳ 組內列之 `[data-progress-org-code]` | 🔴 **2026-09-02 新增（`AC-31` ④-a）**：該列之 `orgCode` **5 碼**代碼片；🔒 進 DOM 之條件、每列個數與呈現語彙同上（對應 `org` 模式群組標題之 `[data-progress-group-code]`）。⚠ **代碼片不帶公司碼**——公司已含於 `orgName` 全名之首段，兩處各帶一份即為分歧之起點 | `AC-31` ④-a |
| `[data-doc-group-basis-note]` | 🔴 **必要載體**（🔒 **全頁恰 1 個**、僅 `document` 模式進 DOM）：明說本區之 X/Y 取自**當下呈現之列**、與儀表板覆蓋率**刻意不同口徑**、兩處數字不相等屬正常。📌 **不是裝飾**——處置比照同頁既有之 `[data-doc-coverage-basis-note]`；逐字見 [§6](#prototype-25-dom-contract) ⑳ | `AC-32` |

> 🔴 **本節之三個負向鎖定（可直接建環）**：**①** `org` 模式下 `document.querySelectorAll('[data-doc-group]').length === 0`、`document` 模式下 `document.querySelectorAll('[data-progress-group]').length === 0`（**兩種群組容器互斥**，先以正向斷言確立當下模式之群組容器存在）。**②** `document.querySelectorAll('[data-ojt-filter]').length === 2` 在**兩種模式下**皆成立。**③** 群組未展開時 `document.querySelectorAll('[data-progress-row]').length === 0`——⚠ **本條必須配一個正向對照**（同一份語料在 `org` 模式下之 `[data-progress-row]` 數 > 0），否則「列根本沒載進來」與「列被折疊起來」在斷言上無從分辨，該負向斷言會退化為恆真。**④（2026-09-02 增補）** 展開後，同一群組內各列之 `[data-progress-org-name]` 逐列比對其 `orgName`（有序陣列 `toEqual`），語料須使**組內至少 3 列之 `orgName` 互不相同**——⚠ **只驗「有出現任一單位名」在「每一列都印第一列名字」之實作下仍會過**；再配 `org` 模式之對偶負向（`[data-progress-org-name]` 恰 0 個、列之主標籤仍為文件編號／書名），防止修過頭把兩邊對調。**⑤（2026-09-02 增補）** `document` 模式下 `document.querySelectorAll('[data-progress-doc-number]').length === 0`／`[data-progress-doc-name]` 同 ——⚠ **本條必須配 `org` 模式之正向對照**（同一份語料在 `org` 模式下該兩掛鉤存在且值為各列自己的文件），否則「兩掛鉤根本沒實作」與「本模式刻意不渲染」在斷言上無從分辨，該負向斷言會退化為恆真。**⑥** 同一 `[data-doc-group]` 內文字等於書名之葉節點**恰 1 個**（`AC-31` ④-c）。
...
| ⑲ | **顯示範圍 `select` 之第四個 `option`**（`[data-doc-coverage-scope]`；🔴 **值域自此為恰四值**，📝 上方 ⑯ 之「三個 `option`」列原文逐字保留） | `僅未指定使用部門`（值 `unassigned`）<br>🔒 **四個 `option` 之順序**＝`僅未全部完成`／`僅已全部完成`／`僅未指定使用部門`／`全部文件`，**第一個仍為預設**；🔒 其 `aria-label` **一字未改**（`依文件逐筆之顯示範圍`） | `25` |
| ⑲ | **第四態之晶片逐字與視覺**（`[data-doc-ojt-state-chip]` 於 `totalUnits === 0` 之列） | 逐字 `未指定使用部門`；icon 鍵 **`circle-slash`**（🔴 **刻意跳出 `file-*-2` 家族**——三態是同一把量尺上的刻度，本態在尺之外）；色票 **`text-slate-500`**（🔴 **刻意不比照 `none` 之 `text-slate-300`**，後者白底約 1.7:1、屬**待裁之既有議題**）<br>🔒 **不取自 `OJT_DOC_STATE`**（與 `13` 共用之三態常數，**不得加第四鍵**） | `25` |
| ⑲ | **第四態之比值與百分比** | `[data-doc-coverage-ratio]` → **`—`**（**非 `0 / 0`**）；`[data-doc-coverage-pct]` → **`—`**（**非 `0%`**，且**不畫進度條**） | `25` |
| ⑲ | **摘要行上行之三個片段**（🔴 **逐掛鉤斷言、禁止對整行串接後下逐字**——同 ⑯ 之既有紀律，各 `<span>` 之 `textContent` 之間無空白字元） | ①`[data-doc-coverage-total="{n}"]` → `共 {n} 份文件`（**既有，一字未動**）<br>②`[data-doc-coverage-tracked="{n}"]` → `已指定使用部門 {n} 份`<br>③`[data-doc-coverage-unassigned="{n}"]` → `未指定使用部門 {n} 份`（前置 `circle-slash` icon，`aria-hidden`；文字色＝`text-slate-500`） | `25` |
| ⑲ | **摘要行下行之標籤**（🔵 **無專屬掛鉤、為裸 `<span>`**——已回報 ux-fix，規格側不代為設計掛鉤） | `已指定使用部門者之細分：`<br>🔴 **本句不是裝飾**：下行之 `尚未開始 {n} 份` 顯示的是「**有義務卻一列都沒完成**」（現行 fixture ＝ **12**），**刻意不等於** `AC-04` 口徑之 `byState.none`（＝ **30**，含 18 份無義務者）；少了這句標籤，讀者會把兩者當成同一個數而判為 bug | `25` |
| ⑲ | ↳ **下行之四個既有片段逐字一字未改** | `[data-doc-coverage-stat="all\|partial\|none"]` 三者 ＋ `[data-doc-coverage-incomplete="{n}"]`（標籤仍為 `尚未全部完成合計`）<br>🔴 **`-incomplete` 之語意就地更正**：其值＝`partial` ＋ **有義務之** `none`（**排除**無義務者）⇒ **不再等於** `byState.partial + byState.none`；現行 fixture ＝ **21**（＝9 ＋ 12），關係式見 [§架構設計 一-2](#architecture) 之不變式 ③′ | `25` |
| ⑲ | **截斷名詞表由三個變體增為恰四個**（🔒 **一 `scope` 一個，其餘句子仍完全相同、不寫四句**） | `incomplete` → `尚未全部完成之文件`／`completed` → `已全部完成之文件`／**`unassigned` → `未指定使用部門之文件`**／`all` → `文件` | `25` |
| ⑲ | **截斷句之排序描述就地更正**（📝 ⑯ 之原句逐字保留於上方） | **`incomplete`／`completed`／`all` 三個範圍**（🔴 **原句補入一個括號段**）：`本表依覆蓋率由低至高排序（未指定使用部門之文件一律排在最後），未列出者之覆蓋率均不低於已列出者`<br>**`unassigned` 範圍**（🔴 **另一句，不得照抄上句**——該範圍下所有列之覆蓋率皆為 `—`，宣稱「依覆蓋率排序」是**假話**）：`本表依程序書編號昇冪排序` | `25` |
| ⑲ | **截斷句之「完整的去哪看」增一個分岔**（🔴 **實質內容改變、非文案潤飾**） | **`incomplete`／`completed`／`all`**：`完整清單請至「OJT 資料清單」分頁逐列檢視。`（**既有，一字未動**）<br>**`unassigned`**：`完整清單與使用部門之設定請至「ICSOP 文件管理」。`<br>🔴 **不得把本範圍之使用者導去「OJT 資料清單」**——這些文件沒有使用部門就**沒有進度列**，過去只會看到空的；正確去處是去把使用部門補上（去處與既有之全域空狀態補充提示同源，指向同一個頁面，**不另造詞**） | `25` |
| ⑲ | ↳ **完整句型（四個 `scope` 共用之骨架，僅兩處分岔）** | `本表僅列出前 {maxRows} 份，另有 {hidden} 份{名詞}未列出；{排序描述}。{去處}`<br>🔒 `{maxRows}` 仍**取自回應之 `docCoverage.maxRows`**，🔴 **前端不得再硬寫一份 15**（既有規則未動） | `25` |
| ⑲ | **範圍空狀態由兩句增為三句**（`[data-doc-coverage-empty="{scope}"]`） | `unassigned` → `所有文件皆已指定使用部門`<br>🔒 `incomplete`／`completed` 兩句與「`all` 恆為空字串」**一字未改**；🔒 其共用補充提示（`切換顯示範圍為「全部文件」可檢視全部文件之覆蓋率。`）亦未動<br>⚠ **三句於 prototype 之 fixture 下皆不可達**（13 份已完成／21 份未完成／18 份未指定，三者皆非 0）⇒ **建環須以 fixture 驅動，不得試圖在原型上操作出來** | `25` |
| ⑲ | ↳ **現行 fixture 之對帳值**（供建環對帳，非斷言字面） | `totalDocuments` ＝ **52**（📝 上一輪為 **34**；本輪語料補 `d35`～`d52` 共 **18 份無義務文件**）／`tracked` ＝ **34**／`byState`（`AC-04` 口徑）＝ `{all:13, partial:9, none:30}`／**`byState.unassigned` ＝ 18**／摘要行下行三態 ＝ `13／9／12`／`incompleteTotal` ＝ **21**<br>各範圍之母體／`hidden`（上限 15）＝ `incomplete` **21／6**、`completed` **13／0**、`unassigned` **18／3**、`all` **52／37**<br>🔒 **`incomplete`（預設範圍）之全部對帳值與上一輪完全相同**（21／6、`incompleteTotal` 21）——**可作為「本輪未動既有口徑」之佐證**；🔒 **`completed` 仍是全檔唯一「截斷提示不存在」之可達狀態**（13 ≤ 15；`unassigned` 之 18 > 15 ⇒ 仍會截斷） | `25` |
| ⑳ | **分組模式 `select` 之兩個 `option`**（`[data-ojt-group-mode]`；值域 `org`／`document`，🔒 **順序即此順序、第一個為預設**） | `以使用單位分組`（`org`）／`以文件分組`（`document`） | 本檔 §3-B |
| ⑳ | ↳ 其 `aria-label` | `資料清單之分組方式` | 本檔 §3-B |
| ⑳ | **文件搜尋**（`[data-ojt-doc-search]`）之 `aria-label`／`placeholder` | `搜尋文件`／`搜尋文件（編號或書名）…`（🔒 **句尾為單一刪節號 `…`**，比照既有 `搜尋使用單位（名稱或代碼）…`） | 本檔 §3-B |
| ⑳ | **文件群組之完成度**（`[data-doc-group-ratio]`） | `已完成 {X} / 共 {Y} 單位`（🔴 **半形斜線**；⚠ 與 `[data-ojt-derived-summary]` 之全形 `／` 刻意不同、不得對齊） | 本檔 §3-B |
| ⑳ | **口徑說明行**（`[data-doc-group-basis-note]`；🔴 **必要載體、非裝飾**——見 `AC-32`） | `本區各文件之「已完成 X / 共 Y 單位」取自本清單當下呈現之進度列（含已裁撤單位與已移出使用部門之單位），與儀表板「文件-訓練覆蓋率」之口徑刻意不同；兩處數字不相等屬正常，請勿互相對帳。` | 本檔 §3-B |
| ⑳ | ↳ **空狀態一律沿用既有常數、不另造詞** | 篩選（含文件搜尋）無結果 ⇒ `查無符合條件的進度列`（既有 `EMPTY_ROWS_TEXT`，一字未改）；全域無任何進度列 ⇒ `目前沒有任何 OJT 進度列`（既有 `EMPTY_ALL_TEXT`）＋其既有補充提示（`EMPTY_ALL_HINT`） | `25`（既有） |
...
> ✅ **2026-08-28 第二輪修正（[`OQ-E11-22`](../open-questions.md#e11-post-impl)，第四種呈現態）**：🔴 **未新增任何「不得建環」項**——其全部定值（判準 `totalUnits === 0`、四值範圍、三段排序鍵、`incompleteTotal` 之排除、`—` 之退化呈現、摘要行兩行、四個名詞變體、第三句範圍空狀態、`circle-slash`／`text-slate-500`）**皆為使用者裁決或已凍結之 prototype 定稿值**，逐字見 [§6](#prototype-25-dom-contract) ⑲ ⇒ **可逕行建環**；🔒 **本節現存之唯一排除項仍為「側選單新項之位置」**。
> 🔵 **另有二項為「已回報 ux-fix、規格側不代為設計」，環側同樣不得自行造掛鉤**（沿用 `-total` 之既有處置路徑）：① 摘要行**兩個行容器**無專屬掛鉤；② 下行標籤 `已指定使用部門者之細分：` 為裸 `<span>`。⇒ **兩行結構之可觀測形狀＝ `[data-doc-coverage-summary]` 恰 2 個元素子節點、各自所含掛鉤集合固定**，斷言請釘該形狀，**不得**自行加一個新的 `data-*`。
...
## UX16 delta — 2026-09-22 OJT 進度管理三項（項 14／15／16；`AC-UX#` 批） {#ux16-delta}

> **來源**＝[stories/2026-09-22-ux-delta-16.md](../../stories/2026-09-22-ux-delta-16.md) 第 14／15／16 項；裁決紀錄＝[open-questions §UX16](../open-questions.md#ux16-2026-09-22)（`OQ-UX16-19`／`27`～`31`，🟢 全數 APPROVED）。
> **本 delta 之 AC 編號採 `AC-UX#`**（編號規則與跨檔範圍見 [F002 §UX16 delta](F002-role-based-routing.md#ux16-delta) 檔頭）。🔴 **本 delta 未消耗任何 `AC-J#`**（`AC-J27` 仍為下一支可用之編號，配發條件仍為「跨六檔之 delta AC」）；亦**不沿用** F042 自身之 `AC-##` 序列。
> 🔴 **人類已裁決之方向（2026-09-22，不得再翻案）**：**C**＝新增獨立的「制定本部」下拉篩選（與項 10／11 一致）。
> 🔴 **本輪之約束環為簡化版（只有 `backend jest` ＋ `frontend vitest`）** ⇒ **AC 是唯一防線**。
> 🔒 **本 delta 不動**：TAB1 三區之全部統計口徑（`AC-14`～`AC-17`）、`AC-03`／`AC-04` 之完成判定、孤兒與裁撤之既有語意、場次新增流程、版次追蹤（`AC-37`～`AC-40`）。

### 一、TAB2 單位名稱加入本部（項 14）

- **AC-UX45**（🔴 **`orgName` 由三段改四段，併入既有組裝點**；`OQ-UX16-27`＝選項 A）：Given TAB2 進度列之 `orgName` 由 `OjtOrgDirectory.nameOf(companyCode, orgCode)` 組裝（`ojt-progress.service.ts:598`，單一組裝點）, When 該單位可沿 `parentCode` 上溯至一個本部層（`tier === 'DIVISION'`）之單位, Then `orgName` **逐字**為 **`{公司簡稱} / {本部全名} / {部全名} / {處室簡稱}`**（四段，分隔符逐字沿用既有 `ORG_PATH_SEPARATOR` ＝ ` / `，前後各一個半形空格）。<br>
  🔒 **既有三段之取值規則一字不改**：公司簡稱＝`resolveCompanyShortName(companyCode)`；部全名＝部層之 `descFull`；處室簡稱＝自身 `name` 以 `/` 分段之**最末段**（`org-path.ts` 之契約 §8.2／§8.3）。**新增的只有第二段。**<br>
  🔴 **空段自動收合、不產生連續分隔符**（沿用既有規則）：Given 該單位**上溯不到任何本部層**, Then `orgName` **仍為既有三段**（🔴 **不得**插入空字串或任何 sentinel 文字如 `無本部` ——那會讓一個內部分類概念出現在使用者看得到的單位名稱裡）。<br>
  🔴 **另一個分支見 [`AC-UX57`](#ux16-delta)**：Given 傳入之 `orgCode` **本身即為本部層代碼**（`X0000` 形，**真實可達**——使用部門候選不依層級過濾）, Then 本部段與部段會解析為**同一個單位** ⇒ 該段**只輸出一次**（恰兩段）。🔒 **與上一款是不同的分支**（上款＝本部段**不存在**；`AC-UX57`＝本部段**存在但與部段同一個單位**），**須各有向量**。<br>
  🔒 **`companyCode` 之解析失敗（未登錄之 COMPID）仍只呈現組織路徑**、**`orgCode` 查無仍退回代碼本身**——兩條既有 fallback 一字不改。<br>
  📌 **本部推導須與 [F017](F017-backend-document-list.md#ux16-delta) `AC-UX43` ④ 共用同一份實作**，🔴 不得於 OJT 側另寫一套。

- **AC-UX46**（🔴 **本項之假綠形狀：語料裡沒有本部那一列，測試會綠而功能沒動**）：Given 本 delta 之任一測試語料, Then 其 `ORG_UNIT` 組織目錄語料**必須成對備妥兩組，且兩組之預期 `orgName` 必須不同**——
  - ⓐ **含本部層那一列**（`{公司}X0000`，`tier === 'DIVISION'`）⇒ 預期為**四段**；
  - ⓑ **不含本部層那一列**（同一組織但 `X0000` 於目錄中缺席）⇒ 預期為**三段**。
  - 🔴 **為何非有不可（理由逐字，不得省略）**：`backend/src/ojt-progress/typeorm-ojt-org-directory.ts` **沒有任何 spec 檔**，而 `orgName` 在 **7 個前端 OJT 測試檔**裡全是**手寫 fixture**（字串直接寫死）。若改寫 `backend/src/ojt-progress/ojt-progress.cross-company.spec.ts` 時**沒有在 fixture 補上該公司的 `X0000` 那一列**，本部段會依 `AC-UX45` 之「空段收合」規則**自動消失** ⇒ 期望值維持舊的三段字串 ⇒ **測試變綠而功能完全沒動**。
  - 🔴 **`backend/src/ojt-progress/ojt-progress.cross-company.spec.ts:184-185` 之兩句逐字期望值為預期轉紅、須就地改寫（非回歸）**：現行為 `expect(as?.orgName).toBe('和潤企業 / 車輛分期營業一部')` 與 `expect(aj?.orgName).toBe('和勁企業 / 商用車輛一部')`。<br>　🔒 **改寫方向＝同時補 fixture 與期望值**：於 `seedTwoCompanies` 之組織目錄中補上兩家公司各自之 `X0000` 列，期望值改為含本部之四段字串。<br>　🔴 **只改期望值而不補 fixture 會讓它永遠紅；只補 fixture 而不改期望值會讓它永遠紅——兩者都不是本條要的**；本條要的是**兩者同批改，且另留一條 ⓑ 案例證明「沒有本部那一列時仍是三段」**。
  - 🔴 **同一義務擴及前端七個 OJT 測試檔之手寫 `orgName` fixture**：凡其期望值涉及單位名稱者，一律同批改為四段；🔴 **不得**以「前端 fixture 是自己寫的、改成什麼都行」為由只改字面——那會讓前端測試與後端真實輸出**永久分歧**且無人察覺。

- **AC-UX57**（🔴 **本部段與部段解析為同一單位時只輸出一次——輸入為「本部層代碼本身」之情形**；2026-09-22 第七輪補訂【lead 收斂】；`AC-UX45` 之未定義輸入）：Given `createOrgPathResolverWithDivision(orgCode)`（`backend/src/org-directory/org-division.ts`）收到之 `orgCode` **本身即為本部層代碼**（`X0000` 形）, When 組裝 `orgName`, Then 其**本部段所解析出的單位與部段為同一個單位**時，該單位**只輸出一次** ⇒ 結果為 **`{公司簡稱} / {本部全名}`**（恰兩段），🔴 **不得**輸出 `{公司簡稱} / {本部全名} / {本部全名}`。<br>
  🔴 **① 判準以 `orgCode` 相等為準，明文禁止以顯示名相等為準（理由不得省略）**：不同層級之兩個單位**理論上可能有相同顯示名**，以名稱去重會把「**兩個真的不同的單位**」誤併成一段；以代碼去重則精確。<br>
  🔒 **② 收合行為沿用 `org-path.ts` 既有之同一紀律**（「空欄自動收合、不產生尾綴分隔符」）——🔴 **不是新發明一條規則**，是既有規則在「該段被判定為重複」時的延伸；分隔符仍為 `ORG_PATH_SEPARATOR`（` / `）。<br>
  🔴 **③ 為什麼這條看起來像邊界案例卻必須寫（本條存在的全部理由）**：<br>
  　**它是真實可達的輸入，不是防禦性程式設計。**OJT 進度列由 `DOC_USING_DEPT` 衍生（`AC-01`），而**使用部門的候選不依層級過濾**——`frontend/src/pages/DocumentEditPage.tsx:410` 之 `usingDeptOptions = orgUnits.map((u) => ({ value: u.orgCode, … }))`，其來源為 `getOrgUnits(company)`（`:291`）之該公司**全部**組織單位（該檔 `:408` 之註解逐字寫著「使用部門候選＝**全部組織單位（任意層級）**」）。⇒ 使用者**可以**把某份文件的使用部門設成一個**本部層**單位，該進度列之 `orgCode` 就是 `X0000`。<br>
  　🔴 **機制上為何會撞**：`departmentCodeOf(orgCode)`（`backend/src/org-directory/org-path.ts:119`）＝ `orgCode.slice(0, 2).padEnd(5, '0')` ⇒ 對 `D0000` 回傳 **`D0000`（映射回自己）**；而本部段亦解析為 `D0000` ⇒ **同一個單位被印兩次**。<br>
  　⚠ **「既有呼叫端都是拿部／處室的 `orgCode` 呼叫」這個推論對本頁不成立**：它對 [F017](F017-backend-document-list.md)／[F019](F019-public-list-browsing.md) 之**制定部門**成立（[F014](F014-accountable-dept-chief.md) 三級相依），但 **F042 的使用部門是另一個欄位、另一條資料來源** ⇒ 🔒 **兩者不得互相援引**。<br>
  🔴 **④ 可測形狀（三條固定向量，🔴 必須成組，缺任一條即證明不了「收合只在同一單位時發生」）**：<br>
    | # | 輸入 | 期望 | 它守的是 |
    |---|---|---|---|
    | ⓐ | `resolve('D0000')`（**本部代碼本身**） | **恰兩段** `{公司} / {本部}` | 🔴 **本條之主體** |
    | ⓑ | `resolve('DA000')`（該本部底下之**部**） | **恰三段** `{公司} / {本部} / {部}` | 🔒 收合**不得**外溢 |
    | ⓒ | `resolve('DAB00')`（該部底下之**處室**） | **恰四段** `{公司} / {本部} / {部} / {處室}`（`AC-UX45`） | 🔒 收合**不得**外溢 |
  　🔴 **只寫 ⓐ 時，一個「把四段一律砍成三段」的實作照樣綠**——ⓑⓒ 才證明收合**只在本部段與部段為同一 `orgCode` 時**發生。<br>
  🔒 **⑤ 與 `AC-UX45` 之「上溯不到本部 ⇒ 仍為既有三段」互不重疊**：那一款處理的是**本部段不存在**；本條處理的是**本部段存在但與部段是同一個單位**。🔴 **兩者是不同的分支、須各有向量**（`AC-UX46` ⓐⓑ 兩組語料涵蓋前者，本條 ⓐ 涵蓋後者）。<br>
  📝 **本條之由來（留檔）**：`ring-ux16` 建環時發現該輸入之期望行為在 `AC-UX45` 與 [architecture-spec §16.7](../architecture-spec.md) 皆未定義，**判為不可達路徑、移除該斷言並留註解**——🟢 **其處置正確：它沒有自行裁決**。🔴 **但「不可達」這個判斷是錯的**，經 lead 查證 `DocumentEditPage.tsx:410`／`:291` 後確認可達且會產出可見缺陷。<br>
  　🔒 **這是一條值得記住的紀律**：**「這條路徑走不到」是一個關於上游輸入範圍的主張，不是一個關於本函式的主張** ⇒ 🔴 **下判斷前必須指出「誰是呼叫端、它的輸入從哪裡來」**，而不是從「既有測試都沒這樣呼叫」反推。

- **AC-UX47**（🔴 **排序連帶變成先依本部分群——這是後果，不是意外**）：Given TAB2 之列以 `orgName.localeCompare()` 為第一排序鍵（`ojt-progress.service.ts:623-626`，既有）, When `AC-UX45` 之四段格式生效, Then 其排序結果**連帶變為「先依公司、再依本部、再依部、再依處室」**——🔴 **本條明文承認並鎖定此後果**。<br>
  📌 **理由**：`orgName` 是排序鍵之**字串本身**，在第一段（公司簡稱）與第三段（部）之間插入一段，等於插入一層新的分群。這在使用者眼中是合理的（同一本部的單位會相鄰），但它**改變了既有列序**，因此必須是一條寫明的 AC 而不是一個沒人提過的副作用。<br>
  🔴 **可測形狀**：語料須含**同一公司、不同本部、但部名字典序交錯**之至少三個單位（例：本部 A 下之「乙部」、本部 B 下之「甲部」、本部 A 下之「丙部」）⇒ 斷言輸出序為 `A/乙 → A/丙 → B/甲`（**先本部後部**），而非三段時代的 `甲 → 乙 → 丙`。<br>
  　🔴 語料若讓「依本部分群」與「依部名字典序」輸出相同，本條恆真。<br>
  🔴 **本條只能在實作側驗、prototype 側無載體（2026-09-22 第三輪補訂，ui-ux-designer 提報）**：`prototypes/25-ojt-progress.html` 之群組次序為 **`orgCode` 昇冪**（見該檔 `:1779` 之 `renderRows` 註解），與實作端之 `orgName.localeCompare` **不同**。<br>
  　🔒 **這是既有且明文刻意不對齊之落差**（出處＝`prototypes/25-ojt-progress.html:160-161`）：對齊會改掉**未帶參數時本頁的既有次序**，直接違反 [F044](F044-admin-dashboard-analytics.md) `AC-G21`／`AC-G79` 之零漣漪鎖定。⇒ **本輪維持不對齊。**<br>
  　🔴 **⇒ 本條之驗證載體恆為後端純函式／服務層**（以固定語料驅動排序後比對 id 序列），🔴 **明文禁止**以 prototype 之群組次序作為本條之期望值來源——那會是一條**鎖住錯誤事實**的斷言。<br>
  🔒 **第二排序鍵 `documentNumber.localeCompare()` 一字不改**；🔒 **前端 `groupRowsByOrg()` 之群組鍵仍為 `orgGroupKeyOf(companyCode, orgCode)`（`{companyCode}__{orgCode}`）、其群組 `label` 仍直接取用後端組好之 `orgName`**——🔴 **不得**改為以本部為群組鍵（那是另一個裁決，且會與 `AC-11` 之「以使用單位為群組」正面牴觸）。

- **AC-UX48**（🔴 **單位搜尋自動涵蓋本部**）：Given TAB2 之「單位搜尋」以 `includesCi(r.orgName, q) || includesCi(r.orgCode, q)` 比對（`ojt-progress.service.ts:613-614`，既有）, When 使用者輸入**本部名稱之關鍵字**, Then 該本部下轄之全部單位之列**皆被篩出**。<br>
  🔒 **比對實作一行未改**——本款是 `AC-UX45` 之自然後果（本部名已在 `orgName` 字串裡），🔴 **不得**為此另寫一段比對邏輯。<br>
  📌 **可測形狀**：以 ⓐ 組語料（含本部層）輸入本部名關鍵字 ⇒ 斷言**恰篩出該本部下之列**；🔴 並以 ⓑ 組語料（無本部層）輸入同一關鍵字 ⇒ 斷言**篩出 0 列**——成對之後，「比對邏輯被改成什麼都命中」會在 ⓑ 翻紅。

### 二、TAB2 新增「制定本部」篩選（項 15）

- **AC-UX49**（🔴 TAB2 篩選 二 → **三項**，新項置於**最前**；`OQ-UX16-29`）：Given TAB2 已載入, When 檢視篩選控制項, Then 其**恰為 3 個**，順序由左至右（行動：由上而下）依序為 **制定本部** → **單位搜尋** → **完成狀態**。<br>
  🔴 **上一句的三個名字是「條文中的稱呼」，不是 `aria-label` 之逐字值——不得據以斷言**。三者之 `aria-label` **逐字**分別為：新項 **`制定本部`**（本 delta 新增）／既有兩項 **`搜尋使用單位`**（🔒 **非**「單位搜尋」）與 **`完成狀態`**。<br>
  　🔴 **為何要特別寫這一句**：F042 之既有條文（`AC-13` 步驟 8）通篇以「單位搜尋」稱呼第一項，而畫面上那個控制項的 `aria-label` 實際是 `搜尋使用單位`（`placeholder` 為 `搜尋使用單位（名稱或代碼）…`）。⇒ 若下游把本條的「逐字」讀成 `aria-label`，會建出一條**鎖住一個畫面上不存在的字串**的約束，並在「修正」它時改掉既有的可見文案。<br>
  　🔒 **既有兩項之 `aria-label`、`placeholder` 與 `option` 文字一律一字不動**（`AC-UX54` ⑦）。<br>
  🔒 **DOM 契約**：新控制項**必須掛 `data-ojt-filter="division"`**（值域自 `org|status` 擴為 **`division|org|status`**），置於既有 `[data-ojt-filter-bar]` 內；型態為下拉（`select` 或 combobox，由 ui-ux-designer 定稿）。<br>
  🔒 **對未來的提醒（本條自己就是下一個陳舊鎖）**：本條之「**恰為 3 個**」是本輪的新權威，寫成絕對值沒有問題；🔴 **但它自此就是下一批 delta 要處理的那個陳舊絕對值鎖**——日後任何人再新增第四項篩選時，`AC-UX50` 表格裡列的那六處**加上本條自己**都會同時矛盾。<br>
  　📌 **本批的教訓不只屬於 `AC-UX50` 的表格**：凡寫下「恰 N 個」，就同時建立了一筆**日後必須被找到**的技術債；找它的方法是 **grep「值的字面」而非常數名**（`AC-UX50` 末段之通則）。<br>
  🔒 **新控制項之 placeholder option（2026-09-22 第三輪補訂，權威＝`prototypes/25-ojt-progress.html`）**：其「不施加限制」之預設選項**可見文字逐字為 `所有制定本部`**——🔒 句型比照同列既有之 `所有完成狀態`（`完成狀態` select 之第一個 `option`），**不得**寫成 `全部`／`全部本部`／`— 制定本部 —`。<br>
  　📌 **可測形狀**：斷言該 `select` 之第一個 `option` 文字逐字為 `所有制定本部` 且其 `value` 為空字串（＝不施加限制）；🔴 並成對斷言既有之 `所有完成狀態` **仍在**（`AC-UX54` ⑦）。<br>
  📝 **DOM id 之處置（勿誤鎖）**：prototype 依本頁 `f{X}` 慣例給了 `#fDivision`（另 `#fGroupMode`／`#fDocSearch` 為既有已上線控制項在 prototype 側之對應 id）。🔴 **本 AC 不鎖任何 DOM id**——實作端（`OjtProgressPage.tsx`）之篩選控制項**現行完全不帶 `id`**，其定位點一律為 **`data-ojt-filter="division|org|status"`**；把 prototype 端的 id 寫成 src 的約束，等於**要求實作去長出一個 prototype 自己發明的東西**。<br>
  　⚠ **本頁與 [F017](F017-backend-document-list.md) 刻意不同**：該頁之 `cbD_{key}_input`／`cbD_{key}_clear` **是 src 裡真實存在的 id**，故 `AC-D10` 鎖得住；本頁沒有那個事實。
  🔴 **明文禁止「把新篩選做成不掛 `data-ojt-filter` 以規避既有絕對值鎖」**（`OQ-UX16-28` 人類確認）：那會使「TAB2 篩選恰三項」這件事**本身無法被機器驗證**，並且讓既有那三條鎖從此鎖住一個與畫面不符的數字——**比翻紅更糟**。<br>
  🔒 **既有兩項之掛鉤、選項集合與逐字文案一格未動**：`[data-ojt-filter="org"]`（單位搜尋）、`[data-ojt-filter="status"]`（完成狀態，✅ 恰三值 `所有完成狀態`／`已完成`／`尚未完成`）。<br>
  🔒 **`[data-ojt-group-mode]`（分組模式切換）與 `[data-ojt-doc-search]`（文件搜尋）仍不得掛 `data-ojt-filter`**（`AC-30`／`AC-33` ② 之既有規則一字不改）——🔴 **它們不是篩選**：一個不移除任何列、一個只在 `document` 模式存在。<br>
  🔴 **比對語意**：等值比對，語意為「該進度列之使用單位**沿 `parentCode` 上溯所抵達之本部** ＝ 所選值」；未選定時不施加限制；與既有兩項並用為 **AND**。<br>
  🔒 **選項來源＝TAB2 當前語料之 distinct 本部**（比照既有篩選之口徑）；🔴 **推導不出本部之單位不產生選項**，且該單位之列在**未選定任何本部**時**照常呈現**（與 [F019](F019-public-list-browsing.md#ux16-delta) `AC-UX24` 同構：沒有選項就是篩不到，**不加 sentinel**）。<br>
  🔒 **兩種分組模式下皆持續生效**（`AC-30` 之既有規則擴及第三項）。

- **AC-UX50**（🔴 **既有三處絕對值鎖之就地改寫；一律改為回歸鎖，不得只把 2 改成 3**）：Given `AC-UX49` 實作完成, Then 下列**每一處**皆為預期轉紅、須就地改寫（非回歸）——

  | # | 位置 | 現行內容 | 改寫方向 |
  |---|---|---|---|
  | ① | 本檔 `AC-13`（＋[§Main Flow](#acceptance-criteria) 步驟 8「提供**恰兩項**篩選」） | 「其**恰為 2 個**——① 單位搜尋；② 完成狀態」 | 就地改為 **恰 3 個**、順序 `制定本部` → `單位搜尋` → `完成狀態`；🔒 其「完成狀態**恰三值**、比對列自身之二態、**明文不含「部分完成」**」之既有裁決（`OQ-E11-18`）**一字不動** |
  | ② | 本檔 `AC-30` | 「🔴 **本控制項不得掛 `data-ojt-filter`**：`AC-13` 之『篩選**恰兩項**』為既有鎖 ⇒ `[data-ojt-filter]` 之數量在**兩種模式下**恆為 **2**」 | 就地改為**恆為 3**；🔒 其真正在守的事（**分組模式切換不是篩選、不得計入**）一字不動 |
  | ③ | 本檔 `AC-36`／`AC-33` ② 之既有掛鉤清單 | `[data-ojt-filter="org|status"]` | 值域擴為 `division|org|status`；🔒 既有兩值之行為與逐字文案不變（本 delta 為 **additive**，無既有掛鉤改名） |
  | ④ | `frontend/src/pages/OjtProgressPage.test.tsx:648` | `expect(document.querySelectorAll('[data-ojt-filter]')).toHaveLength(2)` | 見下方回歸鎖形狀 |
  | ⑤ | `frontend/src/pages/OjtProgressPage.test.tsx:1552`（`AC-30` 分組模式案例） | 同上（`sel.hasAttribute('data-ojt-filter') === false` ＋ `toHaveLength(2)`） | 同上；🔒 **`hasAttribute(...) === false` 那一句一字不動** |
  | ⑥ | `frontend/src/pages/OjtProgressPage.test.tsx:1743`（`AC-33` ② 文件搜尋案例） | 同上 | 同上；🔒 **`hasAttribute(...) === false` 那一句一字不動** |

  - 🔴 **④⑤⑥ 之改寫方向＝回歸鎖，不是 `toHaveLength(3)`**：新斷言須**同時**滿足 ⓐ `[data-ojt-filter="org"]` **仍存在**、ⓑ `[data-ojt-filter="status"]` **仍存在**、ⓒ `[data-ojt-filter="division"]` **存在**、ⓓ 總數恰 3。<br>　🔴 **只把 2 改成 3 時**，一個「把 `單位搜尋` 換成 `制定本部`」或「分組模式切換誤掛了 `data-ojt-filter`」的實作**照樣全綠**——而後者正是 `AC-30` 花了整段在防的事。
  - 🔴 **通則（本 repo 血訓，2026-09-02 一天重演三次）**：**新斷言要 N+1、既有他處斷言鎖死 N ⇒ 同一份環不可能全綠。**

### 三、TAB2 新增匯出（項 16）

- **AC-UX51**（🔴 匯出鈕與其範圍）：Given 我在 TAB2, When 檢視頁面, Then 存在**恰一顆**匯出鈕，其 `aria-label` **逐字**為 **`匯出`**、DOM 掛鉤逐字為 **`data-ojt-export`**；When 點擊, Then 產生一個 CSV 檔，其**資料列集合恰等於當前畫面三項篩選（`AC-UX49`）套用後之進度列集合**，**列序恰等於畫面當前之排序**（`AC-UX47`）。<br>
  🔒 **匯出範圍不含 TAB1**（`OQ-UX16-30`）：🔴 三區統計（覆蓋率、完成率、最近完成）**一律不進本檔**；⚠ 若日後需要，須另立 AC。<br>
  🔒 **孤兒列與裁撤單位之列比照畫面**：`AC-17` 之「統計排除 ≠ 操作禁止」既有界線不變——它們在 TAB2 **看得到**，因此**也匯得出來**。<br>
  🔒 **檔案層規則沿用 [error-handling.md#export](../error-handling.md#export) 與 [F017](F017-backend-document-list.md) `AC-X1`／`AC-X2` 之既有規則，一字不改**：UTF-8 BOM／CRLF／RFC 4180 逸出／CSV 注入前綴（`=`／`+`／`-`／`@`／Tab／CR 開頭者先加前導 `'`）／0 筆時僅表頭。<br>
  📌 **可測形狀**：套用 `完成狀態 = 已完成` 後匯出，斷言 CSV 之資料列數**等於**畫面 `[data-ojt-row-count]` 所示之數；🔴 **並另斷言其 `documentNumber` 集合與畫面各列相同**——只比數量時，「匯出了未篩選的前 N 列」照樣綠。

- **AC-UX52**（🔴 **CSV 欄位集合與順序**；`OQ-UX16-30`，人類 2026-09-22 直接裁決＝**TAB2 逐列全欄，畫面欄位扣掉操作類**）：Given 匯出成功, When 檢視第 1 列表頭, Then 其欄位由左至右**逐字**為 ——
  `使用單位全名,單位代碼,程序書編號,程序書書名,文件版次,訓練版次,公告日期,應完成日,場次數,完成狀態,備註`（**11 欄**）。
  - ① **`使用單位全名`** ＝ 該列之 `orgName`（🔴 **四段格式**，`AC-UX45`）；**`單位代碼`** ＝ `orgCode`（🔴 **不併入全名欄**——代碼是給人回查上游用的，併進去就無法以欄為單位比對）；
  - ② **`完成狀態`** 之值**逐字**為 **`已完成`**／**`尚未完成`**（🔴 與 TAB2 徽章之逐字相同，`AC-03`；**不得**輸出 `true`／`false`／`1`／`0`）；
  - ③ **`備註`** 承載**裁撤與孤兒**兩種註記，其值**逐字**為 `已裁撤`／`已移出使用部門`／兩者兼具時以**全形頓號 `、`** 相接（前後無空白）／皆非時為**空儲存格**；<br>　🔴 **明文禁止**改以 `0`／`—` 表示「無註記」——`—` 是畫面之空值佔位符而非資料（[F017](F017-backend-document-list.md) `AC-B9` ③ 之既有通則）；
  - ④ **`公告日期`／`應完成日`** 之格式皆為 **`YYYY-MM-DD`（UTC+8）**，逐字比照 [F017](F017-backend-document-list.md) `AC-X8` 之既有樣板（**不附時分秒**）；🔴 **兩欄須共用單一正規化推導點**——`公告日期` 取 `formatExportTimestamp(announcedDate).slice(0, 10)`，`應完成日` 之 `addMonthsClamped()` 須吃**該已正規化之值**，**不得各自對原始 `announcedDate` 獨立解析**；無值時為**空儲存格**。<br>　📝 **原條文（2026-09-22 實機覆核後就地改寫）**：`OLD>` 「之格式與畫面所見逐字相同；無值時為空儲存格」。🔴 **該措辭在 TAB2 沒有對照物**——本條自己的 📝 段已載明這兩欄「僅有資料層載體、無畫面欄位」⇒「與畫面所見相同」是一個**指向不存在之物**的判準，正是本缺陷得以逃出環的規格側成因。<br>　🔴 **實機量到之缺陷（2026-09-22，測試站）**：`公告日期` 輸出原始 ISO 時間戳 `2026-03-23T00:00:00.000Z`，而同列 `應完成日` 為乾淨之 `2026-04-23` ⇒ **同一檔案兩種日期格式**。建環時之 fixture 為乾淨之 `'2026-01-15'`，在該語料下「原樣透傳」與「正確格式化」**輸出相同**故斷言恆綠 —— 本 repo 血訓「**乾淨 fixture 對顯示型缺陷零鑑別力**」再次發生。<br>　🔴 **不只是格式、是正確性**：`formatExportTimestamp()` 帶明確 **+8 位移**；兩欄若各自解析原始值，接近 UTC 午夜之時間戳會**各差一天**（實測 `2026-06-10T16:00:00.000Z` ⇒ 公告日期應為 `2026-06-11`、應完成日應為 `2026-07-11`，而改寫前為 `2026-07-10`）。⚠ **兩支匯出必須同進退**：本欄之時區語意與 F017 `AC-X8` 為**同一個決定**，日後若裁定日期型欄位不應套時區位移，**兩處必須同批改**，不得只改其一。<br>　📌 **本條由 lead 於 2026-09-22 實機覆核後就地補寫**（spec-writer 該棒已結束）；條文措辭取自 `ring-ux16b` 之建議稿。
  - ⑤ 🔒 **不含任何操作類欄位**（新增場次鈕、下載鈕、展開鈕等）——🔴 `queryByText` 級之對照：CSV 表頭中**不得出現** `操作`。
  - 📌 **可測形狀（🔴 2026-09-22 第三輪就地改寫；原「四條件同列」經 ui-ux-designer 提報為不可構造）**：<br>
    　🔴 **原條文之語料不可達**：孤兒列**依定義必有場次**（它之所以成列，是因為有場次卻已不在使用部門集合內），而 `completed` 由**場次數**推導 ⇒ **孤兒恆為「已完成」** ⇒ 「孤兒 ∧ 尚未完成」在資料層**構造不出來**。<br>
    　🔒 **改為兩列分開覆蓋 ＋ 一組純函式向量**：<br>
    　　ⓐ **列 1「已裁撤 ∧ 尚未完成 ∧ 無公告日」** ⇒ 覆蓋 `備註` ＝ `已裁撤`、`完成狀態` ＝ `尚未完成`、`公告日期`／`應完成日` 之**空儲存格**；<br>
    　　ⓑ **列 2「孤兒（∧ 已完成）」** ⇒ 覆蓋 `備註` ＝ `已移出使用部門`；<br>
    　　ⓒ 🔴 **`、` 相接之分支（兩者兼具）改由純函式向量覆蓋**——以 `{ inactive: true, orphaned: true }` 直接驅動該欄之組字純函式，斷言輸出逐字為 `已裁撤、已移出使用部門`。**不需要一列真的同時具備兩者**。<br>
    　📝 **一項必須留檔的差異（否則下一個人會以為 ⓒ 是憑空發明的）**：ⓒ 之組合在**實作端**是可達的——`AC-37`～`AC-40`（2026-09-02 第五輪版次追蹤）已把完成判定改以「**當下訓練基準版次之場次數**」為準 ⇒ 一個「舊版次有場次、當前版次沒有」的孤兒列**可以同時是孤兒且尚未完成**。🔴 **prototype 之示範模型停在舊的「場次數 > 0」推導**，故其側構造不出來；**兩邊的差異本身就是這一條註記存在的理由**。<br>
    　🔴 語料若每欄都有值，「空儲存格」與「`—`」之分歧無從暴露——**該要求仍然有效**（落在 ⓐ）。
  - 📝 **prototype 側之載體限制（2026-09-22 第三輪，C-2）**：本欄之 `文件版次`／`訓練版次`／`應完成日`／`公告日期` 四個值，在 `prototypes/25-ojt-progress.html` 側**僅有資料層載體、無畫面欄位**（ui-ux-designer 只補資料層、未自造畫面欄位，🟢 **克制得對**）。<br>
  　🔒 **該畫面缺口屬 [F042](F042-ojt-progress-management.md) `AC-37`～`AC-40`（第五輪版次追蹤）之範圍、本輪不處理、另立追蹤項**——🔴 **不得**由本 delta 順手在 prototype 或實作上新增那四個畫面欄位：那是另一個裁決，且會動到 TAB2 既有的欄位集合（`AC-11`／`AC-28` ③④）。<br>
  　⇒ 🔴 **本條之驗證載體恆為 CSV 組字之純函式**，不是 prototype 畫面。

- **AC-UX53**（🔴 **匯出恆為逐列進度資料，與分組模式無關**；`OQ-UX16-31`）：Given TAB2 之分組模式為 `org`（預設）或 `document`（`AC-30`）, When 於**任一模式**下匯出, Then 其 CSV **位元組完全相同**——欄位集合、列集合與列序**皆不隨分組模式改變**。<br>
  🔴 **理由（不得省略）**：`AC-30` 已明文「切換不改變 rows 之資料來源」——分組**純為前端呈現決策**；讓匯出跟著分組模式改變形狀，等於把一個呈現決策寫進資料落地的產物裡，而使用者下一次打開檔案時**完全看不出當時是哪個模式**。<br>
  🔴 **可測形狀（本條唯一的鑑別力）**：同一份語料、同一組篩選，於 `org` 與 `document` 兩種模式下各匯出一次，斷言**兩份輸出字串嚴格相等**。<br>
  🔒 **零漣漪**：`AC-30`～`AC-36` 之分組模式行為（含 `AC-36` 之往返鎖與對偶鎖）、`[data-ojt-row-count]` 之文字不因分組模式改變——**一律不變**；🔒 **本 delta 不新增任何分組模式、不改變 `[data-ojt-group-mode]` 之恰二值值域**。<br>
  📝 **prototype 載體之出處（2026-09-22 第三輪，C-1）**：`prototypes/25-ojt-progress.html` 之分組模式切換（`[data-ojt-group-mode]`）與文件搜尋（`[data-ojt-doc-search]`）係本輪由 ui-ux-designer **補齊之既有已上線行為**（`AC-30`～`AC-36`，2026-09-01 第三輪），🔴 **非本輪新增功能**；其逐字文案取自**已上線之 `frontend/src/pages/ojt-progress-view.ts` 常數**（`GROUP_MODE_*`／`DOC_SEARCH_*`／`DOC_GROUP_BASIS_NOTE_TEXT`），**非自創**。<br>
  　🔴 **為何非補不可**：沒有這兩個控制項，本條之「兩種模式匯出位元組相等」與 `AC-UX55` 第 3 句在 prototype 側**完全沒有可達狀態**。<br>
  　🔒 **補齊 ≠ 納入本批之約束範圍**：分組模式與文件搜尋之**行為**仍由 `AC-30`～`AC-36` 管轄，🔴 **本批之任何 `AC-UX#` 不得對其另立約束**（含群組標題右側之 `已完成 {X} / 共 {Y} 單位`——該字面屬既有 `DOC_GROUP_BASIS_NOTE_TEXT` 與 `AC-32` 之口徑鎖，**不由本批鎖定**）。

- **AC-UX55**（🔴 **匯出範圍須對使用者明示**；`OQ-UX16-33`＝選項 A，人類 2026-09-22 第二輪）<br>
  📝 **編號說明**：本條為第二輪新增、依配發規則**接續編號**（`AC-UX55`），但依主題編排於匯出區塊之末、`AC-UX54`（全檔零漣漪鎖）之前——**條號不連續於版面順序是刻意的，不得為了排版而重新編號**。

  **Given** 我在 TAB2 按下 `AC-UX51` 之匯出鈕且匯出成功, **When** 畫面回饋出現, **Then** 該 toast 之**可見文字**由下列**恰兩句或三句**構成（🔒 逐字）——
  - 🔒 **第 1 句（恆出現）**：**`已匯出 OJT 進度清單（CSV，UTF-8 BOM）：共 {N} 筆`**
  - 🔒 **第 2 句（恆出現）**：**`匯出內容依「制定本部」「搜尋使用單位」「完成狀態」三項條件。`**
  - 🔒 **第 3 句（🔴 僅當「搜尋文件」目前有值時出現）**：**`「搜尋文件」只縮小畫面上顯示的內容，不影響匯出。`**

  🔴 **① `{N}` 必須是「匯出的筆數」，不是「畫面上看得到的列數」**——這是本條存在的全部理由。使用者回報的情境正是「畫面剩 12 列、檔案 340 列」；若 toast 跟著畫面說 12，就從「沒說清楚」惡化為「說了假話」。<br>
  　📌 **可測形狀**：以「畫面 12 列、匯出 340 列」之語料斷言 toast 文字含 `共 340 筆` **且**不含 `共 12 筆`。

  🔴 **② 第 3 句之出現條件必須綁「搜尋文件目前有值」，不是綁分組模式**：`org` 模式下該控制項**完全不進 DOM**（`AC-33` ②），切回 `org` 時關鍵字**會被清空**（既有行為）⇒ 兩種情形下都沒有落差可言，講它只會讓使用者去找一個畫面上沒有的東西。<br>
  　🔴 **可測形狀（成對，缺一即無鑑別力）**：<br>
  　　ⓐ **正向**——語料須為「分組模式＝`document` ∧ 搜尋文件有值 ∧ **它確實拿掉了列**（畫面列數 **嚴格小於** 匯出列數）」⇒ 斷言第 3 句**逐字可見**。<br>
  　　ⓑ **負向**——同一份語料、**清空搜尋文件**（或切回 `org` 模式）⇒ 斷言第 3 句**零命中**，而第 1、2 句**仍在**。<br>
  　🔴 **語料若讓搜尋文件為空、或有值但沒拿掉任何列，本條恆真、等於沒寫**（判準思路同 [F043](F043-business-function-category.md#ux16-delta) `AC-UX34` ① 對 `AS` 之處理）。

  🔒 **③ 三個引號內的名字為「使用者在畫面上看到的那個名字」，須與控制項之 `aria-label` 逐字相同**：`制定本部`（`AC-UX49` 新增）／`搜尋使用單位`（既有）／`完成狀態`（既有）／`搜尋文件`（既有 `DOC_SEARCH_ARIA_TEXT`）。🔴 **不得**改用條文中的稱呼（如「單位搜尋」）——那個字串畫面上不存在。

  🔴 **④ 使用者語言，不得出現內部詞彙**（判準逐字沿用 [F044](F044-admin-dashboard-analytics.md) `AC-G94`：**這句話在回答使用者的哪一個問題**）。本條回答的是「**我拿到的是什麼**」⇒ 🟢 可見。<br>
  　🔴 **明文禁止**出現於可見文字：任何 `data-*` 屬性名（`data-ojt-doc-search`／`data-ojt-filter`）、任何 AC 或不變式編號、以及「**篩選項**」「**不計入篩選**」這類只對實作者有意義的措辭。<br>
  　🔒 **可測形狀**：對上述 ⓐ 狀態之 toast 文字做裸字面掃描，斷言 `data-ojt`／`AC-UX`／`篩選項` **三者零命中**；🔴 並須配一句**正向探針**（斷言第 2 句確實可見），否則 toast 根本沒渲染時掃描也是綠的。

  🔴 **⑤ 數字必須是可見文字**（比照 [F044](F044-admin-dashboard-analytics.md) `AC-G71`）：`{N}` **不得**只存在於 `title` 屬性、`aria-label` 或 tooltip 內。

  🔒 **⑤-b DOM 契約（2026-09-22 第三輪補訂，權威＝`prototypes/25-ojt-progress.html`）**：toast 之容器帶 **`data-ojt-export-toast`**。<br>
  　🔴 **它是本條全部斷言的定位點**：三句話都是**可見文字**，沒有容器掛鉤時只能用全頁 `getByText`——而本頁同時存在「匯出」鈕、`[data-ojt-row-count]` 與群組標題等多處帶數字的文字，裸比對會撞（形狀同 [F019](F019-public-list-browsing.md#ux16-delta) `AC-UX18` 之 D-4）。🔒 ⓐⓑ 兩種狀態之斷言一律 **`within(getByTestId(...))`** 限定於此容器內。<br>
  　⚠ **本掛鉤不承載值**：`{N}` 仍須是容器內的可見文字（⑤）。

  📝 **⑥-b 第 3 句在 prototype 側之可達性（C-1）**：其前提（分組模式為 `document` 且文件搜尋有值）所依賴的兩個控制項，係本輪由 ui-ux-designer **補齊之既有已上線行為**（`AC-30`～`AC-36`），🔴 **非本輪新增功能**——理由與範圍界線見 `AC-UX53` 之 📝 段。

  🔒 **⑥ 逐字文案為本頁專屬，明文禁止與 [F017](F017-backend-document-list.md) 共用常數**：該頁樣板為 `已匯出程序書清單（CSV，UTF-8 BOM）：共 {N} 筆`（`DocumentListPage.tsx:919`）——句型可參照，🔴 **但兩頁的「範圍」語意不同**（該頁是 13／14 項篩選之結果、本頁是三項條件且另有一個不納入的搜尋框），抽成同一個常數會讓其中一頁的範圍說明在下一次改動時**靜默變成另一頁的**。🔵 建議本頁常數名 `OJT_EXPORT_TOAST_TEXT(n)`／`OJT_EXPORT_SCOPE_TEXT`／`OJT_EXPORT_DOC_SEARCH_NOTE_TEXT`，落點由 system-architect 定案。

  🔒 **⑦ `AC-UX51` 之列集合定義與 `AC-UX53` 之位元組一致性，兩條皆一字不動**——本條改的是「**有沒有告訴使用者**」，不是「**匯出什麼**」。<br>
  　⚠ **第 3 句依狀態出現或不出現，不牴觸 `AC-UX53`**：該條鎖的是 **CSV 之位元組**，不是畫面回饋；🔴 **明文禁止**下游為了「讓兩種模式完全一樣」而把第 3 句改成恆顯示或恆不顯示。

  🔴 **⑧ 本缺口之成因必須留檔（否則下一個人會去改 `AC-UX49` 的數字）**：TAB2 上**會拿掉列的控制項有四個**（三項篩選 ＋ 搜尋文件），但**算作「篩選項」的只有三個**。這是**既有設計的既有張力**，不是本 delta 造成的——`AC-30` 讓分組模式不掛 `data-ojt-filter` 的理由是「**它不移除任何列**」，而搜尋文件**會**移除列卻同樣沒掛。<br>
  　🔒 **處置＝在畫面上說清楚，不是改數字**：`AC-UX49` 之「恰為 3 個」與 `AC-30`／`AC-33` ② 之兩條「不得掛 `data-ojt-filter`」**一律不動**。

- **AC-UX54**（🔒 **F042 之零漣漪回歸鎖定**）：Given 本 delta 全數實作完成, When 逐項檢視, Then 下列**一律與本 delta 導入前逐字相同**——
  - ① **TAB1 三區之全部統計口徑**：`AC-14`（區一逐筆表之節流七項與四態）／`AC-15`／`AC-16`（區三之節流八項）／`AC-17`（裁撤單位排除之封閉集合＝三者，🔴 **不得**因新篩選而擴大其適用範圍）；
  - ② **`AC-03`／`AC-04` 之完成判定**（列層級恆為二態、「部分完成」僅存在於文件層）；
  - ③ **孤兒列之呈現與 `[data-row-orphaned]`**、`AC-25` 之「依集合成員關係判定、不讀 `orphanedAt` 旗標」；
  - ④ **版次追蹤 `AC-37`～`AC-40`**（含應完成訓練日期＝公告日 +1 月之單一推導點）；
