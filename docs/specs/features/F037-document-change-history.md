...
- **AC-D11**（🔴 CSV 值層：列舉欄輸出中文標籤；**2026-08-16 補訂**，lead 裁示）：Given 匯出成功, When 檢視資料列之各儲存格值, Then 下列成立——
  ...
  - ② **`來源` 欄之值為產生該變更事件之來源功能之中文名**，值域**恰為八者**：`編輯`（[F011](F011-edit-with-comparison.md) 一般欄位編輯）／`狀態切換`（[F012](F012-document-status-toggle.md)）／`公告日期`（[F011](F011-edit-with-comparison.md) 之公告日期欄變更）／`制定組織`、`當責室長`、`使用部門`（[F014](F014-accountable-dept-chief.md) 之三類）／`附件`（[F016](F016-pdf-ojt-attachment.md) 替換）／**`建立`**（[F010](F010-create-document.md) 之建立事件，`changeType='CREATE'`）。**不得**輸出列舉代碼。<br>📝 **2026-08-16 就地放寬（lead 瀏覽器實測後確認：規格漏列、實作正確）**：本條首版寫「值域**恰為六者**」，遺漏 `公告日期` 與 `建立` 兩值。**兩者皆為合法之變更來源**——前者是真實發生之欄位變更（畫面上多筆可見）、後者為 F010 建立事件之 **additive 超集**（`ChangeHistoryPage.tsx` 註解已載明其為刻意設計）。**放寬枚舉、不改畫面**：改畫面去符合一個不完整的枚舉會使使用者看不到真實發生過的變更來源，本末倒置。<br>⚠ **本次僅動 ②，`AC-D11` ④ 一字未改**——④ 之「CSV 與畫面逐字相同」才是真正防漂移之載體，且該不變式**本即成立**（lead 已查證後端 `document-change-history.service.ts` 之 `sourceLabel`／`fieldLabel` 與前端 `FIELD_LABEL`／`sourceOf()` 逐字同值），不受本次枚舉錯誤影響。<br>📌 **`prototypes/23` demo 之 `狀態` 值**仍屬 demo 噪音（非本次補入之兩值），維持原註記。
