...
- **緩解不變**：§7.3 消失筆數閾值保護（2026-08-31 起 10%）仍為必要防線，**理由已改變，勿因「孤兒率為 0」而移除**
...
- **消失筆數閾值保護（因應 §3.2）**：單次同步若「上次存在、本次消失」之在職帳號比例超過閾值（**10%**；🔵 2026-08-31 由草案 5% 校準，見下），**中止同步並告警系統管理員**，不執行任何停用。<br>🔵 **校準理由**：AS 有 74 筆永久性「消失」帳號（16 轉調／6 離職／52 為 v1.0 `VW_HPMUSER` 污染殘留，全部從未登入），依 US-010 AC4 同步永不停用它們 ⇒ 消失比例恆為 6.6%，5% 使 AS 每次同步必然中止（實測連續失敗 11 次）。⚠ 代價：AS 觸發點 112 人中已有 74 個名額被幽靈帳號佔用，真實異常需再多消失 39 人才會被擋下。
