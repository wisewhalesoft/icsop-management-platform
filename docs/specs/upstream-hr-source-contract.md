...
#### 5.1.1 🔴 `DESC_CHI` **本身命名不一致**——顯示端不得直接吐 `name`（2026-09-04 實測 ＋ 定案）

真人於 2026-09-04 回報「制定部門／制定室別 有的是簡稱、有的是全名」。對 dev SOP 庫 `ORG_UNIT`
之 **139 筆 active 部／處室**逐筆比對 `name`（← `DESC_CHI`）與 `descFull`（← `DESC_FULL`）：

| 層級 | 筆數 | 形態分布 |
|---|---|---|
| **部**（`AN000` 樣式） | 42 | `name` **等於**全名 20 筆（`作業服務部`／`資訊部`）；**缺尾字** 9 筆（`企劃` vs `企劃部`、`稽核` vs `稽核部`、`信用審查` vs `信用審查部`）；**真簡稱** 13 筆（`營管部` vs `營運管理部`、`財會部` vs `財務會計部`、`車輛分期營一` vs `車輛分期營業一部`、`桃園部` vs `桃園營業部`） |
| **處/室**（`ANA00` 樣式） | 97 | `name` 為 **「部段/室段」複合字串** 95 筆（`營管部/審查室`、`作服/文管室`、`債管/法催一室`），且**部段之簡寫程度自己也不一致**（`營管部`／`作服`／`北區營業一部` 三種風格並存）；無斜線者僅 2 筆（`AE/NAA00 業務開發處`、`AS/WAA00 職安室`） |

⚠ **`DESC_FULL` 亦非可機械拆解**：處/室之 `DESC_FULL` 為**無分隔串接全名**（`營運管理部審查室`），
但 `AS/BAJ00`／`AS/BAK00` 兩筆**帶了斜線**（`車輛分期營業一部/台北營業三處`）——故只能整串使用，
不得以 `/` 切段。

**定案（走 A+，2026-09-04）**：凡以「單一組織單位之名字」呈現於畫面之欄位，一律經
`org-path.ts#orgUnitDisplayName(unit, lookupDepartment)`（前後端各一份實作）：

- **部層以上**（ROOT／DIVISION／DEPARTMENT）→ `descFull` 全名；為空才退回 `name`。
- **處/室、課**（SECTION／SUBSECTION）→ 以**部層（`LEFT(CODE,2)+'000'`）之 `descFull` 為前綴、
  自本單位 `descFull` 切除**所得之尾段（`營運管理部審查室` − `營運管理部` ＝ `審查室`）；
  切不出來才退回 `DESC_CHI` 末段。
  🔴 **不得改以 `/` split `descFull`**：`AS/BAJ00`／`AS/BAK00` 之 `descFull` 本身帶斜線，
  切前綴後再去掉殘留之前導 `/` 才是正解；而 `AS/DAA00` 切出之尾段
  （`財會管理室/和潤暨海外事業`）**本身含斜線且必須完整保留**。

**實測涵蓋率**：97 筆 active 處/室中 **95 筆**可由 `DESC_FULL` 切出全名；其餘 2 筆自動退回末段、
輸出與初版相同——`AE/NAA00`（`DESC_FULL` 為 `業務開發處`，非部名 `業務開發部` 之延伸）、
`AS/WAA00`（`WA000` 部層列不存在）。

#### 為何不停在「取 `DESC_CHI` 末段」（初版 A → A+ 之升級理由）

初版只取末段（`作服/文管室` → `文管室`），確實去掉了重複的部段，但留下兩個問題：

1. **粒度仍不一致**：制定部門變全名、制定室別全是簡稱，使用者原本的抱怨只解了一半。
2. 🔴 **同名歧義**：`AS/CCC00`（信用審查部**企金審查室**）與 `AS/CDF00`（債權管理部**企金催收室**）
   之 `DESC_CHI` 末段**皆為 `企金室`**，在同一欄顯示完全相同——兩個不同單位的文件看起來像同一個
   單位發的。切 `DESC_FULL` 前綴後分別為 `企金審查室`／`企金催收室`，歧義消失。

**本次適用範圍＝`制定部門`／`制定室別` 兩欄**（後台清單與 CSV 匯出、前台清單卡與六項篩選選項、
前台詳情、新增／編輯頁下拉與「目前值」欄、唯讀頁）。**刻意未納入**（各有其呈現理由，改動須另議）：
F018 制定部門之**祖鏈路徑** chip（`orgAncestorPathLabel`）、帳號部門欄與 F020 浮水印之
**「部 / 處室」兩段式**（`buildOrgPath`，§8.2）、F018／F039 之**上傳者所屬部門**與 F042 之
**已完成 OJT 單位清單**（兩者仍走 `NameResolutionService.resolveOrgUnitName`＝原字串）。
