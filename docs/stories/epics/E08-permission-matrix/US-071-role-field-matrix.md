...
一般使用者與部門窗口對整份文件（除 ICSOP PDF/使用表單/附錄下載外）僅於前台瀏覽情境下可見，後台管理介面本身即不對其開放（見 [US-070](US-070-role-function-matrix.md)）。
...
**When** 該角色下載 ICSOP PDF、使用表單或附錄附件
...
- [x] **（已定案 2026-08-06，比照 E10）新增「附錄（多）」欄位列**：附錄（[E10](../E10-appendix/epic-brief.md)）之欄位權限模型與「使用表單（多）」完全比照，插於該列之後；權限值＝系統管理員唯讀（可下載）／ICSOP管理員可寫／主管、部門窗口、一般使用者皆唯讀（可下載）。
