...
| 🔵 **F042 OJT 進度管理（E11）——Phase B 進行中：環已交、待實作轉綠** | **[features/F042-test.md](features/F042-test.md)** ← 2026-08-27 新增／**2026-08-28 依裁決收斂**（16 題 OQ ＋ 4 題衍生後續全數結案）。<br>✅ **規格側已凍結、無任何待決項**；`prototypes/04`／`13`／`14`／`15`／`16`／`17`／`25` 均已依裁決改版 ⇒ **`AC-01`～`AC-29` 與 `AC-J1`～`AC-J26` 全數有權威可依、無阻塞項**。<br>🔵 **現況＝test-generator 已交環**：**backend 恰 13 個紅 suite ／ frontend 恰 13 個紅 suite**，待 tdd-implementation 轉綠。<br>🔴 **取值職權分工（建環／實作皆適用）**：**錯誤碼字面**＝[error-handling #ojt-progress](../specs/error-handling.md#ojt-progress)｜**使用者訊息逐字**＝[F042 §prototype 25 §6 ⑬](../specs/features/F042-ojt-progress-management.md#prototype-25-dom-contract)（error-handling 之「使用者訊息」欄已整欄移除，無佔位字可誤抄）。<br>🔴 **本檔含之三類清單，實作期務必逐項對照**：① **預期轉紅盤點**（backend 9 檔／frontend 8 檔／int 1 檔——**轉紅為預期，須就地改寫為新行為之背書、不得刪除**）；② **八個假綠陷阱**（`.ojt-write` 集合互斥退化為恆真、`hasOjt`／`ojtStatus` 之 N+1 在單元層看不見、**`受限CRUD` 矩陣格值擋不住刪除**、**只驗 rollup 結果漏掉「列被展開」**、**分母為零之 `NaN%`**、**`data-has-ojt` 舊值域 `true`／`false` 之語意悄悄變窄**、**顯示邏輯綁 `orphanedAt` 旗標而非集合成員判定**、**TAB1 兩個數字口徑不同被「修成一致」**）；③ **五處不得順手一併刪除者**（`AC-N76` ④ 之 `xls`／`icsop_pdf` 兩列、`AC-N33`、`AC-N52` 前半、**F024 之 `ATTACHMENT_UPLOAD` 歷史標籤**、**`OQ-E01-09` 不得順手償還**）。<br>⚠ **仍屬設計裁量、不入 AC 者僅 2 項**：區三筆數上限、側選單新項之排列位置（見 [F042 §7-B](../specs/features/F042-ojt-progress-management.md#prototype-25-dom-contract)） |
| 🔵 **F017 後台文件清單「匯出（CSV）」delta**（2026-08-31 使用者裁決；`AC-X1`～`AC-X17`；**簡化版環**＝僅 backend jest ／ frontend vitest，無 Playwright／Stryker／e2e ⇒ **AC 是唯一防線**） | **[features/F017-test.md](features/F017-test.md#export-delta-ring)**（末段）← 2026-08-31 追加。9 個新檔（backend 6／frontend 3）；建環後實跑＝backend 189 suites/3036（5 紅 suite／95 紅測試，基線 183/2925 零波及）、frontend 116 files/1774（3 紅 file／34 紅測試，基線 113/1723 零波及）。✅ **兩處規格衝突已由 lead 裁決**（採 architecture 抽出 `orderedLinks`；`formatExportDate()` 兩案都不採、改用既有 `formatExportTimestamp(...).slice(0,10)` 使 `csv-export.ts` 一行未改）與 **`main.ts` body-parser 之原理不可測**，見 [risks-and-gaps.md #f017-export](risks-and-gaps.md#f017-export) |
