...
| `G-L3-03` | **filter-options 之 `chiefs` label（人員姓名）之解析接縫未指定**。前台清單 DTO 現況不解析人員姓名，`PublicDocumentsService` 亦無人員解析器；§10.6 只寫「由 `resolvePersonName` 解析」。 | 中。本環只斷言 `chiefs[].value` 為員編、label 非空且非佔位字；**姓名是否顯示、由誰解析未受約束**。 |
