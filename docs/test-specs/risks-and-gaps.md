...
## F044 後台首頁儀表板改版（E13，2026-09-21 建環） {#f044}

> 🔴 **本輪為簡化環：只有 backend jest ／ frontend vitest。** 沒有 Playwright／fidelity／mutation／
> 效能閘門、**沒有對真庫之整合測試**。⇒ 凡落在 SQL、HTTP 層、視覺幾何與部署之事，本輪**沒有防線**。
> 完整之 12 項盲區清單為 [architecture-spec §15.10](../specs/architecture-spec.md#ch15-blindspots)，
> 已由 test-generator 於交件報告逐項原樣轉述給 lead，**須列為人工實機覆核項**。

| # | 項目 | 理由／處置 |
|---|---|---|
| `G44-01` | 🔴 **`AC-G90` ③ 之型樣 ②（`jobPositionCode`／`JOB_POSITION` 全樹零命中）在本輪之前就已被違反** | 該二識別子早已存在於 `src/api/types.ts`／`src/api/endpoints.ts`／`src/domain/account-profile.ts`／`src/pages/AccountManagementPage.tsx`——[F003](../specs/features/F003-account-management.md) 手動帳號之「職位」下拉（`AC-P29`／`AC-P31`），與本功能無關。照條文字面實作，該斷言**永遠不可能綠**（而 `AC-G90` 自己就警告過「否則本斷言會假陰性而被人關掉」）。<br>**處置**＝`frontend/src/domain/f044-static-guards.test.ts` 之**具名例外清單（四檔，逐檔註明理由）＋ 三道守門**：① 例外清單自我守護（四檔須仍存在且仍命中，防清單腐化為死條目）；② **軸向守門**（例外四檔內不得出現 `defaultDimension`／`orgDimension`／`normalizeDefaultDimension`／`依制定` 之維度詞彙，使放行不等於開後門）；③ 型樣 ①（職位名字面 `董事長`／`總經理`／`本部長`／`副本部長`）**維持全樹零例外**（實測目前零命中，具鑑別力）。<br>🔴 **這是對 🔒 條文的一處偏離，請 spec-writer／人類裁決是否回填 `AC-G90`。** |
| `G44-02` | 🔴 **`AC-G90` ② 之元件層關鍵向量在本輪被結構性削弱** | 條文要求「給一個**職位為 `部長`** 之 session 替身，而端點回 `defaultDimension: 'company'` ⇒ 畫面必須選『依制定公司』」，並稱其為「唯一一個能讓『前端自己算一份』在行為上現形的向量」。但 `ARCH-G2`／`AC-G87` 已裁定 **`SessionUser` 一欄未加** ⇒ **前端根本拿不到職位名**，該替身在本輪**沒有載體**。<br>⇒ 實際落地之斷言（`DashboardHome.f044.test.tsx`）鑑別的是「**有沒有讀端點這個欄位**」（寫死 `'department'` 之實作會翻紅；三個合法值各一條），**不是**「前端有沒有另寫一份職位白名單」。後者之載體只剩 `G44-01` 之原始碼層掃描，而該層明文**只能「降低風險」、不能「證明不存在」**。<br>🔴 **本項不得被下游寫成「已證明前端無第二個定義點」。** |
| `G44-02` 補記 | 🟢 **2026-09-21 實作落地後降級：元件層之鑑別力比原評估強** | 原文之後半（`SessionUser` 無職位名 ⇒ 規格設想的「職位 `部長` 之 session 替身」在本輪沒有載體）**仍然成立**；但「只鑑別得到有沒有讀端點那個欄位」這個評估**已被推翻**——`DashboardHome.f044.matrix.test.tsx` 之 8 條（以**被人為改動的矩陣替身**驅動，含放行 `DeptContact`／撤銷 `ICSOPAdmin`／**只改其他功能鍵**三個方向）在實作落地後**全綠**，代表三道閘門（`AC-G66`／`AC-G18`／`AC-G58`）確實**讀矩陣、且讀的是正確的那一格**。<br>⇒ 「前端有沒有**另寫一份職位白名單**」之載體仍只有 `G44-01` 之原始碼層掃描（明文只能「降低風險」）；但「**閘門是不是角色清單**」這一半**已有真正的行為層鑑別力**（見 `G44-03` 之四種錯誤實作探針）。 |
| `G44-03` | 🟢 **`AC-G18`／`AC-G58`／`AC-G66` 之「讀矩陣 vs 寫角色清單」已取得真正的鑑別力（2026-09-21 就地補強）** | 🔴 **本項原記為「本輪做不到」，該判斷是錯的，已更正。** [§癸 (e)](../specs/features/F044-admin-dashboard-analytics.md#corpus) 要求之載體是「**以一個被人為改動的矩陣替身驅動閘門述詞，斷言其輸出隨矩陣而變**」——這在本輪**做得到**：<br>· **後端**（`dashboard-analytics.service.spec.ts`）：`canPerform` 為具名 export，TS 之 CommonJS 產物編成呼叫期屬性存取 ⇒ `jest.spyOn(matrix, 'canPerform')` 之替身確實被服務層取用。<br>· **前端**（`DashboardHome.f044.matrix.test.tsx`，🔴 **必須獨立成檔**——`vi.mock` 整檔提升，裝進主檔會污染那裡每一條以真實矩陣為前提的斷言）：以 `importOriginal` 保留其餘 export、只換掉 `canPerform`，逐鍵改動其回傳值。<br>**兩側皆斷言兩個方向 ＋ 一組「只改其他功能鍵」之對照**（防「讀矩陣但讀錯鍵」）。<br>🟢 **實跑自證（後端，對四種錯誤實作）**：① 寫死角色清單 ⇒ **3 紅**；② 讀矩陣但用錯功能鍵 ⇒ **5 紅**；③ 寫死永遠放行 ⇒ **6 紅**；④ 🔴 **呼叫了 `canPerform` 卻忽略其回傳值、再自行比對角色字面 ⇒ 2 紅**——④ 正是原 spy 版本**抓不到**、而本項原文列為「本輪做不到」的那個形狀。<br>⚠ **仍存之局限**：本組證明「閘門之輸出隨矩陣而變」，不證明實作沒有**額外**再加一道角色判斷（例：矩陣通過 **且** 角色符合）。前端側之八條在實作落地前為紅（區塊尚不存在），其**替身機制**已由同檔三條自我守護證明生效；🟢 **2026-09-21 實作落地後八條全綠**，見 `G44-02` 補記。 |
| `G44-04` | 🟢 **`AC-G28` 之漣漪已由 team-lead 裁決並由 test-generator 執行完畢（2026-09-21）** | 原記載：§辛 只列 `DashboardHome.test.tsx:185,190`，實查為 **`:77`／`:92`／`:190`** 三處。<br>🟢 **裁決（team-lead，2026-09-21）**：① `:77`／`:92` 所屬之 `describe('DashboardHome — KPI 卡（GAP-07-1）')` **整段刪除**——授權來源逐字＝人類裁決 **`OQ-D44-03` ＝甲「新 4 張卡完全取代舊 5 張」**（落於 `AC-G1`），該兩案之**主題本身**已被使用者裁決刪除，不是「換載體」的情形；② `:190` **不得刪除**（它正是 `AC-G84` 保護的「活動端點失敗不阻斷儀表板」），依 `AC-G28` **只換載體**：案例名「快速進入卡片仍在」→「統計卡仍在」，斷言由 `getByText('快速進入功能區')` 改為 `getByTestId('dashboard-stat-cards')`。<br>🔒 **非靜默刪除**：被刪那兩案所鎖的五個逐字標題，已由 `AC-G1` **反轉為負向鎖**（「舊 5 張 KPI 卡之逐字標題於整頁 DOM 零命中」，載體＝`DashboardHome.f044.test.tsx` 之五條 `it.each` ＋「`aria-label=\待辦提示\` 不再存在」一條）⇒ 同一批字面仍被鎖著，只是從「必須在」翻成「必須不在」。刪除處已留註解逐字記明授權來源。<br>🔴 **連帶發現並一併處置（原 `AC-G28` 與 §辛 皆未涵蓋）**：本檔用的是 `vi.mock('../api/endpoints')` 之 **automock**，F044 落地後 `DashboardHome` 多呼叫的三支端點會被 automock 換成回傳 `undefined` ⇒ `undefined.then(...)` **同步拋 TypeError**（🔴 且發生在 `.then` 之前，實作端的 `.catch()` 接不到）⇒ 本檔**每一案**都會炸掉，且失敗訊息與其主題完全無關。已加入「存在才 stub」之前瞻性 `stubF044Endpoints()`（今天是完全的 no-op，三支一落地即自動生效），🔴 **刻意不寫成 `vi.mocked(endpoints.getDashboardAnalytics).mockResolvedValue(...)`**——那一行在今天會對 `undefined` 取 `.mockResolvedValue` 而當場炸掉整份既有綠燈測試。<br>⚠ **本檔自此為環的一部分**：`:190` 改寫後之新載體（`data-testid="dashboard-stat-cards"`）在**實作落地前**不存在 ⇒ 該案曾為紅（預期）；🟢 **2026-09-21 實作落地後已轉綠，實跑複核 5/5**，該檔期望值自始未被修改。 |
| `G44-05` | ⚠ **`AC-G8` 之孿生向量表前端側改置於姊妹檔** | 條文指名落在 `frontend/src/pages/ojt-progress-view.test.ts` **之新增 describe**；本輪改置於同目錄之 `ojt-progress-view.f044.test.ts`。理由＝在那份既有 F042 綠燈測試裡 import 一個尚不存在的符號，會使**整份既有檔案**於建環當下翻紅，違反 `AC-G21`／§庚「既有測試全數維持綠燈」。🔒 條文之實質要求（**向量陣列逐字相同**、**兩側檔頭互指對方路徑**）逐項滿足。 |
| `G44-06` | ⚠ **`AC-G85` 第 2 層／`AC-G64` 第 2 層之「禁 `localeCompare`」鑑別向量改用 ASCII 大小寫對** | 規格與 prototype 提供的是中文對（`管理本部` vs `風險本部`）。但**中文之序數序與 locale 序是否相異，取決於執行環境之 ICU 版本與 locale 資料**——把它寫進斷言，本測試自己就成了它要防的那個漂移的受害者。<br>⇒ 改用 `'ZB…'` vs `'Za…'`：序數 `0x42 < 0x61`；而任何 locale 之預設定序皆 primary-level 大小寫不敏感（`a < b`）⇒ 兩者**必然相反**，與 ICU 版本無關。⚠ 為**人工 fixture**（真實公司代碼為 2 碼大寫、類別名為中文），🔴 **不得宣稱它在實機上被驗過**；中文對仍以註解保留為真實世界之例證。已實跑自證：改用 `localeCompare` ⇒ 環圖 4 紅、長條圖 2 紅。 |
| `G44-07` | ⚠ **`AC-G85` 第 3 層（`key` 昇冪收尾）與 `AC-G64` 第 3 層（`categoryId` 昇冪收尾）之向量皆為人工 fixture** | 環圖：需要「兩個 `label` 相同、`key` 不同」之段——在 `依制定部門` 維度下標籤含公司前綴，跨公司同名部**不會**碰撞；本環以「同一家公司之兩個部代碼具有相同顯示名」建構，真實語料是否可達**未經查證**。類別：依 `AC-G64` 就地更正第 3 點，以 `{name:'X（Y）', subcategory:null}` ＋ `{name:'X', subcategory:'Y'}` 建構（該碰撞只在「名稱本身含全形括號」這個病態輸入下可達）。🔴 **兩者皆不得宣稱在實機／真實語料上被驗過**；但第 3 層若省略，兩段之相對次序會落到 `Array.prototype.sort` 的實作細節上（`AC-G93` 同一紀律）。 |
| `G44-08` | ⚠ **`ARCH-G0` 之三條 SQL 禁令只能以文字比對承接** | `dashboard-f044-zero-ripple.spec.ts` 對 `dashboard-analytics.sources.ts`／`category-distribution.source.ts` 兩檔**剝除註解後**掃描三組型樣（`GETUTCDATE`／`CASE WHEN`｜`WITH … AS (`／`UNION ALL`／`MAXRECURSION`｜`ORDER BY`／`TOP n`／`.orderBy(`／`.take(`／`.limit(`），並附**正向對照**（`category-distribution.source.ts` **必須**出現 `DISTINCT`——`ARCH-G0` 之唯一例外）與**存在性守門**（檔案不存在時掃描會恆綠）。<br>🔴 **局限**：它擋得住把這些構造直接寫進那兩支檔案，**擋不住**換個寫法達成同樣效果（藏進 view、用 `Raw()` 組字串、把查詢搬到別的檔案）。**是「降低風險」，不是「證明不存在」。** |
| `G44-09` | ⚠ **`AC-G74`（零 migration）刻意不採「檔案數恰為 N」之絕對值鎖** | 依 `AC-G86` 自己給的判準（「該數字改變時，是否有一條**不變式**跟著改變？」）：migration 檔案數改變時沒有任何不變式跟著改變 ⇒ 下一個合法新增 migration 的 feature 會無辜翻紅。<br>⇒ 改鎖「`draftingDivisionId`／`BUSINESS_CATEGORY_DOC_businessCategoryId` 兩個明文禁止之冗餘欄位不存在於任何 migration」＋「不存在以 F044／dashboardAnalytics／categoryDistribution 為名之 migration 檔」。⚠ 這**不等於**證明零 migration，只證明沒有本功能明文禁止的那些欄位。 |
| `G44-10` | ⚠ **服務層之建構子與 sources 介面形狀為 test-generator 之提案，非 architecture-spec 明文** | [§15.9](../specs/architecture-spec.md#ch15-modules) 只寫到「4 個唯讀 provider（文件投影／`ORG_UNIT` 全表／`ACCOUNT.jobPositionCode`／`JOB_POSITION` 全表）」＋ 類別分布之獨立 source，未給簽章。本環落為 `DashboardAnalyticsService(sources, now)` ＋ 五個 provider 方法（逐字見 `dashboard-analytics.service.spec.ts` 檔頭之契約區塊）。<br>🔒 **若實作者認為此形狀有誤，請訊息 test-generator 裁決，不得自行改測試**（mailbox 紀律）。同理適用於 `donutSlices` 之**具名物件入參**（刻意不採位置參數：五個參數中有兩個是字串列舉，對調在型別上完全合法、在測試上很難發現）。 |
| `G44-11` | ⚠ **`AC-G13` 之「委派既有 `coveragePercent`」在後端無法逐字兌現** | `coveragePercent` 只存在於 `frontend/src/pages/ojt-progress-view.ts`，跨 package 無法 import。後端之 `rate` 鍵其首要職責是 `AC-G14` 之**在／不在**；其值以 `Math.round(n/d*100)` 之期望值逐列鎖住（`ojt-ontime.spec.ts`），孿生鎖在 `ojt-progress-view.f044.test.ts`。⚠ 兩份實作之**一致性**本輪由「同一組期望值」保證，不是由共用原始碼保證。 |
| `G44-12` | ⚠ **`AC-G3` 之回歸鎖從未比對真實 SQL（逐字轉述自條文與 §15.10 #5）** | `pendingPublish` 的 SQL 與 `deriveDisplayStatus` **是否真的等價，本輪沒有任何機器閘門驗得到**——`dashboard-f044-zero-ripple.spec.ts` 之回歸鎖由 **fake provider** 驅動，它比較的是兩支 JS。語料已刻意含「**公告日恰為今日**」之文件（§15.10 #5 點名的那一種），並附自我守護證明該語料能分辨兩種常見錯法。🔴 實機覆核項：比對 `GET /admin/dashboard/summary` 之 `pendingPublish` 與首頁卡②。 |
| `G44-13` | ⚠ **`AC-G92`／`AC-G93` 之元件層斷言在實作前為「配對綠」** | 兩者在排序尚未實作時輸出與基線相同 ⇒ 建環當下為綠。它們**不是恆真空斷言**：其正向對照（`AC-G20` 之「未完成在上」與 `AC-G88` 之「只帶 sort 仍生效」）在同一檔內為紅，且實作落地後兩者立刻取得鑑別力（錯誤地把排序套到文件分組、或段內次序被打亂皆會翻紅）。🔴 **日後不得僅因「從未紅過」就判定為無效測試而刪除**（比照 `BC-NOASSERT-8` 之既有裁示）。 |
| `G44-14` | 🔴 **§癸四 第 10 列之 `{n}` 在正式站結構上算不出來** | 該列可見文案為 `共 {n} 份，這裡顯示最新的 {m} 份。`，`{n}` ＝ 符合條件之**總份數**。但 `AC-G54` 明訂端點**先排序、再截斷為 ≤ 10 筆**，而 `DashboardAnalytics.latestAnnouncements` **只回傳截斷後的列、沒有任何總數欄位** ⇒ 🔴 `{n} > 10` 時前端算不出 `{n}`。（prototype 07 算得出來，是因為它在瀏覽器裡握有全部文件陣列；正式站沒有。）<br>⇒ **本環只鎖 `{n} === {m}`（未達上限）之可滿足分支**，並於測試內逐字註明；`{n} > 10` 之分支本輪**沒有防線**。🔴 需 spec-writer 裁決：additive 回應欄位（如 `latestAnnouncementsTotal`），或改寫文案不提總數。**裁決前不得由實作者自行猜一個值填進去。** |
| `G44-15` | ⚠ **`prototypes/07-admin-shell.html` 之第 4 列文案落後於規格** | 規格 §癸四 第 4 列已就地更正為 `已失效或作廢的文件不列入計算。`（原 `只計入儲存狀態為有效的文件。` 與第 8 列「內部欄位語言必須重寫」自相矛盾）；但 prototype 目前仍是**更正前**的字串。<br>🔒 **本環以規格 §癸四 為權威**，並額外斷言更正前之措辭**不得**再出現（它同時違反 `AC-G94` 之「內部欄位語言」）。⇒ 請 ux-f044 同步 prototype，否則下一個照 prototype 實作的人會直接撞上這條紅。 |
| `G44-16` | ⚠ **可見文案 `已排除 {n} 個單位` 之 `{n}` 混合了兩種計數單位** | `{n}` ＝ `excludedInactive + excludedOrphaned + excludedNoAnnouncedDate`（prototype 與規格一致），但第三項在 ⓘ 內逐字為「**{c} 份文件**尚未設定公告日期」——**文件**被加總成**單位**。<br>⇒ 本環依 prototype／規格之一致口徑鎖住總和，**不自行更動**；但這是一處使用者可見的口徑瑕疵，提報 spec-writer 裁決是否改為只加總前兩項、或改寫可見文案。 |
| `G44-17` | 🟢 **建環時自抓並修正：禁用詞掃描必須跑遍四種畫面狀態** | 初版掃描只在「完整語料」下執行 ⇒ `即時聚合`（只在**載入失敗降級態**出現）、`掛載數為 0`／`降冪`（只在**空狀態**出現）三條**全部假綠通過**，而它們正是 §癸四 第 7／8／9 列要處置的違規。<br>🔒 已改為 `describe.each` 跑遍 `full`／`ojtZero`／`loadError`／`empty` 四態（比照 prototype 07 備四個示範態之理由：那些分支在完整語料下**結構上不可達**），並為**每一個狀態**各配一條正向探針。<br>📌 **通則**：凡「某文案不得出現」之掃描，其涵蓋面等於**被渲染到的狀態集合**——只掃一個狀態，就只證明了那一個狀態。 |
| `G44-18` | 🟢 **`DashboardHome.f044.test.tsx` 之 `AC-G15` 一案已依本輪裁決就地改寫期望值** | 第六輪把三種排除之明細與「兩邊數字為何不同」之說明**移入 ⓘ**，可見層只留 `已排除 {n} 個單位` ⇒ 該案原本斷言的 `OJT 資料清單` 與 `1`／`2`／`5` 三個數字**已不在那個節點上**。<br>🔒 依據＝`AC-G15` 新增之指標 ＋ §癸四 第 1 列（lead 明示「本輪既有期望值變動應僅限文案逐字，且須以 §癸四 為依據」）；`OLD>` 兩行已逐字保留於該案註解。<br>🔒 **該條真正要鎖的東西一格未改**：節點恆存在、且與數值節點為兩個節點；逐字文案與 ⓘ 三段內容改由 `DashboardHome.f044.rationale.test.tsx` 承接。 |
| `G44-19` | 🔒 **通則：凡具名例外，都必須配一條「這個例外現在仍然有載體」的斷言** | 具名例外清單（`AC-G90` ③ 之 `jobPositionCode` 四檔、`AC-G94` ④ 之 `最近活動` 區塊）解決的是「裸詞／裸識別子會對合法用法誤報」，但它自己會**腐化**：被豁免的那個載體一旦消失，例外就變成一個沒有對象、卻仍在放行的後門，而且**沒有任何東西會告訴你**。<br>🔒 **處置＝為每個例外配一條正向斷言，證明它現在仍然有載體**。本輪之實例：斷言裸詞「節點」**確實會**在 `最近活動` 區塊內命中。該條同時證明三件事：① 例外不是多餘的防禦；② 有人把裸詞加進清單就會誤報；③ 🔴 **哪天該區塊不再含該詞，這條會紅** ⇒ 屆時應重新評估例外是否仍需要，而不是留著一個沒有載體的豁免。<br>📌 同一手法已回頭套用於 `G44-01` 之四檔例外清單（「四檔皆確實存在且確實命中」之自我守護）。|
| `G44-20` | 🟢 **2026-09-21 裁決回填與第 5 個掃描狀態（本輪收尾）** | **① `G44-14`（`{n}`）已裁決**：lead 查證後端 `listDocuments()` 回**全量投影**、`latestAnnouncements()` 為**純函式**且在手上先 `filter` 出 `pool` 再截斷 ⇒ `{n}` ＝ `pool.length`、零成本；裁定加 additive 回應欄位，🔴 **由同一支純函式在截斷前算出，禁止另寫一次過濾條件**（否則即第二個定義點）。逐字文案待 spec-writer 回填，環內已以 `it.todo` 標記待補，回填後須補**兩種規模**之語料（超過上限／未達上限），否則頁尾兩個分支必有一個恆不可達。<br>**② `G44-16`（`已排除 {n} 個單位` 混單位）裁為必須改**（口徑錯誤，非文案偏好）：可見文字只加總前兩項，第三項移入 ⓘ 獨立成句並說明其屬「**從未進入母體**」而非「被排除」（`OQ-D44-12b`）。環內同樣以 `it.todo` 標記。<br>**③ `G44-15`（prototype 落後）已由 `ux-f044` 同步。** 🔴 **但驗證層級必須是渲染輸出，不是原始碼**——prototype 內另有兩行**記錄這條規則本身**的解釋性註解寫著該舊措辭，grep 原始碼必然命中而被誤判為未同步。🟢 本環之禁用詞斷言**自始即釘在渲染層**（`textContent`，全檔無 `readFileSync`／`fs.`），故不受影響；此點與 `G44-19` 同族——**檢查的對象必須是使用者看到的東西，不是原始碼裡的字**。<br>**④ 掃描狀態由 4 增為 5**：自查發現原本缺的是**未截斷態**（預設語料類別數 ＝ `CATEGORY_LIMIT + 3` ⇒ 三個非空態渲染的**都是已截斷分支**）⇒ `共 {n} 類，已全部顯示。` 與 `本維度共 {TOTAL} 個組織，已全部繪出。` **兩句從未被掃過**。已補 `untruncated` 態，該態另貢獻 9 條新紅。|
| `G44-21` | 🟢 **`AC-G96` 已定案並建環（`G44-14` 結案）** | 端點 additive 新增 `latestAnnouncementsTotal`，由 `latestAnnouncements()` **在截斷前**自**同一個** `pool` 取 `pool.length`。<br>🔒 **純函式回傳型別因此改為 `{ rows, total }`**——`{n}` 必須與 `rows` 同源，另寫一次過濾即是 `AC-G3` 那個「同一件事兩個定義點」的形狀重演（🔴 **兩份初始碰巧相同、漂移前兩份都會綠**）。<br>🔒 環內鑑別力：後端語料下三種最可能的錯寫各給出**不同**的數（`docs.length` ⇒ 16；漏 `announcedDate !== null` ⇒ 15；漏 `status === 'active'` ⇒ 15），而正解為 13 ⇒ 任一錯寫皆翻紅；前端鎖**兩種規模**（`{n} > 10` 與 `{n} === {m}`），且 🔴 **`{m}` 由實際渲染列數導出、不寫死 10**。<br>🔴 **但 `G44-14` 之結案帶但書，不得寫成「已解決」**：環只能以**輸出值**鑑別——`total` 是否**真的取自同一個 `pool`**，本輪驗不到。🔴 **若有人另寫一份過濾條件而初始寫得剛好正確，環是綠的；要等到兩份漂移才會現形。** 這正是 `AC-G3` 已記載的同一形狀（見 `G44-12`），🔒 **本輪無法再強化**——要真正鎖死需要「同一個 `pool` 物件被兩處共用」這件事本身可觀測（例如把 `pool` 也回傳出來並斷言同一參考），而那會為了測試改變生產契約，成本高於收益。⇒ 列為**人工覆核／日後重評**項，不是已完成項。|
| `G44-22` | 🟢 **§癸四 第 1 列「混單位」已裁為必須改並建環（`G44-16` 結案）** | 可見文字改為**恰加總前兩項**（`excludedUnitCount(stats)` ＝ `excludedInactive + excludedOrphaned`），第三項移入 ⓘ **獨立成句**並說明其屬「**從未進入母體**」而非「被排除」（`OQ-D44-12b`）。<br>🔒 **本條唯一的鑑別力來源**（規格逐字指名）已落為向量：`excludedNoAnnouncedDate > 0` 且前兩項為 0 ⇒【可見】**無排除文字**、ⓘ **仍說明那 {c} 份文件**。🔴 若實作仍三項相加，該向量會翻紅（它會印出 `已排除 5 個單位`）。<br>⚠ 少了這個向量，「只加前兩項」與「三項相加」在一般語料下輸出相同——又是恆真。|
| `G44-23` | ⚠ **紅燈的「粒度」本身是一種可維護性資產：`as unknown as` 橋接 vs 讓整份檔案編譯失敗** | `AC-G96` 把 `latestAnnouncements()` 的回傳型別由 `LatestAnnouncementRow[]` 改為 `{ rows, total }`。在實作改完之前，於既有 spec 直接寫 `.rows` 會是**編譯錯誤** ⇒ 整份檔案 `Test suite failed to run`，連同**另外 43 條與本題無關的斷言**一起被蓋掉，且紅燈訊息看不出真正原因。<br>🔒 **處置＝以 `as unknown as` 定義一支本地橋接 helper**，使紅燈**逐條落在真正相關的 15 個案例**上（`.rows` 為 `undefined`），其餘 43 條維持可讀的綠。實作落地後該 cast 即成為恆等轉換。<br>📌 **通則**：當一個 AC 會改變**既有共用符號的型別**時，環的作者要主動選擇紅燈粒度——**讓 50 條無關斷言陪葬，是把診斷訊息也一起丟掉**。|
| `G44-24` | 🔒 **通則：「分支 → 覆蓋來源」對照表 ＋ 涵蓋自證斷言**（`ux-f044` 提出之形狀，本環照抄並落地） | 本輪與上一輪，「掃描漏了某個分支」**兩次都是靠被質疑後回頭查證**才發現的。⇒ 🔒 **把那個偶然變成機制**：逐一列出畫面之可能分支，並新增一條「每一個分支都至少被一個掃描狀態帶進畫面」之斷言——🔴 **讓「漏掉分支」本身變成會紅的東西，不靠人記得**。<br>🔒 本環落地 **12 個分支 × 5 個掃描狀態**，兩條自證皆綠：① 零個未覆蓋分支；② **述詞本身有鑑別力**（恆為真或恆為偽之述詞不是覆蓋證據，是一個沒有鑑別力的常數——該條會抓出寫壞的述詞）。<br>⚠ 述詞一律以**結構標記**（列數、控制項存在性）判定，**不以文案判定**——否則文案一改版，這張表自己就壞掉，而它的職責恰恰是跨文案改版存活。 |
| `G44-25` | 🔴 **交棒警語：覆蓋表是「對當前語料」的實測，不是規格保證** | 🔴 語料一變，覆蓋來源就跟著變。本環之實例：「類別恆落在截斷分支」**唯一**的理由是 `CATEGORY_MANY` ＝ `CATEGORY_LIMIT + 3`；**哪天有人把它改成 `CATEGORY_LIMIT` 或更小，「類別・截斷分支」會悄悄變成「從未被執行過」而不是翻紅**，所有針對該分支的逐字斷言會一起變成恆真。<br>🔒 已於 fixture 定義處逐字加註；🔒 `G44-24` 之涵蓋自證即為該情形之機器防線。⇒ **改語料後必須重跑涵蓋自證，不得沿用結論。**<br>⚠ 另記：本環 fixture 與 prototype 07 **形狀刻意不同**（prototype 兩個環圖之段數天然不同 ⇒ 預設態即同時渲染合併與未合併；本環兩區共用切片 ⇒ 兩分支由兩個狀態分別帶進畫面）。**兩種作法對涵蓋都成立，但覆蓋來源不同** ⇒ 🔴 兩邊的狀態集合**不可互相沿用**——`ux-f044` 與本環的第 5 態因此不是同一個（它補截斷態、本環補未截斷態），**取聯集才是完整涵蓋面**。 |
| `G44-26` | 🔒 **通則：「為了 X 而加的測試其實在守 Y」時，註解必須寫 Y**（`ux-f044` 實例） | 它原本為了「環圖未合併分支」而加的一趟掃描，實測對該分支**毫無貢獻**（早被既有狀態覆蓋）；但它是**「依制定本部」維度標籤（含 `無本部` 段）的唯一來源**——因為每次重繪都把兩區重設回 `department`，不主動切維度就永遠掃不到本部維度的文字。<br>🔴 **該趟要留，但理由寫錯的註解會讓下一個人刪掉它。**<br>📌 與 `G44-19`（具名例外要配腐化偵測）同一家族：**兩條都是在讓下一個人有辦法判斷「這東西還需不需要」**——前者靠斷言、後者靠註解。 |
| `G44-27` | 🔴 **兩條 AC 互相矛盾到「不可能同時綠」——而先翻紅的是環，不是人**（`AC-G89` × `AC-G94`；lead 已於規格內就地改寫，本列為建環側之紀錄） | **成因**：第六輪「實作理由退出畫面」把卡④ 的說明搬進 ⓘ，六條 AC 都加了指標，**唯獨 `AC-G89` 漏了** ⇒ 它仍要求一支帶「覆蓋率頭句＋百分比＋`NO_STATISTICS_TEXT`」的函式。而 `NO_STATISTICS_TEXT` 之字面為 `尚無可統計之進度列`，**內含 `AC-G94` 禁用之內部欄位語言 `進度列`**；改寫後該函式之輸出**恆在 ⓘ 內**、必然進入禁用詞掃描面 ⇒ 🔴 **兩條在舊形狀下不可能同時綠**。<br>🔒 **這正是 `LESSON-G1` 的形狀**：一個後續決策讓一條既有 AC 的載體在無人察覺下蒸發，**條文一字未變、沒有任何東西會翻紅**——這次會紅，只因為環在那裡。<br>🔒 **處置紀律（逐字記錄）**：`impl-f044` 以「四種選項全數不可行」之窮舉證明提報後，本環 **未自行放寬任何斷言**，改為回讀規格——發現 lead 已就地裁定 **(b) `ojtOnTimeNote` 即為 ⓘ 之產生者**，⇒ 🔴 **是環的向量過期，不是實作有錯**，由本環（非實作者）重寫該 describe。<br>📌 **通則**：實作者送來「你的測試和另一條規則互斥」時，第一件事是**回去讀規格**，不是改測試——本例中裁決早已落盤，只是還沒有人把它傳到環上。 |
| `G44-28` | ⚠ **「內容相等」不等於「呼叫關係」：裁定 (b) 的核心主張原本零載體** | 裁定 (b) 之整個理由是「(a) 會留下一支**沒有消費者的死函式**」。但改寫後，ⓘ 的逐字鎖在元件層、函式的逐字鎖在純函式層，**兩組各自獨立**——元件若把四段硬寫在自己身上、`ojtOnTimeNote` 變成死函式，🔴 **兩邊都會綠，而裁定 (b) 想避免的那件事正好發生了**。<br>🔒 **已補載體**：於元件層斷言「ⓘ 之內容**逐字包含** `ojtOnTimeNote(stats)` 之輸出」，把兩者綁在一起 ⇒ 此後任一邊調整措辭都會立刻翻紅、不可能靜默漂移。<br>⚠ **但仍有殘留限制，不得寫成「已解決」**：本斷言證明的是**內容相等**，不是**呼叫關係**；元件若硬寫一份**恰與函式輸出逐字相同**的孿生本，本條仍綠。🔒 要真正證明呼叫關係需 `vi.mock` 之 spy，而 `vi.mock` 為**整檔 hoist** ⇒ 須另立一支測試檔（比照 `AC-G66` 之矩陣檔）。<br>🟢 **建環當下已以探針實測過（不入環，僅記錄）**：將函式輸出末尾接上一個哨兵字串後，ⓘ 中**跟著出現**該哨兵 ⇒ ✅ 元件確實是委派、不是硬寫孿生本。⚠ 該事實是**建環當下**的，本斷言本身無法在每次執行時重驗它 ⇒ 列為**日後重評**項。 |
| `G44-29` | 🔴 **新的假綠形狀：一條正確的斷言，基於「它不可能被滿足」這個錯誤前提被放寬——而放寬後兩種實作都綠，所以放寬本身不會被任何東西抓到** | **經過**：`AC-G95` 之 `Tab 進入（focus）⇒ 開啟` 原為**同步**斷言（`trigger.focus()` 後直接讀 `aria-expanded`）。它翻紅時我套用了正確的紀律（「先查自己，不要先怪實作」），推斷 React 18 把 focus 排在 **continuous 車道**、同步斷言必在沖洗前求值 ⇒ 判為**測試的錯**，改成 `waitFor`。<br>🔴 **但那個前提是假的**：`impl-f044` 早已在 `onFocus`／`onBlur` 包 `flushSync` 並讓**原樣的同步斷言**綠過。⇒ 我放寬的不是一條寫錯的斷言，是一條**寫對而且抓得到缺陷**的斷言。<br>🔴 **代價＝鑑別力歸零**：`waitFor` 會等到下一拍 ⇒ **一般 `setState` 的實作也會綠**，而那正是原本抓得到的缺陷——焦點已在按鈕上、popover 已由 CSS 展開，`aria-expanded` 還停在 `"false"`，對螢幕閱讀器是一個**說謊的中間狀態**。<br>🔒 **紀律（本列之重點，對下一次成立）**：**放寬一條斷言之前，必須先證明「不存在任何正確實作能照原樣滿足它」。** ⚠ 本輪我自己在建環時就用過這個方法（寫完整參考實作跑一遍全綠，證明環可被滿足、不會白擋實作者）——這次少做了這一步，而且**沒有問實作者是否已經滿足它**。<br>🟢 **已還原為同步斷言，並以兩趟探針實測**（皆已還原，`grep` 殘留為 0）：① 把實作之 `flushSync` 換成 `queueMicrotask`（＝「慢一拍」的實作）⇒ 🔴 **同步版翻紅**；② 同樣的「慢一拍」實作下改用 `waitFor` ⇒ 🟢 **149/149 全綠**。⇒ **`waitFor` 版本的鑑別力確實是零，不是推測。**<br>📌 與 `LESSON-G1` 對照：那次是**載體蒸發**（條文沒變、東西消失）；這次是**斷言自我閹割**（斷言還在、鑑別力消失）。🔴 **兩者都不會翻紅，都只能靠有人回頭看。** |
| `G44-30` | 🔒 **通則：當一個裁決的理由是「避免 X」，環必須有一條斷言直接鎖住「X 沒有發生」——而不是只鎖住裁決的產物**（lead 提煉，本環之 `AC-G89` 即實例） | `AC-G89` 裁定 (b) 勝過 (a) 的**唯一**理由是「(a) 會留下一支沒有消費者的死函式」。而改寫後環裡的兩組斷言——ⓘ 的逐字鎖在元件層、函式的逐字鎖在純函式層——**各自獨立**：元件若把四段硬寫在自己身上、`ojtOnTimeNote` 變成死函式，🔴 **兩邊都會綠，而裁決想避免的那件事同時發生了**。<br>⇒ 🔒 **要檢查的不只是「條文有沒有被滿足」，還有「裁決的理由本身有沒有載體」。**<br>🔒 本環之處置見 `G44-28`（以「ⓘ 內容逐字包含 `ojtOnTimeNote(stats)` 之輸出」把兩者綁死）。<br>📌 與上一輪之 `G44-26`（「為了 X 而加、其實在守 Y，註解要寫 Y」）同一家族：**兩條都是在問「這條斷言真正在守的是什麼」。** |
| `G44-31` | 🔒 **通則：拆節點時要 grep 該 `data-testid` 的所有出現處，特別是空狀態／錯誤態分支**（`ux-f044` 提煉，本環照收並落為斷言） | 卡④ 由**一個**數值節點拆為**兩個**（`[data-ojt-ontime-rate]` ＋ `ojt-ontime-value`）。拆之前只有一個數值節點，空狀態之提示文字上**共用**掛著 `data-testid="ojt-ontime-value"`**無妨**；拆之後 `AC-G97` 要求該節點於分母 0 時**不得存在** ⇒ 🔴 **那個殘留會讓「該節點不得存在」的負向斷言直接失效（恆真）**，而且畫面看起來完全正常。<br>🔒 designer 已於 prototype 移除該殘留（文字本身不變）；本環之對應斷言為「三個節點逐一不得存在 ＋ 卡內 `svg` 數為 0」。<br>📌 **為何要數 `svg`**：「沒有那個 `testid`」很容易被繞過（改個名字就好），而「卡內一個 `<svg>` 都沒有」直接對應規格真正要的那件事——🔴 **不得繪空環**。<br>⚠ 與 `G44-19`／`G44-26` 同族：**負向斷言的前提（那個載體確實不存在）本身需要被檢查**。 |
| `G44-32` | 🔴 **閘門合流使 `AC-G18` 失去獨立鑑別力——這是裁決的直接結果，不是環的缺口** | `AC-G17` 第七輪改為 `canPerform(role, OJT_PROGRESS_MANAGEMENT, 'read')`，與 `AC-G18`（`查看明細` 之閘門）**同一支述詞**。⇒ 🔴 `OLD>` 本組原本的鑑別力來自「撤銷 OJT 讀取權 ⇒ **連結消失、卡④ 本體仍在**」（當時卡由 `canViewDashboard` 把關）——**那個狀態在規格上已不可能存在**。<br>🔒 **處置＝改鎖「兩者必須同進同出」**，而那恰是**兩種最可能的錯誤實作唯一會現形的地方**：① 卡仍重用 `canViewDashboard`（撤銷矩陣鍵時卡會留著）；② 卡寫成四角色白名單（🔴 第七輪之後四種後台角色皆可見 ⇒ **白名單與矩陣在畫面上完全同值**，只有「**只撤銷單一角色**」的向量分得出來）。<br>⚠ **誠實標記**：向量 ②（只撤銷 `Supervisor`）於建環當下是**綠的，而且是碰巧綠**——舊實作下 `Supervisor` 本來就看不到卡④。🔴 **它目前零鑑別力，要等 `AC-G17` 落地之後才成為防線**，不得以它現在是綠的主張「該規則已被滿足」。 |
| `G44-33` | ⚠ **`toContain` 的假綠：被拆掉的舊字串往往「同時含有」拆開後的兩段** | `AC-G24` 要求卡④ 之外層 `stat-value` **同時含住**環中央之 `{Z}%` 與環旁之 `已完成 {X} / 應完成 {Y}`。以 `toContain` 逐段驗證看似正確——🔴 **但 `OLD>` 的單一字串 `已完成 3 / 應完成 4（75%）` 同時含有這兩段**，⇒ **完全沒拆節點的舊實作照樣全綠**（本環初稿即如此，跑出來才發現）。<br>🔒 **修法＝鎖結構而非鎖子字串**：斷言外層裡面**真的有那兩個內層節點**（`querySelector('[data-ojt-ontime-rate]')` 不為 null、`getByTestId('ojt-ontime-value')` 在其內）。<br>📌 **通則**：**「把一段文字拆成兩處」這種改動，用文字包含關係去驗一定驗不出來**——舊的那一段是新的兩段的**超字串**。要驗「拆了沒有」，只能驗**結構**。 |
| `G44-34` | 🔴 **「更難繞過」的代理量尺可能量錯東西——交件前必須拿 prototype 自己跑一次** | 為了讓「不得繪空環」比「沒有那個 `data-testid`」更難繞過，本環原本鎖 `card.querySelectorAll('svg')` **長度為 0**。🔴 **該量尺對任何忠於 prototype 的實作都不可滿足**：`lucide` 的每一顆圖示都渲染成 `<svg>`，而卡④ 在空狀態下仍有**四顆各自被別的 AC 要求**的圖示（卡頭 `graduation-cap`／ⓘ 觸發器之 `info`（`AC-G95`）／`EmptyState` 共用版型之 `info`／`查看明細` 之 `arrow-right`（`AC-G18`））。⇒ 🔴 **我量的是「卡內有沒有圖形」，不是「有沒有畫環」**；要變綠只能違反另外三條 AC。<br>🔒 **`impl-f044` 提報，我以 prototype 查證成立**（`:925`／`:940`／`:960`／`:965` 四處逐一對上）⇒ **這是環的缺陷，不是實作的**；由我改環（採其建議之弧本身，並補上 `viewBox`）。<br>🔒 **修正後＝意圖不變、量尺換掉**：`svg[viewBox="0 0 64 64"]`（環的畫布；lucide 一律 `0 0 24 24`）＋ `[stroke-dasharray]`（**弧本身**）。🔒 **兩者皆補正向孿生**（環存在時必須各自命中），否則對「環根本不用 dasharray 畫」零鑑別力；另加自我守護「空狀態下卡內仍必須有圖示」，防止有人為了讓舊量尺變綠而把圖示拿掉。<br>🟢 **鑑別力已實測**（不動生產碼，只改本測試之 fixture 使環真的被畫出來、並逐一停用同一 `it` 內較早的斷言）：兩條鎖**各自獨立翻紅**。<br>📌 **通則（本列重點）**：🔒 **選一個「更嚴格」的代理量尺時，先拿 prototype 自己跑一遍**——prototype 是保真目標，**它過不了的量尺一定是量尺錯**。⚠ 與 `G44-33` 同一天、同一個 `it`：**同一條斷言我先犯了太鬆（子字串恆真），再犯了太緊（量錯對象）**。 |
| `G44-35` | 🔒 **通則（本輪出現三次，收在一起）：量尺選錯，不是意圖選錯——「我這把尺量到的，是不是恰好就是我要禁的那件事？」** | 🔴 三次皆為**意圖正確、量尺量到旁邊的東西**：<br>**①**（`AC-G94` ④）**裸詞黑名單**——把「節點」當禁用詞，會在「最近活動」的 `循環「…」新增節點「…」` 上誤報：那是**真實的領域語言**，不是實作詞彙。⇒ 改為具名例外 ＋ 腐化偵測（`G44-19`）。<br>**②**（`AC-G24`，`G44-33`）**文字包含關係**——驗「一段文字拆成兩個節點」，但 `OLD>` 的單一字串是拆開後兩段的**超字串** ⇒ 沒拆的實作照樣綠。⇒ 改鎖**結構**（外層裡面真的有那兩個內層節點）。<br>**③**（`AC-G97`，`G44-34`）**數 `svg`**——想讓「不得繪空環」比「沒有那個 testid」更難繞過，卻量成「卡內有沒有圖形」：`lucide` 每顆圖示都是 `<svg>`，卡④ 空狀態下有四顆各自被別的 AC 要求 ⇒ **prototype 自己也過不了**。⇒ 🔒 **裁定改鎖 `[stroke-dasharray]`**（弧獨有，圖示皆無；換 testid、改 viewBox 都逃不掉）；🔴 `svg[viewBox="0 0 64 64"]` **經裁定不採用**——綁死一個可被改的常數、規格從未要求。<br>⚠ **本裁定之已知代價，逐字記錄**：本輪自此**沒有任何斷言鎖住卡④ 環的畫布尺寸**（prototype `:925` 之 `0 0 64 64`）。這是刻意的取捨（不凍結版面），不是漏掉。<br>🔒 **三者的共同修法都是同一句**：**鎖結構／鎖那件事本身，別鎖字面、數量或鄰居。**<br>📌 **交棒提醒**：`svg` 數看起來比 `[stroke-dasharray]` 嚴格，下一個人很可能想改回去——三條斷言的註解都已寫明「意圖對、量尺錯」的理由，**不要只留結論**。 |
| `G44-36` | ⚠ **用 stub 取代外部函式庫時，被 stub 掉的那一段行為在你的量測裡就消失了**（lead 自行更正其查證，逐字收錄） | lead 以 jsdom 驗 prototype 時，把 `lucide` 換成 **no-op stub**（`createIcons(){}`）⇒ `<i data-lucide>` 從未被轉成 `<svg>` ⇒ 量到「卡④ 空狀態下卡內 `svg` 數 ＝ 0」，並據此回報。🔴 **那是 stub 造成的假象**——而被量的恰恰就是「lucide 會不會產生 `<svg>`」這一段行為。<br>🔒 **通則**：**stub 的邊界必須在被量測的行為之外。** 若量測對象與被 stub 的函式庫有任何因果關係（它產生 DOM、它決定可見性、它影響時序），該量測**不成立**。<br>📌 與 `G44-29`（基於假前提放寬斷言）同族：**兩者都是「證據看起來支持結論，但證據本身是自己造出來的」**。⇒ 🔒 得到一個「剛好支持我想法」的量測時，先問**這個數字有沒有可能是我的測量裝置產生的**。 |

### UX16（2026-09-22 使用者體驗優化 16 項）— 建環側紀錄

> 對應 `docs/specs/architecture-spec.md` §16、七檔 `## UX16 delta` 節（`AC-UX1`～`AC-UX56`）。本節記錄建環過程中查證所得之事實更正、範圍取捨與已知盲區，供 tdd-implementation 與 lead 銜接。

| # | 事項 | 說明 |
|---|---|---|
| `UX16-01` | 🔴 **`classifyCompanyCounts()` 不需要（也不應該）過濾 `count===0`——`AC-UX34`②「0 不顯示」是 SQL `GROUP BY` 之結構性保證，非本純函式之職責** | 建環時依 `reference-impl-probe-loop` 紀律寫參考實作驗證，發現若依原提報斷言「輸入含 0 之列時輸出須過濾」，會與 architecture-spec §16.6 給定之 `classifyCompanyCounts` 參考實作**矛盾**（該函式本身不做此過濾）。查證後確認：`docCountsByNodeAndCompany()` 之 `GROUP BY nodeId, companyCode` 結構上保證「某公司在某節點之計數為 0」根本不會出現在 `raw` 陣列裡——SQL `GROUP BY` 只為**存在至少一筆**的組合產生一列。⇒ 已就地改寫 `backend/src/business-categories/node-company-counts.spec.ts` 之對應測試，不再斷言函式本身之過濾行為，僅斷言「輸入不含 0 計數列時（真實 SQL 輸出之唯一合法形狀）輸出忠實反映各公司之非 0 計數」。**`AC-UX34`② 之真正驗收落在部署後之人工覆核**（architecture-spec §16.12 #1 既有盲區），非本輪自動化閘門。 |
| `UX16-02` | 🟢 **已結案——`AC-UX57` 落檔（lead 第七輪裁決）**：`createOrgPathResolverWithDivision(orgCode)` 對「orgCode 本身即為 DIVISION 層代碼」之行為**並非不可達**，已定義並補測 | 原判「不可達」之理由（既有呼叫端皆以部/處室 orgCode 呼叫）**對 F017/F019 之制定部門成立，對 F042 之使用部門不成立**——`DocumentEditPage.tsx:291,410` 之使用部門候選**不依層級過濾**（該公司全部組織單位），使用者可把使用部門設為本部層單位本身。⇒ lead 裁定 `AC-UX57`：本部段與部段解析為**同一 `orgCode`** 時只輸出一次（判準為代碼相等，非顯示名相等）。已補三條成組向量：`backend/src/org-directory/org-division.spec.ts`（純函式層，無 company 前綴）＋ `backend/src/ojt-progress/ojt-progress.cross-company.spec.ts`（adapter 層，含 company 前綴，ⓐ`D0000`→恰兩段／ⓑ`DA000`→恰三段／ⓒ`DAA00`→恰四段）。**紀律**：「這條路徑走不到」是關於上游輸入範圍的主張，下判斷前必須指出「誰是呼叫端、輸入從哪裡來」，不得從「既有測試都沒這樣呼叫」反推。 |
| `UX16-03` | ⚠ **`PublicDocumentDetailService` 之真實建構子 arity 已大於既有 spec 假設之 3——第 4 參數 `ojtCompletion` 為建環時之意外發現** | `public-document-detail.service.spec.ts`（既有）僅以 3 參數建構（`store, names, clock`）。`public-document-detail.service.ux16.spec.ts`（本輪新增，測 `AC-UX18` 之 `categories` 注入）以 3 參數呼叫時，於 `detail()` 執行期拋出 `this.ojtCompletion.getCompletionByDocument is not a function`——證實真實建構子已含一個既有 spec 未使用之第 4 參數（型別未知，本輪對此保持全盲）。⇒ 本輪之 `categories` 注入**假設為第 5 參數**（`ojtCompletion` 以 `undefined` 呼叫），依 TypeScript/NestJS「新增依賴附加於既有參數列末端」之慣例推定，**非確認之事實**。🔒 **此為合理 test-dispute 之預先聲明**：若 tdd-implementation 之實際注入位置不同，應改介面呼叫方式、不得弱化 `businessCategories` 欄位本身之行為斷言。 |
| `UX16-04` | 🟢 **已補測——`PublicListPage.subtreeFilter.ux16.test.tsx`（新檔）** | 涵蓋 `AC-UX15`（deep link → chip 存在、逐字文案、⑤ 兩參數恆成對之靜默 no-op）與 `AC-UX16`（chip ✕ 之 DOM 契約、只清 chip 不動既有篩選、「清除篩選」連 chip 一起清）。🔴 **wire 形狀為明文假設之 test-dispute 風險**（清單回應之 additive `subtreeChip` 欄位、`getPublicDocuments()` 直接併入 `bcSubtreeId`／`bcSubtreeNodeId` 兩參數），非規格逐字鎖定——若 tdd-implementation 之實際形狀不同，應改介面呼叫方式與 fixture，不弱化「chip 存在／清除方向性不對稱／靜默 no-op」三件事本身之行為斷言。 |
| `UX16-05` | 🟢 **已結案——F017／F019 兩側皆已補測** | F017 側：`backend/src/documents/documents.service.ux16.spec.ts`。F019 側：`backend/src/public/public-documents.service.ux16.spec.ts`（新檔，2026-09-22 lead 提報後補）——涵蓋 `AC-UX22`：`list()` 富化後之列確實帶有 `draftingDivisionId`／`draftingDivisionName`（值來自組織索引真實上溯，非寫死）＋跨公司同碼防誤取（AS／AJ 各自 `DAA00`）；`AC-UX23`：**`filterOptions()` 之 `draftingDivisions` 於可推導本部之語料下不得為空陣列**——此為本 repo 已發生三次「端點漏代理 → fetch 收到 index.html → 空陣列 → 下拉永遠沒有選項、零錯誤訊息」之直接對策（`/org-units`／`/persons`／`/companies` 前例），選項集合非空本身列為獨立斷言，不只驗證名稱解析正確與否。🔒 兩檔皆依「鎖結果不鎖機制」紀律：不斷言 `listOrgUnitsByCompany` 被呼叫過，只斷言輸出值正確。 |
| `UX16-06` | 🔒 **兩支一次性 migration（`AC-UX9`／`AC-UX30`）之測試依賴動態尋檔（檔名時間戳前綴未知），且僅能做結構性斷言** | `account-user-subtype-derived-backfill.selection.spec.ts`／`business-category-sort-order.selection.spec.ts` 以 `fs.readdirSync` 動態尋找檔名含特定字串之 migration 檔，尚未建檔前之紅燈訊息為「尚未建立…migration」。`AC-UX9` 之選取述詞測試具真實鑑別力（投影 WHERE 子句條件並套用三筆固定語料）；`AC-UX30` 因涉及 `ROW_NUMBER()` 秩對映，僅能做 SQL 文字之結構性斷言（關鍵字／片段存在性），**不構成**「該 migration 對真庫正確落地」之證明——真正驗收條件（真庫實跑＋`SELECT`覆核）已明文寫入各自 AC 本文，屬 architecture-spec §16.12 #2/#3 之既有盲區。 |
| `UX16-07` | 🟢 **已補測——`PublicCategoryTreePage.test.tsx` 新增之 `describe('... AC-UX36 ...')`** | `AC-UX36` 前台負向半句（不存在 `node-company-counts` 容器、不存在任何 `data-company-doc-count`）與既有 `data-visible-doc-count`／掛載徽章零漣漪回歸鎖皆已補上，與 `BusinessCategoryTreePreviewPage.companyCounts.ux16.test.tsx` 之後台正向半句成對。⚠ `AC-UX26`⑤ 之屬性名互斥檢查仍未獨立成案（該規則之核心即 `AC-UX36` 之「不存在 `data-company-doc-count`」，已隱含涵蓋，但未有一條顯式斷言「`data-visible-doc-count` 與 `data-mounted-doc-count` 兩個屬性名不得共用」），建議下一輪視需要補一條顯式互斥案。 |
| `UX16-08` | 🟢 **已裁決——`impl-core` 申訴（2026-09-22）：`role-derivation.spec.ts` 三條既有絕對值鎖與新增之 `AC-UX8` 互斥，已仲裁三案** | 建環時新增 `AC-UX8`（規則 A 停用後 `deriveRoles()` 輸出中 `to==='business'` 恆為 0）只加了新案例，**未逐一檢查既有 `deriveRoles` describe 區塊裡是否有斷言與之字面互斥**——`impl-core` 依實作找出三條：① 「職稱含業務 → subtype 直接寫為 business」（直接斷言 `to==='business'` 出現一次，與 `AC-UX8` 字面互斥）；② 「角色待審降級時子分類仍照常寫入」（語料現值為 `other`，AC-UX8 後 target 與現值相同、不再產生變動，原不變式失去載體）；③ 「職稱以 (companyCode, code) 複合鍵解析」（`isBusinessJobTitleName()` 完全不再被呼叫，`jobTitles` 參數已對 `deriveRoles()` 之任何輸出無可觀測影響，此不變式**目前無載體**）。<br>🔒 **處置**：①②**就地改寫**（①改斷言「現值已是 other 時不產生變動」；②改語料現值為 `business`，保留「兩條規則路徑分離」之原意且維持鑑別力）；③**最終處置＝`it.skip` ＋ 附加一條互補之單元層測試（見下，歷經 team-lead 兩輪覆核）**。<br>🔴 **③ 之處置沿革（`team-lead` 兩輪覆核，2026-09-22）**：第一版裁定為 `it.skip`。**第一輪覆核**：`team-lead` 指出恆真／休眠之斷言比沒有斷言更糟，應優先查「複合鍵解析在別處是否仍有真實載體」——查證後於 `org-directory/job-title-directory.spec.ts` 補上一條原本缺的直接斷言（`jobTitleKey('AS','D04') !== jobTitleKey('AD','D04')`，同代碼跨公司不得互相覆蓋），並**刪除**了 `role-derivation.spec.ts` 之原案。**第二輪覆核**：`team-lead` 推翻自己第一輪的「遷移或刪除」二選一，改為認可 `it.skip`——理由是人類裁決 A 逐字要求「程式碼以註解保留**可復原**」，原案是 `deriveRoles()` 消費 `isBusinessJobTitleName()` 時之**端對端**驗證（帳號→複合鍵→查表→業務判定→子分類輸出全程），日後規則 A 復活時是現成的驗證，刪除的話復原者得重寫一次；`job-title-directory.spec.ts` 之單元層新增**不是替代品、是不同層次之互補**——只證明 `jobTitleKey()` 建鍵本身不會跨公司碰撞，不證明 `deriveRoles()` 有沒有正確地用它（目前不可能證明，因輸出完全不依賴這段解析）。**最終處置**：`role-derivation.spec.ts` 恢復 `it.skip`，並補齊兩件事——① 可判定之解封條件（「當 `deriveRoles()` 重新消費 `isBusinessJobTitleName()` 之回傳值時解封」，非「暫時停用」這種無觸發條件之措辭）；② 誠實現況聲明（`AC-UX8` 保護的是輸出恆為 `'other'`、與本案不是同一件事；`jobTitleKey()` 之跨公司不覆蓋由 `job-title-directory.spec.ts` 保護；但「`deriveRoles()` 有沒有正確把 `companyCode` 併入查表鍵」**本輪起在這一層無任何自動化保護**，休眠中的本案是解封前唯一的現成驗證載體）。`job-title-directory.spec.ts` 之新增測試保留，兩者並存、各自負責不同層次。<br>📌 **通則（本輪之教訓）**：新增一條「輸出恆為某值」型 AC 時，不能只加新案例，**必須同時 grep 同一 describe 區塊裡是否有既有斷言直接要求相反的值**——這正是本 repo 已多次記錄之「新斷言要 N、既有他處斷言鎖死非 N ⇒ 同一份環不可能全綠」之同一形狀，只是這次不是「數量」而是「布林值直接互斥」。<br>📌 **`it.skip` 之附加通則**：任何一條休眠測試都必須同時附上**可判定的解封條件**（一句「當 X 發生時解封」，不是「暫時停用」這種無觸發條件的措辭）與**誠實現況聲明**（此刻誰在保護這件事、若真的無人保護就明說），否則休眠等同刪除，只是留了一具屍體在檔案裡——沒有人會知道什麼時候該打開它。<br>🔴 **同一模組另一種通則之重演，範圍更廣（非同一件事，勿混淆）**：`impl-core` 落地實作後跑全套 backend 閘門，發現同一目錄下 `role-derivation-pipeline.spec.ts`（🔴 2026-08-25 角色自動化 delta 既有檔案，非本輪新增）有 8/14 條測試轉紅。root cause **不是**「絕對值過期」（斷言的值錯了）、**也不是**「載體蒸發」（被斷言的東西不見了）——而是**語料的前提被抽掉**：該檔多處把「業務職稱」語料當成「產生 N 筆 `subtypeChanges`」之**間接手段**，藉此驅動與職稱規則本身**正交**的關懷（5% 閾值擋不擋、畫面一次性放行、降級告警路由——斷言本身完全正確，壞掉的只是抵達那個前置狀態的路）。規則 A 停用後該語料之 side effect 失效（N 從 20 掉到 0），連帶讓正交關懷測不到，且**這種依賴從頭到尾不會出現在任何斷言的字面裡**，字面 grep 結構上必然漏掉。test-generator 主動排查（非 `impl-core` 正式申訴，跑全套時自己發現）並就地改寫：6 處語料一律加上 `userSubtype:'business'`，使 `AC-UX8` 之回填語意（`business→other`）照樣產生同樣筆數的變更，僅語意方向反轉，閾值/計數/百分比等正交邏輯數字完全不變。已對照 `impl-core` 實作重跑 `role-derivation-pipeline.spec.ts`：14/14 全綠。<br>📌 **通則（一般化措辭，適用於任何一次規則變更，不限本次之欄位／檔案）**：改動一條規則前，除了問「誰的斷言直接要求相反的值」，還要問**「誰的 `Given`（語料前置狀態）依賴這條規則之舊行為當作間接手段，去抵達一個與這條規則本身無關的測試目標」**——這類依賴不會出現在任何斷言的字面裡，只會在跑過全套、逐檔看紅燈歸誰時現形，且抓的範圍不能只框在改動所在之同一 describe 區塊或同一檔案，必須含同一模組目錄下的其他測試檔。 |
| `UX16-09` | 🔴 **`AC-UX55` 未涵蓋之缺口——`X-Export-Row-Count` 標頭缺失／不可解析時之降級行為，由 lead 核准機制、test-generator 定案文案，補於執行環而非 docs（未回填 AC 本文）** | `AC-UX55`①之字面只鎖「`{N}` 必須是匯出筆數、不得是畫面列數」之**快樂路徑**，未預期「筆數改由回應標頭 `X-Export-Row-Count` 提供」這個**傳輸層**設計（該設計為本輪 lead 於實作中途核准，晚於 `AC-UX55` 定稿）——標頭可能被中間層吃掉（本專案 nginx／vite 代理白名單**已經漏過同類規則四次**，非假設性風險）。lead 指定唯一硬條件：**降級時不得回退為畫面列數**（否則從「沒說清楚」惡化為「說了假話」，`AC-UX55`①之同一理由）；降級文案由 test-generator 依 `AC-UX55`④（使用者語言、不得出現內部詞彙）定案。⇒ 已於 `frontend/src/pages/OjtProgressPage.ux16.test.tsx`（`AC-UX55` describe 區塊）補四條向量：標頭缺失（`count: null`）、標頭存在但不可解析（`count: NaN`，與缺失收斂同一降級處置）、降級文案之內部詞彙裸掃描（呼應既有④案）、**`count: 0` 與降級狀態之互斥**（guard 常見 `if (!count)` 誤判 falsy 之偷懶寫法，0 為合法值須逐字顯示「共 0 筆」）。**wire 形狀假設**（`exportOjtProgress()` 解析為 `{ count: number \| null }`，additive、向下相容既有 `{ count: 340 }` 夾具）與**降級文案逐字**（`已匯出 OJT 進度清單（CSV，UTF-8 BOM）；系統暫時無法確認筆數，請以下載檔案為準。`）皆為 test-generator 決定、非規格逐字鎖定，屬合理 test-dispute 範圍——若 `impl-ojt` 之實際回傳形狀或文案措辭不同，應改介面呼叫方式／文案字面，**不弱化「不得回退為畫面列數」與「不得捏造數字」兩件事本身**。🔒 **已實跑驗證紅燈成因**：四條新案例中兩條（`count: null`／`count: NaN`）針對現行占位實作（直接字串內插 `count`，印出「共 null 筆」「共 NaN 筆」）為紅——紅在「未產生降級文案」這一正確理由，非測試自身錯誤；`count: 0` 與既有 `count: 340` 案例維持綠。📌 **AC 本文未回填**：本缺口之發現與處置一律記於此處，未回頭修改 `docs/specs/features/F042-ojt-progress-management.md` 之 `AC-UX55` 本文（依紀律「不動 docs 之 AC 條文，發現有誤即回報 lead」），若 lead 認為應正式收斂為新編號 AC，需另行裁決。 |
