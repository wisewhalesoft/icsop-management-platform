...
#### TS-F019-005 置頂比對套用子樹展開（OQ-F019-03 已定案，2026-07-24） [unit]
- Given：使用者 `orgCode=JAC00`（處室層）；文件 D1 `usingDeptIds=[JA000]`（其上層之部層）
- When：排序
- Then：**D1 列入置頂**。OQ-F019-03 定案採 (b) 子樹展開語意：置頂 ⟺ 文件使用部門任一筆為使用者組織代碼之**祖先或自身**。
- **本案期望值已於 2026-07-24 反向修正**（原採 (a) 精確比對、期望不置頂）。依據：prototype 03 內嵌 `USER_SCOPE`/`inScope` 明載「§9.1/§9.2 定案：文件使用部門可指定任意層級，選上層自動涵蓋其下所有單位 → 『您部門相關』＝文件使用部門命中登入者組織路徑上任一層級（含全公司）」；F026 spec AC 亦以 `JA000`／`JAC00` 同例明定相符。實作見 `public-seams` 線之 `isWithinSubtree()`（置頂與部門篩選共用同一純函式，僅引數角色對調）。
- 反向對照案：文件使用部門設為處室層 `JAC00`、使用者為同部之另一處室 → **不置頂**（避免過度放寬）。
- 對應 AC / 錯誤碼：Main Flow 2、F026 使用部門子樹展開 AC
...
- **OQ-F019-03（語意衝突）— 已定案 ✅（2026-07-24）：採 (b) 子樹展開。** 依據 prototype 03 內嵌 `USER_SCOPE`/`inScope` 註解（「§9.1/§9.2 定案…命中登入者組織路徑上任一層級（含全公司）」）與 F026 spec AC 之 `JA000`／`JAC00` 同例，兩份權威來源一致指向 (b)。TS-F019-005 期望值已反向修正；實作抽出共用純函式 `isWithinSubtree(scopeCode, targetCode)`，置頂傳 `(文件使用部門, 使用者代碼)`、部門篩選傳 `(選定單位, 文件使用部門)`。以下為原始待決記錄：F019 Main Flow 2「文件使用部門含使用者部門」拆置頂區之判定，究竟是（a）**精確集合成員比對**（使用者 orgCode 逐字存在於文件 `usingDeptIds` 集合中），或（b）**依 §9 子樹前綴規則展開後之比對**（如使用者掛於 `JAC00`，文件使用部門若設為其上層 `JA000`，是否仍視為相符）？Spec 原文僅在「部門篩選」章節（§9）明確定義子樹展開規則，並未明言置頂比對規則是否比照。兩者語意差異直接影響 TS-005 之期望值與實作複雜度（(a) 為單純 `IN` 查詢；(b) 需額外前綴比對邏輯）。**需 architect 或 product owner 定案**；本設計暫依 spec 字面採 (a)，並於 TS-005 明確標註此為待確認假設。
