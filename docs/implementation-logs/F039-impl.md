...
| 項目 | 結果 |
|---|---|
| Test Suites | **111 passed / 111 total**（103 既有 ＋ 8 由紅轉綠） |
| Tests | **1361 passed / 1361 total**（baseline 1203 ＋ 158 新增／修復） |
| 全套件（最終跑，39.3 s） | **零失敗** |
...
**過程紀錄——一次中途的假紅（非本次變更所致，最終已排除）**：
多 agent 併行期間的一次全套件執行曾出現 2 紅，皆為 `[NFR]` 效能案例
（`src/public/pdf-burner.spec.ts`、`src/lifecycle/lifecycle-change-diff.service.spec.ts`）
之 `Exceeded timeout of 5000 ms`。兩者為 pdf-lib 真實燒錄之耗時案例，與附錄功能
無任何程式路徑交集（附錄下載**不燒錄浮水印**）。證明為 CPU 競用而非迴歸：

```
# 隔離重跑
npx jest src/public/pdf-burner.spec.ts src/lifecycle/lifecycle-change-diff.service.spec.ts --runInBand
→ 2 suites / 13 tests 全綠，29.0 s（該 suite 於競用時單獨耗 518 s）
# 最終全套件（無競用）
npm test → 111 suites / 1361 tests 全綠，39.3 s
```
...
**未執行——使用者裁定本次跳過**（缺 `E2E_COMPANY`／`E2E_LOGIN_ID`／`E2E_PASSWORD` 憑證，
無法建立 ICSOPAdmin storageState）。此部分約束**尚未驗證，不計為通過**。
...
1. **`AuditRow.appendixId` 最終為必填 `string \| null`（無 `?`），與 architecture §3.6 決策三字面一致。**
   過程：初版宣告為選填，因當時 `access-history-filter.spec.ts` 之 `row()` 夾具以完整物件字面
   建構 `AuditRow` 且未帶該鍵，設必填會讓整支 suite 編譯失敗。經向 `tg-appendix` 報備後，
   **它裁定必填**（理由：§4.9 明講「比照現行 `formId`／`lifecycleId`／`documentId`」，
   而該三欄在夾具中本就是顯式帶 `null` 的必填慣例，不應為附錄另開選填先例），
   並自行於夾具補上 `appendixId: null`。收緊後我複查全部建構點：
   生產側僅 `buildAuditRow`（switch 產出的區域變數）與 `TypeOrmAuditStore.toRow()`／`append()`
   兩處，皆已顯式填值；`npx tsc --noEmit` 乾淨、`npx jest src/audit src/appendices src/rbac src/storage`
   → 15 suites / 230 tests 全綠，無其他呼叫端受影響。
...
6. **`DocumentExistenceChecker` 之介面形狀由 test-generator 定義、本 agent 逐字採用**
   （architecture-spec 只以文字寫「主動驗證 documentId 存在性」，未給型別）：
   `exists(documentId: string): Promise<boolean>`，且為 `AppendicesService` 建構子之**第 4 個必要參數**
   （`blob, store, audit, documents, uploaderDir?, orgResolver?`）。本 agent 認為此形狀合理
   （回 boolean 而非文件本身，避免 AppendicesModule 取得非其擁有之 `ICSOP_DOCUMENT` 欄位；
   置於 service 而非 store 層，讓「驗證早於任何寫入」成為服務層可讀的顯式步驟），**未提出異議**。

7. **`appendDocumentAppendices()`（POST）於前端完全不可達**（§3.6 決策二明文「保留 API 完整性但不接入 UI」）：
   後端已實作並有單元測試；前端雖於 `api/endpoints.ts` 提供 wrapper 以維持契約完整，但
   **`DocumentCreatePage`／`DocumentEditPage`／任何頁面皆未匯入或呼叫**，
   排序寫入一律走 `replaceDocumentAppendices()`（`PUT` replace-set）。`unlinkDocumentAppendix()` 同理未接線
   ——編輯頁之「解除關聯」是從已選清單移除後隨整組覆寫送出，非呼叫單一解除端點。

---

## 五、測試爭議與裁決結果（向 `tg-appendix` 申訴 6 項＋報備 1 項，全數獲裁決）
...
| 6 | `frontend/src/pages/PublicDocumentDetailPage.test.tsx:254` | `.closest('section, div')!` 回傳 `Element`，餵給 `within()` → TS2345（Vitest 執行期綠，但 typecheck 閘紅） | 採納，改為 `.closest('div')` |
| 7（報備非申訴） | `backend/src/audit/access-history-filter.spec.ts` 之 `row()` 夾具 | 夾具未帶 `appendixId`，使「必填 vs 選填」二擇一 | **裁定必填**（§4.9「比照 formId／lifecycleId／documentId」之既有顯式 null 慣例）；`tg-appendix` 自行於夾具補 `appendixId: null`，本 agent 據此把型別收緊 |

> 本 agent 對上述 7 處**未動任何一個字元**；`git diff` 顯示測試檔之變更全部來自 `tg-appendix`。
> 另外，**未為了讓測試轉綠而弱化任何實作語意**（驗證順序、錯誤碼、守門鏈皆逐條依 spec 落地）。
>
> **環的洞在哪**：#1–#3、#7 屬「測試作者的掃描白名單不完整」（原任務只列了後端兩支 rbac spec 與
> 5 支 prototype，未含 `frontend/src/domain/` 之鏡射矩陣、prototype 18、以及既有共用夾具的完整性）；
> #5–#6 屬「測試作者自身的 import／型別缺陷」；#4 則是靠 `tg-appendix` 回頭讀 prototype 18 才實證定案
> （banner 文案早已寫成「共 20 欄…與 F039 新增之『附錄（多）』」，非誰的臆測）。
> 此模式可作為下一輪建環時的掃描清單：**凡跨層鏡射之常數表、共用測試夾具、以及被頁面引用但未列於
> 任務白名單的 prototype，都要納入 test-generator 的掃描範圍**。
...
| C | **未於 `PublicDocumentDetailDto` 內嵌 `appendices`**（team-lead 提綱曾列此項；對應 F039-test.md **R-F039-01**） | 非規格衝突，而是後端契約不變下的前端整合彈性，本 agent 選擇「獨立呼叫 `getDocumentAppendices(documentId)`」：(1) F039 Interface Contract 與 architecture §3.6 決策五皆指定詳情頁走**專用端點** `GET /documents/:documentId/appendices`；(2) 若同時內嵌於 public detail DTO，將出現兩條資料路徑，正是 spec 明文禁止之「兩路徑不得產生不同的排序語意」風險來源；(3) 前後台因此共用同一 API，AC-25 之一致性成為**結構性保證**而非紀律保證。既有 `usageForms`／`attachments` 之內嵌寫法維持不動 |
...
1. **Playwright 3 支 fidelity spec 未執行**（使用者裁定跳過，缺 E2E 憑證）。
   其中 `fidelity-document-appendix-ordering.spec.ts` 與 `fidelity-document-appendix-detail.spec.ts` 內建
   `test.skip` 優雅略過（導覽慣例／資料為空時）。
   **路由確認（對應 R-F039-02）**：`fidelity-document-appendix-ordering.spec.ts` 推測之
   `a[href="/admin/documents/new"]` 與實作**一致**——該路由為 `App.tsx` 既有之
   `<Route path="documents/new" element={<DocumentCreatePage />} />`，本次未變更；
   新增之唯一路由為 architecture 明文授權的 `/admin/appendices`。故該支測試的防禦性 skip
   **不會因路由不符而觸發**，無需回頭補強路由斷言。
