...
## 收尾完成（2026-07-21，commit d09f1d3）
- **編輯 modal**（移植 08）：手動帳號改姓名/重設密碼（`updateAccount` 端點）；上游帳號姓名唯讀＋密碼說明。列操作加「編輯」。
- **分頁**：客端每頁 50 筆、上一頁/下一頁、共 N 筆；篩選/關鍵字變更回第 1 頁。後端 `take` 500→5000（覆蓋 AS ~1,114，解掉靜默截斷）。
- 新增 3 前端測（帳號頁 9 測、前端全 69）；後端 254、typecheck/nest build 乾淨。

## 仍為後續（非本功能範圍）
- 帳號數超過 5000/單公司 → 需改後端分頁（skip/take + total）。目前規模無此需求。
